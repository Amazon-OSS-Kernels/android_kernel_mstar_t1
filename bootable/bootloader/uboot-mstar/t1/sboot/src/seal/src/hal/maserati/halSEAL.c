...
int HAL_SEAL_SetSecureRange(unsigned int rangeId, unsigned long long startAddr, unsigned long long endAddr, int attr)
{
    ...
    if (rangeId >= REG_SECURE_RANGE_NUM)
        return 0;
...
//    }
    ...
    if (free >= REG_SECURE_RANGE_OFFSET) // no free range
    {
        return 0;
    }
...
int HAL_SEAL_Init(void)
{
    ...
    // secure AESDMA
    (RIU[REG_TZPC_NONPM_SECURE_BANK + ((135>>4)<<1)]) &= ~(1<<(135&15));
    (RIU[REG_TZPC_NONPM_MIU_HOST + ((135>>4)<<1)]) &= ~(1<<(135&15));
