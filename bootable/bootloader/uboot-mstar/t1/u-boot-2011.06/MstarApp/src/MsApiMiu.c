...
#if ENABLE_NIKON || ENABLE_MILAN || ENABLE_KERES || ENABLE_KIRIN || ENABLE_KRIS || ENABLE_KAYLA
            if (u32DramSize1 == 0)
            {
                printf("#######################################################################\n");
                if (u32DramSize1 == 0)
...
int MsApi_kernelProtect(void)
{
    ...
    if ( getenv("KERNEL_PROTECT") != NULL)
    {
        ...
        {
            if(Get_LX_MemInfo(LX_ID,&u32LxStart,&u32Size)==TRUE)
            {
                u8MiuIndex=MsApi_Miu_GetID(u32LxStart);
                UBOOT_DEBUG("Lx1 start:[0x%x] Size[0x%x] in MIU(%d)\n",u32LxStart,u32Size,u8MiuIndex);
                if(u32Size>0)
                {
                    MsDrv_kernelProtect(u8BlockMiu[u8MiuIndex],u32LxStart,(MS_U64)((MS_U64)u32LxStart + (MS_U64)u32Size));
