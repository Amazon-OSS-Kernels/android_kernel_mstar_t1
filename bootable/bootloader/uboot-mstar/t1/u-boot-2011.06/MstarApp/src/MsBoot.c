...
#define DUMP_BLOCK_SIZE (0x100000) //1M

#include <idme.h>

int _do_RamDump(void)
{
    ulong u32SPIAddr = 0;
    ulong u32LxLen = 0, u32Lx1Len = 0;
    ulong u32LxAddr = 0, u32Lx1Addr = 0;
    int ret = 0,i = 0;
    ...
    idme_get_var_external("bootcount", boot_count, sizeof(boot_count)-1);
    snprintf(filename, sizeof(filename), "%s.bin", boot_count);
#else
    snprintf(filename, sizeof(filename), "%s", "TV_DUMP.bin");
#endif

    if ( appInitUsbDisk() != 0 )
    {
        UBOOT_ERROR("Please plug in USB disk...\n");
        return -1;
    }

    u32LxLen   = strtol( getenv("E_LX_MEM_LEN"),  NULL, 16 );
    u32Lx1Len  = strtol( getenv("E_LX_MEM2_LEN"), NULL, 16 );
    u32LxAddr  = strtol( getenv("E_LX_MEM_ADR"),  NULL, 16 ) + CONFIG_MIU0_BUSADDR;
    u32Lx1Addr = strtol( getenv("E_LX_MEM2_ADR"), NULL, 16 ) + CONFIG_MIU0_BUSADDR + 0x20000000; //MIU BUS VA from 0xC0000000

    printf("LX=0x%lx++0x%lx, LX1=0x%lx++0x%lx\n", u32LxAddr, u32LxLen, u32Lx1Addr, u32Lx1Len);

    if ( (u32LxLen == 0) || (u32Lx1Len == 0) || (u32LxAddr == 0) || (u32Lx1Addr == 0) )
    {
        UBOOT_ERROR("getenv of LX MEM failed\n");
        return -1;
    }
    ...
    if ( dev_desc == NULL )
    {
        UBOOT_ERROR ("Invalid USB device 0\n");
        return -1;
    }

    if ( (fat_register_device(dev_desc, part)!=0) || ((fd = file_fat_open(filename, "w")) < 0) ) 
    {
        UBOOT_ERROR("Unable to open %s on USB disk\n", filename);
        return -1;
    }

    run_command("led_red_pwm 20", 0);

    for ( i = 0; i < u32LxLen / DUMP_BLOCK_SIZE; i++ )
    {
        ret = file_fat_write(fd, (unsigned int)(u32LxAddr+i*DUMP_BLOCK_SIZE), DUMP_BLOCK_SIZE);
        if (ret < 0)
        {
            UBOOT_ERROR("Unable to write LX%d to USB disk\n", 0);
            file_fat_close(fd);
            return -1;
        }
    }

    for ( i = 0; i < u32Lx1Len / DUMP_BLOCK_SIZE; i++ )
    {
        ret = file_fat_write(fd, (unsigned int)(u32Lx1Addr + i * DUMP_BLOCK_SIZE), DUMP_BLOCK_SIZE);
        if (ret < 0)
        {
            UBOOT_ERROR("Unable to write LX%d to USB disk\n", 1);
            file_fat_close(fd);
...
#if 0 ////ALISON-2434 : do not del CMA setting when factory reset
...
EN_BOOT_MODE get_boot_mode_from_mtd0(void)
{
    ...
    snprintf(cmd, sizeof(cmd)-1, "mmc read.p 0x%08lX misc 2048", (unsigned long)BOOT_MODE_TMP_BUF_ADDR);
#else
    snprintf(cmd, sizeof(cmd)-1, "nand read.e 0x%08lX misc 2048", (unsigned long)BOOT_MODE_TMP_BUF_ADDR);
#endif
    if(-1 != run_command(cmd, 0))
    {
	 if(strstr(p_msg->recovery, "--silent")){
		printf("silent recovery, set silent ota flag\n");
                Write2Byte(0x060a, Read2Byte(0x60a)|(PM_SPARE_SPECIAL_MODE_SILENT_OTA)); /* Set silent OTA flag */
	}
...
static EN_BOOT_MODE check_key_long_press(U8 key_value, EN_BOOT_MODE mode, u32 hold_time)
{
    ...
    UBOOT_INFO("Ticking: ");
    while (interval < hold_time)
    {
        msKeypad_GetKey(&u8KeyPad_KeyVal,&u8KeyPad_RepFlag);
        if (u8KeyPad_KeyVal != key_value)
        {
            break;
        }
    ...
    if (interval >= hold_time)
    {
	UBOOT_INFO("0x%x key is pressed more than %d s\n", hold_time/1000);
	return mode;
	}
    else
    {
        UBOOT_INFO("\nNo enough time(%d.%ds < %d.%ds), continue to booting...\n", interval/1000, (interval%1000)/100,
                        hold_time/1000, (hold_time%1000)/100);
...
EN_BOOT_MODE get_boot_mode_from_keypad(void)
{
     ...
     switch(u8KeyPad_KeyVal) // NOTE: read IR Key to decide what boot mode to go
    {
        ...
        case KEYPAD_FORCEUGRADE_KEY:
            mode = check_key_long_press(KEYPAD_FORCEUGRADE_KEY, EN_BOOT_MODE_USB_UPGRADE, KEYPAD_HOLD_VALUE);
            break;
        ...
        case KEYPAD_SYSTEM_RESTORE_KEY:
            ...
            break;
        #endif
	case KEYPAD_POWER_KEY:
		power_button_pressed = 1;
		if((boot_reason == PM_SPARE_COLD_BOOT_POWER_SUPPLY) && (E_PM_WAKEUPSRC_NONE == MDrv_PM_GetWakeupSource()))
		{
			printf("Enter recovery mode because power key is pressed when plugging power \n");
			mode = check_key_long_press(KEYPAD_POWER_KEY, EN_BOOT_MODE_RECOVERY, 5000);
			if (mode == EN_BOOT_MODE_RECOVERY)
				printf("Enter recovery mode because power key is long pressed when plugging power \n");
		}
		break;
        default:
            mode = EN_BOOT_MODE_UNKNOWN;
...
int isSilentRecovery(void)
{
	 return (Read2Byte(0x60a)&PM_SPARE_SPECIAL_MODE_SILENT_OTA);
}

void clearSilentRecovery(void)
{
	Write2Byte(0x60a, Read2Byte(0x60a)&~PM_SPARE_SPECIAL_MODE_SILENT_OTA);
}

int isSilentLED(void)
{
	return (Read2Byte(0x60a)&PM_SPARE_SPECIAL_MODE_SILENT_LED);
}

void clearSilentLED(void)
{
	Write2Byte(0x60a, Read2Byte(0x60a)&~PM_SPARE_SPECIAL_MODE_SILENT_LED);
}

void setSilentLED(void)
{
	Write2Byte(0x60a, Read2Byte(0x60a)|PM_SPARE_SPECIAL_MODE_SILENT_LED);
}
...
int do_bootcheck (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    if (MDrv_WDT_IsReset())
    {
		boot_reason = Read2Byte(0x60a);
		Write2Byte(0x060a, boot_reason|0x2); /* Set WDT reset flag */


        for (i = 0; i < 4; i++)
        {
            val = get_intr_rec_percpu(i);

            if (val != -1) {
                printf("Suspicious interrupt: cpu %d, interrupt number %d\n",
                       i , val);
                wdt_intr_dump[i] = val;
            }
        }
    ...
    if (boot_reason & PM_SPARE_WARM_BOOT_KERNEL_PANIC)
        printf("kernel panic");
    else if (boot_reason & PM_SPARE_WARM_BOOT_SW)
        printf("software");
    else if (boot_reason & PM_SPARE_BOOT_LONG_PWR_KEY_PRESS)
        printf("long press power key");
    else if (boot_reason & PM_SPARE_WARM_BOOT_HW_WDOG)
        printf("watchdog");
    else if (boot_reason & PM_SPARE_THERMAL_BOOT_SOC)
        printf("thermal boot soc");
    else if (boot_reason & PM_SPARE_THERMAL_SHUTDOWN_SOC)
        printf("thermal shutdown");
    else if (boot_reason & PM_SPARE_SHUTDOWN_SW)
        printf("software shutdown");
    else if (boot_reason & PM_SPARE_SPECIAL_MODE_OTA)
        printf("OTA");
    else if (boot_reason & PM_SPARE_SPECIAL_MODE_FACTORY_RESET)
        printf("factory reset");
    else if (boot_reason == PM_SPARE_COLD_BOOT_POWER_SUPPLY)
        printf("power supply or loss");
    else
        printf("unkonwn reason");
    printf("\n");

#ifdef UFBL_FEATURE_IDME
    if( (boot_reason & PM_SPARE_WARM_BOOT_KERNEL_PANIC) ||
        (boot_reason & PM_SPARE_WARM_BOOT_KERNEL_WDOG) ||
        (boot_reason & PM_SPARE_BOOT_LONG_PWR_KEY_PRESS) )
    {
        char flags[12] = {0};
        if ( !idme_get_var_external("fos_flags", flags, sizeof(flags)-1) &&
             (simple_strtoul(flags, NULL, 16) & FOS_FLAGS_RAMDUMP_ON) )
        {
            printf("Saving ramdump...\n");

            while (_do_RamDump() != 0)
            {
                printf("Ramdump failed, retrying...\n");
                run_command("led_red_pwm 100", 0);
                mdelay(10000);
            }

            Write2Byte(0x060a, PM_SPARE_WARM_BOOT_SW);
            run_command("led_red_pwm 0", 0);
            printf("Ramdump finished, continue booting...\n");
        }
    }
#endif
...
#if 0 // don't do force usb upgrade
...
#ifdef MBOOT_TYPE_OBA
#define MBOOT_OBA_TAG "****MBOOT_OBA****"
    printf("\n\n%s\n\n", MBOOT_OBA_TAG);
    setenv(ENV_FORCE_UPGRADE, NULL);
    saveenv();
    run_command("custar", 0);
    //reset command will be called when custar finishes successfully
    //so normal usb upgrade won't come here
    //if usb upgrade is not successful, halt here
    printf("\n\n%s\n\n", MBOOT_OBA_TAG);
    printf("*NEED TO REPLACE MBOOT!*\n\n");
    printf("** HALTING... **\n");
    while (1) ;
#endif
    ...
    switch(BootMode)
    {
        ...
        case EN_BOOT_MODE_FASTBOOT:
            #if defined(CONFIG_ONE_EXT_USB)
            run_command("usb start 4",0);
            #else
            if ( (getenv("dev_flags") != NULL) && ((simple_strtoul(getenv("dev_flags"), NULL, 16))&DEV_FLAGS_USB_NUM_VAL) )
                run_command("usb start 4",0);
            else
...
// Set fcie / tsp boot selection:
// 1 : clk_fcie_p,  0 : clk_tsp_p
int do_fcie_tsp_boot_sel(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	UBOOT_TRACE("IN\n");
	if (argc != 2)
	{
		UBOOT_ERROR("do_fcie_tsp_boot_sel arg error!");
		return cmd_usage(cmdtp);
	}
	UBOOT_DEBUG("argv[1]:%s argc = %d",argv[1], argc);
	if(strcmp(argv[1],"0") == 0)
	{
		*(volatile MS_U32*)(MS_RIU_MAP+(0x101EB4<<1)) &= ~0x20;
		UBOOT_DEBUG("do_fcie_tsp_boot_sel to tsp");
	}
	else if(strcmp(argv[1],"1") == 0)
	{
		*(volatile MS_U32*)(MS_RIU_MAP+(0x101EB4<<1)) |= 0x20;
		UBOOT_DEBUG("do_fcie_tsp_boot_sel to fcie");
	}
	else
	{
		UBOOT_ERROR("do_fcie_tsp_boot_sel arg error!");
