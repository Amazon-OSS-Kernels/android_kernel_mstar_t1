...
#if 0 // don't do this, device may not enter recovery mode
...
int load_HW_AES_BUFFER_info(U32 *u32HwAesPABufAddr, U32 *u32HwAesBufLen)
{
    ...
    get_value_from_env(E_MMAP_ID_HW_AES_BUF_ADR, NO_DEFAULT_MMAP_VALUE, u32HwAesPABufAddr);
    get_value_from_env(E_MMAP_ID_HW_AES_BUF_LEN, NO_DEFAULT_MMAP_VALUE, u32HwAesBufLen);
...
int do_read_optee (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
   ...
   // _dump(optee_real_va_addr+pst_optee_header->init_size-0x100,0x100);
    ...
    load_HW_AES_BUFFER_info(&u32HwAesPABufAddr,&u32HwAesBufLen);
