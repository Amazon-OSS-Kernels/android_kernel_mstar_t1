...
#if((ENABLE_MSTAR_KENYA == 1) || (ENABLE_MSTAR_KERES == 1) || (ENABLE_MSTAR_KIRIN == 1) || (ENABLE_MSTAR_KRIS == 1) || (ENABLE_MSTAR_KAYLA == 1)  || (ENABLE_MSTAR_CLIPPERS==1) || (ENABLE_MSTAR_KRATOS == 1)|| (ENABLE_MSTAR_KIWI == 1))
...
char *get_script_next_line(char **line_buff_ptr)
{
    char *line_buff;
    char *next_line;
    int i;
    ...
    line_buff = (*line_buff_ptr);

    // strip '\r', '\n' and comment
    while (1)
    {
        // strip '\r' & '\n' & ' ' & '\t'
        if (IS_IGNORED_CHAR(line_buff[0]))
        {
            line_buff++;
        }
        // strip comment
        else if (IS_COMMENT(line_buff[0]))
        {
            for (i = 0; !IS_LINE_END(line_buff[0]) && i <= MAX_LINE_SIZE; i++)
            {
                line_buff++;
            }
...
int do_nandprogramforrom ( cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
    ...
    #elif (ENABLE_MONACO) || (ENABLE_MUJI) || (ENABLE_CELTICS) || (ENABLE_MONET) || (ENABLE_MUNICH) || (ENABLE_MACAN) || (ENABLE_MANHATTAN) || (ENABLE_MESSI) || (ENABLE_MASERATI) || (ENABLE_MAXIM)
    filelength -= 0x3800;
    ...
    #elif (ENABLE_MILAN) || (ENABLE_MULAN)  || (ENABLE_KIWI)
    filelength -= 0x3800;
    (*(volatile unsigned short*)(dbgAddress)) = 0x003d;// jh.huang
