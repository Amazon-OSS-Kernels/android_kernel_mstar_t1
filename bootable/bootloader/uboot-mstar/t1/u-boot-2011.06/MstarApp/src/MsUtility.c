...
char* loadscript(char *filedir,U32 *filesize)
{
    char *script = NULL;
    UBOOT_TRACE("IN\n");
    char * _filedir = filedir;

    if (strncmp(filedir, "/tvconfig/", 10) == 0) {

        UBOOT_INFO("loadscript: remove /tvconfig from path %s\n", filedir);
        _filedir = filedir + strlen("/tvconfig");
        UBOOT_INFO("loadscript: path becomes %s\n", _filedir);
    }


    *filesize = vfs_getsize(_filedir);
    ...
    if(vfs_read((void *)script,_filedir,0,(unsigned int)(*filesize))==-1)
    {
        free(script);
...
int chk_cmd_lockdown(const char* command)
{
   if ( (is_lockdown()) &&
        ...
        (strncmp("secflash", command, strlen("secflash"))) &&
        (strncmp("uupdate", command, strlen("uupdate"))))
      return 0;
   else
      return 1;
}
