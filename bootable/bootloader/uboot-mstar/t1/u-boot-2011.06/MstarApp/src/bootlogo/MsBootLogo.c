...
int do_dbtable_init(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    else
    {
        ...
        p_str = getenv ("customer_ini_path_uboot");
        UBOOT_DEBUG("customer_ini_path_uboot %s\n", p_str);
        if(p_str != NULL)
        {
        ...
        {
            UBOOT_DEBUG("No customer_ini_path_uboot\n");
            ret =  write_dbtable(u32LogoOffset);
        }
#else
...
int showLogo(LogoDispConfigParams stConfig,U32 JPD_OUTADDR)
{
    ...
    UBOOT_TRACE("IN\n");
    if (!amzn_check_model_name()) {
      UBOOT_ERROR("model_name check failure. hardware_id:%x\n", amzn_get_hardware_id());
      UBOOT_TRACE("OK\n");
      return 0;
    }
