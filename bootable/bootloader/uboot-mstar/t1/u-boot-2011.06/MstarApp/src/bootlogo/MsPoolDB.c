...
#define DEFAULT_MODEL_NAME_UBOOT "/config/model/Customer_1.ini"
#endif
...
int parse_sys_ini(char* path,char* ModelPath)
{
    ...
    if (!idme_get_var_external("model_name", ModelPath, BUFFER_SIZE-1))
    {
        ...
        setenv("customer_ini_path_uboot",ModelPath);
        UBOOT_INFO("customer_ini_path_uboot env value --> %s\n",ModelPath);
        ...
        if (strncmp(ModelPath, "/config", 7) == 0) {
            ...
            strncpy(ModelPath, full_path, BUFFER_SIZE);
        }
    ...
    else
    {
        ...
        setenv("customer_ini_path_uboot",DEFAULT_MODEL_NAME_UBOOT);
        UBOOT_INFO("customer_ini_path uses default value --> %s\n", DEFAULT_MODEL_NAME);
        UBOOT_INFO("customer_ini_path_uboot uses default value --> %s\n", DEFAULT_MODEL_NAME_UBOOT);
    }
...
//    setenv("panel_ext_type", str);
    ...
    memset(str,0,sizeof(str));
    n = Profile_GetInteger("panel", "u32MaxPWMvalue", 0);// Duty use u16MaxPWMvalue value,fix it later
    snprintf(str,sizeof(str),"0x%x",n);
    UBOOT_DEBUG("u32MaxPWMvalue = %s\n",str);
    gstPWMSetting.m_u32maxPWM = n;

    memset(str,0,sizeof(str));
    n = Profile_GetInteger("panel", "u32MinPWMvalue", 0);// Duty use u16MinPWMvalue value,fix it later
    snprintf(str,sizeof(str),"0x%x",n);
    UBOOT_DEBUG("u32MinPWMvalue = %s\n",str);
    gstPWMSetting.m_u32minPWM = n;
    memset(str,0,sizeof(str));
...
int Load_PWMSetting_ToFlash(U32 u32DbtableOffset)
{
    ...
    UBOOT_DEBUG("gstPWMSetting.m_u32maxPWM                 = 0x%x\n", gstPWMSetting.m_u32maxPWM);
    UBOOT_DEBUG("gstPWMSetting.m_u32minPWM                 = 0x%x\n", gstPWMSetting.m_u32minPWM);
...
int Read_PWMSetting_ToFlash(st_pwm_setting * pwm_data)
{
    ...
    UBOOT_DEBUG("pwm_data->m_u32maxPWM                 = 0x%x\n", pwm_data->m_u32maxPWM);
    UBOOT_DEBUG("pwm_data->m_u32minPWM                 = 0x%x\n", pwm_data->m_u32minPWM);
