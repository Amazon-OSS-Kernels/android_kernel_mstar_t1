...
#if defined(CONFIG_URSA6_VB1) && (!defined(CONFIG_MSTAR_MONACO)) && (!defined(CONFIG_MSTAR_MUJI))&& (!defined(CONFIG_MSTAR_MONET)) && (!defined(CONFIG_MSTAR_MANHATTAN)) && (!defined(CONFIG_MSTAR_MASERATI))&& (!defined(CONFIG_MSTAR_MACAN) && (!defined(CONFIG_MSTAR_MAXIM)))
...
static char* HdmiTxTimingIndexTable[] = {
    ...
    "",//10
    "",//11
    "HDMITX_VB1_1920x1080_50Hz",//12
    "HDMITX_VB1_1920x1080_60Hz",//13
    "HDMITX_VB1_3840x2160_30Hz",//14
    "",//15
    "",//16
    "",//17
    "",//18
    "",//19
    "",//20
    "HDMITX_VB1_3840x2160_25Hz",//21
    "",//22
    "HDMITX_VB1_3840x2160_50Hz",//23
    "HDMITX_VB1_3840x2160_60Hz",//24
    "",//25
};
#else
#include <apiHDMITx.h>
...
#if (ENABLE_HAS_LD==1)
MS_S8 do_localdimm_init(MS_U16 panel_width, MS_U16 panel_height)
{
    //local dimming init
    char *pLDFlag = getenv("ld_on");
    MS_BOOL bPreSet = TRUE;

    if(NULL == pLDFlag)
    {
        UBOOT_ERROR("local dimming init,get env ld_on fail ...>>>\n");
        bPreSet = FALSE;
        return -1;
    }

    if(1 == simple_strtoul(pLDFlag, NULL, 10))
    {
        MS_PHYADDR u32Addr = 0;
        MS_U32 u32Size = 0;
        MS_U16 u16LDPanelType = 0;  //env ld_type, relate to led num
        stLD_Init_Parameters stLDArgs;

        //get mmap addr
        if(get_value_from_env(E_MMAP_ID_LOCAL_DIMMING_ADR, NO_DEFAULT_MMAP_VALUE, &u32Addr)!=0)
        {
        ...
        if((((size_t)-1) == u32Addr) || (0x0000 == u32Addr) || (0x0000 == u32Size))
        {
            UBOOT_ERROR("local dimming init, Get LD Mmap fail...>>>\n");
            bPreSet = FALSE;
            return -1;
        }

        //get type
        pLDFlag = getenv("ld_type");
        if(NULL == pLDFlag)
        {
            UBOOT_ERROR("local dimming init,get env ld_type fail ...>>>\n");
            pLDFlag = "0";
            return -1;
        }
        u16LDPanelType = simple_strtoul(pLDFlag, NULL, 10);

        //set parametre
        memset(&stLDArgs, 0x00, sizeof(stLDArgs));
        stLDArgs.ePnlLDType = u16LDPanelType;
        stLDArgs.u32MemAdrress = u32Addr;
        stLDArgs.u16MemSize = u32Size;
        stLDArgs.u16PnlWidth = panel_width;
        stLDArgs.u16PnlHeight = panel_height;
        stLDArgs.pu8RawData = (MS_U8 *)&u32Addr;  //panel  data, use later
        stLDArgs.u16DataSize = 0;

        //call utopia
        if(TRUE == bPreSet)
        {
            MS_U16 u16ParaSize = sizeof(stLDArgs) + stLDArgs.u16DataSize;
            MS_U16 u16Inited = MApi_LD_Init(&stLDArgs, u16ParaSize);
            if(E_APILD_STATUS_SUCCESS != u16Inited)
            {
                UBOOT_ERROR("local dimming init fail, init return:%d ...>>>\n", u16Inited);
                return -1;
            }
        }
        else
        {
            UBOOT_ERROR("local dimming init fail ...>>>\n");            //return -1;
            return -1;
        }
    }
    return 0;
...
int panel_dinit(void)
{
...
#if (ENABLE_HAS_LD==1)
    if(0 != do_localdimm_init(panelpara.m_wPanelWidth, panelpara.m_wPanelHeight))
        {
        UBOOT_ERROR("local dimming init fail ...>>>\n");
    }
#endif
    ...
    Read_MiscSetting_ToFlash(&misc_setting);
    if(strncmp(misc_setting.m_Panel_LVDS_Connect_Type,"0x04",4)==0)
    {
