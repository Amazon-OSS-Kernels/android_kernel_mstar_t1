...
int do_panel_init(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if(bPanleReady)
	{
		printf("call MsDrv_PNL_BackLigth_On %s %d \n", __func__, __LINE__);
...
extern void clearSilentRecovery(void);
extern int isCodBootByPowerLoss(void);
extern int isScreenStateOn(void);
...
int do_backLigth_on(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	{
		if(isSilentRecovery()) {
			if (isCodBootByPowerLoss() || isScreenStateOn())
				MsDrv_PNL_BackLigth_On();
			else
				printf(" Do not turn on backlight for reboot after silent OTA\n");
			clearSilentRecovery();
		} else {
