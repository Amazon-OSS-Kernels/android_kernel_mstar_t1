...
int do_MsSd(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	else if (strcmp(argv[1], "scanphase") == 0)
	{
		...
		if(strcmp(argv[2], "read") == 0)
		{
			...
			gu8TuningMode = (strcmp(argv[3], "log") == 0) ? TUNE_NOW:(TUNE_SILENCE+TUNE_NOW);

			for(trig_level=0; trig_level<8; trig_level++)
			{
				if(!(gu8TuningMode&TUNE_SILENCE)) printf("trig_level = %d\n", trig_level);
		...
		else if(strcmp(argv[2], "write") == 0)
		{
			...
			gu8TuningMode = (strcmp(argv[argc-1], "log") == 0) ? TUNE_NOW:(TUNE_SILENCE+TUNE_NOW);

			for(skew1=skew1_start; skew1<skew1_end; skew1++)
			{
				if(!(gu8TuningMode&TUNE_SILENCE)) printf("skew1 = %d\n", skew1);
				for(skew4=0; skew4<18; skew4++)
				{
					if(!(gu8TuningMode&TUNE_SILENCE)) printf("skew4 = %d\n", skew4);
					//printf("\nS1[%d]S4[%d]\n", skew1, skew4);
