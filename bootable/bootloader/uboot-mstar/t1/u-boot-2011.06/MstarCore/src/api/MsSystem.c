...
#define INTR_REC_L 0x60C
#define INTR_REC_H 0x60E
#define INTR_HW_INTR_BASE (128)
#define INTR_SYS_BASE (224)
#define INTR_CLEAR_BASE (240)

signed int get_intr_rec_percpu(unsigned int cpuid)
{
    volatile U32 *riu_addr_off;
    unsigned int bank_off, byte_off;
    signed int IntrNum = 0;
    ...
    riu_addr_off = bank_off + byte_off;

    IntrNum = (signed int)ReadByte((MS_U32)riu_addr_off);

    if (IntrNum == INTR_CLEAR_BASE)
        return -1;

    if (IntrNum >= INTR_SYS_BASE)
        return IntrNum - INTR_SYS_BASE;
    else
        return IntrNum + INTR_HW_INTR_BASE;
}
