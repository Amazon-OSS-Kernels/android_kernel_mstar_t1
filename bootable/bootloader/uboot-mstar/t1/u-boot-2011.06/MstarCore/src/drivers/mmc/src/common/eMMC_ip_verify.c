...
U32 eMMCTest_Read_disturbance(void)
{
    ...
    {
        if(eMMC_ReadData(&mpart, eMMC_SECTOR_512BYTE, u32_i) == eMMC_ST_SUCCESS)
        {
    ...
    if((u32_mboot_start ==0) && (u32_mboot_size ==0))
    {
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR,1,"No mboot partition:%x\n");
        return eMMC_ST_SUCCESS;
    }
    ...
    while(1)
    {
        ...
        u32_err = eMMC_ComapreData(pu32_R, u32_mboot_addr, u32_mboot_size);
        if(u32_err != eMMC_ST_SUCCESS)
        {
