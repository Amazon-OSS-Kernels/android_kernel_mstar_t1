...
int oad_prepare(void)
{
    ...
    DBG_PRINT( printf("%s: at %d\n", __func__, __LINE__) );
#if ENABLE_STB_ECOS_BOOT
    u32DmxSecBuffADR = OADSEC_BUFFER_ADR;
#else
    ...
    u32OadbufMonitorAddr = u32DmxSecBuffADR; //(_VA2PA((MS_U32)demuxBuf.monitor)) ;
    u32OadbufDownloadAddr = (MS_U32)(u32OadbufMonitorAddr + 0x4000);//(_VA2PA((MS_U32)demuxBuf.download)) ;
