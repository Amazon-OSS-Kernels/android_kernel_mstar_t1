...
#include <../hal/maxim/panel/mhal_panel.h>
#else
#endif
...
void SetPWM(U32 u32Period, U32 u32Duty, U32 u32PwmDiv, PWM_ChNum PWMPort)
{
    ...
    MDrv_PWM_Period(PWMPort, u32Period);
    MDrv_PWM_DutyCycle(PWMPort, u32Duty);
    MDrv_PWM_Div(PWMPort, u32PwmDiv);
	if(bPolPWM==1)
...
int PWM_init(void)
{
    ...
    u32maxPWM = pwm_setting.m_u32maxPWM;
    u32minPWM = pwm_setting.m_u32minPWM;
    ...
    u32PWMDuty = (u32maxPWM+u32minPWM) * u32PWMPeriod / 200;

    UBOOT_DEBUG("u32PWMPeriod_T = %d\n",u32PWMPeriod);
    UBOOT_DEBUG("u32PWMDuty_T = %d\n",u32PWMDuty);
    UBOOT_DEBUG("u32PWMDIV_T = %d\n",u32PWMDIV);
    UBOOT_DEBUG("u32maxPWM_T = %d\n",u32maxPWM);
    UBOOT_DEBUG("u32minPWM_T = %d\n",u32minPWM);
...
/* For Maserati 120hz with LG panel*/
    ...
    { //ALISON-1414: do not sned vx1 data when RTC wake up
...
/* For Maserati 120hz with LG panel*/
   ...
   else
   {
    ...
    {
        MApi_PNL_SetSSC_Fmodulation(VBY1_SSC_MODULATION);   //Modulation: 40 kHz, unit: 0.1kHz
        MApi_PNL_SetSSC_Rdeviation(VBY1_SSC_DEVIATION);       //Deviation: 0.5%, unit: 1/10000
