...
//  Extern Functions
//-------------------------------------------------------------------------------------------------
extern int isSilentRecovery(void);
extern int isSilentLED(void);
extern void setSilentLED(void);
extern void clearSilentLED(void);
//-------------------------------------------------------------------------------------------------
//  Private Functions
//-------------------------------------------------------------------------------------------------
...
int do_led(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (isSilentRecovery())
	{
		printf(" Skip LED setting for second boot during silent OTA \n");
		setSilentLED();
		return 0;
	}
	...
	if (isSilentLED())
	{
		printf(" Skip LED setting for third reboot during silent OTA \n");
		clearSilentLED();
	}
	if (argc < 1)
	{
...
int do_led_red_pwm(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    char u8Percentage = 0;
    UBOOT_TRACE("IN\n");
    if (argc < 2)
    ...
    if(u8Percentage > 100)
    {
        UBOOT_ERROR("led percentage should be between 0~100\n");
        return 0;
    }

    UBOOT_INFO("Set red led to %d%%\n", u8Percentage);
    LED_RED_ON_PWM(u8Percentage);
