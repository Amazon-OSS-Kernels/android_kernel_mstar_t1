...
static int PM51_PowerDown(void)
{
    ...
    //Must Set IR Select before PM running.
	#ifdef UFBL_FEATURE_IDME
	idme_get_var_external("model_name", ModelName, 127);
	temp_p = strstr(ModelName, "_");
	if(temp_p)
	{
		if ((*(temp_p+1) ==  'F') || (*(temp_p+1) ==  'B') || (*(temp_p+1) ==  'E'))
			WriteByte(IR_SWITCH_PROTOCOL, E_IR_NEC_Toshiba);
		else
			WriteByte(IR_SWITCH_PROTOCOL, E_IR_NEC_IN);
	}
	else
		WriteByte(IR_SWITCH_PROTOCOL, E_IR_NEC_IN);
	#endif
...
static int SetPm2Sram(void)
{
...
#if defined(AMZN_FTVE_PM_SIGNING_ENABLED)
    UBOOT_DEBUG("AMZ signing Enabled\n");
    if (is_lockdown()) {
        UBOOT_DEBUG("is lockdown\n");
    ...
    {
        UBOOT_DEBUG("is NOT lockdown\n");
    }
#else
    UBOOT_DEBUG("MTK signing Enabled\n");
#endif
...
static BOOLEAN check_pm_standby(void)
{
	...
	else
	{
		...
		{
			printf("Screen is off when watch dog or kernel panic happens, Enter PM standy moe \n");
			return TRUE;
		}
		ret = FALSE;
	}
