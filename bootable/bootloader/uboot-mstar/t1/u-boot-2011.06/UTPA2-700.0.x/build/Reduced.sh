...
do_parsingHeadfile2()
{
	...
	cp -f $ROOTLIB/api/njpeg_ex/apiJPEG_priv.h  $BSPINC_PATH

}
do_parsingHeadfile_Contents()
{
  #Remove_First_line=0
	for ((jndex=1; jndex<${#TARGET_AIP_NAME[@]}; jndex++)); do
			...
			if [ ${TARGET_AIP_use[$jndex]} == "0" ]; then
				...
				if [  -f $BSPINC_PATH/${TARGET_AIP_File[$jndex]} ]; then
					#remove unused item
					bash $Build_PATH/mcomment.sh ${TARGET_AIP_NAME[$jndex]} $BSPINC_PATH/${TARGET_AIP_File[$jndex]} " "
...
bash $Build_PATH/special_case.sh $LOCAL_PATH
do_RemoveTab
