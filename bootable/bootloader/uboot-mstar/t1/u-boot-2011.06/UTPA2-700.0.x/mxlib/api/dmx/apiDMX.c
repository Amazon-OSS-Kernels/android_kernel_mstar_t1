...
static MS_BOOL _DMX_InfoSet_Sect(MS_U8 u8DmxId, DMX_Flt_info* pDmxFltInfo)
{
    ...
    if(_DMX_ChkBufOverlap(u8DmxId, pInfo->SectBufAddr, pInfo->SectBufSize))
    {
#if (!(defined(TSP_VERSION) && ((TSP_VERSION == TSP_VER_2_0) || (TSP_VERSION == TSP_VER_4_0))))
    ...
    MDrv_TSP_SecFlt_SetBuffer(0, _pdmx_res->_FltList[u8DmxId].SecBufId, pInfo->SectBufAddr, (MS_U32)pInfo->SectBufSize);
...
static MS_U32 _DMX_ApiDrv_FltSrcMapping(MS_U32 u32TSPFltSource)
{
    switch (u32TSPFltSource & DMX_FILTER_SOURCE_TYPE_MASK)
    {
        ...
        default:
            DMX_DBGMSG(DMX_DBG_ERR,  ULOGE("DMX", "[MAPI DMX][%06d] Bad filter src %ld\n", __LINE__, (MS_U32)u32TSPFltSource));
            return E_TSP_FLT_SRC_LIVE0;
    }
}
#endif
...
static DMX_FILTER_STATUS _DMX_Open_Stream(MS_U8* pu8DmxId, DMX_FILTER_TYPE DmxFltType)
{
    ...
    DMX_ASSERT(FltId< DMX_MAX_FLTID, DMX_DBGMSG(DMX_DBG_ERR, ULOGE("DMX", "[MAPI DMX][%06d] Bad FltId %lu\n", __LINE__, FltId)));
...
static DMX_FILTER_STATUS _DMX_GetRingBuffer(MS_U8 u8DmxId, MS_U8* pu8Buf, MS_U32 u32BufSize, MS_U32* pu32SecSize, MS_U32* pu32RmnSize, DMX_CheckCb pfCheckCB)
{
    ...
    *pu32RmnSize = u32RbSize= _DMX_RingBuffer_DataSize(virtRead, virtWrite, virtStart, virtEnd);
    ...
    //---------------------------------------------
    if((_pdmx_res->_FltList[u8DmxId].DmxFltType == DMX_FILTER_TYPE_PES) && (_u32PESCpy_ContMode == 0))
    {
    ...
    *pu32RmnSize-= *pu32SecSize;
    ...
    MDrv_TSP_SecFlt_SetReadAddr(0, _pdmx_res->_FltList[u8DmxId].SecBufId, MS_VA2PA((MS_VIRT)pu8Read));
    return DMX_FILTER_STATUS_OK;
}
...
static DMX_FILTER_STATUS _DMX_GetSect(MS_U8 u8DmxId, MS_U8* pu8Buf, MS_U32 u32BufSize, MS_U32* pu32SecSize, MS_U32* pu32RmnSize, DMX_CheckCb pfCheckCB)
{
    ...
    *pu32RmnSize = 0;
    ...
    if(virtRead >= virtEnd || virtRead < virtStart)
    {
        ...
        ULOGE("DMX", "[DMX][%d] Filter %d, Start %x, End %x , Read %x, Write %x \n", __LINE__, (int)u8DmxId, (unsigned int)virtStart, (unsigned int)virtEnd, (unsigned int)virtRead, (unsigned int)virtWrite);
        return DMX_FILTER_STATUS_ERROR;
    }
    ...
    *pu32RmnSize = u32RbSize = _DMX_RingBuffer_DataSize(virtRead, virtWrite, virtStart, virtEnd);
    ...
    MS_ASSERT(virtStart == (MS_VIRT)((MS_VIRT)_pdmx_res->_FltList[(u8DmxId)].FltInfo.Info.SectInfo.SectBufAddr));
    if ((virtRead==0)||(virtWrite==0)||(virtStart==0))
    {
    ...
    if (u32SecLen > u32RbSize)
    {
        DMX_DBGMSG(DMX_DBG_ERR, ULOGE("DMX", "[MAPI DMX][%06d] Section length is larger than what section buffer has %u %u  DMXID : %u\n", __LINE__,
            (unsigned int)u32SecLen, (unsigned int)u32RbSize, (unsigned int)u8DmxId ));
    ...
    if ((u32SecLen > u32BufSize) || (!pu8Buf))
    {
        DMX_ASSERT(u32SecLen <= u32BufSize, DMX_DBGMSG(DMX_DBG_ERR, ULOGE("DMX", "[MAPI DMX][%06d] fitler %d. Section data size > given buffer size (%u, %u)\n",
            __LINE__, (unsigned int)u8DmxId, (unsigned int)u32SecLen, (unsigned int)u32BufSize)));
        _DumpSecHeader(pu8Start, pu8End, pu8Read, pu8Write);
        bCopy = FALSE;
    }
    ...
    *pu32RmnSize -= *pu32SecSize;
    ...
    if (DRVTSP_OK != MDrv_TSP_SecFlt_SetReadAddr(0, _pdmx_res->_FltList[u8DmxId].SecBufId, MS_VA2PA((MS_VIRT)pu8Read)))
    {
...
DMX_FILTER_STATUS _MApi_DMX_Close(MS_U8 u8DmxId)
{
    ...
    do
    {
        ...
        if (FALSE == bPidReady)
        {
            ...
            {
                DMX_DBGMSG(DMX_DBG_ERR, ULOGE("DMX", "[%s][%d] Try to set pid failed,  Demux id: %d Filter Type: %d\n", __FUNCTION__, __LINE__, (int)u8DmxId, (unsigned int)_pdmx_res->_FltList[u8DmxId].DmxFltType));
                _DMX_RETURN(DMX_FILTER_STATUS_ERROR);
            }
...
DMX_FILTER_STATUS _MApi_DMX_SectPatternSet(MS_U8 u8DmxId, MS_U8* pPattern, MS_U8* pMask, MS_U8 *pu8NotMask, MS_U32 MatchSize)
{
    ...
    DMX_ASSERT(DMX_MAX_FLTID > u8DmxId, DMX_DBGMSG(DMX_DBG_ERR, ULOGE("DMX", "[MAPI DMX][%06d] Bad Demux id %d\n", __LINE__, u8DmxId)));
    DMX_ASSERT(0xFF > _pdmx_res->_FltList[u8DmxId].SecBufId, DMX_DBGMSG(DMX_DBG_ERR, ULOGE("DMX", "[MAPI DMX][%06d] Demux id has not been opened yet %d\n", __LINE__, u8DmxId)));
...
DMX_FILTER_STATUS _MApi_DMX_SectReset(MS_U8 u8DmxId)
{
    DMX_DBGMSG(DMX_DBG_FUNC, ULOGD("DMX", "[%s] %d\n", __FUNCTION__, __LINE__));
    _DMX_ENTRY();
    DMX_ASSERT2(DMX_MAX_FLTID > u8DmxId, DMX_DBGMSG(DMX_DBG_ERR, ULOGE("DMX", "[MAPI DMX][%06d] Bad Demux id %d\n", __LINE__, u8DmxId)));
    DMX_ASSERT2(0xFF > _pdmx_res->_FltList[u8DmxId].SecBufId, DMX_DBGMSG(DMX_DBG_ERR, ULOGE("DMX", "[MAPI DMX][%06d] No section buffer, Demux Id %d\n", __LINE__, u8DmxId)));
...
DMX_FILTER_STATUS _MApi_DMX_SectReadSet(MS_U8 u8DmxId, MS_PHY Read)
{
    ...
    _DMX_ENTRY();
    DMX_ASSERT2(DMX_MAX_FLTID > u8DmxId, DMX_DBGMSG(DMX_DBG_ERR, ULOGD("DMX", "[MAPI DMX][%06d] Bad Demux id %d\n", __LINE__, u8DmxId)));
...
DMX_FILTER_STATUS _MApi_DMX_Pvr_Eng_Open(DMX_PVR_ENG Eng ,DMX_Pvr_info* pPvrInfo)
{
    ...
    if(MDrv_TSP_REE_SendMBXMsg(E_DRVTSP_REE_TO_TEE_MBX_MSG_SET_PvrBuf, (void*)&Eng, (void*)&u8Opt, (void*)u32data) != DRVTSP_OK)
    {
        _DMX_PVR_RETURN(DMX_FILTER_STATUS_ERROR);
    }
    else
    {
        pPvrInfo->pPvrBuf0 = ((MS_PHY)u32data[0]) & 0xFFFFFFFFUL;
        pPvrInfo->PvrBufSize0 = u32data[1] >> 1;
...
DMX_FILTER_STATUS _MApi_DMX_Pvr_Eng_Pid_Open(DMX_PVR_ENG Eng ,MS_U32 Pid, MS_U8* pu8DmxId)
{
    ...
    *pu8DmxId =  FltId;
    ...
    DMX_PVRDBGMSG(DMX_DBG_PVR, ULOGD("DMX", "[%s] %d (%d, 0x%x, %lx\n", __FUNCTION__, __LINE__, (int)Eng, (unsigned int)Pid, (unsigned long)pu8DmxId));
    _DMX_PVR_ENTRY();
...
DMX_FILTER_STATUS _MApi_DMX_Pvr_Eng_WriteGet(DMX_PVR_ENG Eng ,MS_PHY* phyWrite)
{
    ...
    if(MDrv_TSP_REE_SendMBXMsg(E_DRVTSP_REE_TO_TEE_MBX_MSG_GET_PvrWPtr, (void*)&Eng, (void*)&u32Addr, 0) != DRVTSP_OK)
    {
        _DMX_PVR_RETURN(DMX_FILTER_STATUS_ERROR);
    }
    else
    {
        *phyWrite = (MS_PHY)u32Addr;
        _DMX_PVR_RETURN(DMX_FILTER_STATUS_OK);
    }
    #else
...
{
    DMX_DBGMSG(DMX_DBG_FUNC, ULOGD("DMX", "[%s](%d, 0x%08x, 0x%lx) %d\n", __FUNCTION__, (int)Eng, (unsigned int)Pid, (unsigned long)pu8DmxId, __LINE__));
...
DMX_FILTER_STATUS _MApi_DMX_Filein_Start(DMX_FILEIN_DST Dst, MS_PHY pBuf, MS_U32 u32BufSize)
{
    ...
    #ifdef SECURE_PVR_ENABLE
    MS_U8 u8Eng = 0;
    if(MDrv_TSP_REE_SendMBXMsg(E_DRVTSP_REE_TO_TEE_MBX_MSG_SET_FileinBuf, (void*)&u8Eng, (void*)(&pBuf), (void*)(&u32BufSize)) != DRVTSP_OK)
    {
        _DMX_RETURN(DMX_FILTER_STATUS_ERROR);
    }
    #else
...
DMX_FILTER_STATUS _MApi_DMX_Filein_Eng_Start(DMX_FILEIN_PATH ePath, DMX_FILEIN_DST Dst, MS_PHY pBuf, MS_U32 u32BufSize)
{
    ...
    switch (ePath)
    {
        ...
        case DMX_FILEIN_TSIF3:
            ...
            break;
        default:
            DMX_DBGMSG(DMX_DBG_ERR, ULOGE("DMX", "[MAPI DMX][%06d] Bad DMX_FILEIN_ENG %lu\n", __LINE__, (MS_U32)ePath));
            _DMX_RETURN(DMX_FILTER_STATUS_ERROR);
    }
    ...
    #endif
        default:
            DMX_DBGMSG(DMX_DBG_ERR, ULOGE("DMX", "[MAPI DMX][%06d] Bad DMX_FILEIN_DST %lu\n", __LINE__, (MS_U32)Dst));
            _DMX_RETURN(DMX_FILTER_STATUS_ERROR);
    }
...
DMX_FILTER_STATUS _MApi_DMX_Open_MultiFlt(DMX_FILTER_TYPE DmxFltType , MS_U8* pu8DmxId, MS_U8 u8TargetDmxId)
{
    ...
    ULOGD("DMX", "input Filter Type = %08lX \n",DmxFltType);

    DmxFltType = DmxFltType & (DMX_FILTER_TYPE)(~DMX_FILTER_FLT_MASK);
    _FLT_LIST_TYPE_GET(u8TargetDmxId,&TgtDmxFltType);

    ULOGD("DMX", "Get FlType %08lX \n",TgtDmxFltType);
    ...
    {
        ULOGE("DMX", "[MAPI DMX][%06d] MultiFlt Type mismatch %lu\n", __LINE__, FltId);
        _DMX_RETURN(DMX_FILTER_STATUS_ERROR);
    }
    ...
    DMX_ASSERT2(FltId< DMX_MAX_FLTID, DMX_DBGMSG(DMX_DBG_ERR, ULOGE("DMX", "[MAPI DMX][%06d] Bad FltId %lu\n", __LINE__, FltId)));

    ULOGD("DMX", "MultiFlt FltId = %ld \n",FltId);

    MDrv_TSP_PidFlt_SelSecFlt(0, FltId, BufId);

    ULOGD("DMX", "MultiFlt AttachBuffer flt %ld Buf %ld\n",FltId,BufId);
...
DMX_FILTER_STATUS _MApi_DMX_FQ_Configure(DMX_FQ_Cfg* pstFqCfg)
{
    ...
    if(E_DMX_FQ_CFG_SEL_SRCMUX == pstFqCfg->eCfgCmd)
    {
        if(pstFqCfg->bSet)
        {
            ...
            if(DRVFQ_OK != MDrv_FQ_SetMuxSwitch(pstFqCfg->u32FQEng,eFQSrcif))
                u32Ret = DMX_FILTER_STATUS_ERROR;
        }
        else
        {
            if(DRVFQ_OK != MDrv_FQ_GetMuxSwitch(pstFqCfg->u32FQEng,&eFQSrcif))
                u32Ret = DMX_FILTER_STATUS_ERROR;
...
DMX_FILTER_STATUS _MApi_DMX_CMD_Run(MS_U32 u32Cmd, MS_U32 u32Config, MS_U32 u32DataNum, void *pData)
{
    ...
    if(u32Cmd == DMX_CMD_SET_LIB_MODE)
    {
        _pdmx_res->_u32LibMode = u32Config;
    }
    else if(u32Cmd == DMX_CMD_SET_PESCPY_CONT_MODE)
    {
