...
MS_U32 Ioctl_GOP_GetCaps(void* pInstance,EN_GOP_CAPS eCapType, MS_U32* pRet, MS_U32 ret_size)
{
    ...
    switch(eCapType)
    {
        ...
        {
            *pRet = g_pGOPCtxLocal->pGopChipProperty->WordUnit;
            break;
        }
        ...
        case E_GOP_CAP_GOP_MUX:
            {
                if(pRet != NULL)
                {
                    PST_GOP_MUX_CAP pstDrvMuxCap = (PST_GOP_MUX_CAP)(void*)pRet;
                    if(Mdrv_GOP_GetMuxPriority(g_pGOPCtxLocal, (PST_DRV_GOP_MUX_CAP)pstDrvMuxCap))
                    {
                        pstDrvMuxCap->bNeedSetMUX1ToIP0 = g_pGOPCtxLocal->pGopChipProperty->bNeedSetMUX1ToIP0;
                        pstDrvMuxCap->bNeedSetMUX3ToIP0 = g_pGOPCtxLocal->pGopChipProperty->bNeedSetMUX3ToIP0;
                        pstDrvMuxCap->bNewMux = g_pGOPCtxLocal->pGopChipProperty->bNewMux;
                    }
                    else
                    {
                        enRet = GOP_API_FAIL;
