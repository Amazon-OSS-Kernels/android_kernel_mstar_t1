...
#if defined(CHIP_MONET) || defined(CHIP_MESSI) /*|| defined(CHIP_MASERATI)*/
...
static VDEC_EX_Result _VDEC_EX_NdecAllocator_GetBitstreamBufferProperty(MS_U8 u8Idx, VDEC_EX_NDecBufferProperty *pstProperty)
{
    ...
    if((bNDecode == TRUE) && (bMonoPoly == FALSE))
    {
...
static VDEC_EX_Result _VDEC_EX_NdecAllocator_GetFrameBufferProperty(MS_U8 u8Idx, VDEC_EX_NDecBufferProperty *pstProperty)
{
    ...
    MS_BOOL bCMADynamic = pVdecExContext->_Pre_Ctrl[u8Idx].bEnableDynamicCMA;
    MS_BOOL bNDecode    = _VDEC_EX_NdecAllocator_IsNDecode();
...
static MS_U16 _VDEC_EX_NdecAllocator_GetMainBlock(VDEC_EX_NDecAllocator *pNDecAllocator,
    MS_PHY phyBufAddr, MS_SIZE szBufSize, VDEC_EX_NDecBufferProperty *pstProperty, MS_BOOL bBuffer2)
{
    ...
    // If using CMA, get CMA phyBufAddr and szBufSize
    if(pstProperty->bCMA== TRUE)
    {
        MS_PHY phyCMAStartAddr = 0;
        MS_SIZE szCMAHeapLength = 0;
        MS_PHY phyUnUseSize = 0;
        VDEC_EX_CMA_POOL_SELECT eCMAPoolSelect = (bBuffer2 == FALSE) ? E_VDEC_EX_CMA_POOL_BUFFER_FB1 : E_VDEC_EX_CMA_POOL_BUFFER_FB2;
        VDEC_EX_Result eRet = _VDEC_EX_NdecAllocator_CMAInit(eCMAPoolSelect, phyBufAddr, &phyCMAStartAddr, &szCMAHeapLength, &phyUnUseSize);
        if(eRet != E_VDEC_EX_OK)
        {
        ...
        if(pstProperty->bNDecode == FALSE && bBuffer2 == FALSE)
        {
            ...
            if(szBufSize == 0 || szBufSize > szAlloclength)
            {
                szBufSize = szAlloclength;
            }

            phyBufAddr = phyCMAStartAddr - phyUnUseSize;
        }
        else
        {
            ...
            phyBufAddr = phyCMAStartAddr - phyUnUseSize;
            szBufSize = szCMAHeapLength - phyUnUseSize;
        }
    }
    ...
    stCriterion.phyAddr = phyBufAddr;
    stCriterion.szSize = szBufSize;
    stCriterion.u32Alignment = 0;

    ALLOCATOR_PRINTF_DBG("[ALLOC][DBG][%s][%d] stCriterion.phyAddr=0x%llx, szSize=0x%x\n", __FUNCTION__, __LINE__, phyBufAddr, szBufSize);
...
static MS_U16 _VDEC_EX_NdecAllocator_GetSubBlock(VDEC_EX_Allocator_Block *pstBlockMain,
    VDEC_EX_NDecBufferProperty *pstProperty, MS_BOOL bBuffer2)
{
    ...
    if(pstProperty->bNDecode == TRUE)
    {
        ...
        ALLOCATOR_PRINTF_DBG("[ALLOC][DBG][%s][%d] eTaskSpec=0x%x, stCriterion.szSize=0x%x\n", __FUNCTION__, __LINE__,
            pstProperty->eTaskSpec, stCriterion.szSize);
...
static VDEC_EX_Result _VDEC_EX_NdecAllocator_GetBitstreamBufferTotalAddrSize(MS_PHY *pphyBSTotalAddrStart, MS_SIZE *pszBSToltalSize)
{
    ...
    MS_PHY phyBSTotalAddrStart;
    MS_PHY phyBSTotalAddrEnd;
    MS_SIZE szBSToltalSize;
    MS_BOOL bFind = FALSE;
    int i;
...
static VDEC_EX_Result _VDEC_EX_NdecAllocator_MallocBitstreamBuffer(
    MS_U8 u8Idx, MS_PHY phyBufAddr, MS_SIZE szBufSize, VDEC_EX_Allocator_Block **ppstBlockResult)
{
    VDEC_EX_Result eRet = E_VDEC_EX_FAIL;
    VDEC_EX_NDecAllocator *pstNDecAllocator = &(pVdecExContext->stNDecAllocator);
    ...
    _VDEC_EX_NdecAllocator_GetBitstreamBufferProperty(u8Idx, &stBufProperty);
    ...
    u16BlockIdMain = _VDEC_EX_NdecAllocator_GetMainBlock(pstNDecAllocator, phyBufAddr, szBufSize, &stBufProperty, FALSE);
    if(_VDEC_EX_Allocator_BlockIsValidId(u16BlockIdMain) == FALSE)
    {
    ...
    else
    {
        ...
        u16BlockIdSub = _VDEC_EX_NdecAllocator_GetSubBlock(pstBlockMain, &stBufProperty, FALSE);
        if(_VDEC_EX_Allocator_BlockIsValidId(u16BlockIdSub) == FALSE)
        {
...
static VDEC_EX_Result _VDEC_EX_NdecAllocator_MallocFrameBuffer(
    MS_U8 u8Idx, MS_BOOL bFrameBuf2, MS_PHY phyBufAddr, MS_SIZE szBufSize, VDEC_EX_Allocator_Block **ppstBlockResult)
{
    VDEC_EX_Result eRet = E_VDEC_EX_FAIL;
    VDEC_EX_NDecAllocator *pstNDecAllocator = &(pVdecExContext->stNDecAllocator);
    VDEC_EX_NDecBufferProperty stBufProperty;
    ...
    u16BlockIdMain = _VDEC_EX_NdecAllocator_GetMainBlock(pstNDecAllocator, phyBufAddr, szBufSize, &stBufProperty, bFrameBuf2);
    if(_VDEC_EX_Allocator_BlockIsValidId(u16BlockIdMain) == FALSE)
    {
        ALLOCATOR_PRINTF_ERR("[ALLOC][ERR][%s][%d] GetMainBlock failed\n", __FUNCTION__, __LINE__);
        goto exit;
    ...
    else
    {
        ...
        u16BlockIdSub = _VDEC_EX_NdecAllocator_GetSubBlock(pstBlockMain, &stBufProperty, bFrameBuf2);
        if(_VDEC_EX_Allocator_BlockIsValidId(u16BlockIdSub) == FALSE)
        {
            ALLOCATOR_PRINTF_ERR("[ALLOC][ERR][%s][%d] GetSubBlock failed\n", __FUNCTION__, __LINE__);
            goto exit;
    ...
    {
        VDEC_EX_CMA_POOL_SELECT eCMAPoolSelect = (bFrameBuf2 == FALSE) ? E_VDEC_EX_CMA_POOL_BUFFER_FB1 : E_VDEC_EX_CMA_POOL_BUFFER_FB2;
...
static VDEC_EX_Result _MApi_VDEC_EX_GetDVSupportProfiles(void *param)
{
    ...
    if(pDV_Info->eDVCmdType == E_VDEC_EX_GET_DV_SUPPORT_PROFILE)
    {
...
static VDEC_EX_Result _MApi_VDEC_EX_GetDVSupportHighestLevel(void *param)
{
    ...
    if(pDV_Info->eDVCmdType == E_VDEC_EX_GET_DV_SUPPORT_LEVEL)
    {
        MS_U32 u32DVLevel;
...
static VDEC_EX_Result _VDEC_Init_Share_Mem(VDEC_EX_Stream eStreamType)
{
                                          ...
                                          MSOS_SHM_QUERY))
    {
        ...
        else
        {
            ...
            _VDEC_EX_Context_Init();
#if defined(CHIP_MONACO) || defined(CHIP_CLIPPERS) || defined(CHIP_MUJI) || defined(CHIP_MONET) || defined(CHIP_MANHATTAN) || defined(CHIP_MESSI) || defined(CHIP_MACAN) || defined(CHIP_MASERATI)
...
VDEC_EX_Result _MApi_VDEC_EX_V2_SetEvent_MultiCallback(VDEC_StreamId *pStreamId, VDEC_EX_CB_TYPE cb_type, MS_U32 u32EventFlag, VDEC_EX_EventCb pfn, void *param)
{
    ...
    //_VDEC_EX_MutexLock(u8Idx);
    if (u32EventFlag & E_VDEC_EX_EVENT_DEC_CMA_ACTION)
    {
...
VDEC_EX_Result _MApi_VDEC_EX_V2_GetErrCode(VDEC_StreamId *pStreamId,VDEC_EX_ErrCode* eErrCode)
{
    ...
    if(pVdecExContext->s32ResViolationIdx == u8Idx)
    {
...
VDEC_EX_Result _MApi_VDEC_EX_V2_SetPowerState(EN_POWER_MODE u16PowerState)
{
    ...
    MS_U32 i;
    MS_U8 u8Idx;
    VDEC_EX_CodecType codec_type[2];
...
static VDEC_EX_Result _MApi_VDEC_EX_SetDVInfo(VDEC_StreamId *pStreamId,VDEC_EX_DV_Info *pDV_Info)
{
    ...
    MS_U32 u32Id = _VDEC_GET_DRV_ID(pStreamId->u32Id);
    MS_U16 u16Para;
    u16Para = (((pDV_Info->eDVProfile << 8) & 0xff00) | (pDV_Info->eDVLevel & 0x00ff));
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        case E_VDEC_EX_DECODER_MVD:
            break;
        case E_VDEC_EX_DECODER_HVD:
            _HVD_RET_HANDLE(MDrv_HVD_EX_SetDVInfo(u32Id, (MS_U32)u16Para));
            break;
        case E_VDEC_EX_DECODER_MJPEG:
        default:
            eRet = E_VDEC_EX_RET_ILLEGAL_ACCESS;
...
static VDEC_EX_Result _MApi_VDEC_EX_V2_SetDVXCShmAddr(VDEC_StreamId *pStreamId, MS_PHY u32DVXCShmAddr)
{
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        case E_VDEC_EX_DECODER_MVD:
            ...
            break;
        case E_VDEC_EX_DECODER_HVD:
            _HVD_RET_TO_VDEC_RESULT(ret, MDrv_HVD_EX_SetDVXCShmAddr(u32Id, pVdecExContext->_Attr[u8Idx].vdecExInitParam.SysConfig.u32CodeBufAddr,
                                    u32DVXCShmAddr));
            break;
        case E_VDEC_EX_DECODER_MJPEG:
            ret = E_VDEC_EX_RET_UNSUPPORTED;
            ...
            case E_VDEC_EX_DECODER_HVD:
            {
                ...
                if (E_VDEC_EX_CODEC_TYPE_MVC == eCodecType)
                {
                    ...
                    else
                    {
                        ...
                        || (E_VDEC_EX_CODEC_TYPE_HEVC == eCodecType)
                        || (E_VDEC_EX_CODEC_TYPE_HEVC_DV == eCodecType))   //EVD support one esb now
                {
                    for (i = 0; i < VDEC_MAX_SUPPORT_STREAM_NUM; i++)
                    {
                        if (((E_VDEC_EX_CODEC_TYPE_VP8 == pVdecExContext->_Attr[i].eCodecType) && (E_VDEC_EX_CODEC_TYPE_VP8 == eCodecType))
                                ...
                                || ((E_VDEC_EX_CODEC_TYPE_HEVC_DV == pVdecExContext->_Attr[i].eCodecType) && (E_VDEC_EX_CODEC_TYPE_HEVC_DV == eCodecType)))
                        {
                            _VDEC_EX_API_MutexUnlock();
                            return E_VDEC_EX_FAIL;
                        }
...
//#endif
    ...
    {
        VDEC_EX_Allocator_Block *pstBlockFB = NULL, *pstBlockBS = NULL;
        VDEC_EX_Result eRet;

        _VDEC_EX_API_MutexLock();

        eRet = _VDEC_EX_NdecAllocator_MallocFrameBuffer(u8Idx, FALSE,
            pVdecExContext->_Attr[u8Idx].vdecExInitParam.SysConfig.u32FrameBufAddr,
            pVdecExContext->_Attr[u8Idx].vdecExInitParam.SysConfig.u32FrameBufSize, &pstBlockFB);
        if(eRet != E_VDEC_EX_OK)
        {
            _MApi_VDEC_EX_ReturnCMABuffer(pStreamId);
            _VDEC_EX_API_MutexUnlock();
        ...
        pVdecExContext->_Attr[u8Idx].vdecExInitParam.SysConfig.u32FrameBufAddr = pstBlockFB->stBlockInfo.phyAddr;
        pVdecExContext->_Attr[u8Idx].vdecExInitParam.SysConfig.u32FrameBufSize = pstBlockFB->stBlockInfo.szSize;

        eRet = _VDEC_EX_NdecAllocator_MallocBitstreamBuffer(u8Idx,
            pVdecExContext->_Attr[u8Idx].vdecExInitParam.SysConfig.u32BitstreamBufAddr,
            pVdecExContext->_Attr[u8Idx].vdecExInitParam.SysConfig.u32BitstreamBufSize, &pstBlockBS);
        if(eRet != E_VDEC_EX_OK)
        {
            _MApi_VDEC_EX_ReturnCMABuffer(pStreamId);
            _VDEC_EX_API_MutexUnlock();
    ...
    {
#if 0  // NDec doesn't use frame buffer2
            ...
            pVdecExContext->_Attr[u8Idx].eSrcMode != E_VDEC_EX_SRC_MODE_DTV)
        {
            ...
            eRet = _VDEC_EX_NdecAllocator_MallocFrameBuffer(u8Idx, TRUE, 0, 0, &pstBlockFB2);
            if(eRet != E_VDEC_EX_OK)
            {
                _MApi_VDEC_EX_ReturnCMABuffer(pStreamId);
    ...
    if(bNDecAllocator == FALSE)
    {
             ...
             E_VDEC_EX_CODEC_TYPE_H264 == pVdecExContext->_Attr[u8Idx].eCodecType))
        {
            ...
            VDEC_PRINT("[VDEC] Set max CMA size=0x%x size2=0x%x \n", VDEC_FRMBUFF_CMA_SIZE, pVdecExContext->cmaInitParam[1].heap_length);
    ...
    else
    {
        if(pVdecExContext->bCMAUsed && pVdecExContext->_Pre_Ctrl[u8Idx].bEnableDynamicCMA &&
           (E_VDEC_EX_CODEC_TYPE_HEVC == pVdecExContext->_Attr[u8Idx].eCodecType ||
            ...
            E_VDEC_EX_CODEC_TYPE_H264 == pVdecExContext->_Attr[u8Idx].eCodecType))
        {
            MS_SIZE szFrameBuf1 = 0, szFrameBuf2 = 0;
            if(_VDEC_EX_NdecAllocator_GetFrameBufferSize(u8Idx, &szFrameBuf1, &szFrameBuf2) != E_VDEC_EX_OK)
            {
                _MApi_VDEC_EX_ReturnCMABuffer(pStreamId);
                ALLOCATOR_PRINTF_ERR("[ALLOC][ERR][%s][%d][%d] _VDEC_EX_NdecAllocator_GetFrameBufferSize fail\n", __FUNCTION__, __LINE__, u8Idx);
                pVdecExContext->_Attr[u8Idx].eStage = E_VDEC_EX_STAGE_STOP;
                return E_VDEC_EX_FAIL;
            }

            MDrv_HVD_EX_SetMaxCMASize(u32Id, szFrameBuf1, szFrameBuf2);
            VPRINTF("[ALLOC][DBG][%s][%d][%d] szFrameBuf1=0x%x, szFrameBuf2=0x%x\n", __FUNCTION__, __LINE__,
                u8Idx, szFrameBuf1, szFrameBuf2);
        }
    }
...
VDEC_EX_Result MApi_VDEC_EX_V2_SetControl(VDEC_StreamId* pStreamId, VDEC_EX_V2_User_Cmd cmd_id, void** param,MS_BOOL IsResume)
{
    ...
    switch (cmd_id)
    {
        ...
        case E_VDEC_EX_V2_USER_CMD_SET_ENABLE_HDR:
            ret = _MApi_VDEC_EX_V2_SetEnalbeHDR(pStreamId, (MS_BOOL)(*((MS_U32*)(param[0]))));
            break;
#ifdef VDEC_CAP_DV_OTT_API
...
MS_VIRT VPU_CodeAddr = 0;
MS_U32 VPU_CodeSize = 0;
MS_VIRT Bitstream_StartAddr_Main = 0;
MS_U32 Bitstream_Size_Main = 0;
MS_VIRT Bitstream_StartAddr_Sub = 0;
MS_U32 Bitstream_Size_Sub = 0;
MS_VIRT Framebuffer_StartAddr_Main = 0;
MS_U32 Framebuffer_Size_Main = 0;
MS_VIRT Framebuffer_StartAddr_Sub = 0;
MS_U32 Framebuffer_Size_Sub = 0;
MS_VIRT VPU_SHMAddr = 0;
MS_U32 VPU_SHMSize = 0; // need at least 256k bytes
MS_VIRT MIU1_StartAddr = 0;
