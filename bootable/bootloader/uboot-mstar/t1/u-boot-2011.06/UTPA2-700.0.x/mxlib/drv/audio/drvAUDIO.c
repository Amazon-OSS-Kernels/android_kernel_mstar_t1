...
MS_S32 MDrv_AUDIO_PCM_converter(void *pDstData, MS_U32 u32DstChannel, MS_U32 u32DstSize, const void *pSrcData, AUDIO_PCM_INFO_t *pSrcPCMInfo)
{
    ...
    u32BigEndien = pSrcPCMInfo->u32BigEndien;
    if ((u32BigEndien != TRUE) && (u32BigEndien != FALSE))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! u32BigEndien(%u) is invalid!\n", __FUNCTION__, (unsigned int)u32BigEndien);
        return -A_EINVAL;
    }

    /* TODO: convert endien here */
    /* TODO: convert bit width here */
    /* TODO: convert channel here */
...
void MDrv_AUDIO_PCM_Init(void)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN, "%s: Init PCM device\n", __FUNCTION__);
    ...
    for (s32DeviceId = 0; s32DeviceId < AUDIO_PCM_MAXNUM; s32DeviceId++)
    {
        ...
        g_PCM[s32DeviceId]->tPcmInfo = &g_AudioVars2->g_PcmInfo[s32DeviceId];
    }
...
void MDrv_AUDIO_PCM_DeInit(void)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN, "%s: DeInit PCM device\n", __FUNCTION__);
    ...
    for (s32DeviceId = 0; s32DeviceId < AUDIO_PCM_MAXNUM; s32DeviceId++)
    {
        ...
        g_PCM[s32DeviceId]->tPcmInfo = NULL;
    }
...
MS_S32 MDrv_AUDIO_PCM_Open(void *pData)
{
    ...
    MS_S32 s32DeviceId = 0;
    MS_S32 s32Ret = 0;
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN, "%s: Open PCM device\n", __FUNCTION__);
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! The version of PCM Info mismatch(%u, %d)!\n", __FUNCTION__, (unsigned int)pUserPcmInfo->u32StructVersion, AUDIO_PCM_INFO_VERSION);
        return -A_EINVAL;
    }
    ...
    for (s32DeviceId = 0; s32DeviceId < AUDIO_PCM_MAXNUM; s32DeviceId++)
    {
        ...
        if (pPCM->tPcmInfo->u8ConnectFlag == TRUE)
        {
            continue;
        }
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN, "%s: return device-id %d\n", __FUNCTION__, (int)s32DeviceId);
...
MS_S32 MDrv_AUDIO_PCM_Close(MS_S32 s32DeviceId)
{
    AUDIO_PCM_t *pPCM = NULL;
    MS_S32 s32Ret = 0;
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN, "%s: Close PCM device-id %d\n", __FUNCTION__, (int)s32DeviceId);

    DRVAUDIO_CHECK_SHM_INIT;

    if (s32DeviceId >= AUDIO_PCM_MAXNUM)
    {
    ...
    if (pPCM->tPcmInfo->u8ConnectFlag == FALSE)
    {
...
MS_S32 MDrv_AUDIO_PCM_Start(MS_S32 s32DeviceId)
{
    AUDIO_PCM_t *pPCM = NULL;
    MS_S32 s32Ret = 0;
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN, "%s: Start PCM device-id %d\n", __FUNCTION__, (int)s32DeviceId);
    ...
    if (pPCM->tPcmInfo->u8StartFlag == FALSE)
    {
...
MS_S32 MDrv_AUDIO_PCM_Stop(MS_S32 s32DeviceId)
{
    AUDIO_PCM_t *pPCM = NULL;
    MS_S32 s32Ret = 0;
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN, "%s: Stop PCM device-id %d\n", __FUNCTION__, (int)s32DeviceId);
...
MS_S32 MDrv_AUDIO_PCM_Set(MS_S32 s32DeviceId, MS_U32 u32Cmd, const void *pData)
{
    ...
    AUDIO_PCM_INFO_t *pUserPcmInfo = NULL;
    MS_S32 s32Ret = 0;
    //AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN, "%s: Set command %u to PCM device-id %d\n", __FUNCTION__, u32Cmd, s32DeviceId);
...
MS_S32 MDrv_AUDIO_PCM_Get(MS_S32 s32DeviceId, MS_U32 u32Cmd, void *pData)
{
    ...
    AUDIO_PCM_INFO_t *pUserPcmInfo = NULL;
    MS_S32 s32Ret = 0;
    //AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN, "%s: Get command %u from PCM device-id %d\n", __FUNCTION__, u32Cmd, s32DeviceId);
...
MS_U32 MDrv_AUDIO_PCM_Read(MS_S32 s32DeviceId, void *pBuf, MS_U32 u32Size)
{
    ...
    MS_U32 u32SizeToReadTmp = 0;
    MS_U32 u32BufferLevel = 0;
    MS_U32 u32ReadSize = 0;
    MS_U32 u32Loop = 0;
    //AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN, "%s: Read %u bytes from PCM device-id %d\n", __FUNCTION__, u32Size, s32DeviceId);
    ...
    if (pPCM->tPcmInfo->u8NonBlockingFlag == TRUE)
    {
        if (pPCM->tPcmOps.get == NULL)
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! s32DeviceId(%d) doesn't register get()!\n", __FUNCTION__, (int)s32DeviceId);
    ...
    u32SizeToReadTmp = u32SizeToRead;

    do
    {
        u32ReadSize = pPCM->tPcmOps.read((void *)pBufTmp, u32SizeToReadTmp);
        if (u32ReadSize == 0)
        {
            u32Loop++;
            if (u32Loop > 250)
        ...
        else if (u32ReadSize > u32SizeToReadTmp)
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! s32DeviceId(%d) get incorrect PCM size(%u, %u)!\n", __FUNCTION__, (int)s32DeviceId, (unsigned int)u32ReadSize, (unsigned int)u32SizeToRead);
            break;
        }
        else
        {
        ...
        pBufTmp += u32ReadSize;
        u32SizeToReadTmp -= u32ReadSize;
    } while (u32SizeToReadTmp > 0);
...
MS_U32 MDrv_AUDIO_PCM_Write(MS_S32 s32DeviceId, const void *pBuf, MS_U32 u32Size)
{
    AUDIO_PCM_t *pPCM = NULL;
    MS_U8 *pPcmBuf = NULL;
    MS_U8 *pPcmBufTmp = NULL;
    MS_U32 u32SizeToWrite = 0;
    MS_U32 u32SizeToWriteTmp = 0;
    MS_U32 u32BufferLevel = 0;
    ...
    MS_U32 u32Loop = 0;
    MS_S32 s32Ret = 0;
    //AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN, "%s: Write %u bytes to PCM device-id %d\n", __FUNCTION__, u32Size, s32DeviceId);
    ...
    if (pPCM->tPcmInfo->u8MultiChFlag == TRUE)
    {
    ...
    u32SizeToWrite = (u32Size / pPCM->tPcmInfo->u32Channel) * u32TargetChannel;

    if (pPCM->tPcmInfo->u8NonBlockingFlag == TRUE)
    {
        u32BufferSize = (pPCM->tPcmInfo->u32BufferDuration * pPCM->tPcmInfo->u32SampleRate * u32TargetChannel *  (pPCM->tPcmInfo->u32BitWidth / 8)) / 1000;
        ...
        u32SizeToWrite = (u32SizeToWrite > u32AvailSize) ? u32AvailSize : u32SizeToWrite;
    }
#ifdef MSOS_TYPE_LINUX_KERNEL
    pPcmBuf = (MS_U8 *)vmalloc(u32SizeToWrite);	//stanley
#else
    pPcmBuf = (MS_U8 *)malloc(u32SizeToWrite);
#endif

    if (pPcmBuf == NULL)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! s32DeviceId(%d) can't allocate %u bytes for PCM buffer!\n", __FUNCTION__, (int)s32DeviceId, (unsigned int)u32SizeToWrite);
        return -A_ENOMEM;
    }
    memset((void *)pPcmBuf, 0x00, u32SizeToWrite);

    /* convert PCM data here */
    s32Ret = MDrv_AUDIO_PCM_converter((void *)pPcmBuf, u32TargetChannel, u32SizeToWrite, pBuf, pPCM->tPcmInfo);
    if (s32Ret < 0)
    {
    ...
    pPcmBufTmp = pPcmBuf;
    u32SizeToWriteTmp = u32SizeToWrite;

    do
    {
        u32WriteSize = pPCM->tPcmOps.write(pPcmBufTmp, u32SizeToWriteTmp);
        if (u32WriteSize == 0)
        {
            u32Loop++;
            if (u32Loop > 250)
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: s32DeviceId(%d) write PCM timeout!\n", __FUNCTION__, (int)s32DeviceId);
                break;
            }

            MsOS_DelayTask(2);
        }
        else if (u32WriteSize > u32SizeToWriteTmp)
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! s32DeviceId(%d) get incorrect PCM size(%u, %u)!\n", __FUNCTION__, (int)s32DeviceId, (unsigned int)u32WriteSize, (unsigned int)u32SizeToWrite);
            break;
        }
        else
        {
        ...
        pPcmBufTmp += u32WriteSize;
        u32SizeToWriteTmp -= u32WriteSize;
    } while (u32SizeToWriteTmp > 0);

#ifdef MSOS_TYPE_LINUX_KERNEL
    vfree(pPcmBuf);	//stanley
#else
    free(pPcmBuf);
#endif
...
MS_S32 MDrv_AUDIO_PCM_Flush(MS_S32 s32DeviceId)
{
    AUDIO_PCM_t *pPCM = NULL;
    MS_S32 s32Ret = 0;
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN, "%s: Flush PCM device-id %d\n", __FUNCTION__, (int)s32DeviceId);
