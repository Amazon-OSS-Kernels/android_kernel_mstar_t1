...
void MDrv_AUDIO_CheckHashkey(void)
{
    ...
    else
    {
        ...
        //AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "Hash Key Check Dolby MS12 C profile Fail, no Dolby MS12 C profile license!!\n");
    }
...
MS_BOOL MDrv_ADVSOUND_ProcessEnable(ADVSND_TYPE type)
{
    ...
    if (!ADVSND_TYPE_Compare(type, MDrv_CurrAdvSndtype))
    {
        ...
        {
            if (MDrv_AUTH_IPCheck(AU_DVB_AUTHBIT_DOLBY_MS12_D) || MDrv_AUTH_IPCheck(AU_DVB_AUTHBIT_DOLBY_MS12_LC))
            {
...
{
    if (MDrv_CurrAdvSndtype == (ADVSND_TYPE)SRS_TSXT)
    {
    ...
    else if (MDrv_CurrAdvSndtype == (ADVSND_TYPE)SRS_TSHD)
    {
        if (ADVFUNC_Compare(proc, SRS_THEATERSOUND_3D_EN) ||
	          ADVFUNC_Compare(proc, SRS_THEATERSOUND_CC3D_EN) ||
	          ADVFUNC_Compare(proc, SRS_TSHD_TRUBASS) ||
    ...
    else if ((MDrv_CurrAdvSndtype == (ADVSND_TYPE)DTS_STUDIOSOUND_3D) || (MDrv_CurrAdvSndtype == (ADVSND_TYPE)SRS_THEATERSOUND))
    {
        if (ADVFUNC_Compare(proc, SRS_THEATERSOUND_3D_EN) ||
            ADVFUNC_Compare(proc, SRS_THEATERSOUND_TSHD) ||
            ADVFUNC_Compare(proc, SRS_THEATERSOUND_TRUBASS) ||
    ...
    else if (MDrv_CurrAdvSndtype == (ADVSND_TYPE)SRS_PURESND)
    {
    ...
    else if (MDrv_CurrAdvSndtype == (ADVSND_TYPE)BONGIOVI_DPS)
    {
    ...
    else if (MDrv_CurrAdvSndtype == (ADVSND_TYPE)DOLBY_DAP)
    {
