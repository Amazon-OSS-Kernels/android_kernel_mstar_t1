        ...
        case MDrv_CMD_AESDMA_Start:
            {
                ...
                if(FALSE == MsOS_SHM_GetId((MS_U8*)"AESDMA driver", sizeof(AESDMA_RESOURCE_SHARED), &u32ShmId, &phyAddr, &u32BufSize, MSOS_SHM_QUERY))
                {
                    if(FALSE == MsOS_SHM_GetId((MS_U8*)"AESDMA driver", sizeof(AESDMA_RESOURCE_SHARED), &u32ShmId, &phyAddr, &u32BufSize, MSOS_SHM_CREATE))
                    {
                        printf("[AESDMA INFO]create Resouce shared memory fail\n");
                        return UTOPIA_STATUS_ERR_NOMEM;
                    }
                    memset((MS_U8 *)MsOS_PA2KSEG1(phyAddr), 0, sizeof(AESDMA_RESOURCE_SHARED));
                }

                psAesResourceShared = (AESDMA_RESOURCE_SHARED*)&phyAddr;
...
//                (psAesInstPri->fpAESDMAReset)();
                    ...
                    (psAesInstPri->eMode == E_DRVAESDMA_CIPHER_CTR))
                {
                    ...
                    if(psAesInstPri->eMode != E_DRVAESDMA_CIPHER_CTR)
                    {
                        for(i = 0; i < 16; i++)
                        {
                            psAesInstPri->InitVector[i] = *(MS_U8 *)((MS_VIRT)FileinVirtAddr + psAesInstPri->u32FileInNum-1-i);
        ...
        case MDrv_CMD_AESDMA_IsFinished:
            {
                ...
                else
                {
                    ...
                    psAesResourceShared = (AESDMA_RESOURCE_SHARED*)phyAddr;
#else
                    psAesResourceShared = (AESDMA_RESOURCE_SHARED*)(MS_U32)phyAddr;
#endif
                    ...
                    && psAesInstPri->bDescrypt == FALSE)
                {
                    ...
                    for(i = 0; i < 16; i++)
                    {
                        psAesInstPri->InitVector[i] = *(MS_U8 *)((MS_VIRT)FileOutStartVirtAddr+psAesInstPri->u32FileInNum-1-i);
                        MS_DEBUG_MSG(printf("[AESDMA INFO]Init Vector[%d] = 0x%x\n",(int)i, psAesInstPri->InitVector[i]));
                    }
                }
        ...
        case MDrv_CMD_RSA_Calculate:
            {
                ...
                if(FALSE == MsOS_SHM_GetId((MS_U8*)"RSA driver", sizeof(RSA_RESOURCE_SHARED), &u32ShmId, &phyAddr, &u32BufSize, MSOS_SHM_QUERY))
                {
                    if(FALSE == MsOS_SHM_GetId((MS_U8*)"RSA driver", sizeof(RSA_RESOURCE_SHARED), &u32ShmId, &phyAddr, &u32BufSize, MSOS_SHM_CREATE))
                    {
                        printf("[RSA INFO]create Resouce shared memory fail\n");
                        return UTOPIA_STATUS_ERR_NOMEM;
                    }
                    memset((MS_U8 *)MsOS_PA2KSEG1(phyAddr), 0, sizeof(RSA_RESOURCE_SHARED));
                }
