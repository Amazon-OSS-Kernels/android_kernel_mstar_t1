...
#if DMD_DTMB_UTOPIA_EN || DMD_DTMB_UTOPIA2_EN
static MS_BOOL _DEFAULT_EXT_IOCTL_CMD(DMD_DTMB_HAL_COMMAND eCmd, void *pArgs)
{
...
MS_BOOL HAL_EXTERN_DTMB_IOCTL_CMD(DMD_DTMB_HAL_COMMAND eCmd, void *pArgs)  __attribute__((weak, alias ("_DEFAULT_EXT_IOCTL_CMD")));
#else
...
static DMD_DTMB_LOCK_STATUS _DTMB_CheckLock(void)
{
    ...
    {
        #if DMD_DTMB_UTOPIA_EN || DMD_DTMB_UTOPIA2_EN
        if ((pInfo->u32DTMBLockStatus & DMD_DTMB_LOCK_DTMB_FEC_LOCK) && ((MsOS_GetSystemTime()-pInfo->u32DTMBFECLockTime) < 100))
        #else
        if ((pInfo->u32DTMBLockStatus & DMD_DTMB_LOCK_DTMB_FEC_LOCK) && ((pInit->GetSystemTimeMS()-pInfo->u32DTMBFECLockTime) < 100))
        #endif
        {
            return DMD_DTMB_LOCK;
        }
...
#endif
{
    DMD_DBG(printf("_MDrv_DMD_DTMB_MD_Init() enter \n"));
    ...
    if (1)//(sizeof(DMD_DTMB_InitData) == u32InitDataLen) //data length always different on kernel mode, wayne@20151028
    {
        DMD_DBG(printf("memcpy(&(pRes->sDMD_DTMB_InitData) enter \n"));
        ...
        DMD_DBG(printf("memcpy(&(pRes->sDMD_DTMB_InitData) exit \n"));
    }
    else
    {
        DMD_DBG(printf("MDrv_DMD_DTMB_Init input data structure incorrect 1\n"));
        return FALSE;
    }
    ...
    {
        pRes->sDMD_DTMB_PriData.HAL_DMD_DTMB_IOCTL_CMD = HAL_INTERN_DTMB_IOCTL_CMD;
    }
    #else
    pRes->sDMD_DTMB_PriData.HAL_DMD_DTMB_IOCTL_CMD = HAL_EXTERN_DTMB_IOCTL_CMD;
    #endif
...
#endif
{
    ...
    DMD_DBG(printf("_MDrv_DMD_DTMB_MD_Exit(), enter\n"));
    ...
    DMD_DBG(printf("_MDrv_DMD_DTMB_MD_Exit(), exit\n"));
...
MS_BOOL MDrv_DMD_DTMB_MD_Init(MS_U8 id, DMD_DTMB_InitData *pDMD_DTMB_InitData, MS_U32 u32InitDataLen)
{
    ...
    DMD_DBG(printf("MDrv_DMD_DTMB_MD_Init()\n"));

    if (_u32DTMBopen == 0)
    {
        if (UtopiaOpen(MODULE_DTMB /*| KERNEL_MODE*/, &_ppDTMBInstant, 0, pAttribte) == UTOPIA_STATUS_SUCCESS)
        {
            _u32DTMBopen = 1;
        }
        else
...
MS_BOOL MDrv_DMD_DTMB_MD_Exit(MS_U8 id)
{
    ...
    DMD_DBG(printf("MDrv_DMD_DTMB_MD_Exit()\n"));
