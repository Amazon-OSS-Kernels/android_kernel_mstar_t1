...
static DMD_DVBS_DbgLv _u8DMD_DVBS_DbgLevel=DMD_DVBS_DBGLV_NONE;
...
static MS_U32 u32DMD_DVBS_PrevScanTime=0, u32DMD_DVBS_ScanCount=0;
...
MS_BOOL DMD_DVBS_Init(DMD_DVBS_InitData_Transform *pDMD_DVBS_InitData, MS_U32 u32InitDataLen)
{
    ...
    {
        DMD_DBG(printf("MDrv_DMD_DVBS_Init more than once\n"));
        return FALSE;
    }

    if (NULL == strncpy(pDMD_DVBS_MutexString,"Mutex DMD DVBS",16))
    {
        DMD_DBG(printf("MDrv_DMD_DVBS_Init strcpy Fail\n"));
        return FALSE;
    }
    _s32DMD_DVBS_Mutex = MsOS_CreateMutex(E_MSOS_FIFO, pDMD_DVBS_MutexString, MSOS_PROCESS_SHARED);
    if ( _s32DMD_DVBS_Mutex == -1)
    {
        DMD_DBG(printf("MDrv_DMD_DVBS_Init Create Mutex Fail\n"));
        return FALSE;
    }
    ...
    {
        DMD_DBG(printf("MDrv_DMD_DVBS_Init input data structure incorrect\n"));
        return FALSE;
    }
    ...
    if (_sDMD_DVBS_InitData.u8DMD_DVBS_DSPRegInitExt != NULL)
    {
        ...
        {
            printf("u8DMD_DVBS_DSPRegInitExt Error\n");
        }
    }
    else
    {
...
const DMD_DVBS_Info* DMD_DVBS_GetInfo(void)
{
    ...
    if (_u8DMD_DVBS_DbgLevel >= DMD_DVBS_DBGLV_INFO)
    {
        printf("MDrv_DMD_DVBS_GetInfo\n");
    }
    #endif
//for DBG
#if 1
	MDrv_SYS_DMD_VD_MBX_ReadReg(FEC_REG_BASE + 0xE0, &state);
	printf(" @INTERN_DVBS_GetLock FSM 0x%x\n",state);
...
MS_BOOL DMD_DVBS_GetLibVer(const MSIF_Version **ppVersion)
{
    ...
    if (_u8DMD_DVBS_DbgLevel >= DMD_DVBS_DBGLV_INFO)
    {
        printf("MDrv_DMD_DVBS_GetLibVer\n");
    }
    #endif
    DMD_LOCK();
    if (!ppVersion)
...
MS_BOOL DMD_DVBS_GetDSPReg(MS_U16 u16Addr, MS_U8 *pu8Data)
{
    ...
    if (_u8DMD_DVBS_DbgLevel >= DMD_DVBS_DBGLV_INFO)
    {
        printf("MDrv_DMD_DVBS_GetDSPReg %x %x\n", u16Addr, *pu8Data);
    }
    #endif

    return bRet;
#else
  	printf("Not Support function: MDrv_DMD_DVBS_GetDSPReg\n");
	return FALSE;
#endif
}
...
MS_BOOL DMD_DVBS_SetDSPReg(MS_U16 u16Addr, MS_U8 pu8Data)
{
    ...
    if (_u8DMD_DVBS_DbgLevel >= DMD_DVBS_DBGLV_INFO)
    {
        printf("MDrv_DMD_DVBS_SetDSPReg %x %x\n", u16Addr, pu8Data);
    }
    #endif

    return bRet;
#else
  	printf("Not Support function: MDrv_DMD_DVBS_SetDSPReg\n");
	return FALSE;
#endif
}
...
MS_BOOL DMD_DVBS_GetReg(MS_U16 u16Addr, MS_U8 *pu8Data)
{
    ...
    if (_u8DMD_DVBS_DbgLevel >= DMD_DVBS_DBGLV_INFO)
    {
        printf("MDrv_DMD_DVBS_GetReg %x %x\n", u16Addr, *pu8Data);
    }
    #endif
    return bRet;
}
...
MS_BOOL DMD_DVBS_SetReg(MS_U16 u16Addr, MS_U8 u8Data)
{
    ...
    if (_u8DMD_DVBS_DbgLevel >= DMD_DVBS_DBGLV_INFO)
    {
        printf("MDrv_DMD_DVBS_SetReg %x %x\n", u16Addr, u8Data);
    }
    #endif
    DMD_LOCK();
...
MS_BOOL DMD_DVBS_SetSerialControl(MS_BOOL bEnable)
{
    ...
    if (_u8DMD_DVBS_DbgLevel >= DMD_DVBS_DBGLV_INFO)
    {
        printf("MDrv_DMD_DVBS_SetSerialControl %d\n", bEnable);
    }
    #endif
...
MS_BOOL DMD_DVBS_SetConfig_symbol_rate_list(MS_U32 u32SymbolRate, DMD_DVBS_MODULATION_TYPE eQamMode, MS_U32 u32IFFreq, MS_BOOL bSpecInv, MS_BOOL bSerialTS, MS_U16 *pu16_symbol_rate_list,MS_U8 u8_symbol_rate_list_num)
{
    ...
    if (_u8DMD_DVBS_DbgLevel >= DMD_DVBS_DBGLV_INFO)
    {
        printf("MDrv_DMD_DVBS_SetConfig %ld %d %ld %d %d\n", u32SymbolRate, eQamMode, u32IFFreq, bSpecInv, bSerialTS);
    }
    #endif
...
MS_BOOL DMD_DVBS_SetActive(MS_BOOL bEnable)
{
    ...
    if (_u8DMD_DVBS_DbgLevel >= DMD_DVBS_DBGLV_INFO)
    {
        printf("MDrv_DMD_DVBS_SetActive %d\n", bEnable);
    }
    #endif
...
MS_BOOL DMD_DVBS_GetLockWithRFPower(DMD_DVBS_GETLOCK_TYPE eType, DMD_DVBS_LOCK_STATUS *eLockStatus, MS_U32 fCurrRFPowerDbm, MS_U32 fNoChannelRFPowerDbm)
{
    ...
    if (_u8DMD_DVBS_DbgLevel >= DMD_DVBS_DBGLV_INFO)
    {
        printf("MDrv_DMD_DVBS_GetLock %x %x\n", eType, *eLockStatus);
    }
    #endif
    bRet=TRUE;
    return bRet;
...
{
	printf("Doesn't support DVBS_ActiveDmdSwitch function!!!\n");
	return false;
}
...
MS_BOOL DMD_DVBS_GetCellID(MS_U16 *u16CellID)
{
    ...
    if (_u8DMD_DVBS_DbgLevel >= DMD_DVBS_DBGLV_INFO)
    {
        printf("MDrv_DMD_DVBS_GetCellID %x\n", *u16CellID);
    }
    #endif
    return bRet;
}
...
MS_BOOL DMD_DVBS_GetStatus(DMD_DVBS_MODULATION_TYPE *pQAMMode, MS_U32 *u32SymbolRate, MS_S16  *s16CFO)
{
    ...
    DMD_LOCK();
    INTERN_DVBS_GetCurrentModulationType(pQAMMode);
    INTERN_DVBS_GetCurrentSymbolRate(u32SymbolRate);
    INTERN_DVBS_Get_FreqOffset(s16CFO);
    DMD_UNLOCK();

    #ifdef MS_DEBUG
    if (_u8DMD_DVBS_DbgLevel >= DMD_DVBS_DBGLV_INFO)
    {
        printf("MDrv_DMD_DVBS_GetStatus %d %ld \n", *pQAMMode, *u32SymbolRate);
    }
    #endif
    return bRet;
}
