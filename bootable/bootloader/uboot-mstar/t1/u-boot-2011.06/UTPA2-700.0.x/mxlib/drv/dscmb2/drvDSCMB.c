...
#define DRVDSCMB2_DBG(lv, x, args...) \
    ...
    {   \
        if(lv == DSCMB2_DBGLV_ERR){ ULOGE("DSCMB", "[ERROR]"); } \
        else if(lv == DSCMB2_DBGLV_WARNING){ ULOGW("DSCMB", "[WARNING]"); }\
        else{ ULOGD("DSCMB", "[DEBUG]"); }\
        if(_u32DSCMB2DbgLv >= DSCMB2_DBGLV_DEBUG){ ULOGD("DSCMB", "[%05d][%-35s]", __LINE__, __FUNCTION__); }\
        ULOGD("DSCMB", " ");\
        ULOGD("DSCMB", x, ##args); \
    }
...
MS_BOOL _MDrv_DSCMB2_Init(void)
{
    ...
    if( HAL_DSCMB_OTPEnDSCMB() == TRUE)
    {
        ...
        ULOGD("DSCMB", "\033[31mGo through DSCMB\n\033[m");
    }
    else
    {
        ULOGD("DSCMB", "\033[33m ByPass DSCMB\n\033[m");
    }
...
//////////////////////////////////////////////////////////////////////*/
MS_BOOL MDrv_DSCMB2_CAPVR_FlowSet(DSCMB_CAPVR_MODE eCaMode, DSCMB_TSIF ePvrSrcTSIf)
{
    DSCMB_INIT_CHECK(FALSE);
    ...
    return HAL_DSCMB_Cipher_Output_Ctrl(u32ProgId, u32Channel, TRUE, FALSE, u32Tsid, u32CaVid);
}
...
KL_Fail:
    ...
    HAL_DSCMB_KL_TA_EN(eKLEng, FALSE);
    MDrv_DSCMB_KLadder_Unlock();
    ULOGE("DSCMB", "[%s][%d]TCSA3 TA DSCMB KL Fail = %08lX \n",__FUNCTION__,__LINE__,*u32Status );
    DSCMB_ASSERT_AND_RETURN(FALSE);
...
void _MDrv_DSCMB2_ReadPidSlotMap(MS_U32 u32FltId)
{
    ...
    ULOGD("DSCMB", "[%s]    FltId: %ld\n", __FUNCTION__, u32FltId);
    ULOGD("DSCMB", "       CA_VID: %d\n", tReadData.CA_VID);
    ULOGD("DSCMB", "    PidSlotEn: %d\n", tReadData.PidSlotEn);
    ULOGD("DSCMB", "    EvenSloEn: %d\n", tReadData.EvenSloEn);
    ULOGD("DSCMB", "     OddSloEn: %d\n", tReadData.OddSloEn);
    ULOGD("DSCMB", "     ClrSloEn: %d\n", tReadData.ClrSloEn);
    ULOGD("DSCMB", "    DualSloEn: %d\n", tReadData.DualSloEn);
    ULOGD("DSCMB", "EvenSlotIndex: %d\n", tReadData.EvenSlotIndex);
    ULOGD("DSCMB", " OddSlotIndex: %d\n", tReadData.OddSlotIndex);
    ULOGD("DSCMB", " ClrSlotIndex: %d\n", tReadData.ClrSlotIndex);
}
...
void _MDrv_DSCMB2_PacketView(MS_U32 u32Eng, MS_U32 u32FltId, MS_U32 Mode, MS_U32 u32Range, MS_BOOL bRst)
{
    ...
    //ULOGD("DSCMB", "%s, u32tsif = %lx, reset = %d\n", __FUNCTION__, u32tsif, bRst);
