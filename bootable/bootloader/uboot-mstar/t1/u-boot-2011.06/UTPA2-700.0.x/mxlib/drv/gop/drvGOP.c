...
MS_BOOL bGOPPowerDown[MAX_GOP_SUPPORT]={0};
#endif
//-------------------------------------------------------------------------------------------------
//  Debug Functions
//-------------------------------------------------------------------------------------------------
...
#ifdef CONFIG_MSTAR_SRAMPD
            if(bGOPPowerDown[u8GopType]==TRUE)
            {
...
static MS_U32 _GOP_GWIN_FB_AddrAlignChecker(MS_GOP_CTX_LOCAL*pGOPCtx, MS_U8 u8GOPNum, MS_PHY Address, MS_U16 fmtbpp)
{
    ...
    {
        GOP_D_ERR("%s,%d FB=0x%tx address need %d-bytes aligned!\n",__FUNCTION__,__LINE__, (ptrdiff_t)Address, (MS_U16)alignfactor);
        return CHECKALIGN_PARA_FAIL;
    }
    else
    {
...
#ifdef CONFIG_MSTAR_SRAMPD
    if((bGopAck==TRUE)&&(bGOPPowerDown[enGopType]==TRUE))
    {
...
void MDrv_GOP_GWIN_UpdateRegWithMaskSync(MS_GOP_CTX_LOCAL*pGOPCtx, MS_U16 u16GopMask, MS_BOOL bSync)
{
    ...
    {
        if((u16GopMask&(GOP_BIT0<<u8GOP))&&(bGOPPowerDown[u8GOP]==TRUE))
        {
            _GetBnkOfstByGop(u8GOP, &u32BankOffSet);
            HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet+GOP_4G_MULTI_ALPHA, GOP_BIT11, GOP_BIT11);
        }
...
MS_BOOL Mdrv_GOP_GetMuxPriority(MS_GOP_CTX_LOCAL*pGOPCtx, PST_DRV_GOP_MUX_CAP pstMuxPri)
{
    MS_U8 i=0,j=0;
    ...
    if(  (pGOPDrvLocalCtx == NULL)
       ||(pstMuxPri == NULL)
       ||(pstMuxPri->pu8MuxProrityArray == NULL))
    {
        return FALSE;
    }
    ...
    if(pstMuxPri->u8MuxNumber > MAX_MUX_NUMBER_SUPPORT)
    {
        pstMuxPri->u8MuxNumber = MAX_MUX_NUMBER_SUPPORT;
    }

    for(i=0; i<pstMuxPri->u8MuxNumber; i++)
    {
        pstMuxPri->pu8MuxProrityArray[i] = (MS_U8)(au16Mux[i][0]); //Store sorted mux number
    }
    return TRUE;
}
#ifdef CONFIG_GOP_GWIN_MISC
...
GOP_Result MDrv_GOP_GetIPInterlace(MS_GOP_CTX_LOCAL*pGOPCtx,MS_BOOL *bInterlace,E_DRV_GOP_SCALER_WIN eWindow)
{
	...
	if(eWindow == E_DRV_GOP_MAIN_WINDOW)
	{
	    HAL_GOP_Read16Reg(&pGOPDrvLocalCtx->halCtxLocal, GOP_SC_IP_MAIN_INTERLACE, &reg_val);
        if(reg_val & BIT(11))
            *bInterlace = TRUE;
        else
            *bInterlace = FALSE;
