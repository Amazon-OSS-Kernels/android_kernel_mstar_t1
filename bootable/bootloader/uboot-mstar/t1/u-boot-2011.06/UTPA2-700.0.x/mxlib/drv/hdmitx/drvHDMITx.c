...
MsHDMITX_HDCP_AUTH_STATUS _MDrv_HDMITx_HdcpAuthExangedAndComputated(MsHDMITX_OUTPUT_MODE tmds_mode)
{
    ...
    /*
     * Base on HDCP Compilance Test Specification,
     * DUT begins the first part of authentication after successful transition to HDMI mode.
     * Test Pass on Quantum Data882E 20140710. (wait within 30 frames )
     */
    #define HDCP_MODE_CHANGE_TIMEOUT 600
    if (gHDMITxInfo.output_mode == E_HDMITX_HDMI_HDCP)
    {
        MS_U32 u32StartTime = 0;
        u32StartTime = MsOS_GetSystemTime();

        while( !(bstatus[1] & 0x10) && ( (MsOS_GetSystemTime() - u32StartTime) < HDCP_MODE_CHANGE_TIMEOUT ))
        {
            if(MHal_HDMITx_Rx74ReadBytes(0x41, 2, bstatus) == FALSE)
            {
                DBG_HDCP(printf("HDCP check BStatus failed because I2C\r\n"));
                return E_HDMITX_HDCP_RX_IS_VALID;
            }
...
void MDrv_HDMITx_SetVideoOutputMode(void)
{
    ...
    if( (gHDMITxInfo.output_video_timing == E_HDMITX_RES_1920x1080p_24Hz) || (gHDMITxInfo.output_video_timing == E_HDMITX_RES_1920x1080p_25Hz) || (gHDMITxInfo.output_video_timing == E_HDMITX_RES_1920x2205p_24Hz)
      ||(gHDMITxInfo.output_video_timing == E_HDMITX_RES_3840x2160p_24Hz) || (gHDMITxInfo.output_video_timing == E_HDMITX_RES_3840x2160p_25Hz) || (gHDMITxInfo.output_video_timing == E_HDMITX_RES_4096x2160p_24Hz)) // 24/25 Hz
        gHDCPCheckRiTimer = 5400;
    // 33.3 ms * 128 = 4.26 s
    else if ((gHDMITxInfo.output_video_timing == E_HDMITX_RES_1920x1080p_30Hz)||(gHDMITxInfo.output_video_timing == E_HDMITX_RES_3840x2160p_30Hz)) // 30 Hz
...
MS_U32 MDrv_HDMITx_SetPowerState(EN_POWER_MODE u16PowerState)
{
    ...
    if (u16PowerState == E_POWER_SUSPEND)
    {
		...
		//store previous state
       	memcpy(&preHDMITxInfo, &gHDMITxInfo, sizeof(gHDMITxInfo));
    ...
    else if (u16PowerState == E_POWER_RESUME)
    {
        ...
        {
			//store back tx info
			memcpy(&gHDMITxInfo, &preHDMITxInfo, sizeof(gHDMITxInfo));
