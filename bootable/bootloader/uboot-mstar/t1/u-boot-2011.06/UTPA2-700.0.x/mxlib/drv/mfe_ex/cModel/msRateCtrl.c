...
//	ct->m_nMaxOffset = (MS_S32)((MS_S64)(ct->m_nMaxBitrate - ct->m_nBitrate)*ct->m_nTargetFrameRateDeNum / ct->m_nTargetFrameRateNum);
	...
	ct->m_nBitsPerFrame[0] = (MS_S32)MFE_DIV_S64((MS_S64)ct->m_nBitrate*ct->m_nTargetFrameRateDeNum*IFRAME_WEIGHT*i, j*ct->m_nTargetFrameRateNum);
	ct->m_nBitsPerFrame[1] = (MS_S32)MFE_DIV_S64((MS_S64)ct->m_nBitrate*ct->m_nTargetFrameRateDeNum*PFRAME_WEIGHT*i,j*ct->m_nTargetFrameRateNum);
	ct->m_nBitsPerFrame[2] = (MS_S32)MFE_DIV_S64((MS_S64)ct->m_nBitrate*ct->m_nTargetFrameRateDeNum*BFRAME_WEIGHT*i,j*ct->m_nTargetFrameRateNum);
#ifdef _SUPPORT_MBLEVEL_RC_
...
void cvbr_CloseRateControl(CVBRRateControl* ct)
{
    if (ct->m_rcMethod == CONSTRAINED_VARIABLE_BITRATE) {
        if (ct->m_BitrateGauge) {
            MfeDrvMemFree(ct->m_BitrateGauge, (const MS_S8*)("m_BitrateGauge"));
