...
#if !defined(MSOS_TYPE_LINUX_KERNEL)
void _MDrv_PQ_LoadPQBin(MS_PQ_Init_Info *pstPQInitInfo)
{
    ...
    else
    {
       ...
       //Skip USFC now, which may cause kernel panic, wait PQ Driver complated.
        ...
        if(MDrv_PQ_GetCaps(&stPQCaps))
        {
            ...
            {
                MS_U8 *pu8SubBuf, *pu8SubExBuf ;
                const char SubDefaltPath[] = "/config/pq/";
                ...
                if(pFile == NULL)
                {
                    // read from default path
                    memset(pm_PQBinFilePathName, 0, PQ_FILE_NAME_LENGTH);
                    strncpy(pm_PQBinFilePathName,SubDefaltPath,strlen(SubDefaltPath));
                    strncat(pm_PQBinFilePathName,SubBinName,strlen(SubBinName));
...
void MDrv_PQ_Init_U2(void* pInstance,MS_PQ_Init_Info *pstPQInitInfo)
{
    ...
    _MDrv_PQ_LoadPQBin(pstPQInitInfo);
...
void MDrv_PQ_LoadUFSCSettings_U2(void *pInstance,PQ_WIN eWindow)
{
    ...
    {
        _u16PQSrcType_UFSC[eWindow] = QM_InputSourceToIndex_UFSC(eWindow, _enInputSourceType[eWindow]);
...
#if (defined (UFO_XC_DS_PQ))
extern void MDrv_PQBin_DS_GetTable(void* pInstance,PQ_WIN eWindow, MS_U16 u16TabIdx, MS_U16 u16PQIPIdx);
MS_BOOL _MDrv_PQ_LoadDSModeTable(void* pInstance, SCALER_WIN eWindow, MS_U16 u16PQ_GameMode_Idx)
{
    MS_U16 i, u16IPIdx=0, u16TabIdx=0;
    ...
    for(i=0; i<PQ_GRULE_DS_PARAMETERS_IP_NUM_Main; i++)
    {
        ...
        MDrv_PQBin_DS_GetTable(pInstance,eWindow, u16TabIdx, u16IPIdx);
    }
    return TRUE;
}
void _MDrv_PQ_Load_DS_Table(void* pInstance,PQ_WIN eWindow,MS_U32 V_Size, MS_U32 H_Size)
{
    ...
    }else if( (H_Size < 3840) && (H_Size >= 720) )
    {
    ...
    _MDrv_PQ_LoadDSModeTable(pInstance,eWindow,u16PQ_GameModeIdx);
}
#endif
