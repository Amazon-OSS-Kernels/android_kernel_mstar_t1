        ...
        }else
        {
            for(x=0;x < DS_REG_CNT;x++)
            {
                if((DS_REG[x].u16Addr/2) == (u32RegAddr/2))
                {
                    ...
                    {
                        DS_REG[x].u16Addr = u32RegAddr;
                        DS_REG[x].u16Value = (DS_REG[x].u16Value<<8)    | u8Value;
                        DS_REG[x].u16Mask  = (DS_REG[x].u16Mask<<8)     | u8Mask;
                    }
                    else
                    {
                        DS_REG[x].u16Value = (u8Value<<8)      | DS_REG[x].u16Value;
                        DS_REG[x].u16Mask  = (u8Mask<<8)        | DS_REG[x].u16Mask;
                    }
                    break;
                }
            }
...
void MDrv_PQBin_DS_GetTable(void* pInstance,PQ_WIN eWindow, MS_U16 u16TabIdx, MS_U16 u16PQIPIdx)
{
