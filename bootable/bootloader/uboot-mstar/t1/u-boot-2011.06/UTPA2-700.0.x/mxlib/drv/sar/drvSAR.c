...
void SARRegisterToUtopia(FUtopiaOpen ModuleType)
{
    ...
    UtopiaModuleCreate(MODULE_SAR, 0, &pUtopiaModule);
    ...
    UtopiaModuleAddResourceStart(pUtopiaModule, SAR_POOL_ID_SAR0);

    // create a resouce and regiter it to a certain Resource_Pool, resource can alloc private for internal use
    UtopiaResourceCreate("sar0", sizeof(SAR_RESOURCE_PRIVATE), &psResource);
    UtopiaResourceRegister(pUtopiaModule, psResource, SAR_POOL_ID_SAR0);
    ...
    UtopiaModuleAddResourceEnd(pUtopiaModule, SAR_POOL_ID_SAR0);

    ULOGD(TAG_SAR, "\033[35mFunction = %s, Line = %d, SAR Register finished\033[m\n", __PRETTY_FUNCTION__, __LINE__); // joe.liu
}
...
MS_U32 SAROpen(void** ppInstance, MS_U32 u32ModuleVersion, void* pAttribute)
{
    ...
    //Check DRV_REVISION
    if(u32ModuleVersion != SAR_VERSION)
    {
        //printf("\033[35mFunction = %s, Line = %d, ERROR, DRV_VERSION is not CMDQ_VERSION\033[m\n", __PRETTY_FUNCTION__, __LINE__);
        //return FALSE;
    }

    // for multi-process safe, check if already other Instance exist
    SAR_MUTEX_LOCK();
    ULOGE(TAG_SAR, "\033[35mFunction = %s, Line = %d\033[m\n", __PRETTY_FUNCTION__, __LINE__); // joe.liu
    u32Ret = MsOS_SHM_GetId((MS_U8*)"SAR_Instance", sizeof(MS_U32), &u32RetShmID, &virtRetAddr, &u32RetShmSize, MSOS_SHM_QUERY);
    if(u32Ret == 0)
    {
        // first create, set value as 0
        MsOS_SHM_GetId((MS_U8*)"SAR_Instance", sizeof(MS_U32), &u32RetShmID, &virtRetAddr, &u32RetShmSize, MSOS_SHM_CREATE);
        *(MS_VIRT *)virtRetAddr = 0;
    }

    if(*(MS_VIRT *)virtRetAddr == 1)
    {
        ULOGE(TAG_SAR, "\033[35mFunction = %s, Line = %d, [Multi-process Safe] A Instance is existed!!\033[m\n", __PRETTY_FUNCTION__, __LINE__);
        SAR_MUTEX_UNLOCK();
        return FALSE;
    }
    else
    ...
    ULOGD(TAG_SAR, "\033[35mFunction = %s, Line = %d, [SAR INFO] OPEN INSTANCE...\033[m\n", __PRETTY_FUNCTION__, __LINE__); // joe.liu

    // instance is allocated here, also can allocate private for internal use, ex, SAR_INSTANT_PRIVATE, ppInstance will point to a pointer of the created UTOPIA_INSTANCE
    UtopiaInstanceCreate(sizeof(SAR_INSTANT_PRIVATE), ppInstance);
    // set the pSARPri point to the private of UTOPIA_INSTANCE
    UtopiaInstanceGetPrivate(*ppInstance, (void**)&pSARPri);



	pSARPri->fpSARKpdGetKeyCode = MDrv_SAR_Kpd_GetKeyCode;
...
MS_U32 SARIoctl(void* pInstance, MS_U32 u32Cmd, void* pArgs)
{
    ...
    switch(u32Cmd)
    {
    	case MDrv_CMD_Kpd_GetKeyCode:
			{
				SAR_Kpd_GetKeyCode_PARAM *param=(SAR_Kpd_GetKeyCode_PARAM *) 	pArgs;
				u32Ret = psSARInstPri->fpSARKpdGetKeyCode(param->pu8Key, param->pu8Repeat);
				return u32Ret;
    		}
    }
...
MS_U32 SARClose(void* pInstance)
{
    ...
    if( (u32Ret == 0) || (*(MS_VIRT *)virtRetAddr == 0) )
    {
            ...
            ULOGE(TAG_SAR, "\033[35mFunction = %s, Line = %d, [Multi-process Safe] u32IsInstanceCreated value is incorrect, which is %lu!!\033[m\n", __PRETTY_FUNCTION__, __LINE__, *(unsigned long*)virtRetAddr);
        #else
            ULOGE(TAG_SAR,"\033[35mFunction = %s, Line = %d, [Multi-process Safe] u32IsInstanceCreated value is incorrect, which is %u!!\033[m\n", __PRETTY_FUNCTION__, __LINE__, *(unsigned int *)virtRetAddr);
        #endif
        return FALSE;
    }
    else
    {
        ULOGD(TAG_SAR,"\033[35mFunction = %s, Line = %d, [Multi-process Safe] Release a Instance!!\033[m\n", __PRETTY_FUNCTION__, __LINE__);
        SAR_MUTEX_LOCK();
        *(MS_VIRT *)virtRetAddr = 0;
        SAR_MUTEX_UNLOCK();
        return TRUE;
