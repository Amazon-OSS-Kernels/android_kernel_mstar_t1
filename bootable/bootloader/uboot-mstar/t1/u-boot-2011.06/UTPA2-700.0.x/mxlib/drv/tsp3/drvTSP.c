...
#define _TSP_EXT_LOCK()                                     while(!OSAL_TSP_MutexObtain(_s32ExtMutexId, _TSP_MUTEX_TIMEOUT))  \
                                                            {   \
                                                                printf("TSP FATAL ERR: [%s][%d] EXT mutex get error\n", __FUNCTION__, __LINE__);   \
                                                                MsOS_DelayTask(1);  \
                                                            }

#define _TSP_EXT_UNLOCK()                                   if(!OSAL_TSP_MutexRelease(_s32ExtMutexId)){ printf("TSP FATAL ERR: [%s][%d] EXT mutex free error\n", __FUNCTION__, __LINE__); }
...
TSP_TEE_TO_REE_MBX_ACK_TYPE _TSP_REE_SendMBXMsg(TSP_REE_TO_TEE_MBX_MSG_TYPE msg_type, void* param1, void *param2, void *param3)
{
    MBX_Result result;
    MS_U8 u8Id = 0;
    ...
    {
        return E_TSP_TEE_TO_REE_MBX_ACK_MSG_INVALID;
    }
    ...
    while((MsOS_GetSystemTime() - u32StartTime) < u32TotTimeout)
    {
        ...
        switch(msg_type)
        {
            case E_DRVTSP_REE_TO_TEE_MBX_MSG_FW_LoadCode:
            case E_DRVTSP_REE_TO_TEE_MBX_MSG_SET_VqBuf:
                break;
            case E_DRVTSP_REE_TO_TEE_MBX_MSG_SET_PvrBuf:
                ...
                break;
            case E_DRVTSP_REE_TO_TEE_MBX_MSG_SET_FileinBuf:
                ...
                u8Id = *((MS_U8*)param1);
                u32Addr = *((MS_U32*)param2);
                u32Size = *((MS_U32*)param3);
                ...
                break;
            case E_DRVTSP_REE_TO_TEE_MBX_MSG_GET_PvrWPtr:
                _TSPReeToTeeMbxMsg.u8Parameters[0] = *((MS_U8*)param1);
        ...
        {
            return E_TSP_TEE_TO_REE_MBX_ACK_MSG_ACTION_SUCCESS;
        }
    ...
    return E_TSP_TEE_TO_REE_MBX_ACK_MSG_ACTION_FAIL;
}
...
static void _TSP_SW_Exit(void)
{
    ...
    if(_u32LibMode != DRVTSP_CMD_LIB_MODE_KRN)
    {
        ...
        while(_s32TspTaskId > 0)
        {
            ...
            {
                ULOGD("TSP", "[%s][%d] Systemtime Change %u \n", __FUNCTION__, __LINE__, (unsigned int)u32CurTime);
                u32Timer = u32CurTime;
            }
            if ((u32CurTime-u32Timer) > 1000UL)
            {
...
//#endif
    ...
    if (!bAlive)
    {
        ...
        if(_TSP_REE_SendMBXMsg(E_DRVTSP_REE_TO_TEE_MBX_MSG_FW_LoadCode, 0, 0, 0) != E_TSP_TEE_TO_REE_MBX_ACK_MSG_ACTION_SUCCESS)
        {
            return DRVTSP_FAIL;
        }
        #endif
...
#ifdef MSOS_TYPE_LINUX_KERNEL
    if(_ptsp_res->_TspState[0].TSPStatus != E_DRVTSP_SW_RESUME){
#endif
...
TSP_Result MDrv_TSP_STCClk_Adjust(MS_U32 u32EngId, MS_BOOL bUpClk, MS_U32 u32Percentage)
{
    ...
    if(bUpClk)
    {
        u32CW_New = (((STC_SYNTH_DEFAULT >> 16UL) * 100UL) / u32Percentage) << 16UL;
        u32CW_New += (((STC_SYNTH_DEFAULT & 0xFFFFUL) * 100UL) / u32Percentage);
    }
    else
    {
        u32CW_New = (((STC_SYNTH_DEFAULT >> 16UL) * u32Percentage) / 100UL) << 16UL;
        u32CW_New += (((STC_SYNTH_DEFAULT & 0xFFFFUL) * u32Percentage) / 100UL);
    }
...
#else
    if(_TSP_REE_SendMBXMsg(E_DRVTSP_REE_TO_TEE_MBX_MSG_SET_VqBuf, 0, 0 ,0) != E_TSP_TEE_TO_REE_MBX_ACK_MSG_ACTION_SUCCESS)
    {
...
{
    if(u32Cmd == DRVTSP_CMD_SET_LIB_MODE)
    {
...
TSP_Result MDrv_TSP_REE_SendMBXMsg(TSP_REE_TO_TEE_MBX_MSG_TYPE msg_type, void* param1, void *param2, void *param3)
{
    MS_U32 u32data = 0, *pu32data = 0;

    _TSP_ENTRY();

    if(_TSP_REE_SendMBXMsg(msg_type, param1, param2, param3) != E_TSP_TEE_TO_REE_MBX_ACK_MSG_ACTION_SUCCESS)
    {
        _TSP_RETURN(DRVTSP_FAIL);
    }
    else
    {
        ...
        {
            case E_DRVTSP_REE_TO_TEE_MBX_MSG_SET_PvrBuf:
                pu32data =(MS_U32*)param3;
                pu32data[0] = 0;
                pu32data[1] = 0;
                _TSP_REE_MbxParamToBufParam(&_TSPTeeToReeMbxMsg, pu32data, pu32data+1);
                break;
            case E_DRVTSP_REE_TO_TEE_MBX_MSG_GET_PvrWPtr:
                pu32data =(MS_U32*)param2;
                ...
                u32data = ((MS_U32)_TSPTeeToReeMbxMsg.u8Parameters[1]) & 0xFFUL;
                u32data += ((((MS_U32)_TSPTeeToReeMbxMsg.u8Parameters[2]) << 8UL) & 0xFF00UL);
                u32data += ((((MS_U32)_TSPTeeToReeMbxMsg.u8Parameters[3]) << 16UL) & 0xFF0000UL);
                u32data += ((((MS_U32)_TSPTeeToReeMbxMsg.u8Parameters[4]) << 24UL) & 0xFF000000UL);
                _miu_offset_to_phy(_TSPTeeToReeMbxMsg.u8Parameters[0], u32data, *pu32data);
                break;
            default:
                break;
        }
