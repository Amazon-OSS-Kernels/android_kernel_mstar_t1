...
static void _TSP_FLT_Init(MS_U32 u32FltId)
{
    ...
    if (TSP_PCRFLT_PCR0 == u32FltId)
    {
        ...
        HAL_TSP_INT_Disable((MS_U16)(TSP_HWINT2_PCR0_UPDATE_END_EN << 8));
    ...
    else if (TSP_PCRFLT_PCR1 == u32FltId)
    {
        ...
        HAL_TSP_INT_Disable((MS_U16)(TSP_HWINT2_PCR1_UPDATE_END_EN << 8));
        //always read PCR after PCR interrupt has been disabled or cleared to avoid that interrupt's not trggering again.
...
TSP_Result MDrv_TSP_Proc(MS_U32 u32TSPId, MS_U32 u32FltId, TSP_Event* pEvent)                        // for non-OS TSP scheduling
{
    ...
    if(u32FltId == HAL_TSP_GetCaps(E_TSP_HAL_CAP_TYPE_RECFLT_IDX))
        return E_TSP_OK;
...
TSP_Result MDrv_TSP_FLT_Alloc(MS_U32 u32TSPId, TSP_FltType eFltType, MS_U32 *pu32FltId)
{
    ...
    *pu32FltId = 0xFFFFFFFF;
    ...
    if ((eFltType & E_TSP_FLT_SRC_TSIF0) && (_ptsp_res->_Current_Live != E_TSP_FLT_SRC_TSIF0)) // if current live not @ playback channel
    {
     ...
     //[NOTE] In TSP2 : HW_PCRFLT doesn't have dedicated FltId, so we can only initiate HW_PCRFLT by FltType.
     //       In TSP4 : There are 2 HW_PCRFLT, so we can't distinguish them by FltType. But in TSP4, each  HW_PCRFLT
     //                 has its' dedicated FltId,  we can initiate each HW_PCRFLT by each FltId.

        MS_U32                 Pcr_H, Pcr;
        if (TSP_PCRFLT_PCR0 == i)
        {
            HAL_TSP_PcrFlt_SetPid(0, TSP_PID_NULL);
            HAL_TSP_PcrFlt_SetSrc(0, E_TSP_PCR_SRC_INVALID);
            HAL_TSP_PcrFlt_Enable(0, FALSE);
...
TSP_Result MDrv_TSP_FLT_SetPID(MS_U32 u32FltId, MS_U32 u32PID)
{
    ...
    if ((_ptsp_res->_tspInfo.FltType[u32FltId]& E_TSP_FLT_USER_MASK) == E_TSP_FLT_USER_PCR )
    {
        ULOGD("TSP", "Set PCR FLT=%ld PID = %8lX \n\n",u32FltId,u32PID);
        if ( TSP_PCRFLT_PCR0 == u32FltId)
        {
                ...
                switch (type)
                {
                    ...
                    case E_TSP_FLT_USER_PCR:
                        ...
                        if (TSP_PCRFLT_PCR0 == u32FltId)
                        {
                            ...
                            _ptsp_res->_u32_StcOffset[0].u32StcOffset = 0;
                            _ptsp_res->_u32_StcOffset[0].bAdd = TRUE;
                            _ptsp_res->_u32_StcUpdateCtrl[0].bEnable = TRUE;
                            _ptsp_res->_u32_StcUpdateCtrl[0].bUpdateOnce = FALSE;
                            HAL_TSP_PcrFlt_Enable(0, TRUE);
                            HAL_TSP_INT_Enable((MS_U16)(TSP_HWINT2_PCR0_UPDATE_END_EN << 8) );
                        }
                        else if (TSP_PCRFLT_PCR1 == u32FltId)
                        {
                            ...
                            _ptsp_res->_u32_StcOffset[1].u32StcOffset = 0;
                            _ptsp_res->_u32_StcOffset[1].bAdd = TRUE;
                            _ptsp_res->_u32_StcUpdateCtrl[1].bEnable = TRUE;
                            _ptsp_res->_u32_StcUpdateCtrl[1].bUpdateOnce = FALSE;
                            HAL_TSP_PcrFlt_Enable(1, TRUE);
                            HAL_TSP_INT_Enable((MS_U16)(TSP_HWINT2_PCR1_UPDATE_END_EN << 8) );
                        }
...
TSP_Result MDrv_TSP_STC_UpdateCtrl(MS_U8 u8Eng, MS_U8 u8Mode)
{
    ...
    if (u8Mode ==0x00)
    {
        TSP_RETURN((HAL_TSP_STC_UpdateCtrl(u8Eng,u8Mode)==TRUE)?E_TSP_OK:E_TSP_FAIL);
    }
    else if( u8Mode ==0x01)
    {
...
TSP_Result MDrv_TSP_FLT_ChangeSource(MS_U32 u32FltId, TSP_FltType eTSPFltTypeSrc)
{
    ...
    if(TSP_PCRFLT_PCR0 == u32FltId)
    {
        TSP_PCR_SRC ePcrSrc = HAL_FltSrc2PCRSrc(eTspPidSrc);
        HAL_TSP_PcrFlt_SetSrc(0, ePcrSrc);
    }
    else if (TSP_PCRFLT_PCR1 == u32FltId)
    {
