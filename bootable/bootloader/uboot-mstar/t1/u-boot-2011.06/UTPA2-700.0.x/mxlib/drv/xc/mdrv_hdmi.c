...
MS_U8 mdrv_HDMI_CheckHDCPState(void* pInstance, E_MUX_INPUTPORT enInputPortType)
{
    ...
    else
    {
        ...
        {
            if(Hal_HDCP_GetEncryptionFlag(enInputPortType)) // HDCP RX detection ENC_EN status
            {
                if(Hal_HDCP_getstatus(enInputPortType) & BIT(13)) // HDCP encryption is enabled: HDCP1.4
                {
                    ucHDCPState = HDMI_HDCP_1_4;

                    Hal_HDCP_ClearStatus(enInputPortType, BIT(5));
...
MS_U8 mdrv_HDCP_GetInterruptStatus(void* pInstance, E_MUX_INPUTPORT enInputPortType)
{
    ...
    else
    {
        ...
        Hal_HDCP_ClearStatus(enInputPortType, ucRetIntStatus);
    }
...
MS_BOOL mdrv_HDCP22_Handler(void* pInstance, MS_U8 ucPortIdx, MS_U8* ucPortType, MS_U8* pucMsgData, MS_U32* wMsgLen)
{
    ...
    if(ucMsgData[0] == 0x00)
    {
        ...
        {
            if (pHDMIRxResourcePrivate->ucHDCP22RecvIDListSend[ucPortIdx]== TRUE)
            {
