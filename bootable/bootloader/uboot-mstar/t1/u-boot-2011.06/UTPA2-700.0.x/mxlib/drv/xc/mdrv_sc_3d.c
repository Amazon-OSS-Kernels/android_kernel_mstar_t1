                                   ...
                                   SCALER_WIN eWindow)
{
       ...
       && (IS_OUTPUT_FRAME_ALTERNATIVE()||IS_OUTPUT_TOP_BOTTOM()))
    {
        ...
        if (gSrcInfo[eWindow].Status2.stMiuDualModCfg.bEnableMiuDualMod ==  TRUE)
        {
...
static void _MDrv_SC_3D_DD_setting(void *pInstance, SCALER_WIN eWindow)
{
    ...
    else
    {
        ...
        {
            u16DDVSize = MIN(u16PanelHeight>>1, 1080);
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK62_69_L, BIT(4), BIT(4));//DD v-sacling dn mode
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK15_02_L, u16DDVSize, 0x0FFF);//src height
        }
        else
...
MS_BOOL MDrv_XC_3D_CfgSubWin(void *pInstance, MS_BOOL bEn)
{
    ...
    if(ENABLE == bEn)
    {
        ...
        u32tmp = SC_R4BYTE(psXCInstPri->u32DeviceID, REG_SC_BK02_04_L);
        SC_W4BYTE(psXCInstPri->u32DeviceID, REG_SC_BK04_04_L, u32tmp);   // H pre-scaling
        u32tmp = SC_R4BYTE(psXCInstPri->u32DeviceID, REG_SC_BK02_08_L);
        SC_W4BYTE(psXCInstPri->u32DeviceID, REG_SC_BK04_08_L, u32tmp);   // V pre-scaling
        u32tmp = SC_R4BYTE(psXCInstPri->u32DeviceID, REG_SC_BK23_07_L);
        SC_W4BYTE(psXCInstPri->u32DeviceID, REG_SC_BK23_27_L, u32tmp);  // H post-scaling
        u32tmp = SC_R4BYTE(psXCInstPri->u32DeviceID, REG_SC_BK23_09_L);
        SC_W4BYTE(psXCInstPri->u32DeviceID, REG_SC_BK23_29_L, u32tmp);  // V post-scaling
...
static void _XC_Set_3D_MiuDualModeBaseAddress(XC_RESOURCE_PRIVATE *pXCResourcePrivate, MS_U32 u32DeviceID, SCALER_WIN eWindow)
{
#ifdef SUPPORT_DUAL_MIU
    if(gSrcInfo[eWindow].Status2.stMiuDualModCfg.bEnableMiuDualMod)
    {
        SC_W4BYTE(u32DeviceID, REG_SC_BK52_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32DualDNRBase0);  //F2_1 IPM base addr
        SC_W4BYTE(u32DeviceID, REG_SC_BK52_10_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32DualOPMBase0);  //F2_1 OPM base addr
    }
#endif
}
                           ...
                           SCALER_WIN eWindow)
{
    ...
    _XC_RETURN(pInstance);
#elif (HW_DESIGN_3D_VER >= 2)
    MS_BOOL b3Dstate =    (!IS_INPUT_MODE_NONE(eWindow))
                       && (!IS_OUTPUT_MODE_NONE())
    ...
    if(!IS_INPUT_MODE_NONE(eWindow)
       || !IS_OUTPUT_MODE_NONE())
    {
        // disable 12 line mode for 3D
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK20_10_L, 0x00, BIT(4));
    ...
    if(b3Dstate)
    {
        ...
        if (eWindow == MAIN_WINDOW)
        {
            ...
            _XC_Set_3D_MiuDualModeBaseAddress(pXCResourcePrivate, psXCInstPri->u32DeviceID, eWindow);
#if(HW_DESIGN_3D_VER == 5) //monaco mode
...
/////------------------
...
#if (HW_DESIGN_3D_VER == 3) //manhattan mode
    ...
    else if(IS_OUTPUT_TOP_BOTTOM() || IS_OUTPUT_TOP_BOTTOM_HW())
    {
            ...
            if (IsVMirrorMode(eWindow))
            {
...
#endif
                {
                    ...
                    if (pXCResourcePrivate->stdrvXC_MVideo_Context.g_bFSCEnabled)
                    {
                        ...
                        {
                            MHal_SWDS_AddTGENMdeStartEndX(pInstance, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, SUB_WINDOW);
                            MHal_SWDS_AddTGENMdeStartEndY(pInstance, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + pSrcInfo->stDispWin.height/2-1, SUB_WINDOW);
                            MHal_SWDS_AddTGENMdeStartEndX(pInstance, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, MAIN_WINDOW);
        ...
        if(gSrcInfo[eWindow].Status2.stMiuDualModCfg.bEnableMiuDualMod == TRUE)
        {
            SC_W4BYTE(psXCInstPri->u32DeviceID, REG_SC_BK52_50_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32DualOPMBase0);
        }
#endif
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK20_15_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen, 0x0FFF);//Set the maximum request lines for second channel
            ...
            && (!IS_INPUT_NORMAL_2D(eWindow))) //real 3d adjust depth using hw 2d to 3d function
    {
        if(MDrv_XC_3D_IsHWDepthAdjSupported(pInstance, eInputMode, eOutputMode, eWindow)) //real 3d adjust depth using hw 2d to 3d function
        {
            ...
            if(IS_OUTPUT_TOP_BOTTOM())
            {
                if(!MHal_FRC_IsForce4K2KToFSC(pInstance, MAIN_WINDOW))// non-maserati manhattan mode FRC 3D
                {
