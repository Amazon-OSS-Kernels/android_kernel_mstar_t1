...
#define FPLL_SWITCH_LOCKVFREQ_TIME  (1300)      // if exceed this stable time, the FPLL would switch to LockVfreq mode
...
void _MApi_XC_FPLL_EnableSpreadMode(void *pInstance, EN_FPLL_SPREAD_MODE eSpreadMode)
{
    ...
    if(eSpreadMode!=FPLL_SPREAD_MODE_NONE)
    {
        ...
        W2BYTEMSK(L_BK_LPLL(0x7A), u8FrameOutdiv-1 , 0x000F);
    ...
    else
    {
        ...
        W2BYTEMSK(L_BK_LPLL(0x7A), 0, 0x000F);
                             ...
                             SCALER_WIN eWindow)
{
    ...
    XC_LOG_TRACE(XC_DBGLEVEL_SETTIMING, "u16DefaultVFreq=%d,Final u8FrcOut = %d, u16OutputVFreq = %d\n", u16DefaultVFreq, *u8FrcOut, u16OutputVFreq) ;
#ifdef ENABLE_SPREADMODE
    ...
    )
    {
        ...
        {
            _MApi_XC_FPLL_EnableSpreadMode(pInstance,FPLL_SPREAD_MODE_1_2); //1:2 spreadmode
        }
        else if((*u8FrcIn==2) && (*u8FrcOut==5))
        {
            _MApi_XC_FPLL_EnableSpreadMode(pInstance,FPLL_SPREAD_MODE_2_5); //2:5 spreadmode
        }
        else
        {
    ...
    if(u8Lpll_bank == 0)
    {
...
#ifdef ENABLE_SPREADMODE
        if (!(MDrv_XC_FPLL_IsSupportLockFreqOnly(pInstance)) && (((*u8FrcIn==1) && (*u8FrcOut==2)) || ((*u8FrcIn==2) && (*u8FrcOut==5)))
#if (FRC_INSIDE && (HW_DESIGN_4K2K_VER == 6)) // for maserati 4K2K@120Hz case, not use spread mode
        && !((pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Pnl_Misc.FRCInfo.bFRC)
            && IsVBY1_16LANE((MS_U8)(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.eLPLL_Type)))
#endif
        ...
        MDrv_WriteByte(L_BK_LPLL(0x0E), (((u8FrcOutTmp-1)>>4) << 1) | ((*u8FrcIn-1)>>4));
    }
    else
    {
                                                     ...
                                                     MS_U16 u16UserStep, MS_U16 u16UserDelay)
{
    ...
    if (AdjustMode==EN_OUTPUT_TIMING_VTOTAL)
    {
        ...
        if(MDrv_XC_MLoad_GetStatus(g_pDevice1Instance) == E_MLOAD_ENABLED)
        {
            ...
            MDrv_XC_MLoad_WriteCmd(g_pDevice1Instance, REG_SC_BK10_0D_L,(u16Target_Value - 1), VOP_VTT_MASK  );
            _MLOAD_RETURN(g_pDevice1Instance);
        }
        else
        {
            SC_W2BYTE(1, REG_SC_BK10_0D_L,(u16Target_Value - 1));
        }
#endif
...
MS_BOOL MDrv_Scaler_IsPRDLock(void *pInstance, MS_U16 *u16OrigPrd)
{
    ...
    if((pXCResourcePrivate->stdrvXC_MVideo_Context.g_bFRCEnabled && (u32NewPhaseDiff < PHASE_OFFSET_LIMIT)) ||   // FRC enable case
       ((u16NewPrd < FPLL_PRD_THRESHOLD) && (abs(*u16OrigPrd - u16NewPrd) < FPLL_PRD_VAR_LIMIT))) // non-FRC case
    #else
...
static void _MApi_XC_FPLL_AdjustOvsVtt(void *pInstance, MS_U32 u32AccurateVPeriod)
{
    ...
    if(u32AccurateVPeriod != 0)
    {
        MS_U32 u32OutDiv = ((((MDrv_Read2Byte(L_BK_LPLL(0x0E)) & 0x0002)>>1)<< 4 )|((MDrv_Read2Byte(L_BK_LPLL(0x0C)) & 0xF000) >> 12)) + 1;
        MS_U32 u32InDiv  = ((((MDrv_Read2Byte(L_BK_LPLL(0x0E)) & 0x0001)>>0)<< 4 )|((MDrv_Read2Byte(L_BK_LPLL(0x0C)) & 0x0F00) >>  8)) + 1;
        ...
        //if(u16OutVtt > u16OriOutVtt - 2 && u16OutVtt < u16OriOutVtt + 2)
        {
            ...
            if (psXCInstPri->u32DeviceID == 0)
            {
                SC_W2BYTE(1, REG_SC_BK10_0D_L, u16OutVtt - 1);
            }
#endif
        }
#if defined(PATCH_HW_VTT_LIMITATION)
...
#ifdef ENABLE_SPREADMODE
        if(pXCResourcePrivate->stdrvXC_MVideo_Context.g_bSpreandEnable==TRUE)
        {
            W2BYTEMSK(L_CLKGEN0(0x59), BIT(10) | BIT(11),BIT(8) | BIT(10) | BIT(11));//spread mode
        }
        else
#endif
        {
...
MS_BOOL MDrv_XC_FPLL_IsSupportLockFreqOnly(void *pInstance)
{
    ...
    if(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.eLPLL_Type == E_XC_PNL_LPLL_LVDS)
    {
        if((gSrcInfo[MAIN_WINDOW].bFBL || gSrcInfo[MAIN_WINDOW].bR_FBL)|| pXCResourcePrivate->stdrvXC_Display._bForceFreerun
            ...
            #endif
            #endif
            )/*|| gSrcInfo[MAIN_WINDOW].Status2.eMirrorMode */
        {
            bResult = FALSE;
        }
    }
...
//    XC_LOG_TRACE(XC_DBGLEVEL_SETTIMING, "[%s][%d]pXCResourcePrivate->stdrvXC_MVideo_Context.g_bEnableLockFreqOnly= %x\n",__FUNCTION__, __LINE__ ,pXCResourcePrivate->stdrvXC_MVideo_Context.g_bEnableLockFreqOnly);
    ...
    if(!bResult)
    {
        MDrv_Write2ByteMask(L_BK_LPLL(0x70), 0, BIT(15));
#ifdef ENABLE_SPREADMODE
        if(pXCResourcePrivate->stdrvXC_MVideo_Context.g_bSpreandEnable==FALSE)
#endif
        {
            W2BYTEMSK(L_CLKGEN0(0x59), BIT(8), BIT(8) | BIT(10) | BIT(11));
        }
...
static MS_BOOL _MApi_XC_FPLL_LockFreqOnly(void *pInstance)
{
    ...
    #endif
    {
        ...
        if (psXCInstPri->u32DeviceID == 0)
        {
            SC_W2BYTE(1, REG_SC_BK10_0D_L, u32MaxOutVtt - 1);
        }
#endif
    ...
    #endif
    {
        ...
        if(MDrv_ReadByte(REG_CHIP_REVISION) < HW_VTT_LIMITATION_CHIPREV)
        {
            u64Vtt = SC_R2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK68_36_L, VOP_VTT_MASK)+1;
        }
        else
#endif
        {
...
void MApi_XC_SetPanelTiming_FSM(void *pInstance, XC_SetTiming_Info *pTimingInfo, SCALER_WIN eWindow)
{
   ...
   if(pTimingInfo->u16InputVFreq > 0)
    ...
    if(MDrv_ReadByte(REG_CHIP_REVISION) < HW_VTT_LIMITATION_CHIPREV)
        ...
        if (psXCInstPri->u32DeviceID == 0)
        {
            SC_W2BYTE(1, REG_SC_BK10_0D_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16DefaultVTotal/2));
        }
#endif
 ...
 #if LOCK_FREQ_ONLY_WITHOUT_LOCK_PHASE
        if(MDrv_XC_FPLL_IsSupportLockFreqOnly(pInstance) == FALSE)
            bEnable = FALSE;
        else
 #endif
...
void MApi_XC_SetFLock(void *pInstance, XC_SetTiming_Info *pTimingInfo, SCALER_WIN eWindow)
{
    ...
    if(MDrv_XC_MLoad_GetStatus(pInstance) == E_MLOAD_ENABLED)
    {
        ...
        if (psXCInstPri->u32DeviceID == 0)
        {
            _MLOAD_RETURN(pInstance);
            _MLOAD_ENTRY(g_pDevice1Instance);
            MDrv_XC_MLoad_WriteCmd(g_pDevice1Instance, REG_SC_BK10_0D_L,(u16VTotal - 1)&VOP_VTT_MASK, VOP_VTT_MASK);
    ...
    else
    {
        ...
        if (psXCInstPri->u32DeviceID == 0)
        {
            SC_W2BYTE(1, REG_SC_BK10_0D_L,(u16VTotal - 1)&VOP_VTT_MASK);
    ...
    */
    ...
    if (eWindow == MAIN_WINDOW)
    {
        ...
        if ((pXCResourcePrivate->stdrvXC_MVideo_Context.g_bFSCEnabled) &&(psXCInstPri->u32DeviceID != 1))
        {
            ...
            if (pXCResourcePrivate->stdrvXC_MVideo_Context.g_bFRCEnabled &&
               (gSrcInfo[eWindow].bEnableFPLL && (gSrcInfo[MAIN_WINDOW].stCapWin.height < pTimingInfo->u16InputVTotal)) &&
        ...
        if (IsVBY1_16LANE(u8LPLL_Type))
        {
            MHal_FRC_Set_Mload_Trig_Mode(pInstance, E_FRC_MLOAD_TRIG_MODE_FROM_OP1); // menuload trig always form tgen
        }
        else // Manhattan mode
        if ((pXCResourcePrivate->stdrvXC_MVideo_Context.g_bFRCEnabled)&&(psXCInstPri->u32DeviceID != 1))
        {
            MHal_FRC_Set_Mload_Trig_Mode(pInstance, E_FRC_MLOAD_TRIG_MODE_FROM_OP2); // with FRC case, menuload trig form stgen
        }
        else
        {
            MHal_FRC_Set_Mload_Trig_Mode(pInstance, E_FRC_MLOAD_TRIG_MODE_FROM_OP1); // no FRC case, menuload trig form tgen
        }
...
static void MDrv_Scaler_SetFrameLock(void *pInstance, MS_BOOL bSignal)
{
    ...
    MS_BOOL bUseMainFB = !gSrcInfo[MAIN_WINDOW].bFBL;
    ...
    if(MDrv_SYS_GetChipType() == E_SYS_CHIP_TV) // TV zorder
    {
        ...
        for (u8Layer = 0; u8Layer < OSD_LAYER_NUM; u8Layer++)
        {
            if (MAIN_WINDOW == eWindow)
            {
                ...
                bEnable = SC_R2BYTEMSK(psXCInstPri->u32DeviceID, u32Addr[u8Layer], BIT(8)) == BIT(8);
#elif (VIDEO_OSD_SWITCH_VER < 3)    // ver 2
                bEnable = SC_R2BYTEMSK(psXCInstPri->u32DeviceID, u32Addr[u8Layer], BIT(8)|BIT(9)) == (BIT(8)|BIT(9));
#endif
            }
            else
            {
                bEnable = SC_R2BYTEMSK(psXCInstPri->u32DeviceID, u32Addr[u8Layer], BIT(8)|BIT(10)) == (BIT(8)|BIT(10));
            }
            if (!bEnable)
            {
                break;
            }
            u8Alpha = SC_R2BYTEMSK(psXCInstPri->u32DeviceID, u32Addr[u8Layer], BITMASK(7:0));
...
#elif (VIDEO_OSD_SWITCH_VER == 2) // clippers mode
        switch (enlayer) // translate layer to register value
        {
            case E_VIDEO_ON_OSD_LAYER_DEAFULT:
                u16RegVal = 0;
...
E_APIXC_ReturnValue MDrv_XC_HDR_GetCaps(EN_XC_HDR_TYPE enHDRType, MS_BOOL *pbSupported)
{
    E_APIXC_ReturnValue eRet = E_APIXC_RET_FAIL;
