...
#if (SUPPORT_KERNEL_MLOAD == 1)
#ifdef MSOS_TYPE_LINUX_KERNEL
extern void KApi_XC_MLoad_Init(EN_MLOAD_CLIENT_TYPE _client_type,MS_PHY PhyAddr, MS_U32 u32BufByteLen);
...
void MDrv_XC_MLoad_Init(void *pInstance, MS_PHY phyAddr)
{
    ...
    */
    Hal_XC_MLoad_set_watch_dog_time_delay(pInstance, 0x0040);
    /*
    ...
    if(MDrv_XC_MLoad_Check_Done(pInstance) && pXCResourcePrivate->stdrvXC_MVideo_Context.stdrvXC_Menuload[psXCInstPri->u32DeviceID].stMLoadInfo.u16FPoint)
    {
        if(pXCResourcePrivate->stdrvXC_MVideo_Context.stdrvXC_Menuload[psXCInstPri->u32DeviceID].stMLoadInfo.u16FPoint > pXCResourcePrivate->stdrvXC_MVideo_Context.stdrvXC_Menuload[psXCInstPri->u32DeviceID].stMLoadInfo.u16RPoint)
        {
            ...
            if (IS_SUPPORT_64BITS_COMMAND(ENABLE_64BITS_COMMAND, psXCInstPri->u32DeviceID))
            {
                ...
                {
                    u32FireMemShift = _MDrv_XC_GetMloadMemOffset(pInstance, u16FireIdx);
                }
                ...
                else
                {
                    ...
                    u32FireMemShift = _MDrv_XC_GetMloadMemOffset(pInstance, pXCResourcePrivate->stdrvXC_MVideo_Context.stdrvXC_Menuload[psXCInstPri->u32DeviceID].stMLoadInfo.u16RPoint);
...
/// for another device
#if PIP_PATCH_USING_SC1_MAIN_AS_SC0_SUB
    _MLOAD_RETURN(pInstance); /// exit current instance
    ...
    if (bRet)
    {
        ...
        {
            _MLOAD_ENTRY(pInstanceTmp); /// enter another instance
            ...
            _MLOAD_RETURN(pInstanceTmp); /// exit another instance
        }
    }
    _MLOAD_ENTRY(pInstance); /// restore to current instance
#endif
    return bRet;
}
