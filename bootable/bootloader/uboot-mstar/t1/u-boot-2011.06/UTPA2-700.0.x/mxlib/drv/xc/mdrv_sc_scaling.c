...
#define MAX_1P_MODE_CLK          (300*1024*1024)   // max 1p mode clk value
...
void _MDrv_SC_set_dual_opm_write_limit(void *pInstance, MS_BOOL bEnable, MS_BOOL bFlag, MS_PHY u32OPWlimitAddr, SCALER_WIN eWindow)
{
    ...
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    //No register for dual need check with hw
    //SC_W4BYTE(psXCInstPri->u32DeviceID, REG_SC_BK12_5E_L, u32OPWlimitAddr);
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK0D_38_L, (bEnable?BIT(4):0),BIT(4)); //OPW memory address limit enable, refer to F2 memory limit from BK0D_40/BK0D_42
}
...
void MDrv_SWDS_Fire(void *pInstance, P_SC_SWDB_INFO pDBreg, SCALER_WIN eWindow )
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
...
#else
    Hal_SC_SWDS_Fire(pInstance,pDBreg, eWindow);
#endif
}
...
#endif
#ifdef SUPPORT_DUAL_MIU
    MDrv_SC_set_dual_write_limit(pInstance,SWDBreg.u32FRCMWritelimitBase, eWindow);
#endif
            ...
            if(bBilinearMode)
            {
                ...
                else
                {
                    if (bFixPreVRatio != 0)
                    {
                        ...
                        u64_result = (u16ScaleSrc -1) * 1048576UL;
            ...
            else
            {
                ...
                else
                {
                    if (bFixPreVRatio != 0)
                    {
                        ...
                        u64_result = (u32TmpPre -1) *u16ScaleSrc;
...
//printf("MDrv_SC_set_fetch_number_limit  %d \n",u8BytesPer2Pixel );
    ...
    {
        u32MsIPMBase0 = MS_IPM_BASE0(eWindow);
#ifdef SUPPORT_DUAL_MIU // Monet & Manhattan
...
//#if (HW_IPM_FETCH_ALIGNMENT==FALSE)
    if ((pSrcInfo->bMemFmt422)
#ifdef SUPPORT_DUAL_MIU
        && (gSrcInfo[eWindow].Status2.stMiuDualModCfg.bEnableMiuDualMod == FALSE)
#endif
        )
    ...
    else
    {
...
#ifdef SUPPORT_BWD
        {
            if (bBwdEnable == TRUE) // Designer request: CropWin.y must be even number when BWR enable
            {
                ...
                u16CropY &= ~0x1;
                u32OPMPixelOffset = (MS_U32)(u16CropY/2) * u16DNROffsetFor3D;
            }
            else
            {
                u32OPMPixelOffset = (MS_U32)(gSrcInfo[eWindow].ScaledCropWin.y/2) * u16DNROffsetFor3D;
...
#ifdef SUPPORT_BWD
        {
            if (bBwdEnable == TRUE) // Designer request: CropWin.y must be even number when BWR enable
            {
                MS_U16 u16CropY = gSrcInfo[eWindow].ScaledCropWin.y;
                u16CropY &= ~0x1;
                u32OPMPixelOffset = (MS_U32)((u16CropY) * u16DNROffsetFor3D);
            }
        else
        {
            u32OPMPixelOffset = (MS_U32)((gSrcInfo[eWindow].ScaledCropWin.y) * u16DNROffsetFor3D);
...
#endif
#ifdef SUPPORT_DUAL_MIU
        if ((gSrcInfo[eWindow].Status2.stMiuDualModCfg.bEnableMiuDualMod == TRUE) && (bIsLeftFactor == FALSE))
        {
            XC_LOG_TRACE(XC_DGBLEVEL_CROPCALC, "Skip CropWin.x for dual right side on view for IPM.\n");
        }
        else
#endif
        {
        ...
        if (IsHMirrorMode(eWindow) && pSrcInfo->bMemFmt422)
        {
            ...
            if (gSrcInfo[eWindow].Status2.stMiuDualModCfg.bEnableMiuDualMod == TRUE)
            {
                MS_U16 u16AfterPreScaling = pSrcInfo->u16H_SizeAfterPreScaling;
                ...
                if (IsHMirrorMode(eWindow))
                {
                    u16AfterPreScaling = u16AfterPreScaling*u32RightFactor /(u32RightFactor + u32LeftFactor);
                    u16AfterPreScaling = (u16AfterPreScaling + (OFFSET_PIXEL_ALIGNMENT*2 - 1)) & ~(OFFSET_PIXEL_ALIGNMENT*2 - 1);
                    if (bIsLeftFactor == TRUE)
                    {
                       u16AfterPreScaling = pSrcInfo->u16H_SizeAfterPreScaling - u16AfterPreScaling;
                    }
                }
                else
                {
                    u16AfterPreScaling = u16AfterPreScaling*u32LeftFactor /(u32RightFactor + u32LeftFactor);
                    u16AfterPreScaling = (u16AfterPreScaling + (OFFSET_PIXEL_ALIGNMENT*2 - 1)) & ~(OFFSET_PIXEL_ALIGNMENT*2 - 1);
                    if (bIsLeftFactor == FALSE)
                    {
                       u16AfterPreScaling = pSrcInfo->u16H_SizeAfterPreScaling - u16AfterPreScaling;
                ...
                if (bIsLeftFactor == TRUE)
                {
                u32OPMPixelOffset += (u16DNROffset - u16AfterPreScaling);
            }
            else
                {
                    XC_LOG_TRACE(XC_DGBLEVEL_CROPCALC, "Skip CropWin.x for dual right side on view for IPM.\n");
...
static MS_BOOL _MDrv_SC_check_2p_mode(void *pInstance, MS_BOOL bEnable,SCALER_WIN eWindow)
{
...
#endif
    {
        ...
        else if(((MS_U32)(u16Width)*(MS_U32)(u16Height)*(u32Vfreqx10/10)) >= MAX_1P_MODE_CLK)//ip clk >= 300Mhz,will be setted 2p mode.op clk>= ip clk,will be also setted  2p mode.
        {
            if(gSrcInfo[eWindow].bR_FBL)
            {
