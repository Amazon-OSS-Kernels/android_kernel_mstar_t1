...
//DISABLE Interrupt
    ...
    MApi_CMA_Pool_Release(stXC_CMA_Config[CMA_XC_SELF_MEM].u32HeapID);
    if (stXC_CMA_Config[CMA_XC_COBUFF_MEM].u32HeapID != XC_INVALID_HEAP_ID)
    {
        ...
        MApi_CMA_Pool_Release(stXC_CMA_Config[CMA_XC_COBUFF_MEM].u32HeapID);
    }
#endif
#endif
    XC_LOG_TRACE(XC_DBGLEVEL_FUNCTION_TRACE, "Exit\n");
...
static MS_BOOL _MApi_XC_Init_WithoutCreateMutex(void *pInstance, XC_INITDATA *pXC_InitData, MS_U32 u32InitDataLen)
{
    ...
    if(pXCResourcePrivate->stdrvXC_MVideo._bSkipSWReset == FALSE)
    {
        ...
        if (pXCResourcePrivate->stdrvXC_MVideo._enPrevPowerState != E_POWER_RESUME)
        {
            ...
            stXC_CMA_Config[CMA_XC_SELF_MEM].u32HeapID = ION_XC_HEAP_ID;
#if defined(CHIP_MONET)
            stXC_CMA_Config[CMA_XC_COBUFF_MEM].u32HeapID = XC_DUAL_MIU_HID;
#else
            stXC_CMA_Config[CMA_XC_COBUFF_MEM].u32HeapID = XC_INVALID_HEAP_ID;
#endif
            ...
            {
                /* 1. do CMA_POOL_INIT */
                CMA_Pool_Init_PARAM[CMA_XC_COBUFF_MEM].heap_id     = stXC_CMA_Config[CMA_XC_COBUFF_MEM].u32HeapID;
...
#ifndef MSOS_TYPE_LINUX_KERNEL
        || (pXCResourcePrivate->stdrvXC_MVideo._enPrevPowerState == E_POWER_RESUME)
#endif
        )
        {
#ifdef MSOS_TYPE_LINUX_KERNEL
...
#endif
#ifdef K3_U2
        //abc123 issue of 0266525 . Need to toggle IP write mask field count clear before input V sync .
        //Main window
        ...
        //enable F2 IPM wbank freeze alginment mode //abc123:0747407
...
void MApi_XC_ChangePanelType_U2(void* pInstance, XC_PANEL_INFO *pstPanelInfo)
{
    ...
    _XC_SEMAPHORE_ENTRY(g_pDevice1Instance,E_XC_ID_VAR);
    ...
    _XC_SEMAPHORE_RETURN(g_pDevice1Instance,E_XC_ID_VAR);
#endif
}
...
#if PIP_PATCH_USING_SC1_MAIN_AS_SC0_SUB
                    if(s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_ioctl((psXCInstPri->u32DeviceID == 1)?PQ_SUB_WINDOW:PQ_MAIN_WINDOW, E_PQ_IOCTL_ADC_SAMPLING, (void *)&stPQADCSamplingInfo, sizeof(PQ_ADC_SAMPLING_INFO)) == FALSE)
#else
...
static void _MDrv_XC_CheckDisplayWin(void *pInstance, XC_SETWIN_INFO *pstXC_SetWin_Info, SCALER_WIN eWindow)
{
    ...
    //maserati 4k2k@120hz 3D: SC transforms to TB, FRC transforms to FA/LA, 3D mode is FA/LA in SDK.
...
MS_BOOL MDrv_XC_ConfigCMA(void *pInstance, XC_CMA_CONFIG *pstXC_CMA_Config, XC_CMA_CLIENT enCmaClient, MS_U32 u32DataLen, SCALER_WIN eWindow)
{
    ...
    {
        memcpy(&stXC_CMA_Config[enCmaClient], pstXC_CMA_Config, u32DataLen);
    }
    else
    {
...
//    MS_BOOL bEnableHighSpeedFclk = FALSE;
    ...
    if (eWindow == MAIN_WINDOW)
    {
        ...
        else
        {
            ...
            if (IsVBY1_16LANE(u8LPLL_Type)
                ||MHal_FRC_IsForce4K2KToFSC(pInstance, MAIN_WINDOW))
            {
                pXCResourcePrivate->stdrvXC_MVideo_Context.g_bFRCEnabled = TRUE;
                pXCResourcePrivate->stdrvXC_MVideo_Context.g_bFSCEnabled = TRUE;
            }
...
if(MDrv_XC_Is_SupportSWDS(pInstance))
{
    ...
    && (! MDrv_XC_Is_SupportSWDS(pInstance)))
    {
           ...
           ||((bIsGetCMA[CMA_XC_COBUFF_MEM] == TRUE)&&((u32DualMiuMemSize!=gSrcInfo[eWindow].Status2.u32PreCMAMemDualSCMSize)||(au32CMAMemSCMSize[CMA_XC_COBUFF_MEM]!=gSrcInfo[eWindow].u32PreCMAMemSCMSize[CMA_XC_COBUFF_MEM])||(au32CMAMemFRCMSize[CMA_XC_COBUFF_MEM]!=gSrcInfo[eWindow].u32PreCMAMemFRCMSize[CMA_XC_COBUFF_MEM]))))
        {
            ...
            MS_U32 u32CMAAddr[CMA_XC_MEM_NUM] = {0, };
...
#endif
                        MHal_XC_Release_CMA(pInstance, CMA_XC_SELF_MEM, MAIN_WINDOW);
                        u32CMAAddr[CMA_XC_SELF_MEM] = (MS_U32)MHal_XC_Get_CMA_Addr(pInstance, CMA_XC_SELF_MEM,  au32CMAMemSCMSize[CMA_XC_SELF_MEM]+au32CMAMemFRCMSize[CMA_XC_SELF_MEM], eWindow);
#ifdef MSOS_TYPE_LINUX_KERNEL
                    }
                    else
                    {
                        u32CMAAddr[CMA_XC_SELF_MEM] = pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32DNRBase0*BYTE_PER_WORD;
                    }
#endif
            ...
            if (bIsGetCMA[CMA_XC_COBUFF_MEM] == TRUE)
            {
                   ...
                   ||(au32CMAMemFRCMSize[CMA_XC_COBUFF_MEM]!=gSrcInfo[eWindow].u32PreCMAMemFRCMSize[CMA_XC_COBUFF_MEM]))
                {
                    ...
                    if(!str_setwindow)
                    {
                        ...
                        {
                            u32CMAAddr[CMA_XC_COBUFF_MEM] = (MS_U32)MHal_XC_Get_CMA_Addr(pInstance, CMA_XC_COBUFF_MEM, u32DualMiuMemSize, eWindow);
                        }
                        else
                        {
                            u32CMAAddr[CMA_XC_COBUFF_MEM] = (MS_U32)MHal_XC_Get_CMA_Addr(pInstance, CMA_XC_COBUFF_MEM, au32CMAMemSCMSize[CMA_XC_COBUFF_MEM]+au32CMAMemFRCMSize[CMA_XC_COBUFF_MEM], eWindow);
                        }
#ifdef MSOS_TYPE_LINUX_KERNEL
                    }
                    else
                    {
                        u32CMAAddr[CMA_XC_COBUFF_MEM] =  pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32DNRBase0*BYTE_PER_WORD;
                    }
#endif
                    ...
                    XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"[%s,%5d][CMA], u32CMAAddr[CMA_XC_SELF_MEM]=%x \n",__func__, __LINE__, u32CMAAddr[CMA_XC_COBUFF_MEM]);
                }
            }
            else
            {
            ...
            {
                // default frcm frame buffer address init
                MDrv_XC_SetFRCMFrameBufferAddress(pInstance, (MS_U32)u32CMAAddr[enXCCMAClient], au32CMAMemFRCMSize[enXCCMAClient], eWindow);
                // default frame buffer address init
                MDrv_XC_SetFrameBufferAddress(pInstance, (MS_U32)u32CMAAddr[enXCCMAClient], au32CMAMemFRCMSize[enXCCMAClient], eWindow);
            }
            else
#endif
            {
#if (HW_DESIGN_4K2K_VER == 4)
                // default frcm frame buffer address init
                MDrv_XC_SetFRCMFrameBufferAddress(pInstance, (MS_U32)u32CMAAddr[enXCCMAClient]+au32CMAMemSCMSize[enXCCMAClient], au32CMAMemFRCMSize[enXCCMAClient], eWindow);
#endif
                // default frame buffer address init
                MDrv_XC_SetFrameBufferAddress(pInstance, (MS_U32)u32CMAAddr[enXCCMAClient], au32CMAMemSCMSize[enXCCMAClient], eWindow);
            }
#ifdef SUPPORT_DUAL_MIU
            if (u32DualMiuMemSize != 0)
            {
                // default dual frame buffer address init
                MDrv_XC_SetDualFrameBufferAddress(pInstance, (MS_U32)u32CMAAddr[CMA_XC_COBUFF_MEM], u32DualMiuMemSize, eWindow);
            }
#endif
#if (HW_DESIGN_4K2K_VER == 4)
    ...
    if(MDrv_XC_Is_SupportSWDS(pInstance))
    {
        ...
        MDrv_SWDS_Fire(pInstance,&pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg,eWindow);
    }
#endif
#endif
    ...
    #if (HW_DESIGN_4K2K_VER == 4)
    //SW patch for HDMI 4K@60 BW issue, abc123: 782590
    if((pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16HTotal > 3000)          // 4K output
        && (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VTotal > 2000)
   ...
   //several lines of garbage on the top FPI to SBSH
    ...
    //DTV 1080i patch for abc123:0735773
    ...
    */
    ...
    if(!(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Init_Misc.u32MISC_B & E_XC_INIT_MISC_B_HDMITX_ENABLE))
    {
        if(pXCResourcePrivate->stdrvXC_3D._bFRC3DEnabled )//FRC 3D enabled
        {
            ...
            if(MHal_FRC_IsForce4K2KToFSC(pInstance, MAIN_WINDOW))
            {
...
#ifdef SUPPORT_DUAL_MIU
        _XC_ENTRY(pInstance);
        if(gSrcInfo[MAIN_WINDOW].Status2.stMiuDualModCfg.bEnableMiuDualMod == TRUE)
        {
...
#ifndef DONT_USE_CMA
#if (XC_SUPPORT_CMA == TRUE)
        if ((dest != OUTPUT_SCALER2_MAIN_WINDOW))
        {
            MHal_XC_Release_CMA(pInstance, CMA_XC_SELF_MEM, MAIN_WINDOW);
            MHal_XC_Release_CMA(pInstance, CMA_XC_COBUFF_MEM, MAIN_WINDOW);
        }
...
MS_U16 MApi_XC_GetStatusEx_U2(void* pInstance, XC_ApiStatusEx *pDrvStatusEx, SCALER_WIN eWindow)
{
    ...
    if(DrvStatusEx.stDispWin.x >= gSrcInfo[eWindow].Status2.stXCPanelDefaultTiming.u16HStart)
    {
        DrvStatusEx.stDispWin.x -= gSrcInfo[eWindow].Status2.stXCPanelDefaultTiming.u16HStart;
    }
    if(DrvStatusEx.stDispWin.y >= gSrcInfo[eWindow].Status2.stXCPanelDefaultTiming.u16VStart)
    {
        DrvStatusEx.stDispWin.y -= gSrcInfo[eWindow].Status2.stXCPanelDefaultTiming.u16VStart;
    }
...
//  UNUSED(function_Info);    if(function_Info == NULL)
//  UNUSED(function_Info);    {
//  UNUSED(function_Info);        MS_ASSERT(0);
//  UNUSED(function_Info);        s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_deside_srctype = NULL;
//  UNUSED(function_Info);        s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_disable_filmmode = NULL;
//  UNUSED(function_Info);        s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_load_scalingtable = NULL;
//  UNUSED(function_Info);        s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_set_420upsampling = NULL;
//  UNUSED(function_Info);        s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_set_csc = NULL;
//  UNUSED(function_Info);        s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_get_memyuvfmt= NULL;
//  UNUSED(function_Info);        s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_set_memformat = NULL;
//  UNUSED(function_Info);        s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_set_modeInfo = NULL;
//  UNUSED(function_Info);        s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_ioctl = NULL;
//  UNUSED(function_Info);#ifdef ENABLE_TV_SC2_PQ
//  UNUSED(function_Info);        s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_deside_srctype_ex = NULL;
//  UNUSED(function_Info);        s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_disable_filmmode_ex = NULL;
//  UNUSED(function_Info);        s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_load_scalingtable_ex = NULL;
//  UNUSED(function_Info);        s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_set_420upsampling_ex = NULL;
//  UNUSED(function_Info);        s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_set_csc_ex = NULL;
//  UNUSED(function_Info);        s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_get_memyuvfmt_ex = NULL;
//  UNUSED(function_Info);        s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_set_memformat_ex = NULL;
//  UNUSED(function_Info);        s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_set_modeInfo_ex = NULL;
//  UNUSED(function_Info);        s_PQ_Function_Info[psXCInstPri->u32DeviceID].pq_ioctl_ex = NULL;
//  UNUSED(function_Info);#endif
//  UNUSED(function_Info);    }
//  UNUSED(function_Info);    else
//  UNUSED(function_Info);    {
        // Path create.
...
MS_U32 MApi_XC_SetPowerState_U2(void* pInstance, EN_POWER_MODE enPowerState)
{
    ...
    else if (enPowerState == E_POWER_RESUME)
    {
        if (pXCResourcePrivate->stdrvXC_MVideo._enPrevPowerState == E_POWER_SUSPEND)
        {
            ...
            if(IsSrcTypeHDMI(currentSrc[MAIN_WINDOW])|IsSrcTypeVga(currentSrc[MAIN_WINDOW])|IsSrcTypeYPbPr(currentSrc[MAIN_WINDOW]))
                _MApi_XC_PCMonitor_Exe(pInstance, MAIN_WINDOW, currentSrc[MAIN_WINDOW]);
...
MS_BOOL MApi_XC_EnableMiuDualMode_U2(void* pInstance, MS_BOOL bEnable, MS_U32 u32LeftFactor, MS_U32 u32RightFactor, SCALER_WIN eWindow)
{
    ...
    if((u32LeftFactor < 3*u32RightFactor) || (3*u32RightFactor < u32LeftFactor))
    {
...
E_APIXC_ReturnValue MApi_XC_AutoDownload_Write_U2(void* pInstance, XC_AUTODOWNLOAD_DATA_INFO *pstDataInfo)
{
    ...
    if (stDataInfo.enClient == E_XC_AUTODOWNLOAD_CLIENT_HDR)
    {
        ...
        XC_LOG_TRACE(XC_DBGLEVEL_PARAMETER_TRACE, "Write auto download, Size: %d, SubClient: %d, AddrRange(enable, start, end) = (%d, %d, %d).\n",
            stDataInfo.u32Size, pstFormatInfo->enSubClient, pstFormatInfo->bEnableRange, pstFormatInfo->u16StartAddr, pstFormatInfo->u16EndAddr);
