        ...
        if((AudioInitTbl_4[i].u32Addr == 0xFFFFFF))
        {
            if(AudioInitTbl_4[i].u8Mask == 1)
            {
                if(AudioInitTbl_4[i].u8Value!=0)
                    AUDIO_DELAY1MS((MS_U32)(AudioInitTbl_4[i].u8Value));
            }
        }
        else
        {
        ...
        if((AudioPreInitTbl_Clock[i].u32Addr == 0xFFFFFF))
        {
            if(AudioPreInitTbl_Clock[i].u8Mask == 1)
            {
                if(AudioPreInitTbl_Clock[i].u8Value!=0)
                    AUDIO_DELAY1MS((MS_U32)(AudioPreInitTbl_Clock[i].u8Value));
            }
        }
        else
        {
...
void HAL_AUDIO_SetInputPath(AUDIO_INPUT_TYPE input , AUDIO_PATH_TYPE u8Path)
{
    ...
    // Input source selection for DAP      0:from Downmix   1:from multi-channel
    if (u8Path == AUDIO_PATH_MAIN )
    {
        if ((g_AudioVars2->eAudioSource == E_AUDIO_INFO_DTV_IN) ||             // DTV mode
            ((g_AudioVars2->eAudioSource == E_AUDIO_INFO_HDMI_IN) && (input != AUDIO_HDMI_INPUT))) //HDMI-nonpcm
        {
            HAL_AUR2_WriteMaskByte(REG_SNDR2_MBOX_BYTE_DAP_SEL,0x02,0x02); // DAP source from Multi-channel
        }
        else
        {
            HAL_AUR2_WriteMaskByte(REG_SNDR2_MBOX_BYTE_DAP_SEL,0x02,0x00); // DAP source from Down-mix
        }
    }
}
    ...
    switch(eAudioSource)
    {
        ...
        case E_AUDIO_INFO_GAME_IN:
        {
            if (((eAudioSource == E_AUDIO_INFO_MM_IN) || (eAudioSource == E_AUDIO_INFO_GAME_IN)) &&
                (HAL_MAD_GetDSP2DecCmd() == AU_DVB_DECCMD_PLAYFRAME_GS))
            {
            ...
            else
            {
                ...
                if (HAL_MAD_GetDecCmd() == AU_DVB_DECCMD_STOP)
                {
...
MS_S32 HAL_AUDIO_Pcm_HwDma_Reader1_Init(void *pData)
{
    ...
    if (pUserPcmInfo->u32StructSize != sizeof(AUDIO_PCM_INFO_t))
    {
		...
		AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! The size of PCM Info mismatch(%u, %lu)!\n", __FUNCTION__, pUserPcmInfo->u32StructSize, sizeof(AUDIO_PCM_INFO_t));
#else
...
MS_S32 HAL_AUDIO_Pcm_HwDma_Reader1_Set(MS_U32 u32Cmd, const void *pData)
{
    ...
    if (pPcmInfo->u8StartFlag != FALSE)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! Running now, not allow to be configured!\n", __FUNCTION__);
        return -A_EBUSY;
    }
    ...
    switch(u32Cmd)
    {
        ...
        case AUDIO_PCM_CMD_BIGENDIEN:
        {
            ...
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! invalid u32BigEndien(%u)!\n", __FUNCTION__, (unsigned int)u32BigEndien);
                return -A_EINVAL;
            }
...
MS_U32 HAL_AUDIO_Pcm_HwDma_Reader2_Write(const void *pBuf, MS_U32 u32Size)
{
    ...
    //AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "u32PcmLevel: %x u32RequestSize: %x u32BufferSize: %x\n", u32PcmLevel, u32RequestSize, u32BufferSize);
    /* copy data to PCM buffer */
    if ((u32PcmLevel + u32RequestSize) < u32BufferSize)
    {
        for (u32Loop = 0; u32Loop < u32RequestSampleCount; u32Loop++)
