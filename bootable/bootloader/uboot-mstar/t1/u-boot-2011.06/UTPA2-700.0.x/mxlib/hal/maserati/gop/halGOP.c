...
GOP_Result HAL_GOP_SetWinFmt(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U8 regForm, MS_U8 u8GOPNum, MS_U8 u8GwinNum, MS_U16 colortype)
{
    ...
    {
        HAL_GOP_Write16Reg(pGOPHalLocal, u32BankOffSet + GOP_4G_GWIN0_CTRL(Gop23_GwinCtl_Ofet), colortype, GOP_REG_COLORTYPE_MASK);
    }
    else
    {
        HAL_GOP_Write16Reg(pGOPHalLocal, u32BankOffSet + GOP_4G_GWIN0_CTRL(u8GwinNum), colortype, GOP_REG_COLORTYPE_MASK);
    }
...
void HAL_GOP_Read16Reg(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U32 u32addr, MS_U16* pu16ret)
{
    ...
    switch (u32addr & 0xFF00)
    {
        ...
        case (MIU0_ARBB_BASE & 0xFF00):
        {
            direct_addr = MIU0_ARBB_BASE + (u32addr & 0xFF);  //Direct_Base + addr_offset
            *pu16ret = GOP_READ2BYTE((direct_addr&0xFFFFF));
            break;
        }
        case (MIU1_ARBB_BASE & 0xFF00):
        {
            direct_addr = MIU1_ARBB_BASE + (u32addr & 0xFF);  //Direct_Base + addr_offset
            *pu16ret = GOP_READ2BYTE((direct_addr&0xFFFFF));
            break;
        }
        case (MIU2_ARBB_BASE & 0xFF00):
        {
            direct_addr = MIU2_ARBB_BASE + (u32addr & 0xFF);  //Direct_Base + addr_offset
            *pu16ret = GOP_READ2BYTE((direct_addr&0xFFFFF));
            break;
        }
        default:
...
#if (GOP_SC_MIU_ARBITOR_HW_BUG_PATCH == TRUE)
MS_BOOL _GOP_MIU_Mask(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U8 u8GOP,  MS_BOOL bMask)
{
    MS_U16 mask_shift=0xFF;
    ...
    switch(u8GOP)
    {
        ...
        case E_GOP_Dwin:
            ...
            break;
        default:
            mask_shift = GOP_MIU_CLIENT_INVALID;
            MS_CRITICAL_MSG(GOP_H_DBUG("ERROR gop miu client\n"));
            break;
    }
    if(mask_shift == GOP_MIU_CLIENT_INVALID )
        return FALSE;

    HAL_GOP_Write16Reg(pGOPHalLocal, GOP_MIU0_ARBB, bMask<<mask_shift, 1<<mask_shift );
    HAL_GOP_Write16Reg(pGOPHalLocal, GOP_MIU1_ARBB, bMask<<mask_shift, 1<<mask_shift );
    HAL_GOP_Write16Reg(pGOPHalLocal, GOP_MIU2_ARBB, bMask<<mask_shift, 1<<mask_shift );
...
GOP_Result HAL_GOP_Set_MIU(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U8 u8GOP,MS_U8 miusel)
{
...
#if (GOP_SC_MIU_ARBITOR_HW_BUG_PATCH == TRUE)
        //miu mask before miu select
        _GOP_MIU_Mask(pGOPHalLocal, u8GOP, MIU_MASK_ENABLE);
#endif
...
#endif
#if (GOP_SC_MIU_ARBITOR_HW_BUG_PATCH == TRUE)
        //miu unmask after miu select
        _GOP_MIU_Mask(pGOPHalLocal, u8GOP, MIU_MASK_DISABLE);
#endif
    }
    ...
    if (pGOPHalLocal->pHALShared->GOP_Dst[u8_mainGop] == E_DRV_GOP_DST_OP_DUAL_RATE)
    {
        ...
        if(pGOPHalLocal->pGopChipPro->bInternalMIUSelect[u8GOP]==TRUE)
        {
            HAL_GOP_Write16Reg(pGOPHalLocal, u8GOP+GOP_4G_MIU_SEL, miusel<<0, GOP_BIT0|GOP_BIT1 );//GWIN Palette MIU Select
            HAL_GOP_Write16Reg(pGOPHalLocal, u8GOP+GOP_4G_MIU_SEL, miusel<<2, GOP_BIT2|GOP_BIT3 );//GWIN MIU Select
            HAL_GOP_Write16Reg(pGOPHalLocal, u8GOP+GOP_4G_MIU_SEL, miusel<<4, GOP_BIT4|GOP_BIT5 );//GWIN_3D MIU Select
        }
        else
        {
            switch(u8GOP)
    ...
    )
{
    ...
    else
    {
        ...
        // crop left part
        crop_hstart = 0x0;
        crop_hend = (u16HScalingRatio != 0) ? ((SCALING_MULITPLIER * (MS_U32)gop_stretch_width) / u16HScalingRatio) - 1 : 0x77F;
        crop_vstart = 0x0;
        crop_vend = (u16VScalingRatio != 0) ? ((SCALING_MULITPLIER * (MS_U32)gop_stretch_height) / u16VScalingRatio) : 0x870;
        ...
        // crop left part
        crop_hstart = (u16HScalingRatio != 0) ? ((SCALING_MULITPLIER * (MS_U32)gop_stretch_width) / u16HScalingRatio) : 0x780;
        crop_hend = (u16HScalingRatio != 0) ? ((SCALING_MULITPLIER * (MS_U32)gop_stretch_width) / u16HScalingRatio * 2) - 1: 0xeff;
        crop_vstart = 0x0;
        crop_vend = (u16VScalingRatio != 0) ? ((SCALING_MULITPLIER * (MS_U32)gop_stretch_height) / u16VScalingRatio) : 0x870;
...
GOP_Result HAL_GOP_CMDQ_SetGOPACK(GOP_CTX_HAL_LOCAL *pGOPHalLocal,MS_U8 gop)
{
    ...
    MS_U32 fireGOP=0;
#if (GOP_SC_MIU_ARBITOR_HW_BUG_PATCH == TRUE)
    MS_U16 u16Regmiu0=0,u16Regmiu1=0,u16Regmiu2 = 0;
    HAL_GOP_Read16Reg(pGOPHalLocal, GOP_MIU0_ARBB, &u16Regmiu0);
    HAL_GOP_Read16Reg(pGOPHalLocal, GOP_MIU1_ARBB, &u16Regmiu1);
    HAL_GOP_Read16Reg(pGOPHalLocal, GOP_MIU2_ARBB, &u16Regmiu2);
#endif
    ...
    HAL_GOP_CMDQ_WriteCommand(pGOPHalLocal,fire_struct,&number,u32BankOffSet+GOP_4G_BANK_FWR,(u16RegVal2&(~GOP_BIT0)) ,0xFFFF);//current GOP force write en
#if (GOP_SC_MIU_ARBITOR_HW_BUG_PATCH == TRUE)
    u16Regmiu0 |= (1<<u16MiuClient);
    u16Regmiu1 |= (1<<u16MiuClient);
    u16Regmiu2 |= (1<<u16MiuClient);
    HAL_GOP_CMDQ_WriteCommand(pGOPHalLocal,fire_struct,&number,GOP_MIU0_ARBB,u16Regmiu0,0xFFFF);
    HAL_GOP_CMDQ_WriteCommand(pGOPHalLocal,fire_struct,&number,GOP_MIU1_ARBB,u16Regmiu1,0xFFFF);
    HAL_GOP_CMDQ_WriteCommand(pGOPHalLocal,fire_struct,&number,GOP_MIU2_ARBB,u16Regmiu2,0xFFFF);
#endif
...
GOP_Result HAL_GOP_CMDQ_SetGOPACKMask(GOP_CTX_HAL_LOCAL *pGOPHalLocal,MS_U16 u16GopMask)
{
    ...
    {
#if (GOP_SC_MIU_ARBITOR_HW_BUG_PATCH == TRUE)
        u16Regmiu0 |= (1<<u16GOPmiumask);
        u16Regmiu1 |= (1<<u16GOPmiumask);
        u16Regmiu2 |= (1<<u16GOPmiumask);
        HAL_GOP_CMDQ_WriteCommand(pGOPHalLocal,fire_struct,&number,GOP_MIU0_ARBB,u16Regmiu0,0xFFFF);
        HAL_GOP_CMDQ_WriteCommand(pGOPHalLocal,fire_struct,&number,GOP_MIU1_ARBB,u16Regmiu1,0xFFFF);
        HAL_GOP_CMDQ_WriteCommand(pGOPHalLocal,fire_struct,&number,GOP_MIU2_ARBB,u16Regmiu2,0xFFFF);
#endif
