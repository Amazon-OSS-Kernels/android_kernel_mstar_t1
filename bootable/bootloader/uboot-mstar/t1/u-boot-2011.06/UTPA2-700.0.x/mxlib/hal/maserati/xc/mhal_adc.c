...
void Hal_ADC_disable_csc(void *pInstance, SCALER_WIN eWindow)
{
    ...
    if(eWindow == SUB_WINDOW)
    {
        if(0 != (SC_R2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK04_2B_L) & 0xFF))
        {
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK04_2B_L, 0, BIT(4)|BIT(5));
        }
    }
    else
    {
        if(0 != (SC_R2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK02_2B_L) & 0xFF))
        {
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK02_2B_L, 0, BIT(4)|BIT(5));
        }
    }
}
