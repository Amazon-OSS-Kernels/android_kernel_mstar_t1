...
void MHal_FRC_TGEN_Init(void *pInstance)
{
    ...
    #if 1 // for abc123 0915469: MM zoom display window position wrong, need wait for DS ready
...
void MHal_FRC_TGEN_Enable_Lock_Source(void *pInstance, MS_BOOL bEn, E_FRC_TGEN_LOCK_SOURCE_MODE eMode, MS_BOOL bMenuload)
{
    if (ENABLE == bMenuload)
    {
        ...
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK68_20_L, u16STGEN_Setting, (BIT(2)|BIT(5))); // reg_stgen_en: use simple tgen to trigger op/vip
...
void MHal_FRC_IPM_SetBaseAddr(void *pInstance, MS_U32 addr, MS_U32 u32FrameYcout, MS_U16 u16FB_YcountLinePitch, MS_U8 u8MirrorMode )
{
    MS_U8  u8MIUSel;
...
void MHal_FRC_IPM_R_SetBaseAddr(void *pInstance, MS_U32 addr)
{
    MS_U8  u8MIUSel;
    //MS_PHY u32StartOffset;
    MS_U32 u32MemSize = 0;
...
void MHal_FRC_OPM_SetBaseAddr(void *pInstance, MS_U32 addr, MS_U32 u32FrameYcout)
{
    MS_U8  u8MIUSel;
    //MS_PHY u32StartOffset;
    MS_U32 u32MemSize = 0;
...
void MHal_FRC_OPM_R_SetBaseAddr(void *pInstance, MS_U32 addr)
{
    MS_U8  u8MIUSel;
    //MS_PHY u32StartOffset;
    MS_U32 u32MemSize = 0;
...
void MHal_FRC_ME1_SetBaseAddr(void *pInstance, MS_U32 addr, MS_U32 u32Size)
{
    MS_U8  u8MIUSel;
    //MS_PHY u32StartOffset;
    MS_U32 u32MemSize = 0;
...
void MHal_FRC_ME2_SetBaseAddr(void *pInstance, MS_U32 addr, MS_U32 u32Size)
{
    MS_U8  u8MIUSel;
    //MS_PHY u32StartOffset;
    MS_U32 u32MemSize = 0;
...
void MHal_FRC_Halo_SetBaseAddr(void *pInstance, MS_U32 addr, MS_U32 u32Size)
{
    MS_U8  u8MIUSel;
    //MS_PHY u32StartOffset;
    MS_U32 u32MemSize = 0;
    ...
    W2BYTEMSK(REG_FRC_BK22C_62_L, (u32Width/4), 0x0F);
    W2BYTEMSK(REG_FRC_BK22C_68_L, (u32Width/4), 0x0F);
    ...
    W2BYTEMSK(REG_FRC_BK22C_53_L, (u32Width/8), 0x0F);
    W2BYTEMSK(REG_FRC_BK22C_54_L, (u32Width/8), 0x0F);
...
void MHal_FRC_Init(void *pInstance, PMST_PANEL_INFO_t pPanelInfo, PFRC_INFO_t pFRCInfo)
{
    ...
    MS_U32 u32ME2Addr = 0, u32ME2Size = 0;
    MS_U32 u32HRAddr = 0, u32HRSize = 0;
    MS_U32 u32MEMC_R_Addr = 0, u32MEMC_R_Size = 0;
    MS_U8 u8LPLL_Type = (MS_U8)pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.eLPLL_Type;
    ...
    if (IsVBY1_16LANE(u8LPLL_Type))
    {
        ...
        //---------------------------------------------------------------
        MHal_FRC_FO_TGEN_SetVTotal(pInstance, pPanelInfo->u16VTotal);
        MHal_FRC_FO_TGEN_SetHTotal(pInstance, pPanelInfo->u16HTotal);
        MHal_FRC_FO_TGEN_SetFdeStartEndY(pInstance, pPanelInfo->u16DE_VStart, (pPanelInfo->u16DE_VStart + pPanelInfo->u16Height - 1));
        MHal_FRC_FO_TGEN_SetFdeStartEndX(pInstance, pPanelInfo->u16HStart, (pPanelInfo->u16HStart + pPanelInfo->u16Width - 1));
        MHal_FRC_FO_TGEN_SetMdeStartEndY(pInstance, pPanelInfo->u16DE_VStart, (pPanelInfo->u16DE_VStart + pPanelInfo->u16Height - 1));
        MHal_FRC_FO_TGEN_SetMdeStartEndX(pInstance, pPanelInfo->u16HStart, (pPanelInfo->u16HStart + pPanelInfo->u16Width - 1));
        MS_U16 u16AlignHStart = (pPanelInfo->u16HStart +3) &~3; // Maserati 4P mode, Hstart need to 4 align
        MHal_FRC_FO_TGEN_SetMdeStartEndX_L(pInstance, (u16AlignHStart/4), (u16AlignHStart/4 + pPanelInfo->u16Width/4 - 1));
        MHal_FRC_FO_TGEN_SetMdeStartEndX_R(pInstance, (u16AlignHStart/4), (u16AlignHStart/4 + pPanelInfo->u16Width/4 - 1));

        MHal_FRC_FO_TGEN_SetHSyncStartEndX(pInstance, pPanelInfo->u16HSyncStart, pPanelInfo->u16HSyncEnd);
        MHal_FRC_FO_TGEN_SetVSyncStartEndY(pInstance, pPanelInfo->u16VSyncStart, pPanelInfo->u16VSyncEnd);
    ...
    // for abc123 1028207: YUV/RGB use same MEDS setting to avoid MIU HIT issue
    ...
    if (IsVBY1_16LANE(u8LPLL_Type))
    {
        ...
        if (gSrcInfo[MAIN_WINDOW].bUseYUVSpace)
        {
            ...
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKC1_2F_L, 0x0005, 0x0035); // enable Y2R
        }
        else
        {
            MDrv_Write2ByteMask( REG_FRC_BK339_20, 0x0000, 0xE010); // disable R2Y
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BKC1_2F_L, 0x0000, 0x0035); // disable Y2R
        }
    }
...
void MHal_FRC_Set_Pipe_Delay_Value(void *pInstance, MS_U16 u16Hcnt, MS_U16 u16Vcnt)
{
    ...
    // abc123: 1025485: VG849 timing 851, video flicker with Raptor
    // --------|------------|**** bk68 V delay delay ****|-----------
...
void MHal_FRC_Set_STGEN_ODCLK(void *pInstance, E_FRC_STGEN_ODCLK eMode, MS_BOOL bMenuload)
{
    ...
    if (ENABLE == bMenuload)
    {
        ...
        MS_U16 u16FSCSetting = MDrv_Read2Byte(0x142004);
        u16FSCSetting = (u16FSCSetting & ~(0x300)) | ((eFSCIDCLK<<8) & 0x300);
        MDrv_XC_MLoad_WriteCommand_NonXC(pInstance, 0x1420, 0x04, u16FSCSetting, 0xFFFF);
    }
    else
    {
        XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
...
MS_BOOL MHal_FRC_IsForce4K2KToFSC(void *pInstance, SCALER_WIN eWindow)
{
    MS_BOOL bRet = FALSE;
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
...
#if (FRC_INSIDE && (HW_DESIGN_4K2K_VER == 6))
    // For Maserati 3D 4k1k@120Hz FA out, force scaler scaling up to 4K2K, need to sent 4k2k to FSC
    MS_U8 u8LPLL_Type = (MS_U8)pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.eLPLL_Type;
    if (!IsVBY1_16LANE(u8LPLL_Type) &&
        pXCResourcePrivate->stdrvXC_MVideo_Context.g_bFSCEnabled &&
        (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16DefaultHTotal > 3500) && ///4k
        (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16DefaultVTotal > 1000) && ///1K
        (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16DefaultVTotal < 1200) && ///1K
        (1180 < pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16DefaultVFreq) && ///120Hz
        (1220 > pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16DefaultVFreq) && ///120Hz
        (psXCInstPri->u32DeviceID != 1)  //sc1 no fsc
        )
