...
MS_BOOL MsOS_MPool_Mapping(MS_U8 u8MiuSel, MS_SIZE tOffset, MS_SIZE tMapSize, MS_BOOL bNonCache)
{
    ...
    MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("u32Offset = %lX, u32MapSize = %lX, bNonCache = %02x\n", (unsigned long)tOffset, (unsigned long)tMapSize, bNonCache));
...
MS_BOOL MsOS_MPool_Mapping_Dynamic(MS_U8 u8MiuSel, MS_SIZE u32Offset, MS_SIZE u32MapSize, MS_BOOL bNonCache)
{
    ...
    MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("u32Offset = %tX, u32MapSize = %tX, bNonCache = %02x\n", (ptrdiff_t)u32Offset, (ptrdiff_t)u32MapSize, bNonCache));
    ...
    for (i = 0; i < MAX_MAPPINGSIZE; i++)
    {
        if(mpool_info[i].bIsUsed == true)
        {
            if( (mpool_info[i].u64VirtStart == ptrVirtStart) && (mpool_info[i].u64MpoolSize == tMapSize) )
            {
                ...
                {
					printf("\033[35mFunction = %s, Line = %d, release handle id %td\033[m\n", __PRETTY_FUNCTION__, __LINE__, (ptrdiff_t)mpool_info[i].handle_id);
