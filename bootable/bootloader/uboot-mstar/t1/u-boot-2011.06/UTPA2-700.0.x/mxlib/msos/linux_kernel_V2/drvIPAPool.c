...
#include "../../include/drvIPAPool_v2.h"
...
int test_case_2()
{
    ...
    Init_Param.space_id= DFB_TEMP_HID;
...
#if DEBUG_IPA_KERNEL_MODE
    printk("[%s:%d]Init_Param->space_id=%u,Init_Param->offset=0x%llx,Init_Param->len=0x%llx\n",__FUNCTION__,__LINE__,Init_Param.space_id
    ...
    if(err ==FALSE)
    {
        printk("[%s:%d] ipa_init fail  Init_Param->space_id=%u,Init_Param->offset=0x%llx,Init_Param->len=0x%llx\n",__FUNCTION__,__LINE__,Init_Param.space_id
                          ,Init_Param.offset_in_heap,Init_Param.len);
        return -1;
    }
    ...
    printk("[%s:%d] will MApi_IPA_Pool_Register_Notify  polling_param.pool_handle_id=%u\n",__FUNCTION__,__LINE__,polling_param.pool_handle_id);
    MApi_IPA_Pool_Register_Notify(&polling_param);
    printk("[%s:%d] after MApi_IPA_Pool_Register_Notify\n",__FUNCTION__,__LINE__);
