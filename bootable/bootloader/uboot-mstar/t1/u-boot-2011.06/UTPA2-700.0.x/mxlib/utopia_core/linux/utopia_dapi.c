...
#if defined(DISABLE_SYSCALL)
    //flag =-1 : try lock. flag =0 : lock. others : not implement yet.
        if (FALSE == MsOS_ObtainNamedSemaphore(pLocatedRPool->u32ResourcesSemaphore, 0))//coverity
	{
...
static MS_U32 set_rpool_sem(void* pModuleTmp, MS_U32 u32RPoolID, MS_U32 reset)
{
    ...
    if (reset)
    {
        if (TRUE == MsOS_ResetNamedSemaphore(pLocatedRPool->u32ResourcesSemaphore))
