...
cflags := \
    ...
    -DFW_EMBEDDED_ASC      \
    -DCONFIG_MASERATI        \
    -DCONFIG_UTOPIAXP_CONF_PATH="\"/data/utopia.conf\"" \
...
define _install_bsp
   $(shell bash $(LOCAL_PATH)/build/MXlibBk.sh $(LOCAL_PATH) "BACKUP_INC")
   ...
   $(shell bash $(LOCAL_PATH)/build/Reduced.sh $(LOCAL_PATH)/build/api_with_header_file.csv $(LOCAL_PATH) $$Version)
   $(shell bash $(LOCAL_PATH)/build/CPBackup2mxlib.sh $(LOCAL_PATH))
