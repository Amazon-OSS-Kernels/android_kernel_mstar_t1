...
void board_init_r (gd_t *id, ulong dest_addr)
{
	...
	ulong flash_size;
#endif
#if defined(UFBL_FEATURE_IDME) && defined(CONFIG_CMD_SERIALNO)
        char serialno[18] = {0};
        char serialno_cmd[32] = {0};
#endif
    gd = id;
	bd = gd->bd;
...
#ifdef UFBL_FEATURE_IDME
        idme_initialize();
      //  boot_mode = idme_boot_mode();

        char idmeipaddr[32] = {0};
        char ipaddrcmd[64] = {0};
        if (!idme_get_var_external("eth_ip_addr", idmeipaddr, sizeof(idmeipaddr)-1) &&
            strchr((const char*)idmeipaddr, '.')) {
                 snprintf(ipaddrcmd, sizeof(ipaddrcmd), "setenv ipaddr %s", idmeipaddr);
        } else {
           snprintf(ipaddrcmd, sizeof(ipaddrcmd), "setenv ipaddr %s", DEFAULT_IP_ADDR); // default ipaddr
        }
        ...
        if (!idme_get_var_external("eth_mac_addr", idmemacaddr, sizeof(idmemacaddr)-1) &&
            strlen((const char*)idmemacaddr) == 12) {
                 snprintf(macaddrcmd, sizeof(macaddrcmd), "setenv macaddr %c%c:%c%c:%c%c:%c%c:%c%c:%c%c",
...
//bootmode set for diag/normal boot separation
        ...
        if (!idme_get_var_external("bootmode", bootmode_buf, sizeof(bootmode_buf)-1)) {
                ...
                printf("idme bootmode failed", bootmode_buf);
                bootmode = 1;
        }
        if(bootmode < 0 || bootmode > IDME_BOOTMODE_MAX)
                bootmode = 1;

        printf("idme bootmode: %d\n", bootmode);
        ...
        if (!idme_get_var_external("dev_flags", dev_flags_buf, sizeof(dev_flags_buf)-1)) {
                dev_flags = atoi(dev_flags_buf);
                printf("idme dev_flags read [%d]\n",dev_flags);
        }else {
                printf("idme dev_flags read failed %s", dev_flags_buf);
        }

	snprintf(dev_flags_cmd, sizeof(dev_flags_cmd), "setenv dev_flags %d",dev_flags);
        ...
        if (!idme_get_var_external("serial", serialno, sizeof(serialno)-1)) {
                snprintf(serialno_cmd, sizeof(serialno_cmd), "serialno %s;", serialno);
                run_command(serialno_cmd, 0);
}
#endif
#endif
