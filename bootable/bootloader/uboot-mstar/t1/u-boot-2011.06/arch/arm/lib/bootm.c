...
//MStar end
	...
	if (ibootmode == IDME_BOOTMODE_DIAG) {//boot into diag mode
        ...
        sprintf(__cmdline, "androidboot.serialno=%s %s %s %s %s %s %s %s %s",  serialno,
		__unlocked, __prod, __secure_cpu, __rpmb_state, __selinux, __console, __wdt_intr_dump, commandline);
    }
