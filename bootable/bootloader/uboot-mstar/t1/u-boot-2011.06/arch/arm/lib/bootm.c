...
//MStar end
	...
	if (ibootmode == IDME_BOOTMODE_DIAG) {//boot into diag mode
        ...
        if (*(volatile unsigned short *)(0x1f000c08) == 0xEBAB) {
        ...
        if (amzn_dm_verity_is_off(iunlocked)) {
	...
	if (isBootToRecovery()) {
        ...
        sprintf(__cmdline, "androidboot.serialno=%s %s %s %s %s %s %s %s %s %s mboot_build_time=%s-%s %s",
		serialno, __unlocked, __prod, __secure_cpu, __rpmb_state, __selinux, __dm_verity_mode,
		__console, __wdt_intr_dump, __recovery_mode, U_BOOT_DATE, U_BOOT_TIME, commandline);
    }
	...
	__asm__ __volatile__(
#if defined(ENABLE_MASERATI) || (ENABLE_KANO) || (ENABLE_KASTOR) || (ENABLE_MAXIM) || (ENABLE_CURRY)
	"ldr     r3, =0x16001080\n\t"
	"ldr     r2, [r3]\n\t"
	"ldr     r4, =(0x3 << 30)\n\t"
	"orr     r2, r2, r4\n\t"
