...
MS_U32 g_audioDumpInfo_interval = 2500;
    ...
    if (bRecoveryOn == TRUE)
    {
        ...
        for (s32DeviceId = 0; s32DeviceId < AUDIO_PCM_MAXNUM; s32DeviceId++)
        {
            if (g_AudioVars2->g_PcmInfo[s32DeviceId].u8ConnectFlag == TRUE)
            {
                ...
                MDrv_AUDIO_PCM_Start(s32DeviceId);
            }
        }
    }
...
void _MApi_AUDIO_SetOutConnectivity(void)
{
    ...
    {   // used in U4, T3, T8, T9, T11, T12, T13, J2, A1, A2, A5,
     ...
     //Set path for HP out
            ...
            {
                MDrv_AUDIO_SetInternalPath(INTERNAL_PCM_DELAY, g_AudioVars2->AudioOutputInfo.HpOut);
            }
...
AUDIO_DEC_ID _MApi_AUDIO_OpenDecodeSystem(AudioDecStatus_t * p_AudioDecStatus)
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "%s() is AD\n",__FUNCTION__);
    }
...
MS_BOOL _MApi_AUDIO_SetAudioParam2( AUDIO_DEC_ID DecId, Audio_ParamType paramType, MS_VIRT Param )
{
    ...
    if(paramType == Audio_ParamType_CommonCmd)
    {
        AUDIO_COMMON_CMD_t * cmd = (AUDIO_COMMON_CMD_t *)Param;
       ...
       if (!(strcmp(name, "MApi_AUDIO_Resume")))
       {
           _MApi_AUDIO_Resume();
           return TRUE;
       }
       else if(!(strcmp(name, "MApi_AUDIO_Suspend")))
       {
           _MApi_AUDIO_Suspend();
           return TRUE;
       }
