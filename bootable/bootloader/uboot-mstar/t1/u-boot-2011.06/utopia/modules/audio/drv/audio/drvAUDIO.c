...
void MDrv_AUDIO_PCM_Init(void)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: Init PCM device\n", __FUNCTION__);

    DRVAUDIO_CHECK_SHM_INIT;

    if (g_Common_PCM_IO_Init == TRUE)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "%s: already initialized\n", __FUNCTION__);
        return;
    }
...
void MDrv_AUDIO_PCM_DeInit(void)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: DeInit PCM device\n", __FUNCTION__);

    DRVAUDIO_CHECK_SHM_INIT;

    if (g_Common_PCM_IO_Init == FALSE)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "%s: no need to de-initialize\n", __FUNCTION__);
        return;
    }
...
MS_S32 MDrv_AUDIO_PCM_Open(void *pData)
{
    ...
    MS_S32 s32DeviceId = 0;
    MS_S32 s32Ret = 0;
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: Open PCM device\n", __FUNCTION__);
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: return device-id %d\n", __FUNCTION__, (int)s32DeviceId);
...
MS_S32 MDrv_AUDIO_PCM_Start(MS_S32 s32DeviceId)
{
    AUDIO_PCM_t *pPCM = NULL;
    MS_S32 s32Ret = 0;
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: Start PCM device-id %d\n", __FUNCTION__, (int)s32DeviceId);
...
MS_S32 MDrv_AUDIO_PCM_Flush(MS_S32 s32DeviceId)
{
    AUDIO_PCM_t *pPCM = NULL;
    MS_S32 s32Ret = 0;
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: Flush PCM device-id %d\n", __FUNCTION__, (int)s32DeviceId);
