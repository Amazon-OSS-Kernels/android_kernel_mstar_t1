...
void MDrv_AUDIO_CheckHashkey(void)
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash-key Support Dolby MS12 B profile.\n");
        DolbyAACFlag = 1;
        DolbyEncFlag = 1;
    }
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash-key Support Dolby Pulse(MS10-DDT).\n");
        DolbyAACFlag = 1;
        DolbyEncFlag = 1;
    }
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash-key Support Generic HE-AAC!!\n");
        /* See if need to translate to Generic HE-AAC package */
    }
    else
    {
        ip_auth |= 0x100; /* Disable Generic HE-AAC */
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash Key Check AAC Fail, no any AAC license!!\n");
    }

    if (MDrv_AUTH_IPCheck(AU_DVB2_AUTHBIT_DDE))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash-key Support Dolby DDCO \n");
        DolbyEncFlag = 1;
    }
    else
    {
        ip_auth |= 0x4;    /* Disable DDCO */
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash Key Check DDCO Fail, no DDCO license!!\n");
    }

    if (MDrv_AUTH_IPCheck(AU_DVB_AUTHBIT_DTSDEC))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash-key Support DTS DMP.\n");
    }
    else
    {
        ip_auth |= 0x8;    /* Disable DTS */
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash Key Check DTS Fail, no DTS license!!\n");
    }

    if (MDrv_AUTH_IPCheck(AU_DVB2_AUTHBIT_WMA))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash-key Support WMA.\n");
    }
    else
    {
        ip_auth |= 0x20;    /* Disable WMA */
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash Key Check WMA Fail, no WMA license!!\n");
    }

    if (MDrv_AUTH_IPCheck(AU_DVB_AUTHBIT_DRA))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash-key Support DRA.\n");
    }
    else
    {
        ip_auth |= 0x40;    /* Disable DRA */
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash Key Check DRA Fail, no DRA license!!\n");
    }

    if (MDrv_AUTH_IPCheck(AU_DVB_AUTHBIT_DTS_LBR))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash-key Support DTSLBR.\n");
    }
    else
    {
        ip_auth |= 0x80;    /* Disable DTSLBR */
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash Key Check DTSLBR Fail, no DTSLBR license!!\n");
    }

    if (MDrv_AUTH_IPCheck(AU_DVB2_AUTHBIT_DTSE))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash-key Support DTSE.\n");
    }
    else
    {
        ip_auth |= 0x400;    /* Disable DTSE */
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash Key Check DTSE Fail, no DTSE license!!\n");
    }

    if (MDrv_AUTH_IPCheck(AU_DVB2_AUTHBIT_DTSNeoUltra))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash-key Support DTSNeoUltra.\n");
    }
    else
    {
        ip_auth |= 0x800;    /* Disable DTSNeoUltra */
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash Key Check DTSNeoUltra Fail, no DTSNeoUltra license!!\n");
    }

    if (MDrv_AUTH_IPCheck(AU_DVB2_AUTHBIT_TSHD))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash-key Support SRS_TSHD.\n");
    }
    else
    {
        ip_auth |= 0x2000;    /* Disable SRS_TSHD */
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash Key Check SRS_TSHD Fail, no SRS_TSHD license!!\n");
    }

    if (MDrv_AUTH_IPCheck(AU_DVB2_AUTHBIT_SRS_StudioSND))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash-key Support SRS_THEATERSOUND.\n");
    }
    else
    {
        ip_auth |= 0x4000;    /* Disable SRS_THEATERSOUND */
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash Key Check SRS_THEATERSOUND Fail, no SRS_THEATERSOUND license!!\n");
    }

    if (MDrv_AUTH_IPCheck(AU_DVB2_AUTHBIT_DTS_StudioSound3D))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash-key Support DTS_StudioSound3D.\n");
    }
    else
    {
        ip_auth |= 0x8000;    /* Disable DTS_StudioSound3D */
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash Key Check DTS_StudioSound3D Fail, no DTS_StudioSound3D license!!\n");
    }

    if (MDrv_AUTH_IPCheck(AU_DVB_AUTHBIT_COOK))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash-key Support COOK.\n");
    }
    else
    {
        ip_auth |= 0x10000;    /* Disable COOK */
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash Key Check COOK Fail, no COOK license!!\n");
    }

    if (MDrv_AUTH_IPCheck(AU_DVB_AUTHBIT_DTSHD))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash-key Support DTS_HD.\n");
        ip_auth &= 0xFFFFFBFF;  //if DTSHD then enable DTSE
    }
    else
    {
        ip_auth |= 0x20000;    /* Disable DTS_HD */
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash Key Check DTS_HD Fail, no DTS_HD license!!\n");
    }

    if (MDrv_AUTH_IPCheck(AU_DVB2_AUTHBIT_DolbyVolume))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash-key Support Dolby volume.\n");
    }
    else
    {
        ip_auth |= 0x400000;    /* Disable Dolby volume */
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash Key Check Dolby volume Fail, no Dolby volume license!!\n");
    }

    if (MDrv_AUTH_IPCheck(AU_DVB2_AUTHBIT_SRS_PURESND))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash-key Support SRS_PURESND.\n");
    }
    else
    {
        ip_auth |= 0x800000;    /* Disable SRS_PURESND */
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Hash Key Check SRS_PURESND Fail, no SRS_PURESND license!!\n");
    }

    if (MDrv_AUTH_IPCheck(AU_DVB_AUTHBIT_DEMOMODE))
    {
        ip_auth |= 0x1000;
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "This is Demo Mode!!\n");
    }

    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "===== Check Protection IP End                         =====\n");
