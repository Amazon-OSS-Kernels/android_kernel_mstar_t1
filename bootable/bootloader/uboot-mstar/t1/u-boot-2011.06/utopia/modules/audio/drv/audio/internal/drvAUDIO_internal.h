...
//  Macro and Define
//=====================================================
#define  Audio_DumpInfo_TvDecoderStatus    0x10
...
#define  AU_DVB2_AUTHBIT_DOLBY_MS12_D_WO_AAC     117
  ...
  #define AUDIO_LOG(tag,fmt,...)  ALOG(LOG_ERROR, "AnUtopia", "[" tag "]" fmt, ##__VA_ARGS__)
#else
    #ifdef CONFIG_MBOOT
      //  #define AUDIO_LOG(fmt, args...)    printf("[[utopia]]      " fmt, ## args)
        #define AUDIO_LOG(tag,fmt,...)      printf("[Utopia][" tag "]: " fmt, ##__VA_ARGS__)
    #else
      //  #define AUDIO_LOG(fmt, args...)  printf("[[utopia]]      " fmt, ## args)
        #define AUDIO_LOG(tag,fmt,...)      printf("[Utopia][" tag "]: " fmt, ##__VA_ARGS__)
    #endif
...
{
    MS_BOOL bConnect;
    MS_U8 eConnectSource;
} AUDIO_CONNECTION_t;

typedef struct __attribute__((packed))
{
    AUDIO_CONNECTION_t AUOUT0;
    AUDIO_CONNECTION_t AUOUT1;
    AUDIO_CONNECTION_t AUOUT2;
    AUDIO_CONNECTION_t AUOUT3;
    AUDIO_CONNECTION_t I2S;
    AUDIO_CONNECTION_t I2S2;
    AUDIO_CONNECTION_t SPDIF;
    AUDIO_CONNECTION_t HDMI_ARC;
} AUDIO_CONNECTION_INFO_t;
...
typedef struct
{
    ...
    AUDIO_SPDIF_OUTPUT_TYPE g_UserSpdifType;
    ...
    MS_U16   g_DecoderInfoTag;
    AUDIO_CONNECTION_INFO_t g_ConnectInfo;
    MS_BOOL  g_spdif_force_ddp;
    MS_BOOL  g_system_alarm_mode;
