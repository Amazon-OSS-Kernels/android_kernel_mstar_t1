...
MS_BOOL HAL_AUDIO_ResetDefaultVars(AUDIO_SHARED_VARS2 * pAUDIOShared)
{
    ...
    pAUDIOShared->g_DecoderInfoTag = 0xFFFF;

//==============================
// Audio connection initialize
    pAUDIOShared->g_ConnectInfo.AUOUT0.bConnect = 1;
    pAUDIOShared->g_ConnectInfo.AUOUT1.bConnect = 1;
    pAUDIOShared->g_ConnectInfo.AUOUT2.bConnect = 1;
    pAUDIOShared->g_ConnectInfo.AUOUT3.bConnect = 1;
    pAUDIOShared->g_ConnectInfo.I2S.bConnect = 1;
    pAUDIOShared->g_ConnectInfo.I2S2.bConnect = 1;
    pAUDIOShared->g_ConnectInfo.SPDIF.bConnect = 0;
    pAUDIOShared->g_ConnectInfo.HDMI_ARC.bConnect = 1;

    pAUDIOShared->g_ConnectInfo.AUOUT0.eConnectSource = 0xE; //Connect to NULL source
    pAUDIOShared->g_ConnectInfo.AUOUT1.eConnectSource = 0xE;
    pAUDIOShared->g_ConnectInfo.AUOUT2.eConnectSource = 0xE;
    pAUDIOShared->g_ConnectInfo.AUOUT3.eConnectSource = 0xE;
    pAUDIOShared->g_ConnectInfo.I2S.eConnectSource = 0xE;
    pAUDIOShared->g_ConnectInfo.I2S2.eConnectSource = 0xE;
    pAUDIOShared->g_ConnectInfo.SPDIF.eConnectSource = 0xE;
    pAUDIOShared->g_ConnectInfo.HDMI_ARC.eConnectSource = 0xE;

    pAUDIOShared->g_system_alarm_mode = FALSE;
    pAUDIOShared->g_spdif_force_ddp = FALSE;
    pAUDIOShared->g_dts_drc_percent= 0;
...
AUDIO_SHARED_VARS2 * HAL_AUDIO_AllocateVars(void)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Start %s InitShmFlag=%d\r\n", __FUNCTION__, g_bInitShmFlag);
    ...
    if (MsOS_SHM_GetId((MS_U8 *)AUDIO_SHM_NAME, sizeof(AUDIO_SHARED_VARS2), &u32ShmId, (MS_VIRT *)&virtAddr, &u32BufSize, MSOS_SHM_QUERY) == TRUE)
    {
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "%s QUERY ClientCounter=%u\r\n", __FUNCTION__, pAUDIOShared->g_u32ClientCounter);

        if (pAUDIOShared->g_u32ClientCounter == 0)
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Resume from Suspend mode, reset Audio SHM data ...\n");
        }
    }
    else
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "create Audio SHM data ...\n");
        ...
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Audio SHM data is created\n");
            pAUDIOShared = (AUDIO_SHARED_VARS2 *) virtAddr;

            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "reset Audio SHM data ...\n");
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "End %s InitShmFlag=%d, ClientCounter=%u \r\n", __FUNCTION__, g_bInitShmFlag, pAUDIOShared->g_u32ClientCounter);
...
void HAL_AUDIO_SetPowerOn(MS_BOOL bPower_on)
{
    ...
    {
        //HAL_AUDIO_DeInitialVars();   //share memory can not de-init when STR
...
void HAL_AUDIO_SwResetMAD(void)
{
    ...
    HAL_AUDIO_WriteMaskByte(REG_AUDIO_DECIMATION_CFG,      0x04, 0x04); //[2]=1       HDMI TX  Full Decimation mode.
    HAL_AUDIO_WriteMaskByte(REG_AUDIO_DECIMATION_CFG,      0x02, 0x02); //[1]=1       HDMI ARC Full Decimation mode.
}
...
{
    MS_BOOL bUpdateConnectStatus = 0;
    MS_BOOL bConnectStatus = 1;
    MS_U8   path = 0x0E;
    MS_U16 regValue = 0x0000;
    ...
    if((u8Path == INTERNAL_ATVENC) ||(u8Path == INTERNAL_TONE) )
    {
        bUpdateConnectStatus = 1;
        if(u8Path == INTERNAL_TONE)  // TONE ==> connect   ;  ATVENC ==> Disconnect
        {
            bConnectStatus = 1;
        }
        else
      ...
      switch(u8Output)
    {
        case AUDIO_AUOUT0_OUTPUT:
            if(bUpdateConnectStatus == 1)  // update connection
            {
                g_AudioVars2->g_ConnectInfo.AUOUT0.bConnect = bConnectStatus;
            }
            else  // update source
            {
                g_AudioVars2->g_ConnectInfo.AUOUT0.eConnectSource = (MS_U8)u8Path;
            }

            path = (g_AudioVars2->g_ConnectInfo.AUOUT0.bConnect == 1) ?  g_AudioVars2->g_ConnectInfo.AUOUT0.eConnectSource : 0x0E;
        ...
        case AUDIO_HP_OUTPUT:
            if(bUpdateConnectStatus == 1)  // update connection
            {
                g_AudioVars2->g_ConnectInfo.AUOUT1.bConnect = bConnectStatus;
            }
            else  // update source
            {
                g_AudioVars2->g_ConnectInfo.AUOUT1.eConnectSource = (MS_U8)u8Path;
            }

            path = (g_AudioVars2->g_ConnectInfo.AUOUT1.bConnect == 1) ?  g_AudioVars2->g_ConnectInfo.AUOUT1.eConnectSource : 0x0E;
        ...
        case AUDIO_AUOUT2_OUTPUT:
            if(bUpdateConnectStatus == 1)  // update connection
            {
                g_AudioVars2->g_ConnectInfo.AUOUT2.bConnect = bConnectStatus;
            }
            else  // update source
            {
                g_AudioVars2->g_ConnectInfo.AUOUT2.eConnectSource = (MS_U8)u8Path;
            }

            path = (g_AudioVars2->g_ConnectInfo.AUOUT2.bConnect == 1) ?  g_AudioVars2->g_ConnectInfo.AUOUT2.eConnectSource : 0x0E;
        ...
        case AUDIO_AUOUT3_OUTPUT:
            if(bUpdateConnectStatus == 1)  // update connection
            {
                g_AudioVars2->g_ConnectInfo.AUOUT3.bConnect = bConnectStatus;
            }
            else  // update source
            {
                g_AudioVars2->g_ConnectInfo.AUOUT3.eConnectSource = (MS_U8)u8Path;
            }

            path = (g_AudioVars2->g_ConnectInfo.AUOUT3.bConnect == 1) ?  g_AudioVars2->g_ConnectInfo.AUOUT3.eConnectSource : 0x0E;
        ...
        case AUDIO_I2S_OUTPUT:
            if(bUpdateConnectStatus == 1)  // update connection
            {
                g_AudioVars2->g_ConnectInfo.I2S.bConnect = bConnectStatus;
            }
            else  // update source
            {
                g_AudioVars2->g_ConnectInfo.I2S.eConnectSource = (MS_U8)u8Path;
            }

            path = (g_AudioVars2->g_ConnectInfo.I2S.bConnect == 1) ?  g_AudioVars2->g_ConnectInfo.I2S.eConnectSource : 0x0E;
        ...
        case AUDIO_SPDIF_OUTPUT:
            if(bUpdateConnectStatus == 1)  // update connection
            {
                g_AudioVars2->g_ConnectInfo.SPDIF.bConnect = bConnectStatus;
            }
            else  // update source
            {
                g_AudioVars2->g_ConnectInfo.SPDIF.eConnectSource = (MS_U8)u8Path;
            }

            regValue = (g_AudioVars2->g_ConnectInfo.SPDIF.bConnect) ? 0: SPDIF_OUTPUT_DISABLE_MASK;
            HAL_AUDIO_WriteMaskReg(REG_AUDIO_SPDIF_OUT_CFG, SPDIF_OUTPUT_DISABLE_MASK, regValue);
            HAL_AUDIO_WriteMaskByte(M2S_MBOX_INPUT_MUX_SEL3+1, 0x0F, g_AudioVars2->g_ConnectInfo.SPDIF.eConnectSource);
            break;

        case AUDIO_HDMI_ARC_OUTPUT:
            if(bUpdateConnectStatus == 1)  // update connection
            {
                g_AudioVars2->g_ConnectInfo.HDMI_ARC.bConnect = bConnectStatus;
            }
            else  // update source
            {
                g_AudioVars2->g_ConnectInfo.HDMI_ARC.eConnectSource = (MS_U8)u8Path;
            }

            regValue = (g_AudioVars2->g_ConnectInfo.HDMI_ARC.bConnect) ? 0: SPDIF_OUTPUT_DISABLE_MASK;
            if(g_AudioVars2->g_spdif_force_ddp == TRUE) // force enable spdif DDP case
            {
                regValue = 0 ;   // Connect ARC for spdif ddp output
            }
            HAL_AUDIO_WriteMaskReg(REG_AUDIO_SPDIF2_OUT_CFG, SPDIF_OUTPUT_DISABLE_MASK, regValue);
            HAL_AUDIO_WriteMaskByte(M2S_MBOX_INPUT_MUX_SEL3+1, 0xF0, (g_AudioVars2->g_ConnectInfo.HDMI_ARC.eConnectSource<<4));
            break;

        case AUDIO_I2S2_OUTPUT:
            if(bUpdateConnectStatus == 1)  // update connection
            {
                g_AudioVars2->g_ConnectInfo.I2S2.bConnect = bConnectStatus;
            }
            else  // update source
            {
                g_AudioVars2->g_ConnectInfo.I2S2.eConnectSource = (MS_U8)u8Path;
            }

            path = (g_AudioVars2->g_ConnectInfo.I2S2.bConnect == 1) ?  g_AudioVars2->g_ConnectInfo.I2S2.eConnectSource : 0x0E;
...
const char * _internal_path_name(MS_U8 path)
{
    AUDIO_INTERNAL_PATH_TYPE ePath = (AUDIO_INTERNAL_PATH_TYPE) path;

    switch (ePath)
    {
        case INTERNAL_MULTI_1:
            return "multi_1";

        case INTERNAL_MULTI_2:
            return "multi_2";

        case INTERNAL_MULTI_3:
            return "multi_3";

        case INTERNAL_MULTI_4:
            return "multi_4";

        case INTERNAL_PCM:
            return "pcm";

        case INTERNAL_PCM_DELAY:
            return "pcm_delay";

        case INTERNAL_PCM_SE:
            return "pcm_delay_se";
...
const char * _spdif_mode_name(MS_U8 mode)
{
    AUDIO_SPDIF_OUTPUT_TYPE eMode = (AUDIO_SPDIF_OUTPUT_TYPE) mode;
...
const char * _audio_source_name(MS_U8 source)
{
    AUDIO_SOURCE_INFO_TYPE eSource = (AUDIO_SOURCE_INFO_TYPE) source;
...
const char * _audio_path_name(MS_U8 path)
{
    switch (path)
    {
...
const char * _audio_codec_name(AUDIO_DSP_CODE_TYPE eCodec)
{
    switch (eCodec)
    {
        case AU_DVB_NONE:
        ...
        case AU_DVB_STANDARD_MPEG_AD:
            return "mpeg-ad";

        case AU_DVB_STANDARD_DolbyTrueHD_Bypass:
            return "dolby-true-hd-bypass";

        case AU_DVB_STANDARD_DTSHD_ADO:
            return "dts-hd-ado";
...
const char * _audio_channel_mode(AUD_CH_MODE_TYPE eChannelMode)
{
    switch (eChannelMode)
    {
        ...
        case AUD_CH_MODE_3_0_MODE:
        case AUD_CH_MODE_2_1_MODE:
        case AUD_CH_MODE_3_1_MODE:
        case AUD_CH_MODE_2_2_MODE:
        case AUD_CH_MODE_3_2_MODE:
...
const char * _specified_codec_name(MS_U32 Codec)
{
    DIGITAL_OUTPUT_SPECIFIED_CODEC eCodec = (DIGITAL_OUTPUT_SPECIFIED_CODEC) Codec;

    switch (eCodec)
    {
        case DIGITAL_OUTPUT_CODEC_AC3:
            return "ac3";

        case DIGITAL_OUTPUT_CODEC_AC3P:
            return "eac3";

        case DIGITAL_OUTPUT_CODEC_AAC:
            return "aac";

        case DIGITAL_OUTPUT_CODEC_DTS:
            return "dts";

        case DIGITAL_OUTPUT_CODEC_NOT_SPECIFIED:
        case DIGITAL_OUTPUT_CODEC_MAX:
        default:
            return "pcm";
            break;
    }
...
const MS_U32 _get_sample_rate(MS_U64 sr)
{
    AUDIO_FS_TYPE eFsType = (AUDIO_FS_TYPE) sr;
...
void HAL_AUDIO_RebootDecDSP(void)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "*** Load code and reset DEC DSP \n");
    HAL_MAD_SetMemInfo();
    HAL_AUDIO_ResetDSP();
...
MS_BOOL HAL_AUDIO_SeSystemLoadCode(void)
{
    ...
    if (time_out>=100) {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "DSP2 Re-Active\n");
    }
    else {
...
AUDIO_DEC_ID HAL_AUDIO_OpenDecodeSystem(AudioDecStatus_t * p_AudioDecStatus)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG,"DecID = %d %s() %d \n",DecRet,__FUNCTION__,__LINE__);
    return DecRet;
}
...
MS_BOOL HAL_AUDIO_SetDecodeSystem(AUDIO_DEC_ID DecId, AudioDecStatus_t *p_AudioDecStatus)
{
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "%s: eAudFormat:(0x%x) \n", __FUNCTION__, p_AudioDecStatus->eAudFormat);
	...
	AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "%s: DecId:%d \n", __FUNCTION__, DecId);
    ...
    {
		AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "%s: MainDecoderId:%d \n", __FUNCTION__, MainDecoderId);
        ...
        if (MainDecoderId != -1)
        {
            ...
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "%s: 2MainDecoderId:%d \n", __FUNCTION__, MainDecoderId);
...
{
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "\033[1;32m==========Audio R2 & DSP Version Check==================\033[0m");
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "\033[1;32m         DEC-R2:0x%06X     SND-R2:%06X    DSP System:0x%06X    \033[0m \r\n", DEC_R2_VERSION, SND_R2_VERSION, system_version_num);
#if ASND_R2_SUPPORT
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN, "\033[1;32m        Audio  DEC-R2  Version  Mismatch!!!!!!!    \033[0m \r\n");
    }
    return TRUE;
}
...
MS_S32 HAL_AUDIO_PCM_HwDma_Reader1_Apply_Setting(void)
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN, "%s: Error! Too large buffer size(%u) is required! Please check the settings: duration %u, sample rate %u\n", __FUNCTION__, u32TargetBufferSize, pPcmInfo->u32BufferDuration, pPcmInfo->u32SampleRate);
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN, "%s: Error! Force duration to %u, which is the maximun value\n", __FUNCTION__, pPcmInfo->u32BufferDuration);
    }
...
MS_S32 HAL_AUDIO_PCM_HwDma_Reader1_Open(void *pData)
{
    ...
    MS_U32 u32TargetAlignmentSize = 0;
    MS_S32 s32Ret = 0;
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: Open %s\n", __FUNCTION__, pPCM->u8Name);
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: The size of PCM Info mismatch(%u, %zu)!\n", __FUNCTION__, pUserPcmInfo->u32StructSize, sizeof(AUDIO_PCM_INFO_t));
        return -A_EINVAL;
    }
...
MS_S32 HAL_AUDIO_PCM_HwDma_Reader1_Close(void)
{
    ...
    AUDIO_PCM_INFO_t *pPcmInfo = pPCM->pPcmInfo;
    MS_S32 s32Ret = 0;
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: Close %s\n", __FUNCTION__, pPCM->u8Name);
...
MS_S32 HAL_AUDIO_PCM_HwDma_Reader1_Start(void)
{
    ...
    AUDIO_PCM_INFO_t *pPcmInfo = pPCM->pPcmInfo;
    MS_S32 s32Ret = 0;
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: Start %s\n", __FUNCTION__, pPCM->u8Name);
...
MS_S32 HAL_AUDIO_PCM_HwDma_Reader1_Stop(void)
{
    ...
    AUDIO_PCM_INFO_t *pPcmInfo = pPCM->pPcmInfo;
    MS_S32 s32Ret = 0;
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: Stop %s\n", __FUNCTION__, pPCM->u8Name);
...
MS_S32 HAL_AUDIO_PCM_HwDma_Reader1_Get(MS_U32 u32Cmd, void *pData)
{
    ...
    switch(u32Cmd)
    {
        case AUDIO_PCM_CMD_ALL:
        {
            ...
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN, "%s: Warning! The version of PCM Info mismatch(0x%08X, 0x%08X)!\n", __FUNCTION__, pUserPcmInfo->u32StructVersion, AUDIO_PCM_INFO_VERSION);
            }
...
MS_U32 HAL_AUDIO_PCM_HwDma_Reader1_Write(const void *pBuf, MS_U32 u32Size)
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN, "%s: Warning! PCM buffer empty!\n", __FUNCTION__);
        ...
        for (u32Loop = 0; u32Loop < u32RequestSampleCount; u32Loop++)
        {
            ...
            *pPCM->tPcmBufferInfo.pWritePtr++ = 0;
            ...
            if (pPCM->tPcmBufferInfo.pWritePtr >= (pPCM->tPcmBufferInfo.pBuffer + u32BufferSize))
            {
                if (pPCM->tPcmBufferInfo.pWritePtr > (pPCM->tPcmBufferInfo.pBuffer + u32BufferSize))
                {
                    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN, "%s: Warning! it's an abnormal copy!\n", __FUNCTION__);
                }
                pPCM->tPcmBufferInfo.pWritePtr = pPCM->tPcmBufferInfo.pBuffer;
            }
        }
...
MS_S32 HAL_AUDIO_PCM_HwDma_Reader1_Flush(void)
{
    ...
    AUDIO_PCM_INFO_t *pPcmInfo = pPCM->pPcmInfo;
    MS_S32 s32Ret = 0;
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: Flush %s\n", __FUNCTION__, pPCM->u8Name);
...
MS_S32 HAL_AUDIO_PCM_HwDma_Reader2_Get(MS_U32 u32Cmd, void *pData)
{
    ...
    switch(u32Cmd)
    {
        ...
        case AUDIO_PCM_CMD_BUFFERLEVEL:
        {
            ...
            if (u32BufferLevel1 != u32BufferLevel2)
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN, "%s: Error! fail to get correct buffer level (%u, %u)\n", __FUNCTION__, u32BufferLevel1, u32BufferLevel2);
            }
...
MS_S32 HAL_AUDIO_PCM_R2Dma_Reader1_Apply_Setting(void)
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN, "%s: Error! Too large buffer size(%u) is required! Please check the settings: duration %u, sample rate %u, channel %u\n", __FUNCTION__, u32TargetBufferSize, pPcmInfo->u32BufferDuration, pPcmInfo->u32SampleRate, u32TargetChannel);
...
void HAL_AUDIO_DebugLog(void)
{
    ...
    MS_U16 input_vol;
    MS_U16 speaker_vol;
    MS_U16 headphone_vol;
    MS_U8 system_mute;
    MS_U8 input_mute;
    MS_U8 speaker_mute;
    MS_U8 headphone_mute;
    MS_U8 spdif_mute;
    MS_U8 spdif_pcm_mode = (g_AudioVars2->g_eSpdifOutputType == SPDIF_OUT_PCM) ? 1 : 0;

    // Input
    system_vol = ((HAL_AUDIO_ReadByte(REG_SOUND_CH8_MIX_VOL_INT) & 0x7F) << 8) | (((HAL_AUDIO_ReadByte(REG_SOUND_CH8_MIX_VOL_FRC) & 0xE0) >> 5) & 0x00FF);
    multi_vol = (MS_U16)HAL_MAD2_Read_DSP_sram(DSP2DmAddr_Multi_Channel_VOL, DSP_MEM_TYPE_DM) & 0x7FFF;
    input_vol = ((HAL_AUDIO_ReadByte(REG_SOUND_CH5_MIX_VOL_INT) & 0x7F) << 8) | (((HAL_AUDIO_ReadByte(REG_SOUND_CH5_MIX_VOL_FRC) & 0xE0) >> 5) & 0x00FF);

    system_mute = (HAL_AUDIO_ReadByte(REG_SOUND_CH8_MIX_VOL_INT) & 0x80) >> 7;
    multi_vol = ((MS_U16)HAL_MAD2_Read_DSP_sram(DSP2DmAddr_Multi_Channel_VOL, DSP_MEM_TYPE_DM) & 0x8000) >> 7;
    input_mute = (HAL_AUDIO_ReadByte(REG_SOUND_CH5_MIX_VOL_INT) & 0x80) >> 7;

    if ((((g_AudioVars2->eAudioSource == E_AUDIO_INFO_MM_IN) || (g_AudioVars2->eAudioSource == E_AUDIO_INFO_GAME_IN)) && (HAL_MAD_GetDecCmd(DVB_Audio_Decoder2) == AU_DVB_DECCMD_PLAYFRAME_GS)) ||
        HAL_MAD_GetAC3PInfo(Audio_AC3P_infoType_hdmiAC3inSE))
    {
        if ((((HAL_AUDIO_AbsReadByte(REG_R2_DECODE2_TYPE) & adec_type_regmask) == adec_type_ac3) ||
            ((HAL_AUDIO_AbsReadByte(REG_R2_DECODE2_TYPE) & adec_type_regmask) == adec_type_ac3p)) &&
            (HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_STAUS, ADEC2) & 0x0001))
        {
            MainDspCodeType = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DOLBY_DEC_TYPE, ADEC2) ? AU_DVB_STANDARD_AC3P : AU_DVB_STANDARD_AC3;
        }
        else if ((HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_STAUS, ADEC2) & 0x0001))
        {
            MainDspCodeType = AU_DVB_STANDARD_INVALID; // pcm
        }
        else
        {
            MainDspCodeType = g_AudioVars2->g_Dsp2CodeType;
        }
    }
    else
    {
        ...
        {
            MainDspCodeType = HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DOLBY_DEC_TYPE, ADEC1) ? AU_DVB_STANDARD_AC3P : AU_DVB_STANDARD_AC3;
        }
        else if ((HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_STAUS, ADEC1) & 0x0001))
        {
            MainDspCodeType = AU_DVB_STANDARD_INVALID; // pcm
        }
        else
    ...
    switch (g_AudioVars2->eAudioSource)
    {
        ...
        case E_AUDIO_INFO_GAME_IN:
            if (MainDspCodeType == AU_DVB_STANDARD_AC3 ||
                ...
                snprintf(input, MAX_STR_LEN, "passthrough[na/0x%04x/%s/%s]", input_vol, input_mute ? "y" : "n", _audio_codec_name(MainDspCodeType));
            }
            break;
        case E_AUDIO_INFO_DTV_IN:
            snprintf(input, MAX_STR_LEN, "dtv[na/0x%04x/%s/%s]", input_vol, input_mute ? "y" : "n", _audio_codec_name(MainDspCodeType));
            break;
        case E_AUDIO_INFO_ATV_IN:
        case E_AUDIO_INFO_ADC_IN:
        case E_AUDIO_INFO_KTV_IN:
            snprintf(input, MAX_STR_LEN, "av[na/0x%04x/%s/pcm]", input_vol, input_mute ? "y" : "n");
            break;
        case E_AUDIO_INFO_HDMI_IN:
        {
            if (HAL_AUDIO_HDMI_NonpcmMonitor())
            {
                snprintf(input, MAX_STR_LEN, "hdmi[na/0x%04x/%s/%s]", input_vol, input_mute ? "y" : "n", _audio_codec_name(MainDspCodeType));
            }
            else
            {
                snprintf(input, MAX_STR_LEN, "hdmi[na/0x%04x/%s/pcm]", input_vol, input_mute ? "y" : "n");
            }
            break;
        }
        default:
    ...
    // Output
    speaker = (HAL_AUDIO_ReadByte(M2S_MBOX_INPUT_MUX_SEL2) & 0x0F);
    headphone = (HAL_AUDIO_ReadByte(M2S_MBOX_INPUT_MUX_SEL1) & 0xF0) >> 4;
    spdif = !((HAL_AUDIO_ReadByte(0x2C8B) & 0x08) >> 3);
    arc = !((HAL_AUDIO_ReadByte(0x2C5F) & 0x08) >> 3);

    speaker_vol = ((HAL_AUDIO_ReadByte(REG_SOUND_I2S_VOLUME) & 0x7F) << 8) | (((HAL_AUDIO_ReadByte(REG_SOUND_I2S_VOL_FRAC) & 0xE0) >> 5) & 0x00FF);
    headphone_vol = ((HAL_AUDIO_ReadByte(REG_SOUND_AUOUT1_VOLUME) & 0x7F) << 8) | (((HAL_AUDIO_ReadByte(REG_SOUND_AUOUT1_VOL_FRAC) & 0xE0) >> 5) & 0x00FF);

    speaker_mute = (HAL_AUDIO_ReadByte(REG_SOUND_I2S_VOLUME) & 0x80) >> 7;
    headphone_mute = (HAL_AUDIO_ReadByte(REG_SOUND_AUOUT1_VOLUME) & 0x80) >> 7;
    spdif_mute = (HAL_AUDIO_ReadByte(REG_M2D_MAILBOX_SPDIF_CTRL) & 0x01);

    snprintf(output, MAX_STR_LEN, "speaker[%s/0x%04x/%s/pcm] headphone[%s/0x%04x/%s/pcm] arc[%s/na/%s/%s] spdif[%s/na/%s/%s]",
        _internal_path_name(speaker), speaker_vol, (speaker_mute == 1) ? "y" : "n",
        _internal_path_name(headphone), headphone_vol, (headphone_mute == 1) ? "y" : "n",
        arc ? "y" : "n", (spdif_mute == 1) ? "y" : "n", spdif_pcm_mode ? "pcm" : _specified_codec_name(g_AudioVars2->g_AudioSpecifiedDigitalOutput.u32HdmiArcOutputCodec),
        spdif ? "y" : "n", (spdif_mute == 1) ? "y" : "n", spdif_pcm_mode ? "pcm" : _specified_codec_name(g_AudioVars2->g_AudioSpecifiedDigitalOutput.u32SpdifOutputCodec));

    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: device[path/volume/force mute/format], in:AndroidMix[na/0x%04x/%s/pcm] %s, out:%s", __FUNCTION__, system_vol, system_mute ? "y" : "n", input, output);
}
...
MS_S32 HAL_AUDIO_SetEDIDOutputInterface(DIGITAL_OUTPUT_TYPE outpath)
{
    ...
    {
        // Not Support now
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Warning! it's a not support output HBR path!\n", __FUNCTION__);
        return FALSE;
    }
...
MS_U32 HAL_AUDIO_MP3_ENCODER1_Read(void *pBuf, MS_U32 u32Size)
{
    ...
    //AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "[%u] %s: u32BufferSize %u, u32Threshold %u, u32Mp3Level %u, u32RequestSize %u\n", MsOS_GetSystemTime(), __FUNCTION__, u32BufferSize, u32Threshold, u32Mp3Level, u32RequestSize);
...
#endif
            //AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "[%u] %s: g_u32Mp3Encoder1RptrOffset 0x%08X, SizeToCopy %u, ReadPtr 0x%08X, Buf 0x%08X, BufEnd 0x%08X\n", MsOS_GetSystemTime(), __FUNCTION__, g_u32Mp3Encoder1RptrOffset, u32SizeToCopy, pEncoderReadPtr, pEncoderBuf, (pEncoderBuf + u32BufferSize));
...
MS_U32 HAL_AUDIO_MP3_ENCODER2_Read(void *pBuf, MS_U32 u32Size)
{
    ...
    if (u32Mp3Level >= u32RequestSize)
    {
        do {
            ...
            memcpy((void *)pBufTmp, (void *)pEncoderReadPtr, u32SizeToCopy);
#endif
            //AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "[%u] %s: g_u32Mp3Encoder2RptrOffset 0x%08X, SizeToCopy %u, ReadPtr 0x%08X, Buf 0x%08X, BufEnd 0x%08X\n", MsOS_GetSystemTime(), __FUNCTION__, g_u32Mp3Encoder2RptrOffset, u32SizeToCopy, pEncoderReadPtr, pEncoderBuf, (pEncoderBuf + u32BufferSize));
