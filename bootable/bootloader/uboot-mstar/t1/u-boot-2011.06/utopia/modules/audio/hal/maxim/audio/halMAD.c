...
MMA_AudioType _GetAudioCaps(AUDIO_DEC_ID eDecId)
{
    ...
    if ( eDecId == AU_DEC_ID1 )
    {
        eAudioCaps = Audio_DEC_PCM        | Audio_DEC_MPEG     |
                     ...
                     Audio_DEC_DRA        | Audio_DEC_AC4      |
                     Audio_DEC_MPEG_H     | Audio_DEC_OPUS;
    }
...
static MS_BOOL _getIntByKeyword(char *param, const char *keyword, MS_S32 *value)
{
    MS_BOOL ret = FALSE;
    const char * split = "=,;";
    char pParam[256] = {0};

    if (param == NULL || keyword == NULL || value == NULL) {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "param/keyword/value should not be NULL\n");
        goto exit;
    }

    strcpy(pParam, param);

    char *str = strtok(pParam, split);

    if (str == NULL) {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "invalid param(%s)\n", param);
        goto exit;
    }
...
{
    return; // Unused function in 2R1D chip
}
...
MS_BOOL HAL_MAD_MM2_initAesInfo( AUDIO_DEC_ID dec_id )
{
    ...
    if ( dec_id == AU_DEC_ID1 )
    {
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "ID1: HAL_AUDIO_MM2_init end !!\r\n");
        return TRUE;
    }
    else if( dec_id == AU_DEC_ID3 )
    {
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "ID3: HAL_AUDIO_MM2_init end !!\r\n");
        return TRUE;
    }
    else
        return FALSE;
...
MS_BOOL HAL_MAD_MM2_checkAesInfo( AUDIO_DEC_ID  dec_id, AES_INFO *aes_info )
{
    ...
    if ( dec_id == AU_DEC_ID1 )
    {
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "[ID1:HAL_MAD_MM2_checkAesInfo] Ptr[w:%x,r:%x] LvL:%x, FreeSpace:%x, tmpFree:%x \n",
    ...
    else if( dec_id == AU_DEC_ID3 )
    {
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "[ID3:HAL_MAD_MM2_checkAesInfo] Ptr[w:%x,r:%x] LvL:%x, FreeSpace:%x, tmpFree:%x \n",
...
MS_BOOL HAL_MAD_MM2_inputAesFinished( AUDIO_DEC_ID dec_id, MS_U32 es_size, MS_BOOL ptsExist, MS_U64 pts )
{
    ...
    if ( dec_id == AU_DEC_ID1 )
    {
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "[ID1:HAL_MAD_MM2_inputAesFinished] wPtr:%08x => ", (unsigned int)audio_aes_info[0].aes_write_addr);
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "%08x, size=%08x, isPtsExist:%d, PTS=%09lx\n",
#else
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "%08x, size=%08x, isPtsExist:%d, PTS=%09llx\n",
#endif
            (unsigned int)audio_aes_info[0].aes_write_addr,
        ...
        if (ptsExist)
        {
            if (prv_PTS != 0)
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, ",PTS_diff=%08lld,accum_ES = %08x,ES_diff= %08d, ES PTS diff=%08d\n",
            ...
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, ", PTS_diff=%08x, accum_ES = %08x\n", 0, 0);
            }
    ...
    else if (dec_id == AU_DEC_ID3)
    {
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "[ID3:HAL_MAD_MM2_inputAesFinished] wPtr:%08x => ", (unsigned int)audio_aes_info[1].aes_write_addr);
...
void HAL_MAD_Monitor_DDPlus_SPDIF_Rate(void)
{
    ...
    HAL_AUDIO_SPDIF_SetMode(u8SpdifMode, eAudioSource);
...
//                HAL_AUDIO_WriteMaskReg(M2S_MBOX_DBG_CMD2, 0xFFFF, param1);
        ...
        case Audio_Comm_infoType_Set_CallBackFunc:
            ...
            {
                g_FuncPrt_Hal_SetAudioParam2 = HAL_MAD_SetAudioParam2;
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: g_FuncPrt_Hal_SetAudioParam2 hook OK !!\r\n", __FUNCTION__);
            }
            else
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: g_FuncPrt_Hal_SetAudioParam2 already register !!\r\n", __FUNCTION__);
            }
            if ( g_FuncPrt_Hal_GetAudioInfo2 == NULL )
            {
                g_FuncPrt_Hal_GetAudioInfo2 = HAL_MAD_GetAudioInfo2;
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: g_FuncPrt_Hal_GetAudioInfo2 hook OK !!\r\n", __FUNCTION__);
            }
            else
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: g_FuncPrt_Hal_GetAudioInfo2 already register !!\r\n", __FUNCTION__);
            }
            break;
        ...
        ********************************************************************************/
...
#ifdef T1_ONLY
        case Audio_Comm_infoType_Set_Decoder_Info_Tag:
            g_AudioVars2->g_DecoderInfoTag = param1;
            break;
#endif
        ...
        case Audio_Comm_infoType_Multi_Channel:
        {
            ...
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "!!!!!!!!!set Audio_Comm_infoType_Multi_Channel = [0x%x] \n", (unsigned int)param1);
        ...
        case Audio_Comm_infoType_Set_1st_PreferLang:
        {
            Audio_id adec_id = HAL_AUDIO_Convert_DecId_to_ADECId(param1);
            HAL_DEC_R2_Set_SHM_PARAM( R2_SHM_PARAM_AC4_1ST_PREF_LANG, adec_id, param2, 0);   //param1(dec_id)
            break;
        }
        case Audio_Comm_infoType_Set_2nd_PreferLang:
        {
            Audio_id adec_id = HAL_AUDIO_Convert_DecId_to_ADECId(param1);
            HAL_DEC_R2_Set_SHM_PARAM( R2_SHM_PARAM_AC4_2ND_PREF_LANG, adec_id, param2, 0);   //param1(dec_id)
            break;
        }
        case Audio_Comm_infoType_Set_AC4_DE_Gain:
        {
            Audio_id adec_id = HAL_AUDIO_Convert_DecId_to_ADECId(param1);
            HAL_DEC_R2_Set_SHM_PARAM( R2_SHM_PARAM_AC4_DE_GAIN, adec_id, param2, 0);   //param1(dec_id)
            break;
        }
        case Audio_Comm_infoType_Set_AC4_MD_PresentationIndex:
        {
            Audio_id adec_id = HAL_AUDIO_Convert_DecId_to_ADECId(param1);
            HAL_DEC_R2_Set_SHM_PARAM( R2_SHM_PARAM_AC4_MD_PRESENT_INDEX, adec_id, param2, 0);   //param1(dec_id)
            break;
        }
        case Audio_Comm_infoType_Set_AC4_AD_PresentationIndex:
        {
            Audio_id adec_id = HAL_AUDIO_Convert_DecId_to_ADECId(param1);
            HAL_DEC_R2_Set_SHM_PARAM( R2_SHM_PARAM_AC4_AD_PRESENT_INDEX, adec_id, param2, 0);   //param1(dec_id)
            break;
        }
        /* not supported cmd */
        default:
...
#ifdef T1_ONLY
        case Audio_Comm_infoType_Get_Decoder_Info_Tag:
            result =  g_AudioVars2->g_DecoderInfoTag;
            break;
#endif
...
MS_BOOL HAL_MAD_SetAC3PInfo( Audio_AC3P_infoType infoType, MS_VIRT param1, MS_VIRT param2 )
{
    ...
    switch(infoType)
    {
        ...
        case Audio_AC3P_infoType_hdmiTxBypass_enable:
            ...
            {
                #if defined (CONFIG_UTOPIA_ANDROID_L)
                    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "%s: param1[%x,%08x,%08x] out of range\n",__FUNCTION__, infoType, param1, param2);
                #else
                    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "%s: param1[%x,%x,%x] out of range\n",__FUNCTION__, infoType, (unsigned int)param1, (unsigned int)param2);
                #endif
            }
        ...
        case Audio_AC3P_DTV_AVSYNC_OFFSET:
            if ((param1<DELAY_LOWER_LIMIT)||(param1>DELAY_UPPER_LIMIT))              //delay offset range from 30ms ~ 1050ms
            {
                g_ac3_delay = DEFAULT_AVSYNC_OFFSET;
            }
            else
            {
                g_ac3_delay = param1;
...
MS_BOOL HAL_MAD_SetAudioParam2( AUDIO_DEC_ID DecId, Audio_ParamType paramType, MS_VIRT Param )
{
    ...
    switch ( paramType )
    {
        ...
        case Audio_ParamType_UNI_NEED_DECODE_FRMCNT:
            ...
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "ID:%X   Audio_ParamType_UNI_NEED_DECODE_FRMCNT =0x%X\n", adec_id, (unsigned int)Param);
        ...
        case Audio_ParamType_UNI_ES_Wptr:  // OMX Wptr = offset to MAD start address
            ...
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "ID:%X  Audio_ParamType_UNI_ES_Wptr =0x%X\n", adec_id, (unsigned int)Param);
        ...
        case Audio_ParamType_UNI_PCMOutCnt:
        {
            if(adec_id < MAX_ADEC)
            {
                ...
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "Trigger decInt[%d] %d\n", adec_id, (unsigned int)uniPlay_ackCnt[adec_id]);
        ...
        case Audio_ParamType_AllocDecoder:
        {
            ...
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "DecID:%X  Audio_ParamType_AllocDecoder =0x%X\n", (unsigned int)DecId, (unsigned int)Param);
        ...
        case Audio_ParamType_FreeDecoder:
            ...
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_INFO, "%s: DecId %d NOT in USED !!\r\n", __FUNCTION__, DecId);
                bRet = FALSE;
            }
        ...
        case Audio_ParamType_Multi_Channel:
        {
            if ((Param == TRUE) || (Param == FALSE))
            {
                ...
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "%s: DecId(%d) %s Multi-Channel\n", __FUNCTION__, adec_id, Param?"Enable":"disable");
            }
            else
            {
                AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: Error! Invalid value(%u) to enable/disable Multi-Channel\n", __FUNCTION__, (unsigned int)Param);
            }
        ...
        {
            MS_S32 result;
            result = HAL_MAD_CommonCmd((AUDIO_COMMON_CMD_t *) Param);
            if (result < 0)
            {
                bRet = FALSE;
            }
        ...
        case Audio_infoType_Decoder_Type:
        {
            ...
            switch (adec_id)
            {
                case ADEC1:
                {
                    ...
                    if((eDecoder == AU_DVB_STANDARD_AC3) ||(eDecoder == AU_DVB_STANDARD_AC3P)||(eDecoder == AU_DVB_STANDARD_MS10_DDC))
                    {
                        if(HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_STAUS, ADEC1) & 0x0001)
                        {
                            if(HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DOLBY_DEC_TYPE, ADEC1)==1)
                                eDecoder = AU_DVB_STANDARD_AC3P;
                            else
                                eDecoder = AU_DVB_STANDARD_AC3;
                        }
                    }
                ...
                case ADEC2:
                {
                    ...
                    if((eDecoder == AU_DVB_STANDARD_AC3) ||(eDecoder == AU_DVB_STANDARD_AC3P)||(eDecoder == AU_DVB_STANDARD_MS10_DDC))
                    {
                        if(HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DEC_STAUS, ADEC2) & 0x0001)
                        {
                            if(HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_DOLBY_DEC_TYPE, ADEC2)==1)
                                eDecoder = AU_DVB_STANDARD_AC3P;
                            else
                                eDecoder = AU_DVB_STANDARD_AC3;
                        }
...
MS_S32 HAL_MAD_SpecifyDigitalOutputCodec(AUDIO_SPECIFIED_DIGITAL_OUTPUT_t * pAudioSpecifiedDigitalOutput, DIGITAL_OUTPUT_TYPE u32DigitalOutputType)
{
...
#else
    tSpecifiedDigitalOutput.u32SpdifOutputCodec = pAudioSpecifiedDigitalOutput->u32SpdifOutputCodec;
    tSpecifiedDigitalOutput.u32HdmiArcOutputCodec = pAudioSpecifiedDigitalOutput->u32HdmiArcOutputCodec;
    tSpecifiedDigitalOutput.u32HdmiTxOutputCodec = pAudioSpecifiedDigitalOutput->u32HdmiTxOutputCodec;
#endif
    ...
    switch (u32DigitalOutputType)
    {
        ...
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "%s: SpdifOutputCodec %d\n", __FUNCTION__, tSpecifiedDigitalOutput.u32SpdifOutputCodec);
        ...
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "%s: HdmiArcOutputCodec %d\n", __FUNCTION__, tSpecifiedDigitalOutput.u32HdmiArcOutputCodec);
        ...
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "%s: HdmiTxOutputCodec %d\n", __FUNCTION__, tSpecifiedDigitalOutput.u32HdmiTxOutputCodec);
...
MS_S32 HAL_MAD_Debug_Log(void * pData)
{
    MS_S32 s32Ret  = 0;
    ...
    result = _getIntByKeyword(pData, "all", &value);
    if (result)
    {
        HAL_AUDIO_DebugLog();
    }
...
MS_S32 HAL_MAD_GetDecoderInfo(AUDIO_DECODER_INFO_t * pDecoderInfo)
{
    ...
    if ((u32UserStrucVer & AUDIO_MAJOR_VERSION_MASK) != (u32DriverStrucVer & AUDIO_MAJOR_VERSION_MASK))
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s(): structure version is mismatch, user is 0x%08X, driver is 0x%08X!\n", __FUNCTION__, (unsigned int)u32UserStrucVer, (unsigned int)AUDIO_DECODER_INFO_VERSION);
        return -A_EPERM;
    }
...
MS_S32 HAL_MAD_CommonCmd(AUDIO_COMMON_CMD_t * cmd)
{
    ...
    else if (!(strcmp(name, "Get_Digital_Delay")))
    {
        (*(MS_U64 *)cmd->pData) = (MS_U64)HAL_MAD2_Read_DSP_sram(DSP2PmAddr_spdifDelay,  DSP_MEM_TYPE_PM);
    }
    else if (!(strcmp(name, "Get_BufferBase_SPDIF")))
    {
        (*(MS_U32 *)cmd->pData) = (MS_U32)(HAL_AUDIO_GetDspMadBaseAddr(DSP_SE) + ASND_DSP_DDR_SIZE + OFFSET_SPDIF_NONPCM_DRAM_BASE);
    }
    else if (!(strcmp(name, "Get_BufferSize_SPDIF")))
    {
        (*(MS_U32 *)cmd->pData) = (MS_U32)SPDIF_NONPCM_DRAM_SIZE;
    }
    else if (!(strcmp(name, "Get_BufferBase_HDMI")))
    {
        (*(MS_U32 *)cmd->pData) = (MS_U32)(HAL_AUDIO_GetDspMadBaseAddr(DSP_SE) + ASND_DSP_DDR_SIZE + OFFSET_HDMI_NONPCM_DRAM_BASE);
    }
    else if (!(strcmp(name, "Get_BufferSize_HDMI")))
    {
        (*(MS_U32 *)cmd->pData) = (MS_U32)HDMI_NONPCM_DRAM_SIZE;
    }
    else if (!(strcmp(name, "Get_BufferOffset_ES1")))
    {
        (*(MS_U32 *)cmd->pData) = (MS_U32)HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_ES_RD_PTR, ADEC1);
    }
    else if (!(strcmp(name, "Get_BufferOffset_ES3")))
    {
        (*(MS_U32 *)cmd->pData) = (MS_U32)HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_ES_RD_PTR, ADEC2);
    }
#ifdef T1_ONLY    
    else if (!(strcmp(name, "Get_BufferOffset_SPDIF")))
    {
        (*(MS_U32 *)cmd->pData) = (MS_U32)HAL_AUR2_ReadReg(REG_SE_SPDIF_RDPTR) << 4;
    }
    else if (!(strcmp(name, "Get_BufferOffset_HDMI")))
    {
        (*(MS_U32 *)cmd->pData) = (MS_U32)HAL_MAD2_Read_DSP_sram(DSP2DmAddr_hdmi_report_rdptr, DSP_MEM_TYPE_DM) << 4;
    }
#endif    
    else if (!(strcmp(name, "Debug_Log")))
    {
        s32Ret = HAL_MAD_Debug_Log(cmd->pData);
        if (s32Ret < 0)
        {
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "%s: set debug log fail(%d)\n", __FUNCTION__, s32Ret);
        }
    }
    else if (!(strcmp(name, "Set_Speaker_GainOffset")))
    {
        HAL_AUDIO_WriteByte(REG_SOUND_SPK_GAIN_OFFSET,  (*(MS_U8 *)cmd->pData) );
    }
    else if (!(strcmp(name, "Set_HP_GainOffset")))
    {
        HAL_AUDIO_WriteByte(REG_SOUND_HP_GAIN_OFFSET,  (*(MS_U8 *)cmd->pData) );
    }
    else if (!(strcmp(name, "Set_Spdif_ARC_GainOffset")))
    {
        HAL_AUDIO_WriteByte(REG_SOUND_SPDIF_ARC_GAIN_OFFSET,  (*(MS_U8 *)cmd->pData) );
    }
    else if (!(strcmp(name, "Set_MainSource_GainOffset")))
    {
        HAL_AUDIO_WriteByte(REG_SOUND_MAIN_SOURCE_GAIN_OFFSET,  (*(MS_U8 *)cmd->pData) );
    }
    else if (!(strcmp(name, "Get_Speaker_GainOffset")))
    {
        (*(MS_U8 *)cmd->pData) = HAL_AUDIO_ReadByte(REG_SOUND_SPK_GAIN_OFFSET);
    }
    else if (!(strcmp(name, "Get_HP_GainOffset")))
    {
        (*(MS_U8 *)cmd->pData) = HAL_AUDIO_ReadByte(REG_SOUND_HP_GAIN_OFFSET);
    }
    else if (!(strcmp(name, "Get_Spdif_ARC_GainOffset")))
    {
        (*(MS_U8 *)cmd->pData) = HAL_AUDIO_ReadByte(REG_SOUND_SPDIF_ARC_GAIN_OFFSET);
    }
    else if (!(strcmp(name, "Get_MainSource_GainOffset")))
    {
        (*(MS_U8 *)cmd->pData) = HAL_AUDIO_ReadByte(REG_SOUND_MAIN_SOURCE_GAIN_OFFSET);
    }
    else if (!(strcmp(name, "Set_AlarmMode")))
    {
         g_AudioVars2->g_system_alarm_mode = (*(MS_BOOL *)cmd->pData);
    }
    else if (!(strcmp(name, "Get_DecoderInfoTag")))
    {
        (*(MS_U16 *)cmd->pData) = g_AudioVars2->g_DecoderInfoTag;
    }
    else if (!(strcmp(name, "Set_DecoderInfoTag")))
    {
         g_AudioVars2->g_DecoderInfoTag = (*(MS_BOOL *)cmd->pData);
    }
    else if (!(strcmp(name, "Get_HdmiInAudioFormat")))
    {
       if (HAL_AUDIO_HDMI_NonpcmMonitor())
      {
          switch(HAL_AUDIO_HDMI_DolbyMonitor())
          {
              case 0x01:  (*(MS_U16 *)cmd->pData) = HDMI_RX_DD;        break;
              case 0x0b:  (*(MS_U16 *)cmd->pData) = HDMI_RX_DTS;       break;
              case 0x15:  (*(MS_U16 *)cmd->pData) = HDMI_RX_DDP;       break;
              default:    (*(MS_U16 *)cmd->pData) = HDMI_RX_Other;     break;
          }
      }
      else
          (*(MS_U16 *)cmd->pData) = HDMI_RX_PCM;
    }
#ifdef T1_ONLY    
    else if (!(strcmp(name, "Set_DTS_DrcPercent")))
    {
        g_AudioVars2->g_dts_drc_percent = (*(MS_U32 *)cmd->pData);
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DTS_DRC_PERCENT, ADEC1, g_AudioVars2->g_dts_drc_percent, 0);
        HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_DTS_DRC_PERCENT, ADEC2, g_AudioVars2->g_dts_drc_percent, 0);
    }
    else if (!(strcmp(name, "Set_Spdif_DD_Delay_Offset")))
    {
         g_AudioVars2->g_spdif_dd_delay_offset= (*(MS_S16 *)cmd->pData);
    }
    else if (!(strcmp(name, "Set_Spdif_DDP_Delay_Offset")))
    {
         g_AudioVars2->g_spdif_ddp_delay_offset= (*(MS_S16 *)cmd->pData);
    }
#endif    
    else
    {
