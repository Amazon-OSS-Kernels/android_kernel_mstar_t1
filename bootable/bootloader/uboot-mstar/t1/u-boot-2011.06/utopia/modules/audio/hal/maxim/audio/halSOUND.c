...
{
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "%s: path %s %s", __FUNCTION__, _audio_path_name(u8Path), bEnable ? "mute" : "un-mute");
...
{
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "%s: path %s, volume 0x%X", __FUNCTION__, _audio_path_name(u8Path), (u8u8Vol1 << 8) | (u8u8Vol2 & 0x00FF));
...
void HAL_SOUND_SetBalance(MS_U8 u8Balance)
{
    ...
    //AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "apiAud>> u8Balance = %02bx\n", u8Balance);
    if(u8Balance==50)
    {
        value_l = 0x00;
        value_r = 0x00;
...
void HAL_SOUND_SetDRCThreshold(MS_U8 u8Level, AUDIO_SOUNDEFFECT_TYPE Type)
{
    ...
    switch (Type)
    {
        ...
        default:
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR,"Warning! Invalid AUDIO_SOUNDEFFECT_TYPE(%u). \r\n", Type);
            break;
    }
}
...
void HAL_SOUND_SetCH1AudioDelay(MS_U16 u16Delay)
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN,"Warning! Invalid value (%u), main audio delay should be less than %u ms. \r\n", u16Delay, AUDIO_DELAY_UPPER_BOUND);
        u16Delay = AUDIO_DELAY_UPPER_BOUND;
    }

    if (u16Delay < AUDIO_DELAY_LOWER_BOUND)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN,"Warning! Invalid value (%u), main audio delay should be larger than %u ms. \r\n", u16Delay, AUDIO_DELAY_LOWER_BOUND);
        u16Delay = AUDIO_DELAY_LOWER_BOUND;
    }
...
void HAL_SOUND_SetSpdifAudioDelay(MS_U16 u16Delay)
{
    ...
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN, "Warning! Invalid value (%u), spdif audio delay should be less  than %u ms. \r\n", u16Delay, AUDIO_DELAY_UPPER_BOUND_SPDIF);
        u16Delay = AUDIO_DELAY_UPPER_BOUND_SPDIF;
    }

    if (u16Delay < AUDIO_DELAY_LOWER_BOUND_SPDIF)
    {
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_WARN, "Warning! Invalid value (%u), spdif audio delay should be larger than %u ms. \r\n", u16Delay, AUDIO_DELAY_LOWER_BOUND_SPDIF);
        u16Delay = AUDIO_DELAY_LOWER_BOUND_SPDIF;
    }
