...
#define HALAUDSP_CHECK_SHM_INIT \
    do { \
        ...
        { \
            ...
            AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "%s() : Perform SHM Init here !!!\n", __FUNCTION__); \
...
MS_BOOL HAL_AUDSP_DspLoadCode(AUDIO_DSP_CODE_TYPE dspCodeType)
{
   ...
   HALAUDSP_CHECK_SHM_INIT;
    ...
    {
        for (ii=0; ii< (int)(sizeof(dsp_info)/sizeof(AUDIO_ALG_INFO));ii++)
        {
            if (dspCodeType == dsp_info[ii].dspCodeType)
            break;
        }
        if (ii <(int)(sizeof(dsp_info)/sizeof(AUDIO_ALG_INFO)))
            {
...
//    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "@@@@@@@@@@    ====== %s: %s start ======\r\n", __FUNCTION__, pau_info->AlgName);
    ...
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "    CM addr:     0x%x\r\n", pau_info->cm_addr);
#if defined (__aarch64__)
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "    CM buf addr: 0x%lx\r\n",  (MS_VIRT)pau_info->cm_buf);
#else
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "    CM buf addr: 0x%x\r\n",  (MS_VIRT)pau_info->cm_buf);
#endif
    AUDIO_PRINT(AUDIO_DEBUG_LEVEL_DEBUG, "    CM size:     0x%x\r\n", pau_info->cm_len);
    ...
    if ( pau_info->prefetch_len != 0 )
    {
        ...
        AUDIO_PRINT(AUDIO_DEBUG_LEVEL_ERROR, "    MIU of PreFetch: 0x%X\r\n", (unsigned int) MIU_addr);
