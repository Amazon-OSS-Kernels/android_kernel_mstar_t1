...
*/
      ...
      DMD_DBG(ULOGD("DEMOD","[drvDMD_INTERN_DVBT.c]MDrv_DMD_DVBT_GetSignalStrengthWithRFPower\n"));

	/*bryan temp mark*/
	#if(0)
	if(u32DVBTopen==1)
		UtopiaIoctl(ppDVBTInstant, DMD_DVBT_DRV_CMD_GetSignalStrengthWithRFPower, &Drv_DVBT_GetSignalStrengthWithRFPower_PARAM);
	else
		return false;

	 return Drv_DVBT_GetSignalStrengthWithRFPower_PARAM.ret;
	#else
	*u16Strength=100;
       return true;
	#endif
...
{
	//MS_BOOL return_val;
	DVBT_GetSignalQualityWithRFPower_PARAM Drv_DVBT_GetSignalQualityWithRFPower_PARAM;
	Drv_DVBT_GetSignalQualityWithRFPower_PARAM.u16Quality=u16Quality;
	Drv_DVBT_GetSignalQualityWithRFPower_PARAM.fRFPowerDbm=fRFPowerDbm;

       DMD_DBG(ULOGD("DEMOD","[drvDMD_INTERN_DVBT.c]MDrv_DMD_DVBT_GetSignalQualityWithRFPower\n"));    
	/*bryan temp mark */
	#if(0)
	if(u32DVBTopen==1)
		UtopiaIoctl(ppDVBTInstant, DMD_DVBT_DRV_CMD_GetSignalQualityWithRFPower, &Drv_DVBT_GetSignalQualityWithRFPower_PARAM);
	else
		return false;
	#else
		Drv_DVBT_GetSignalQualityWithRFPower_PARAM.ret=true;
	#endif
	
        return Drv_DVBT_GetSignalQualityWithRFPower_PARAM.ret;
...
//    	MS_BOOL return_val;
	...
	Drv_DVBT_GetSNR_PARAM.fSNR=fSNR;
	*/
//bryan temp mark
...
//	MS_BOOL return_val;
	...
	#if(0)
	DVBT_GetPostViterbiBer_PARAM Pre_DVBT_GetPostViterbiBer_PARAM;
	Pre_DVBT_GetPostViterbiBer_PARAM.ber=ber;
	#endif
	//bryan temp mark 
	#if(0)
	if(u32DVBTopen==1)
		UtopiaIoctl(ppDVBTInstant, DMD_DVBT_DRV_CMD_GetPostViterbiBer, &Pre_DVBT_GetPostViterbiBer_PARAM);
	else
		return false;

	return Pre_DVBT_GetPostViterbiBer_PARAM.ret;
	#else
