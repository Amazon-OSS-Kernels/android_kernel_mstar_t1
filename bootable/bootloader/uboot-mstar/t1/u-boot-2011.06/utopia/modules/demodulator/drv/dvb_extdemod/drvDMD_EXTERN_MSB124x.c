...
#define MSB124X_DVBS2_START_ADDR    0x20000
#define MSB124X_DVBS2_END_ADDR      0x27FFF//0x28FDF
...
#define MSB124X_DVBS2_WINDOWS_OFFSET       (MSB124X_DVBC_WINDOWS_OFFSET + 0x08)
...
MS_BOOL MDrv_DMD_MSB124X_Init(sDMD_MSB124X_InitData *pDMD_MSB124X_InitData, MS_U32 u32InitDataLen)
{
    ...
    if (_sDMD_MSB124X_InitData.bEnableSPILoadCode && _sDMD_MSB124X_InitData.fpMSB124x_SPIPAD_En != NULL)
    {
    ...
    {
        _sDMD_MSB124X_InitData.bEnableSPILoadCode = FALSE;
    }
...
MS_BOOL MDrv_DMD_MSB124X_Exit(void)
{
    ...
    bDMD_MSB124X_Power_init_en = FALSE;
    u8DMD_MSB124X_PowerOnInitialization_Flow = 0;
    u8DMD_MSB124X_Sdram_Code = 0x0;
    u8DMD_MSB124X_Sram_Code  = 0x0;
    bDemodRest = TRUE;
...
static MS_BOOL _Load2Sdram(MS_U8 *u8_ptr, MS_U16 data_length, MS_U16 sdram_win_offset_base)
{
    ...
    if (_sDMD_MSB124X_InitData.bEnableSPILoadCode)
    {
        MS_U32 u32Addr = 0;
...
static MS_BOOL _LoadDspCodeToSDRAM_dvbs2(void)
{
    ...
    if(!(u8DMD_MSB124X_Sdram_Code&MSB124X_DVBS2))
    {
        // dvbt code
        if(sizeof(MSB124X_LIB) > MSB124X_DVBS2_START_ADDR)
        {
         data_ptr = MSB124X_LIB + MSB124X_DVBS2_START_ADDR;
         code_size = MSB124X_DVBS2_END_ADDR - MSB124X_DVBS2_START_ADDR + 1;
         win_offset = MSB124X_DVBS2_WINDOWS_OFFSET;
         bRet &= _Load2Sdram(data_ptr,code_size,win_offset);
         if(bRet == true)
         {
             u8DMD_MSB124X_Sdram_Code |= MSB124X_DVBS2;
         }
        }
        else
        {
...
static MS_BOOL _LoadDspCodeToSRAM_dvbs2(void)
{
    ...
    if(!(u8DMD_MSB124X_Sram_Code&MSB124X_DVBS2))
    {
        // dvbs code
        if(sizeof(MSB124X_LIB) > MSB124X_DVBS2_START_ADDR)
        {
         ...
         if(bRet == true)
         {
             u8DMD_MSB124X_Sram_Code |= MSB124X_DVBS2;
         }
        }
        else
        {
...
static MS_BOOL _LoadDspCodeToSDRAM(MS_U8 code_n)
{
    ...
    if (_sDMD_MSB124X_InitData.bEnableSPILoadCode)
    {
        (*_sDMD_MSB124X_InitData.fpMSB124x_SPIPAD_En)(TRUE);
    ...
    if (_sDMD_MSB124X_InitData.bEnableSPILoadCode)
    {
        ...
        (*_sDMD_MSB124X_InitData.fpMSB124x_SPIPAD_En)(FALSE);
    }
...
static MS_BOOL _LoadDSPCode(void)
{
    ...
    if(_sDMD_MSB124X_InitData.u8WO_Sdram==1)   //  1 means no SDRAM on board
    {
           //no sdram case
        #ifdef MS_DEBUG
    ...
    else if((_sDMD_MSB124X_InitData.u8WO_SPI_Flash == 1)&&(_sDMD_MSB124X_InitData.u8WO_Sdram==0))
    {
        //MS_U8 u8FirmwareType = MSB124X_DVBT;
...
*/
    ...
    if(_sDMD_MSB124X_InitData.u8WO_Sdram==0)//with sdram on board
    {
...
MS_BOOL MDrv_DMD_MSB1245_LoadDSPCodeToSram(void)
{
    ...
    if (_sDMD_MSB124X_InitData.bEnableSPILoadCode)
    {
        ...
        bRet&=_MSB124X_I2C_CH_Reset(3);
        bRet&=_MDrv_DMD_MSB124X_SetReg(0x0B32, 0x01);
        bRet&= MDrv_DMD_SSPI_MIU_Writes(SRAM_BASE, MSB1245_LIB, u16dat_size);
        bRet&=_MDrv_DMD_MSB124X_SetReg(0x0B32, 0x00);
        if (bRet==FALSE)
        {
