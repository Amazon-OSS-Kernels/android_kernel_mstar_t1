...
//@NOTE assume default RASP is enable on TSP_VER_2_0
#if defined(RASP_ENABLE) || (TSP_VERSION == TSP_VER_2_0)
#include "drvRASP.h"
#endif
#endif
...
#define DMX_FLT_DS_SIZE                    260UL //130
#else
...
#define _DMX_RETURN(_ret)           do{  \
                                        if(MsOS_ReleaseMutex(_pdmx_res->_Dmx_Mutex) == FALSE) { \
                                            printf("[MAPI DMX][%s][%06d] DMX Mutex Free fail\n", __FUNCTION__, __LINE__); \
...
#define _DMX_PVR_RETURN(_ret)       do{   \
                                        if(MsOS_ReleaseMutex(_pdmx_res->_Dmx_Pvr_Mutex) == FALSE) { \
                                            printf("[MAPI DMX][%s][%06d] PVR Mutex Free fail\n", __FUNCTION__, __LINE__); \
                                            return DMX_FILTER_STATUS_ERROR; }  \
                                        return _ret;                                            \
                                    }while(0);
...
#define _DMX_MMFI_RETURN(_ret)      do{   \
                                        if(MsOS_ReleaseMutex(_pdmx_res->_Dmx_MMFI_Mutex) == FALSE){ \
                                            printf("[MAPI DMX][%s][%06d] MMFI Mutex Free fail\n", __FUNCTION__, __LINE__); \
                                            return DMX_FILTER_STATUS_ERROR; }  \
                                        return _ret;                                            \
                                    }while(0);
...
#define _DMX_TSO_RETURN(_ret)       do{   \
                                        if(MsOS_ReleaseMutex(_pdmx_res->_Dmx_TSO_Mutex) == FALSE){ \
                                            printf("[MAPI DMX][%s][%06d] TSO Mutex Free fail\n", __FUNCTION__, __LINE__); \
                                            return DMX_FILTER_STATUS_ERROR; }  \
                                        return _ret;                                            \
                                    }while(0);
#endif
...
static DMX_RESOURCE_PRIVATE         _dmx_res = {
    ...
    ._u8InitRefCnt                              = 0,
#ifdef FQ_ENABLE
    #if ((TSP_VERSION == TSP_VER_2_0) || (TSP_VERSION == TSP_VER_4_0))
...
static MS_BOOL _DMX_InfoSet_Sect(MS_U32 u32DmxId, DMX_Flt_info* pDmxFltInfo)
{
    DMX_FILTER_TYPE        DmxFltType;
    DMX_Sect_info*         pInfo = &(pDmxFltInfo->Info.SectInfo);
    DMX_Sect_info*         pInfoPrev = &(_FLT_LIST_INFO_GET_P(u32DmxId)->Info.SectInfo);
    ...
    MDrv_TSP_SecFlt_SetMode(0, _pdmx_res->_FltList[u32DmxId].SecBufId/*u8DmxId*/, (DrvTSP_FltMode)pInfo->SectMode);

    if(_DMX_ChkBufOverlap(u32DmxId, pInfo->SectBufAddr, pInfo->SectBufSize))
    {
#if (!(defined(TSP_VERSION) && ((TSP_VERSION == TSP_VER_2_0) || (TSP_VERSION == TSP_VER_4_0))))
    ...
    MDrv_TSP_SecFlt_SetBuffer(0, _pdmx_res->_FltList[u32DmxId].SecBufId, pInfo->SectBufAddr, (MS_U32)pInfo->SectBufSize);
    ...
    if (pInfo->pNotify || (((pInfo->Event&DMX_EVENT_CB_SELF_TYPE2) == DMX_EVENT_CB_SELF_TYPE2) && pInfo->pType2Notify))
    {
        ...
        MDrv_TSP_SecFlt_Notify(0, _pdmx_res->_FltList[u32DmxId].SecBufId, (DrvTSP_Event)pInfo->Event, _DMX_Notify);
    }
    _FLT_LIST_INFO_SET(u32DmxId, pDmxFltInfo);
    return TRUE;
}
...
{
    switch (u32TSPFltSource & DMX_FILTER_SOURCEID_MASK)
    {
        case DMX_FILTER_SOURCEID_0:
            return E_TSP_FLT_SRCID_0;
        case DMX_FILTER_SOURCEID_1:
            return E_TSP_FLT_SRCID_1;
        case DMX_FILTER_SOURCEID_2:
            return E_TSP_FLT_SRCID_2;
        case DMX_FILTER_SOURCEID_3:
            return E_TSP_FLT_SRCID_3;
        case DMX_FILTER_SOURCEID_4:
            return E_TSP_FLT_SRCID_4;
        case DMX_FILTER_SOURCEID_5:
            return E_TSP_FLT_SRCID_5;
        case DMX_FILTER_SOURCEID_6:
            return E_TSP_FLT_SRCID_6;
        case DMX_FILTER_SOURCEID_7:
            return E_TSP_FLT_SRCID_7;
        case DMX_FILTER_SOURCEID_8:
            return E_TSP_FLT_SRCID_8;
        case DMX_FILTER_SOURCEID_9:
            return E_TSP_FLT_SRCID_9;
        case DMX_FILTER_SOURCEID_10:
            return E_TSP_FLT_SRCID_10;
        case DMX_FILTER_SOURCEID_11:
            return E_TSP_FLT_SRCID_11;
        case DMX_FILTER_SOURCEID_12:
            return E_TSP_FLT_SRCID_12;
        case DMX_FILTER_SOURCEID_13:
            return E_TSP_FLT_SRCID_13;
        case DMX_FILTER_SOURCEID_14:
            return E_TSP_FLT_SRCID_14;
        case DMX_FILTER_SOURCEID_15:
            return E_TSP_FLT_SRCID_15;
        default:
            DMX_DBGMSG(DMX_DBG_ERR,  printf("[MAPI DMX][%06d] Bad filter src id %d\n", __LINE__, (unsigned int)u32TSPFltSource));
            return E_TSP_FLT_SRCID_0;
    }
}
...
static DMX_FILTER_STATUS _DMX_Open_Stream(MS_U32* pu32DmxId, DMX_FILTER_TYPE DmxFltType)
{
    ...
    else if (DMX_FILTER_TYPE_PVR== DmxFltType)
    {
       eTspFltType= E_DRVTSP_FLT_TYPE_PVR | (DrvTSP_FltType)u32TSPFltSource;
    }
    else if (DMX_FILTER_TYPE_PVR1== DmxFltType)
    {
       eTspFltType= E_DRVTSP_FLT_TYPE_PVR1 | (DrvTSP_FltType)u32TSPFltSource;
    }
    else if (DMX_FILTER_TYPE_PVR2== DmxFltType)
    {
       eTspFltType= E_DRVTSP_FLT_TYPE_PVR2 | (DrvTSP_FltType)u32TSPFltSource;
    }
    else if (DMX_FILTER_TYPE_PVR3== DmxFltType)
    {
       eTspFltType= E_DRVTSP_FLT_TYPE_PVR3 | (DrvTSP_FltType)u32TSPFltSource;
    }
    else
    {
...
#if (defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_4_0))
    //default: 172.8/(31+1) = 5.4M
    DrvTSO_OutClk eOutClk = E_DRVTSO_OUTCLK_DIVN;
    MS_U16  u16DivNum = 0x1F;
#else
...
static MS_BOOL _DMX_FlowSet(DMX_FLOW DmxFlow, DMX_FLOW_INPUT DmxFlowInput, MS_BOOL bClkInv, MS_BOOL bExtSync, MS_BOOL bParallel)
{
    ...
    if((DmxFlowInput >= DMX_FLOW_INPUT_EXT_INPUT0_3WIRE) && (DmxFlowInput <= DMX_FLOW_INPUT_EXT_INPUT5_3WIRE))
    {
...
#else
    bCopy = TRUE; //can not callback user function pointer in kernel driver#endif#endif
#endif
...
DMX_FILTER_STATUS _MApi_DMX_Info(MS_U32 u32DmxId, DMX_Flt_info* pDmxFltInfo, DMX_FILTER_TYPE* pDmxFltType, MS_BOOL bSet)
{
    ...
    case DMX_FILTER_TYPE_SECTION_VER:
        ...
        bRet= _DMX_InfoSet_Sect(u32DmxId, pDmxFltInfo);
        break;
    default:
...
#if (!((defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_4_0))) && defined(SECURE_PVR_ENABLE))
...
DMX_FILTER_STATUS _MApi_DMX_Pvr_Eng_Pid_Open(DMX_PVR_ENG Eng ,MS_U32 Pid, MS_U32* pu32DmxId)
{
...
#if defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_2_0)
    DMX_PVRDBGMSG(DMX_DBG_PVR, ULOGD("DMX", "[%s] %d (%d, 0x%lx, %p\n", __FUNCTION__, __LINE__, (MS_S32)Eng, Pid, pu32DmxId));
    _DMX_PVR_ENTRY();
    ...
    DMX_PVR_ASSERT2(FltId< DMX_MAX_FLTID, DMX_DBGMSG(DMX_DBG_ERR, ULOGE("DMX", "[MAPI DMX][%06d] Bad FltId %lu\n", __LINE__, (unsigned int)FltId)));
...
#if defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_2_0)
    MDrv_RASP_PVR_ReleaseFlt(Eng, u32DmxId);
#elif defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_4_0)
    MDrv_TSP_PVR_Eng_FltFree(Eng, u32DmxId);
    _FLT_LIST_REMOVE(u32DmxId);
#else
    MDrv_TSP_PidFlt_Enable(0, u32DmxId, FALSE);
...
DMX_FILTER_STATUS _MApi_DMX_Pvr_Eng_Pid(DMX_PVR_ENG Eng ,MS_U32 u32DmxId , MS_U32 *Pid, MS_BOOL bSet)
{
    ...
    if (bSet)
    {
        MDrv_RASP_PVR_SetPid(Eng, (MS_U16)u32DmxId, *Pid);
    }
    else
    {
       *Pid = 0 ;
        MDrv_RASP_PVR_GetPid(Eng, (MS_U16)u32DmxId,(MS_U16*) Pid);
    }
    _DMX_PVR_RETURN(DMX_FILTER_STATUS_OK);
#elif defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_4_0)
    ...
    if (bSet)
    {
        MDrv_TSP_PVR_Eng_FltSetPID(Eng, u32DmxId, *Pid);
        _FLT_LIST_PID_SET(u32DmxId, *Pid);
    }
    else
    {
       *Pid =  DMX_PID_NULL;
        MDrv_TSP_PVR_Eng_FltGetPID(Eng, u32DmxId,(MS_U32*) Pid);  //@F_TODO not implement yet
    }
    _DMX_PVR_RETURN(DMX_FILTER_STATUS_OK);
#else
...
#if (!(defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_2_0)) && defined(SECURE_PVR_ENABLE))
...
#if defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_2_0)
    if(MDrv_RASP_PVR_GetWriteAddr(Eng, phyWrite) != DRVRASP_OK)
    {
        return DMX_FILTER_STATUS_ERROR;
    }
    else
#elif defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_4_0)
    #ifndef SECURE_PVR_ENABLE
    if (DRVTSP_OK != MDrv_TSP_PVR_Eng_GetWriteAddr(Eng, phyWrite))
    {
        return DMX_FILTER_STATUS_ERROR;
    }
    #endif
...
DMX_FILTER_STATUS _MApi_DMX_PvrCA_Eng_Pid_Open(DMX_PVR_ENG Eng, MS_U32 Pid, MS_U32* pu32DmxId, MS_U8 u8ShareKeyType)
{
    ...
    return (_MApi_DMX_Pvr_Pid_Open(Pid, pu32DmxId));
#elif (defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_4_0))
...
#if (defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_2_0))
    return (MApi_DMX_Pvr_Pid_Close(u32DmxId));
#elif (defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_4_0))
...
DMX_FILTER_STATUS _MApi_DMX_Filein_Eng_MOBF_Enable(DMX_FILEIN_PATH ePath, MS_BOOL bEnable, MS_U32 u32key)
{
    ...
    switch(ePath)
    {
        ...
        case DMX_FILEIN_TSIF3:
            ...
            break;
        default:
            DMX_DBGMSG(DMX_DBG_ERR, printf("[MAPI DMX][%06d] Bad DMX_FILEIN_ENG %u\n", __LINE__, (unsigned int)ePath));
            return DMX_FILTER_STATUS_ERROR;
    }
...
DMX_FILTER_STATUS _MApi_DMX_ResAllocate(DMX_RES_TYPE eResType, void *pRes)
{
    ...
    if(eResType == DMX_RES_STC_ENG)
    {
        if(MDrv_TSP_STC_Alloc((MS_U32*)pRes) == DRVTSP_OK)
...
DMX_FILTER_STATUS _MApi_DMX_ResFree(DMX_RES_TYPE eResType, void *pRes)
{
    ...
    if(eResType == DMX_RES_STC_ENG)
    {
        if(MDrv_TSP_STC_Free(*((MS_U32*)pRes)) == DRVTSP_OK)
            ret = DMX_FILTER_STATUS_OK;
    }
...
TSP_TSIF            eTSIF   = E_TSP_TSIF_INVALID;
    ...
    TSP_TSIF eTSPTsif = MDrv_TSP_TsifMapping(eTSIF);
...
DMX_FILTER_STATUS _MApi_DMX_GetCap_Ex(char* pstrQueryType, MS_S32 s32Strlen, void* pOutput, MS_S32* ps32OutSize)
{
    ...
    *ps32OutSize = 0;
    ...
    ptr += 8;
    for(ii = 0; ii <= (MS_U32)DMX_CAP_FIQ_NUM; ii++)
    {
        if(strcmp(ptr, _tblDmxCapStr[ii].strCap) == 0)
            break;
    }
    if(ii > (MS_U32)DMX_CAP_FIQ_NUM)
    {
        return DMX_FILTER_STATUS_ERROR;
    }
...
static TSP_PktMode _DMX_ApiDrv_PacketModeMapping(DMX_PacketMode ePacketMode)
{
    switch (ePacketMode)
    {
        ...
        default:
            DMX_DBGMSG(DMX_DBG_ERR,  ULOGE("DMX", "[MAPI DMX][%06d] Bad filter src %d\n", __LINE__, (int)ePacketMode));
            return E_TSP_FLT_SRC_LIVE0;
    }
}
#endif
...
DMX_FILTER_STATUS _MApi_DMX_FQ_Configure(DMX_FQ_Cfg* pstFqCfg)
{
    ...
    {
        if(DRVTSP_OK != MDrv_TSP_FQ_FltNullPkt(eFQSrcif, pstFqCfg->bSet))
...
DMX_FILTER_STATUS _MApi_DMX_Get_DisContiCnt(DMX_DisContiCnt_info* pDmxInfo, MS_U32 *pu32Cnt)
{
    ...
    DMX_DBGMSG(DMX_DBG_FUNC, ULOGD("DMX", "[%s] %d\n", __FUNCTION__, __LINE__));
        ...
        _DMX_RETURN(ret);
    #elif (defined(TSP_VERSION) && ((TSP_VERSION == TSP_VER_1_0) || (TSP_VERSION == TSP_VER_3_0)))
...
DMX_FILTER_STATUS MApi_DMX_InitLibResource(void *pResMemAddr)
{
    ...
    //@NOTE assume default RASP is enable on TSP_VER_2_0
    #if defined(RASP_ENABLE) || (TSP_VERSION == TSP_VER_2_0)
    if(MDrv_RASP_InitLibResource((void*)ptrMemPos) != DRVRASP_OK)
    {
        return DMX_FILTER_STATUS_ERROR;
    }
    if(MDrv_RASP_GetCap(E_DRVRASP_CAP_RESOURCE_SIZE, (void*)(&u32size)) != DRVRASP_OK)
...
#if (defined(MERGE_STR_SUPPORT) && defined(TSP_VERSION) && (TSP_VERSION == TSP_VER_4_0))
