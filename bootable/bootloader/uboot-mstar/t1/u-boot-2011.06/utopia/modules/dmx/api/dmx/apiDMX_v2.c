...
#define DMX_MAX_PVR_ENG                                 8UL
#define DMX_MAX_SECFLT_NUM                              256UL
//------------------------------------------------------------------------------
// Local Variable
//------------------------------------------------------------------------------
#ifdef DMX_UTOPIA_20
...
#define _DMX2K_LOCK()                   while(!MsOS_ObtainMutex(_Dmx2K_Mutex, DMX2K_MUTEX_TIMEOUT)) { \
                                            printf("DMX FATAL ERR: [%s][%d] DMX2K mutex get error\n", __FUNCTION__, __LINE__); \
                                            MsOS_DelayTask(1);  \
                                        }

#define _DMX2K_UNLOCK()                 if(!MsOS_ReleaseMutex(_Dmx2K_Mutex)) {  \
                                            printf("DMX FATAL ERR: [%s][%d] DMX2K mutex free error\n", __FUNCTION__, __LINE__); }
                ...
                for(u32jj = 0; u32jj < 32; u32jj++)
                {
                    ...
                    if(stTspIoSig.u32SecCrcErrFlag[u32ii] & u32Mask)
                    {
                        ...
                        if(stCbParam.Event & DMX_EVENT_CB_MASK)
                        {
                            if(((stCbParam.Event&DMX_EVENT_CB_SELF_TYPE2) == DMX_EVENT_CB_SELF_TYPE2) && (stCbParam.pType2Notify))
                            {
                                stCbParam.pType2Notify(u32FltId, DMX_EVENT_SEC_CRCERROR ,stCbParam.Type2NotifyParam1);
                            }
                            else if(stCbParam.pNotify)
                            {
                                stCbParam.pNotify(u32FltId, DMX_EVENT_SEC_CRCERROR);
                            }
                        }
...
DMX_FILTER_STATUS MApi_DMX_ResAllocate(DMX_RES_TYPE eResType, void *pRes)
{
    ...
    if(eResType == DMX_RES_STC_ENG)
    {
        *((MS_U32*)pRes) = u32param[1];
        ret = (DMX_FILTER_STATUS)u32param[2];
    }
