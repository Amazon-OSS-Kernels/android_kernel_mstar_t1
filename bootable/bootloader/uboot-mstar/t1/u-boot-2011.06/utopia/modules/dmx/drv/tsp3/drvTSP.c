...
static void _TSP_SW_Exit(void)
{
    ...
    if(_u32LibMode != DRVTSP_CMD_LIB_MODE_KRN)
    {
        ...
        while(_s32TspTaskId > 0)
        {
            ...
            {
                printf("[%s][%d] Systemtime Change %u %u %x\n", __FUNCTION__, __LINE__, (unsigned int)u32CurTime, (unsigned int)u32Timer, _s32TspTaskId);
                u32Timer = u32CurTime;
            }
            if ((u32CurTime-u32Timer) > 10000UL)
            {
                printf("[%s][%d] TSP task exit timeout %u %u %x !!!!!!\n", __FUNCTION__, __LINE__, (unsigned int)u32CurTime, (unsigned int)u32Timer, _s32TspTaskId);
                break;
            }
            MsOS_DelayTask(3);
        }
