...
// Blocking events
#define TSP_SetEvent(flag)                          ({ MS_BOOL bRet = TSP_OS_EventSet(_s32EventId, (flag)); (bRet == TRUE)? TRUE : FALSE; })
#define TSP_GetEvent(events, flag, opt, timeout)    ({ MS_BOOL bRet = TSP_OS_EventGet(_s32EventId, (flag), (events), (opt), (timeout)); (bRet == TRUE)? TRUE : FALSE; })
...
#define TSP_RES_ALLOC_STC_CHECK(u32StcEng)          {                                                                                                                           \
                                                        if((_ptsp_res->_bIsStcEverAlloc == TRUE) &&  (_ptsp_res->_u32TspStcState[u32StcEng].bUsed == FALSE))                    \
                                                        {                                                                                                                       \
...
typedef struct _TSP_RESOURCE_PRIVATE
{
    ...
    DrvTSP_Stc_State        _u32TspStcState[STC_ENG_NUM];
...
static MS_S32                  _s32CrcErrFlags[(TSP_PIDFLT_NUM+31)>>5] = { [0 ... (((TSP_PIDFLT_NUM+31)>>5)-1)] = -1 };
static MS_S32                  _s32SecOvfFlags[(TSP_PIDFLT_NUM+31)>>5] = { [0 ... (((TSP_PIDFLT_NUM+31)>>5)-1)] = -1 };
static MS_S32                  _s32SecRdyFlags[(TSP_PIDFLT_NUM+31)>>5] = { [0 ... (((TSP_PIDFLT_NUM+31)>>5)-1)] = -1 };
static MS_S32                  _s32FltDisFlags[(TSP_PIDFLT_NUM+31)>>5] = { [0 ... (((TSP_PIDFLT_NUM+31)>>5)-1)] = -1 };
static MS_S32                  _s32FreeFlags  [(TSP_PIDFLT_NUM+31)>>5] = { [0 ... (((TSP_PIDFLT_NUM+31)>>5)-1)] = -1 };
static MS_S32                  _s32DoneFlags  [(TSP_PIDFLT_NUM+31)>>5] = { [0 ... (((TSP_PIDFLT_NUM+31)>>5)-1)] = -1 };

static MS_S32                  _s32ModeFlags[(TSP_PIDFLT_NUM+31)>>5] = { [0 ... (((TSP_PIDFLT_NUM+31)>>5)-1)] = -1 };
static MS_S32                  _s32OvfRstFlags[(TSP_PIDFLT_NUM+31)>>5] = { [0 ... (((TSP_PIDFLT_NUM+31)>>5)-1)] = -1 };
...
static TSP_RESOURCE_PRIVATE         _tsp_res = {
    ...
    ._u32TspStcState                = {[0 ... (STC_ENG_NUM-1)] = {0xFFFFFFFF, FALSE}},
...
static MS_BOOL _TSP_InitResource(TSP_RESOURCE_PRIVATE* presource)
{
    ...
    if(presource == NULL)
    {
        ULOGE("TSP", "Utopia2 Resource address is NULL!\n");
        return FALSE;
    }
...
static TSP_TS_PAD_MUX_MODE _TSP_DrvHal_OutPadSrcMapping(EN_TSP_OutPadMode eTSPadMode)
{
    switch(eTSPadMode)
    {
        case E_TSP_OUT_PADMODE_DEMOD:
            return E_TSP_TS_PAD_MUX_DEMOD;
        case E_TSP_OUT_PADMODE_TSO:
            return E_TSP_TS_PAD_MUX_TSO;
        case E_TSP_OUT_PADMODE_S2P:
            return E_TSP_TS_PAD_MUX_S2P;
        case E_TSP_OUT_PADMODE_S2P1:
            return E_TSP_TS_PAD_MUX_S2P1;
        default:
            return E_TSP_TS_PAD_MUX_INVALID;
    }
}
...
TSP_Result MDrv_TSP_FIFO_Reset(TSP_FltType eFltType, MS_BOOL bReset)
{
    ...
    switch (eFltType)
    {
        ...
        case E_TSP_FLT_FIFO_AUDIO3:
            ...
            break;
        case E_TSP_FLT_FIFO_AUDIO4:
            eDst = E_TSP_DST_FIFO_AUDIO4;
            break;
    }
...
static TSP_Result _TSP_STC_Alloc(MS_U32 *pu32EngId)
{
    ...
    *pu32EngId = 0xFFFFFFFF;
    ...
    {
        if(_ptsp_res->_u32TspStcState[i].bUsed == FALSE)
        {
            _ptsp_res->_u32TspStcState[i].bUsed = TRUE;
            *pu32EngId = i;
            break;
        }
    }
...
{
    if(_ptsp_res->_u32TspStcState[u32EngId].bUsed == FALSE)
        return E_TSP_FAIL;

    _ptsp_res->_u32TspStcState[u32EngId].bUsed = FALSE;
    _ptsp_res->_u32TspStcState[u32EngId].u32PcrFltId = 0xFFFFFFFF;
...
{
    _ptsp_res->_u32TspStcState[u32EngId].u32PcrFltId = u32PcrFltId;
...
static TSP_Result _TSP_PcrId_To_StcId(MS_U32 u32PcrFltId,MS_U32 *pu32EngId)
{
    ...
    *pu32EngId = 0xFFFFFFFF;
    ...
    for(i = 0; i < STC_ENG_NUM; i++)
    {
        if((_ptsp_res->_u32TspStcState[i].u32PcrFltId == u32PcrFltId)
            && (_ptsp_res->_u32TspStcState[i].bUsed == TRUE))
        {
            *pu32EngId = i;
            return E_TSP_OK;
...
static void _TSP_FLT_Init(MS_U32 u32FltId)
{
    ...
    if ((_ptsp_res->_tspInfo.FltType[u32FltId]& E_TSP_FLT_USER_MASK)== E_TSP_FLT_USER_PCR)
    {
        ...
        {
            u32PcrId = u32FltId - TSP_PCRFLT_START_ID;
            MS_U16 u16PcrUpdateEndEn = (u32PcrId == 0)? TSP_HWINT2_PCR0_UPDATE_END_EN : TSP_HWINT2_PCR1_UPDATE_END_EN;
            ...
            HAL_TSP_INT_Disable((MS_U16)(TSP_HWINT2_PCR0_UPDATE_END_EN << (8 + u16PcrUpdateEndEn)));
        ...
        {
            ULOGE("TSP", "[%s][%d] Incorrect PCR filter ID !!\n",__FUNCTION__,__LINE__);
            return;
        }
    }
...
{
    ULOGE("TSP", "%s Wait implementation\n", __FUNCTION__);
}

static void _TSP_ISR_Proc(MS_U32 u32Events)
{
    MS_U32              fltid = TSP_PIDFLT_NUM, flags = 0,ModeMask,dummy,BaseFlt;
    int                 i;
...
#endif
{
    ...
    if (_TSP_ChkHwPcrNotifyId(0) && (hw_int & (TSP_HWINT2_PCR0_UPDATE_END)))
    {
        ...
        _TSP_PcrId_To_StcId(HAL_TSP_PCRFLT_GET_ID(0), &u32PcrDstSTC);
        ...
        if(_ptsp_res->_u8_ResetPcr[0]>=2)
        {
            ...
            if(_ptsp_res->_u32_StcUpdateCtrl[0].bEnable == TRUE)
            {
                HAL_TSP_STC64_Set(u32PcrDstSTC, Pcr_H,u32STC_Add_Offset);

                if(_ptsp_res->_u32_StcUpdateCtrl[0].bUpdateOnce == TRUE)
                {
                    _ptsp_res->_u32_StcUpdateCtrl[0].bEnable = FALSE;
                    _ptsp_res->_u32_StcUpdateCtrl[0].bUpdateOnce = FALSE;
                }
            }

            _ptsp_res->_u8_ResetPcr[0] = 0;
            _ptsp_res->_u32_PrevStcBase[0] = u32STC_Add_Offset;
            HAL_TSP_GetSTCSynth(u32PcrDstSTC, &_ptsp_res->_u32_PrevPllControl[0]);
        }
        ...
        if(( Stc - _ptsp_res->_u32_PrevStcBase[0]) > 0x4000)
        {
            ...
            if(_ptsp_res->_u32_PrevStcBase[0] > Pcr) // faster then PCR comming
            {
                ...
                else if( u32Diff  < 0x1000)
                {
                    ...
                    HAL_TSP_SetSTCSynth(u32PcrDstSTC, _ptsp_res->_u32_PrevPllControl[0] + ((u32Diff)>>6));        //out = now + error x beta
                    _ptsp_res->_u8_ResetPcr[0] = 0;
    ...
    if (_TSP_ChkHwPcrNotifyId(1) && (hw_int & (TSP_HWINT2_PCR1_UPDATE_END ) ))
    {
        ...
        _TSP_PcrId_To_StcId(HAL_TSP_PCRFLT_GET_ID(1), &u32PcrDstSTC);
        ...
        if(_ptsp_res->_u8_ResetPcr[1]>=2)
        {
            ...
            if(_ptsp_res->_u32_StcUpdateCtrl[1].bEnable == TRUE)
            {
                HAL_TSP_STC64_Set(u32PcrDstSTC, Pcr_H,u32STC_Add_Offset);

                if(_ptsp_res->_u32_StcUpdateCtrl[1].bUpdateOnce == TRUE)
                {
                    _ptsp_res->_u32_StcUpdateCtrl[1].bEnable = FALSE;
                    _ptsp_res->_u32_StcUpdateCtrl[1].bUpdateOnce = FALSE;
                }
            }

            _ptsp_res->_u8_ResetPcr[1] = 0;
            _ptsp_res->_u32_PrevStcBase[1] = u32STC_Add_Offset;
            HAL_TSP_GetSTCSynth(u32PcrDstSTC, &_ptsp_res->_u32_PrevPllControl[1]);
        }
        ...
        if(( Stc - _ptsp_res->_u32_PrevStcBase[1]) > 0x4000)
        {
            ...
            if(_ptsp_res->_u32_PrevStcBase[1] > Pcr) // faster then PCR comming
            {
                ...
                else if( u32Diff  < 0x1000)
                {
                    ...
                    HAL_TSP_SetSTCSynth(u32PcrDstSTC, _ptsp_res->_u32_PrevPllControl[1] + ((u32Diff)>>6));        //out = now + error x beta
                    _ptsp_res->_u8_ResetPcr[1] = 0;
                    //ULOGD("TSP", "======== Set STC CONTROL <++> %08X  PCR/Diff = %08X/%08X \n",_ptsp_res->_u32_PrevPllControl + ((_ptsp_res->_ptsp_res->_u32_PrevStcBase - Pcr)>>6),Pcr,_ptsp_res->_u32_PrevStcBase-Pcr);
                }
                else
...
static void _TSP_SW_Exit(void)
{
    ...
    if(_ptsp_res->_s32TaskId != -1)
    {
        ...
        {
            TSP_SetEvent(TSP_EVENT_TASKEND);
...
TSP_Result MDrv_TSP_Init(TSP_InitParam *pParam)
{
    ...
    if (u32Check == NULL)
    {
        ...
        bNeedUnMapping = TRUE;
        _phy_to_miu_offset(u8MiuSel, Offset, pParam->phyFWAddr);

        if(MsOS_MPool_Mapping(u8MiuSel, Offset, pParam->u32FWSize, TRUE) == FALSE)
        {
            TSP_OS_Print("MsOS_MPool_Mapping() Fail !!!\n");
            return E_TSP_FAIL;
        }
...
#ifdef MSOS_TYPE_LINUX_KERNEL
    if (bNeedUnMapping == TRUE)
    {
        if(MsOS_MPool_UnMapping((MS_VIRT)u32Check, pParam->u32FWSize) == FALSE)
        {
            TSP_OS_Print("MsOS_MPool_UnMapping() Fail !!!\n");
            return E_TSP_FAIL;
        }
    }
#endif
...
TSP_Result MDrv_TSP_Exit(void)
{
    ...
    HAL_TSP_SetBank(u32Base);
    TSP_DBG(5, "MMIO(MS_MODULE_PM) 0x" MPRI_VIRT "\n", u32Base);
...
static TSP_TSId _TSP_Drv_SRCSEQ2TSID_Mapping(TSP_SRC_SEQ pktDmxId)
{
    switch (pktDmxId)
    {
    ...
    *eTid = E_TSP_TSID_INVALID;
    ...
    if ((DmxFlow == E_DRV_TSP_FLOW_PLAYBACK0) || (DmxFlow == E_DRV_TSP_FLOW_PLAYBACK1))
    {
        if (DmxFlow == E_DRV_TSP_FLOW_PLAYBACK0)
        {
            ...
            *eTid = _TSP_Drv_SRCSEQ2TSID_Mapping(pktDmxId);
        }
    ...
    {
        MS_U32 u32PvrEng = _TSP_Drv_DmxFlow2PVREng_Mapping(DmxFlow);
...
TSP_Result MDrv_TSP_PVR_SetFlowSource(MS_U32 u32Eng, MS_U32 u32PvrSrc)
{
    ...
    switch(u32FltSrc)
    {
        ...
        #if defined(RASP_ENABLE)
            bRASPSrcPktDmx = FALSE;
        #endif
            break;
    }

    u32Tid = HAL_TSP_FltSrc2PktDmx_Mapping(ePidFltSrc);
    ...
    {
        ULOGE("TSP", "[ERROR][%s][%d]Incorrect Engine type!\n",__FUNCTION__,__LINE__);
    }
...
TSP_Result MDrv_TSP_SetFlowSource(DRV_TSP_FLOW DmxFlow, TSP_TSId eTid)
{
    ...
    else if ((DmxFlow == E_DRV_TSP_FLOW_PVR0) || (DmxFlow == E_DRV_TSP_FLOW_PVR1) || (DmxFlow == E_DRV_TSP_FLOW_PVR2) || (DmxFlow == E_DRV_TSP_FLOW_PVR3))
    {
        ...
        switch(ePktDmx)
        {
            ...
            default:
                u32pktDmxId = 0;
                ULOGE("TSP", "[TSP_ERR][%s][%d] Wrong PktDmx type!!!\n",__FUNCTION__,__LINE__);
                break;
        }
...
TSP_Result MDrv_TSP_SelPad(TSP_TSIF eTSIF, TSP_TSPad eTSPad)
{
    ...
    if((eTSPad >= E_TSP_TS_PAD0_3WIRE) && (eTSPad <= E_TSP_TS_PAD3_3WIRE))
    {
...
{
    TSP_Result          ret;
    TSP_TS_PAD          eOutPad = E_TSP_TS_PAD_INVALID, eInPad = E_TSP_TS_PAD_INVALID;
    TSP_TS_PAD_MUX_MODE eOutPadMode = E_TSP_TS_PAD_MUX_INVALID, eInPadMode = E_TSP_TS_PAD_MUX_INVALID;
    ...
    eOutPad = _TSP_DrvHal_PadMapping(eOutTSPad & ~E_TSP_OUT_PADMODE_MASK);
    eOutPadMode = _TSP_DrvHal_OutPadSrcMapping(eOutTSPad & E_TSP_OUT_PADMODE_MASK);

    eInPad = _TSP_DrvHal_PadMapping(eInTSPad);
    bIs3Wire = (eInTSPad >= E_TSP_TS_PAD0_3WIRE) && (eInTSPad <= E_TSP_TS_PAD3_3WIRE);// S2P / S2P1
    u8Mode = (bIs3Wire << 1) + bInParallel;

    switch(u8Mode)
    {
        case 0:
            eInPadMode = E_TSP_TS_PAD_MUX_4WIRED_SERIAL;
            break;
        case 1:
            eInPadMode = E_TSP_TS_PAD_MUX_PARALLEL;
            break;
        case 2:
            eInPadMode = E_TSP_TS_PAD_MUX_3WIRED_SERIAL;
            break;
        case 3:
            TSP_DBG(7, "[%s][%d] Wrong combination !!\n", __FUNCTION__, __LINE__);
            TSP_RETURN(E_TSP_FAIL);
            break;
        default:
            TSP_DBG(7, "[%s][%d] Impossible !!\n", __FUNCTION__, __LINE__);
            break;
    }

    if (HAL_TSP_TsOutPadCfg(eOutPad, eOutPadMode, eInPad, eInPadMode, bEnable))
    {
        ret = E_TSP_OK;
    }
    else
...
TSP_Result MDrv_TSP_FLT_Alloc(MS_U32 u32TSPId, TSP_FltType eFltType, MS_U32 *pu32FltId)
{
    ...
    *pu32FltId = 0xFFFFFFFF;
    ...
    {
        start_id = TSP_SECFLT_START_ID;
        end_id   = TSP_SECFLT_END_ID;
    }
    else
    {
        end_id = TSP_PIDFLT_END_ID;
#if 0
    ...
    if(TSP_NEED_SEC_FILTER(eFltType))
    {
        for (i = start_id; i < end_id; i++) // check which filter is free
        {
            ...
            #if HW_PCRFLT_ENABLE
                if(TSP_PCRFLT_START_ID <= start_id)
                {
                    break;
                }
            #endif
    ...
    else
    {
        ...
        if ((i < TSP_SECFLT_END_ID) && (TSP_SECFLT_END_ID < TSP_PIDFLT_REC_NUM))
    #else
        if (i < TSP_SECFLT_END_ID)
    #endif
        {
...
#ifdef MERGE_STR_SUPPORT
    {
        ...
        {
            TSP_DBG(0, "[%s][%d] Stream index out of range!!\n", __FUNCTION__, __LINE__);
        }
        ...
        if(E_TSP_FLT_USER_PCR == (eFltType & E_TSP_FLT_USER_MASK))
        {
            ...
            {
                u32PcrId = i - TSP_PCRFLT_START_ID;
...
//    TSP_ENTRY();
    ...
    {
        ULOGE("TSP", "[DMX_ERR][%s][%d] Free flt error!\n", __FUNCTION__,__LINE__);
    }
...
TSP_Result MDrv_TSP_FLT_SetPCRSrc(MS_U32 u32FltId,MS_U32 u32PCRSrc)
{
    ...
    ePcrSrc = HAL_TSP_FltSrc2PCRSrc_Mapping(ePidFltSrc);

    if ((u32FltId >= HAL_TSP_PCRFLT_GET_ID(0)) && (u32FltId <= HAL_TSP_PCRFLT_GET_ID(TSP_PCRFLT_NUM - 1)))
    {
        HAL_TSP_PcrFlt_SetSrc((u32FltId - TSP_PCRFLT_START_ID), ePcrSrc);
        TSP_RETURN(E_TSP_OK);
    }
    else
    {
...
TSP_Result MDrv_TSP_FLT_SetPID(MS_U32 u32FltId, MS_U32 u32PID)
{
    ...
    if ((_ptsp_res->_tspInfo.FltType[u32FltId]& E_TSP_FLT_USER_MASK) == E_TSP_FLT_USER_PCR )
    {
        ULOGD("TSP", "Set PCR FLT=%d PID = %8X \n\n",(unsigned int)u32FltId,(unsigned int)u32PID);

        if ((u32FltId >= HAL_TSP_PCRFLT_GET_ID(0)) && (u32FltId <= HAL_TSP_PCRFLT_GET_ID(TSP_PCRFLT_NUM - 1)))
        {
            HAL_TSP_PcrFlt_SetPid((u32FltId - TSP_PCRFLT_START_ID), u32PID);
...
TSP_Result MDrv_TSP_FLT_SetMode(MS_U32 u32FltId, TSP_FltMode eFltMode)
{
    ...
    TSP_DBG(7, "%s\n", __FUNCTION__);
    TSP_CHECK(TSP_SECFLT_END_ID > u32FltId, "[%04d] Invalid u32FltId %d\n", __LINE__, (unsigned int)u32FltId);
...
TSP_Result _MDrv_TSP_FLT_Enable(MS_U32 u32FltId, MS_BOOL bEnable)
{
    ...
    if (!bEnable)
    {
        ...
        TSP_DBG(3, "[TSP_WARNNING][%04d] Enable an active PidFlt %d\n", __LINE__, u32FltId);
    ...
    if (type)
    {
        switch (type)
        {
            ...
            case E_TSP_FLT_FIFO_AUDIO3:
                ...
                break;
            case E_TSP_FLT_FIFO_AUDIO4:
                HAL_TSP_FIFO_SetSrc(E_TSP_DST_FIFO_AUDIO4,u32Src);
                HAL_TSP_PidFlt_SetFltOut(u32FltId, TSP_PIDFLT_OUT_AFIFO4);
                break;
            ...
            if (E_TSP_FLT_FIFO_PVR == type) // @FIXME WHY??????????
            {
                ...
                switch (type)
                {
                    ...
                    case E_TSP_FLT_USER_PCR:
                        ...
                        if ((u32FltId >= HAL_TSP_PCRFLT_GET_ID(0)) && (u32FltId <= HAL_TSP_PCRFLT_GET_ID(TSP_PCRFLT_NUM - 1)))
                        {
                            MS_U32 u32PcrId = u32FltId - TSP_PCRFLT_START_ID;
                            MS_U16 u16PcrUpdateEndEn = (u32PcrId == 0)? TSP_HWINT2_PCR0_UPDATE_END_EN : TSP_HWINT2_PCR1_UPDATE_END_EN;
                            ...
                            HAL_TSP_INT_Enable((MS_U16)(u16PcrUpdateEndEn << 8));
                        }
                        else
                        {
                            TSP_DBG(0, "[%s][%d] Incorrect PCR filter ID !!\n", __FUNCTION__, __LINE__);
                            return E_TSP_FAIL;
                        }
...
TSP_Result MDrv_TSP_SEC_Alloc(MS_U32 u32TSPId, MS_U32 *pu32BufId)
{
    ...
    *pu32BufId = 0xFFFFFFFF;
    ...
    for (i = TSP_SECBUF_START_ID; i < TSP_SECBUF_END_ID; i++)
    {
...
{
    TSP_DBG(7, "%s\n", __FUNCTION__);
    TSP_CHECK(TSP_SECFLT_END_ID > u32BufId, "[%04d] Invalid u32BufId %d\n", __LINE__, (unsigned int)u32BufId);
...
TSP_Result MDrv_TSP_SEC_SetBuffer(MS_U32 u32BufId, MS_PHYADDR u32Start, MS_U32 u32Size)
{
    ...
    if (_ptsp_res->_tspInfo.MulFlt2Buf[u32BufId] == 1)
    {
        ...
        _phy_to_miu_offset(u8MiuSel, Offset, u32Start);
        if(MsOS_MPool_Mapping(u8MiuSel, Offset, u32Size, TRUE) == FALSE)
        {
            TSP_OS_Print("[%s][%d]: MsOS_MPool_Mapping() Fail !!!\n", __FUNCTION__, __LINE__);
            return;
        }
    #endif
...
TSP_Result MDrv_TSP_FILE_Eng_SetPktMode(TSP_FILE_ENG Eng, TSP_PktMode mode)
{
    ...
    switch (mode)
    {
        ...
        case E_TSP_PKTMODE_134:
            ...
            break;
        default:
            ULOGE("TSP", "Invalid File-in Packet Size!\n");
            TSP_RETURN(E_TSP_FAIL);
            break;
    }
...
{
    return (MS_U32)HAL_TSP_FilePath2Tsif_Mapping((MS_U32)Eng);
}
...
static TSP_HAL_PKT_MODE _TSP_DrvHAL_PktConverter_PktModeMapping(TSP_PktMode ePktMode)
{
    ...
    switch(ePktMode)
    {
        ...
        case E_TSP_PKTMODE_MXL200:
            ...
            break;
        default:
            ULOGE("TSP", "[%s][%d] Invalid Packet Mode !!\n", __FUNCTION__, __LINE__);
            break;
    }
...
TSP_Result MDrv_TSP_FILE_Eng_CmdStart(TSP_FILE_ENG Eng, TSP_FileMode mode)
{
    ...
    switch (Eng)
    {
        ...
        case E_TSP_FILE_ENG_TSIF3:
            break;
        default:
            ULOGE("TSP", "Invalid File-in Engine number!\n");
            TSP_RETURN(E_TSP_FAIL);
            break;
    }
...
static TSP_DST_SEQ _TSP_DrvHal_DstMapping(DrvTSP_AVFIFO eDebugFifo)
{
    ...
    switch(eDebugFifo)
    {
        case E_DRVTSP_AVFIFO_VIDEO:
            eRetDst= E_TSP_DST_FIFO_VIDEO;
            break;
        case E_DRVTSP_AVFIFO_VIDEO3D:
            eRetDst= E_TSP_DST_FIFO_VIDEO3D;
            break;
        case E_DRVTSP_AVFIFO_AUDIO:
            eRetDst= E_TSP_DST_FIFO_AUDIO;
            break;
        case E_DRVTSP_AVFIFO_AUDIOB:
            eRetDst= E_TSP_DST_FIFO_AUDIO2;
            break;
        case E_DRVTSP_AVFIFO_AUDIOC:
            eRetDst= E_TSP_DST_FIFO_AUDIO3;
            break;
        case E_DRVTSP_AVFIFO_AUDIOD:
            eRetDst= E_TSP_DST_FIFO_AUDIO4;
            break;
        default:
            ULOGE("TSP", "[ERROR][%s][%d] UnSupport Debug AV Type!\n",__FUNCTION__,__LINE__);
            break;
    }
    ...
    switch(eDebugFlow)
    {
       ...
       case E_DRVTSP_FLOW_FILE0:
        ...
        case E_DRVTSP_FLOW_MMFI1:
            ...
            break;
        default:
            ULOGE("TSP", "[ERROR][%s][%d] UnSupport Debug Flow !\n",__FUNCTION__,__LINE__);
            break;
    }
    ...
    *pu32Cnt = HAL_TSP_Debug_DropDisPktCnt_Get(ePktDmx, FALSE);
    ...
    switch(eCmd)
    {
        ...
        case E_DRVTSP_DEBUG_CMD_ENABLE:
            ...
            break;
        default:
            ULOGE("TSP", "[%s][%d] UnSupported Debug Cmd  !!\n",__FUNCTION__,__LINE__);
            TSP_RETURN(E_TSP_FAIL);
    }
...
TSP_Result MDrv_TSP_Get_LockPktCnt(DrvTSP_Debug_Cmd eCmd, DrvTSP_Flow eFlow, MS_U32 *pu32Cnt)
{
    ...
    switch(eFlow)
    {
        ...
        case E_DRVTSP_FLOW_LIVE3:
            ...
            break;
        default:
            ULOGE("TSP", "[%s][%d] UnSupport Debug Flow !!\n",__FUNCTION__,__LINE__);
            return E_TSP_FAIL;
    }
    ...
    *pu32Cnt = HAL_TSP_Debug_LockPktCnt_Get(TRUE);
...
TSP_Result MDrv_TSP_MStr_SyncByte(DrvTSP_If eTsIf, MS_U8 u8StrId, MS_U8* pu8SyncByte, MS_BOOL bSet)
{
    ...
    {
        ULOGE("TSP", "[%s][%d] Stream index out of range !!\n",__FUNCTION__,__LINE__);
        return E_TSP_FAIL;
    }

    switch(eTsIf)
    {
        case E_DRVTSP_IF_TSIF0:
            u8Path = TSP_TSIF0;
            break;
        case E_DRVTSP_IF_TSIF1:
            u8Path = TSP_TSIF1;
            break;
        case E_DRVTSP_IF_TSIF2:
            u8Path = TSP_TSIF2;
            break;
        case E_DRVTSP_IF_TSIF3:
            u8Path = TSP_TSIF3;
            break;
        default:
            return E_TSP_FAIL;
...
TSP_Result MDrv_TSP_GetTSIFStatus(TSP_TSIF eTSIF, TSP_TSPad* ePad, MS_BOOL* pbClkInv, MS_BOOL* pbExtSyc, MS_BOOL* pbParl)
{
    ...
    {               // in the opposite, hal translate register state to that enum to drv layer to communicate...
        case REG_TOP_TS_SRC_EXT0:
            if((u16clk & REG_CLKGEN0_TS_SRC_MASK) != REG_CLKGEN0_TS_SRC_EXT0)
            {
        ...
        case REG_TOP_TS_SRC_EXT1:
            if((u16clk & REG_CLKGEN0_TS_SRC_MASK) != REG_CLKGEN0_TS_SRC_EXT1)
            {
                Res = E_TSP_FAIL_FUNCTION;
            }
            *ePad = E_TSP_TS_PAD1;
        ...
        case REG_TOP_TS_SRC_EXT2:
            if((u16clk & REG_CLKGEN0_TS_SRC_MASK) != REG_CLKGEN0_TS_SRC_EXT2)
            {
                Res = E_TSP_FAIL_FUNCTION;
            }
            *ePad = E_TSP_TS_PAD2;
        ...
        case REG_TOP_TS_SRC_EXT3:
            if((u16clk & REG_CLKGEN0_TS_SRC_MASK) != REG_CLKGEN0_TS_SRC_EXT3)
            {
                Res = E_TSP_FAIL_FUNCTION;
            }
            *ePad = E_TSP_TS_PAD3;
        ...
        case REG_TOP_TS_SRC_DMD0:
            if((u16clk & REG_CLKGEN0_TS_SRC_MASK) != REG_CLKGEN0_TS_SRC_DMD0)
            {
                Res = E_TSP_FAIL_FUNCTION;
            }
...
// @TODO Add to menuconfig [bruce]
#ifdef DMD1_IF_SUPPORT //support demod1
        case REG_TOP_TS_SRC_DMD1:
            if((u16clk & REG_CLKGEN0_TS_SRC_MASK) != REG_CLKGEN0_TS_SRC_DMD1)
            {
                Res = E_TSP_FAIL_FUNCTION;
            }
        ...
        case REG_TOP_TS_SRC_TSO0:
            if((u16clk & REG_CLKGEN0_TS_SRC_MASK) != REG_CLKGEN0_TS_SRC_TSO0)
            {
                Res = E_TSP_FAIL_FUNCTION;
            }
...
// @TODO Add to menuconfig [bruce]
#ifdef TSO1_IF_SUPPORT //support TSO1
        case REG_TOP_TS_SRC_TSO1:
            if((u16clk & REG_CLKGEN0_TS_SRC_MASK) != REG_CLKGEN0_TS_SRC_TSO1)
            {
                Res = E_TSP_FAIL_FUNCTION;
            }
...
{
    ST_TSP_HAL_CLK_STATUS stClkStatus;
    MS_VIRT u32Base;
    MS_PHY u32Size;
    ...
    if(HAL_TSP_GetClockSetting(E_TSP_HAL_TSP_CLK, 0, &stClkStatus) == FALSE)
    {
        TSP_OS_Print("[TSP Error][%s][%d] Un-Support Clock Type !!\n", __FUNCTION__, __LINE__);
        return E_TSP_FAIL_NOT_SUPPORTED;
    }

    if((stClkStatus.bEnable == FALSE) || (stClkStatus.u8ClkSrc == REG_CLKGEN0_TSP_SRC_XTAL))
    {
        TSP_OS_Print("[TSP Error][%s][%d] Not init TSP clock !!\n", __FUNCTION__, __LINE__);
        return E_TSP_FAIL;
    }
...
TSP_Result MDrv_TSP_PVR_FltAlloc(TSP_FltType eFltType, MS_U32 *pu32FltId)
{
    ...
    {
        ULOGE("TSP", "[ERROR][%s][%d]Incorrect filter type!\n",__FUNCTION__,__LINE__);
        return E_TSP_FAIL;
    }
#else
    return E_TSP_OK;
...
TSP_Result MDrv_TSP_FLT_ChangeSource(MS_U32 u32FltId, TSP_FltType eTSPFltTypeSrc)
{
    ...
    if ((u32FltId >= HAL_TSP_PCRFLT_GET_ID(0)) && (u32FltId <= HAL_TSP_PCRFLT_GET_ID(TSP_PCRFLT_NUM - 1)))
    {
        HAL_TSP_PcrFlt_Enable(u32FltId - TSP_PCRFLT_START_ID, FALSE);
    }
    else
    {
        ULOGE("TSP", "[%s][%d] Incorrect PCR filter ID !!\n",__FUNCTION__,__LINE__);
...
{
    TSP_CHECK(TSP_PIDFLT_END_ID > PidFltId, "[%04d] Invalid u32FltId %d\n", __LINE__, (unsigned int)PidFltId);
...
TSP_TSIF MDrv_TSP_TsifMapping(TSP_TSIF eTSIF)
{
    ...
    switch(u32TSIF)
    {
        ...
        case TSP_TSIF3:
            return E_TSP_TSIF_3;
        default:
            TSP_DBG(TRUE, "[%04d] TSIF %lu is wrong !! \n", __LINE__, u32TSIF);
            return E_TSP_TSIF_0;
    }
}
...
#if 1
    if((s32Ret = MsOS_WaitEvent_Interrupt(_s32EventId, TSP_TASK_EVENTS, &u32Event, E_OR_CLEAR, MSOS_WAIT_FOREVER)) <= 0)
    {
...
#else
    s32Ret = TSP_GetEvent(&u32Event, TSP_TASK_EVENTS, E_OR_CLEAR, MSOS_WAIT_FOREVER);
