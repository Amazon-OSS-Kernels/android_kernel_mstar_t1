...
MS_BOOL _NSK2_verSC_Reset(MS_U8 u8SCID,SC_ResetType RstType)
{
    ...
    if (MDrv_SC_Reset_ATR(u8SCID, pInitParams, _u8AtrBuf, &_u16AtrLen, history, &history_len) == E_SC_OK)
    {
        ...
        NSK_TRACE(("***Smartcard%d  T=%d  ***\n",u8SCID, u8Protocol));
        {
            int i;
            NSK_TRACE(("SM%d  ATR Message :     \n",u8SCID));
            NSK_TRACE(("Rx :     \n"));
        ...
        MS_U8 sc_clock = (_atrbuf[2]>>4)&0xf;
        ...
        MDrv_SC_SetGuardTime(0,_atrbuf[4]);
        return TRUE;
    }
...
{
    printf("NSK2 driver update at 2015.06.10 \n");
    NSK_TRACE(("Enter\n"));
    ...
    // UART setting
    MDrv_SC_GetSCParam(u8SCID,pInitParams);
    ...
    {
	    pInitParams->eVccCtrl     = E_SC_VCC_CTRL_8024_ON; 	//E_SC_VCC_CTRL_HIGH;//E_SC_VCC_CTRL_LOW;//E_SC_VCC_CTRL_8024_ON
    }
    ...
    if (MDrv_SC_Open(u8SCID, u8Protocol, pInitParams, _SC_Notify) == E_SC_OK)
    {
        /*
         if (MDrv_SC_Activate(0) != E_SC_OK)
        {
            NSK_ERROR(("\nMDrv_SC_Open fail"));
        }
        */
    ...
    {
        NSK_ERROR(("Open fail with \n"));
        return FALSE;
    }


    NSK2HDX_SetScreaderHwInfo(  &sc_reader_id[0],                              // inform verifier
                                sizeof(sc_reader_id)/sizeof(NDS_ULONG));


    // wyc.test-130701
    NSK_TRACE(("pull GPIO-89 high\n"));
    mdrv_gpio_set_high(89);
...
NDS_STATUS32 NSK2HDX_SetVcc(NDS_ULONG screader_id, NDS_UBYTE vcc_switch)
{
    ...
    {
        MDrv_SC_CardVoltage_Config(0,E_SC_VOLTAGE_OFF);
    }
...
MS_U32 _SC_Calc_WWT(MS_U8 u8Ta1Byte, MS_U8 u8Tc2Byte)
{
    ...
    for(i=0; i<u32ConvTabSize; ++i)
    {
        ...
        {
            NSK_TRACE(("clock_rate_conversion=%f, clock_rate=%f\n",
                        SC_Rate_Table[i].clock_rate_conversion, SC_Rate_Table[i].clock_rate));
            wwt = INITIAL_WAITING_TIME * (float)u8Tc2Byte * (SC_Rate_Table[i].clock_rate_conversion / SC_Rate_Table[i].clock_rate);
            wwt /= 1000;    //convert to mSec
            u32WWT = (MS_U32)wwt;
            NSK_TRACE(("work waiting time, float=[%f], hex=[%x]\n", wwt, u32WWT));
            return u32WWT;
        }
    }
    return u32WWT;
                                    ...
                                    NDS_UBYTE *atr_len, NDS_UBYTE *first_history_byte)
{
    ...
    *atr_len = u16atrlen;
    ...
    #else
    if( (atr_data[2] == 0x11) || (atr_data[2] == 0x12) || (atr_data[2] == 0x13) || (atr_data[2] == 0x14) || (atr_data[2] == 0x15) )
    {
        MDrv_SC_SetBlockWaitingTime(screader_id, 794 - SCHWTimeOutOffset);
        MDrv_SC_SetWaitingTime(screader_id, 794 - SCTimeOutOffset);
    }
    else if( (atr_data[2] == 0x36) || (atr_data[2] == 0x5F) )
    {
        MDrv_SC_SetBlockWaitingTime(screader_id, 1058 - SCHWTimeOutOffset);
        MDrv_SC_SetWaitingTime(screader_id, 1058 - SCTimeOutOffset);
    }
    else if(atr_data[2] ==0x6F)
    {
        MDrv_SC_SetBlockWaitingTime(screader_id, 1323 - SCHWTimeOutOffset);
        MDrv_SC_SetWaitingTime(screader_id, 1323 - SCTimeOutOffset);
    }
    else if(atr_data[2] == 0x96)
    {
        MDrv_SC_SetBlockWaitingTime(screader_id, 1094 - SCHWTimeOutOffset);
        MDrv_SC_SetWaitingTime(screader_id, 1094 - SCTimeOutOffset);
    }
    else
    {
        MDrv_SC_SetBlockWaitingTime(screader_id, 1323 - SCHWTimeOutOffset);
                                    ...
                                    NSK2HDX_UARTCONFIGURATION *conf)
{
	...
	if (TRUE == isDefaultUART)
	{
		...
		printf("NSK2HDX_GetUartParams: default wait_time = %x\n", wait_time);
