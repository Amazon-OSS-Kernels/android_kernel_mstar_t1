...
MS_U32 _MDrv_DSCMB2_FltAllocWithCAVid(MS_U32 u32EngId, DSCMB_Flt_Type eFltType, MS_U32 u32CAVid)
{
    ...
    DSCMB_Flt_Type eFltTypeTmp = eFltType & (~E_DSCMB_FLT_SECURE_KEYS_ENABLE);
    DSCMB_ASSERT(u32EngId == 0, "EngId input error", DRV_DSCMB_FLT_NULL);
    DSCMB_ASSERT((eFltTypeTmp == E_DSCMB_FLT_3_KEYS) || (eFltTypeTmp == E_DSCMB_FLT_2_KEYS) || (eFltTypeTmp == E_DSCMB_FLT_1_KEYS), "FltType input error", DRV_DSCMB_FLT_NULL);
    ...
    for ( i = 0 ; i < HAL_DSCMB_KTE_MAX ; i ++)
    {
        ...
        if ( pMap->bUsed == FALSE )
        {
            ...
            if(E_DSCMB_FLT_1_KEYS == eFltTypeTmp)
            {
            ...
            if ( E_DSCMB_FLT_3_KEYS == eFltTypeTmp)
            {
    ...
    if(E_DSCMB_FLT_SECURE_KEYS_ENABLE & eFltType)
    {
        HAL_DSCMB_PidIdx_SetSecure(i, TRUE);
        pMap->bSecure = TRUE;
    }
    else
    {
...
#ifdef FLEXIBLE_PIDSLOTMAP
    HAL_DSCMB_SetTSPPidSlotMap(u32FltId, u32DscmbId);
#else
...
#ifdef FLEXIBLE_PIDSLOTMAP
    HAL_DSCMB_SetTSPPidSlotMap(u32FltId, 0);
#else
    // Reset the Key index
    HAL_DSCMB_PidIdx_ClearSlotKeyIdx(u32FltId, E_DSCMB_KEY_CLEAR);
    HAL_DSCMB_PidIdx_ClearSlotKeyIdx(u32FltId, E_DSCMB_KEY_EVEN);
...
MS_BOOL _MDrv_DSCMB2_EngEnableKey(MS_U32 u32EngId, MS_U32 u32DscmbId, DSCMB_Eng_Type eEngType, DSCMB_Key_Type eKeyType, MS_BOOL bEnable)
{
    ...
    if (!HAL_DSCMB_KTE_Key_Ctrl (u32FltId, eKeyType , eEngType , u32CAVid, bEnable ))
    {
...
MS_BOOL _MDrv_DSCMB2_EngSetIV(MS_U32 u32EngId, MS_U32 u32DscmbId, DSCMB_Key_Type eKeyType, MS_U8* pu8IV)
{
    ...
    DSCMB_Key_Type eKeyTypeTmp = eKeyType & (~E_DSCMB_KEY_SECURE_KEYS_ENABLE);
    DSCMB_ASSERT(u32EngId == 0, "EngId input error", FALSE);
    DSCMB_ASSERT(u32DscmbId < HAL_DSCMB_KTE_MAX, "DscmbId input error", FALSE);
    DSCMB_ASSERT(eKeyTypeTmp <= E_DSCMB_KEY_ODD, "KeyType input error", FALSE);
    ...
    if((pMap->eFltType == E_DSCMB_FLT_2_KEYS) && (eKeyTypeTmp == E_DSCMB_KEY_CLEAR))
    {
...
MS_BOOL _MDrv_DSCMB2_EngSetIV_Ex(MS_U32 u32EngId, MS_U32 u32DscmbId, DSCMB_Eng_Type eEngType, DSCMB_Key_Type eKeyType, MS_U8* pu8IV)
{
    ...
    if((eEngType & E_DSCMB_ENG_LSAS) == E_DSCMB_ENG_LSAS )
    {
        if (!HAL_DSCMB_KTE_Write_IV_Ex(u32FltId, eKeyType, E_DSCMB_ENG_LSAS, u32CAVid, pu8IV))
        {
            DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "Set LSAS IV Fail\n");
            goto MDrv_DSCMB2_EngSetIV_Ex_FAIL;
        }

        if (!HAL_DSCMB_KTE_IV_Ctrl_Ex (u32FltId, eKeyType, E_DSCMB_ENG_LSAS, u32CAVid, TRUE ))
        {
            DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "Set LSAS IV Fail\n");
            goto MDrv_DSCMB2_EngSetIV_Ex_FAIL;
        }
    ...
    if((eEngType & E_DSCMB_ENG_ESA) == E_DSCMB_ENG_ESA )
    {
        if (!HAL_DSCMB_KTE_Write_IV_Ex(u32FltId, eKeyType, E_DSCMB_ENG_ESA, u32CAVid, pu8IV))
        {
            DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "Set ESA IV Fail\n");
            goto MDrv_DSCMB2_EngSetIV_Ex_FAIL;
        }

        if (!HAL_DSCMB_KTE_IV_Ctrl_Ex (u32FltId, eKeyType, E_DSCMB_ENG_ESA, u32CAVid, TRUE ))
        {
            DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "Set ESA IV Fail\n");
            goto MDrv_DSCMB2_EngSetIV_Ex_FAIL;
        }
    ...
    if((eEngType & E_DSCMB_ENG_LSAD) == E_DSCMB_ENG_LSAD )
    {
        if (!HAL_DSCMB_KTE_Write_IV_Ex(u32FltId, eKeyType, E_DSCMB_ENG_LSAD, u32CAVid, pu8IV))
        {
            DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "Set LSAD IV Fail\n");
            goto MDrv_DSCMB2_EngSetIV_Ex_FAIL;
        }

        if (!HAL_DSCMB_KTE_IV_Ctrl_Ex (u32FltId, eKeyType, E_DSCMB_ENG_LSAD, u32CAVid, TRUE ))
        {
            DRVDSCMB2_DBG(DSCMB2_DBGLV_ERR, "Set LSAD IV Fail\n");
            goto MDrv_DSCMB2_EngSetIV_Ex_FAIL;
        }
...
#if 0
MS_BOOL MDrv_DSCMB2_CAPVR_Pid_Open (DSCMB_CAPVR_MODE eCaMode, MS_U32 u32Pid, MS_U8* pu8FltId)
{
    DSCMB_INIT_CHECK(FALSE);
    MS_U32 u32ProgId  = HAL_DSCMB_GetProgId(eCaMode);
    MS_U32 i = 0;
    ...
    {
        if(_bUsedCAFlt[u32ProgId][i] == FALSE)
        {
            bFound = TRUE;
            break;
        }
    ...
    if(bFound)
    {
        *pu8FltId = i + HAL_DSCMB_SPSPVR_FLT_NUM*u32ProgId;
        _bUsedCAFlt[u32ProgId][i] = TRUE;
    }

    return HAL_DSCMB_SPSPVR_SetPid(u32ProgId, i,  u32Pid,  TRUE/*buf0*/, FALSE/*buf1*/); //only enable buffer0
}

MS_BOOL MDrv_DSCMB2_CAPVR_Pid_Close (DSCMB_CAPVR_MODE eCaMode, MS_U8 u8FltId)
{
    DSCMB_INIT_CHECK(FALSE);
    MS_U32 u32ProgId  = HAL_DSCMB_GetProgId(eCaMode);

    MS_U32 i = u8FltId % DSCMB_CAPVR_PIDFLT_MAX;

    _bUsedCAFlt[u32ProgId][i] = FALSE;

    return HAL_DSCMB_SPSPVR_SetPid(u32ProgId, i,  0x0 , FALSE/*buf0*/, FALSE/*buf1*/);
}
...
{
#define E_DSCMB_KL_DST_KT_PIDSLOTMAP_ENABLE  (1 << 14)
