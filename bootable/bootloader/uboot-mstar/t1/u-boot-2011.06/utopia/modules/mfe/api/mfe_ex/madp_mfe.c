...
#if defined(_MFE_MUJI_) || defined(_MFE_MONET_) || defined(_MFE_MESSI_) || defined(_MFE_MANHATTAN_) || defined(_MFE_MASERATI_) || defined(_MFE_MAXIM_) || defined(_MFE_KANO_) || defined(_MFE_KASTOR_)
            set_value = (*((MS_S32*) value));
...
#if defined(MFE_YUV_LOADER)
    if (YUVTILE == appPVR_Info->bColorFormat) {
        if ((YUVPlane.Cur_Y0.miuAddress & 0xFF) != 0)   // 256-byte alignment
            return FALSE;
        if ((YUVPlane.Cur_C0.miuAddress & 0xFF) != 0)
            return FALSE;
    }
    else if (MSTTILE == appPVR_Info->bColorFormat) {
        if ((YUVPlane.Cur_Y0.miuAddress & 0x1FF) != 0)  // 512-byte alignment
            return FALSE;
        if ((YUVPlane.Cur_C0.miuAddress & 0x1FF) != 0)
            return FALSE;
    }
    else if (MFE_YUYV <= appPVR_Info->bColorFormat && appPVR_Info->bColorFormat <= MFE_VYUY) {
        if ((YUVPlane.Cur_Y0.miuAddress & 0x7) != 0)    // 8-byte alignment
            return FALSE;
    }
    else {
        if ((YUVPlane.Cur_Y0.miuAddress & 0x3FF) != 0)  // 1024-byte alignment
            return FALSE;
        if ((YUVPlane.Cur_C0.miuAddress & 0x3FF) != 0)
            return FALSE;
    }
#else
