...
void mfeH264_DeInit(MFE_CONFIG* pConfig)
{
    ...
    if(pInfo->dpb)
        MfeDrvMemFree(pInfo->dpb, (const MS_S8*)("pInfo->dpb"));


    for(i=0;i<pInfo->BufPool.FrameBufPoolSize;i++) {
        if(pInfo->BufPool.addr[i])
            MfeDrvMemFree(pInfo->BufPool.addr[i], (const MS_S8*)("BufPool.addr[i]"));
    }
    if(pInfo->BufPool.addr)
        MfeDrvMemFree(pInfo->BufPool.addr, (const MS_S8*)("BufPool.addr"));
    if(pInfo->BufPool.available)
        MfeDrvMemFree(pInfo->BufPool.available, (const MS_S8*)("BufPool.available"));
