...
MS_BOOL HAL_MSPI_CLOCK_Config(MS_U32 u32MaxClock)
{
    ...
    u16TempClk  = CLK_READ(MSPI_CLK_CFG);
    u16ClkLevel = u16ClkLevel << 2;
    u16TempClk &= (~(7 << 2));
    u16TempClk |= u16ClkLevel;
    CLK_WRITE(MSPI_CLK_CFG, u16TempClk);
    return TRUE;
}
...
MS_BOOL HAL_MSPI_CLK_Config(MS_U8 u8Chanel,MS_U32 u32MspiClk)
{
    ...
    DEBUG_MSPI(E_MSPI_DBGLV_DEBUG,printf("u8ClkSpi_P1 =%d\n", (int)clk_buffer[i].u8ClkSpi_P1));
    DEBUG_MSPI(E_MSPI_DBGLV_DEBUG,printf("u8ClkSpi_P2 =%d\n", (int)clk_buffer[i].u8ClkSpi_P2));
    DEBUG_MSPI(E_MSPI_DBGLV_DEBUG,printf("u8ClkSpi_DIV =%d\n",(int)clk_buffer[i].u8ClkSpi_DIV));
    DEBUG_MSPI(E_MSPI_DBGLV_DEBUG,printf("u32ClkSpi = %u\n",  (unsigned int)clk_buffer[i].u32ClkSpi));

    printf("Select MSPI clock = %uKHz\r\n", (unsigned int)clk_buffer[i].u32ClkSpi/1000);
