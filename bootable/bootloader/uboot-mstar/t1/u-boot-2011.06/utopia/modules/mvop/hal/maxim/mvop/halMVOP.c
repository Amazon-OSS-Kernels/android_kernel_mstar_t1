...
void _HAL_MVOP_MaskMIU(MVOP_DevID eID, MS_BOOL bEnMask)
{
    switch(eID)
    {
        case E_MVOP_DEV_0:
            HAL_WriteRegBit(VOP_MIU_IF, bEnMask, VOP_MIU_REQ_DIS);
            if(bEnMask)
            {
                HAL_WriteByteMask(VOP_REG_MASK, VOP_LSB_REQ_MASK, VOP_LSB_REQ_MASK);
            }
            else
            {
        ...
        case E_MVOP_DEV_1:
#if MVOP_SUPPORT_SUB
            HAL_WriteRegBit(SUB_REG(VOP_MIU_IF), bEnMask, VOP_MIU_REQ_DIS);
            if(bEnMask)
            {
                HAL_WriteByteMask(SUB_REG(VOP_REG_MASK), VOP_LSB_REQ_MASK, VOP_LSB_REQ_MASK);
            }
            else
            {
...
{
    UNUSED(bEnable);
    //using _HAL_MVOP_MaskMIU to mask
    return;
}

void HAL_MVOP_Rst(void)
{
    _HAL_MVOP_MaskMIU(E_MVOP_DEV_0, TRUE);
    HAL_WriteRegBit(VOP_CTRL0, 0, BIT0);
    HAL_WriteRegBit(VOP_CTRL0, 1, BIT0);
    _HAL_MVOP_MaskMIU(E_MVOP_DEV_0, FALSE);
...
void HAL_MVOP_SetOutputTiming( MVOP_Timing *ptiming )
{
        ...
        && (ptiming->u8Framerate > 15)) || g_pHalMVOPCtx->bIs265DV)
    {
        ...
        if(g_pHalMVOPCtx->bIs265DV)
        {
            ...
            {
                g_pHalMVOPCtx->u16VsyncLines = VSYNC_LINE_4KDV_30;//abc123:1186138 (from 0x40->0x60)
            }
    ...
    else if(((ptiming->u16V_TotalCount >= 1080) && (ptiming->u16H_TotalCount >= 1440)) && (ptiming->u8Framerate >= 24) && (ptiming->bInterlace == 1))
    {
        ...
        {
            g_pHalMVOPCtx->bNewVSyncMode = FALSE;
            HAL_WriteRegBit(VOP_CTRL1, 1, VOP_MVD_VS_SEL); // 4 lins forwarding; abc123 ID:1074519
        }
        else
        {
            g_pHalMVOPCtx->bNewVSyncMode = TRUE;
            HAL_MVOP_SetFieldInverse(DISABLE, ENABLE);
            g_pHalMVOPCtx->u16VsyncLines = VSYNC_LINE_fHD_30_I; //abc1231251174: 0x40 -> 0xAD
            HAL_WriteByte(VOP_DMA0, 0x18);
            HAL_WriteByteMask(VOP_DMA1, 0x01, VOP_BURST_EXT);
        }
    }
...
void HAL_MVOP_SetDCSRAMClk(MS_U8 clkNum, MS_BOOL bEnable)
{
    ...
    if (clkNum==0)
    {
        ...
        HAL_WriteRegBit(REG_CKG_DC_SRAM, !bEnable, CKG_DC1_SRAM); //for mfdec1 + mvop0
    }
}
...
void HAL_MVOP_SelMIU(MVOP_DevID eDevID, HALMVOPMIUSEL eMiuMSB0, HALMVOPMIUSEL eMiuMSB1, HALMVOPMIUSEL eMiuLSB0, HALMVOPMIUSEL eMiuLSB1)
{
    ...
    _HAL_MVOP_MaskMIU(eDevID, FALSE);
}
...
void HAL_MVOP_Enable2DCTimingSync(MS_BOOL bEnable)
{
    ...
    _HAL_MVOP_MaskMIU(E_MVOP_DEV_0, TRUE);
    _HAL_MVOP_MaskMIU(E_MVOP_DEV_1, TRUE);
    ...
    _HAL_MVOP_MaskMIU(E_MVOP_DEV_0, FALSE);
    _HAL_MVOP_MaskMIU(E_MVOP_DEV_1, FALSE);
}
...
void HAL_MVOP_SetbyInputTiming(MVOP_DevID eID, MVOP_VidStat *pstVideoStatus)
{
    ...
    if(pstVideoStatus->u16FrameRate == 58000)
    {
        pstVideoStatus->u16FrameRate = 60000;
        MVOP_PRINTF("%s Modify mvop framerate from 58000 to 60000.\n", __FUNCTION__);
    }
}
...
void HAL_MVOP_SubSetOutputTiming( MVOP_Timing *ptiming )
{
            ...
            && (ptiming->u8Framerate > 15)) || g_pHalMVOPCtx->bIs265DV)
    {
        ...
        if(g_pHalMVOPCtx->bIs265DV)
        {
            ...
            {
                g_pHalMVOPCtx->u16SubVsyncLines = VSYNC_LINE_4KDV_30;//abc123:1186138 (from 0x40->0x60)
            }
    ...
    else if(((ptiming->u16V_TotalCount >= 1080) && (ptiming->u16H_TotalCount >= 1440)) && (ptiming->u8Framerate >= 24) && (ptiming->bInterlace == 1))
    {
        ...
        {
            g_pHalMVOPCtx->bSubNewVSyncMode = FALSE;
            HAL_WriteRegBit(SUB_REG(VOP_CTRL1), 1, VOP_MVD_VS_SEL); // 4 lins forwarding; abc123 ID:1074519
        }
        else
        {
            g_pHalMVOPCtx->bSubNewVSyncMode = TRUE;
            HAL_MVOP_SubSetFieldInverse(DISABLE, ENABLE);
            g_pHalMVOPCtx->u16SubVsyncLines = VSYNC_LINE_fHD_30_I; //abc1231251174: 0x40 -> 0xAD
            HAL_WriteByte(SUB_REG(VOP_DMA0), 0x18);
            HAL_WriteByteMask(SUB_REG(VOP_DMA1), 0x01, VOP_BURST_EXT);
        }
    }
...
void HAL_MVOP_SubSetDCSRAMClk(MS_U8 clkNum, MS_BOOL bEnable)
{
    ...
    if (clkNum==0)
    {
        HAL_WriteRegBit(REG_CKG_DC_SRAM, !bEnable, CKG_DC0_SRAM); //for mfdec0 + mvop1
