    ...
    else
    {
       ...
       //Skip USFC now, which may cause kernel panic, wait PQ Driver complated.
        ...
        if(pu8TMOBuf)
        {
            ...
            pstPQInitInfo->stPQBinInfo[PQ_BIN_TMO].PQBin_PhyAddr = (MS_PHYADDR) pu8TMOBuf;
        }
        else
        {
            pstPQInitInfo->u8PQBinTMOCnt = 0;
...
#ifdef STELLAR
if(MApi_XC_Is_SupportSWDS() == FALSE || (!MApi_XC_GetDynamicScalingStatus()))
#else
...
#if (defined (UFO_XC_DS_PQ))
#ifdef STELLAR
    if(MApi_XC_Is_SupportSWDS() == TRUE && (MApi_XC_GetDynamicScalingStatus()))
#else
