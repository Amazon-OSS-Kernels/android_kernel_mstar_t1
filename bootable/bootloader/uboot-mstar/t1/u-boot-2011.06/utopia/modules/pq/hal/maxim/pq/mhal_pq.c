...
#define PEAKING_CAG_MAX_SRAM_SIZE 0x124
#define PEAKING_SRAM1_CAG_COUNT 289//81
#define PEAKING_SRAM2_CAG_COUNT 272//72
#define PEAKING_SRAM3_CAG_COUNT 272//72
#define PEAKING_SRAM4_CAG_COUNT 256//64

#define PEAKING_SRAM1_CAG_OFFSET 0
#define PEAKING_SRAM2_CAG_OFFSET PEAKING_SRAM1_CAG_COUNT
#define PEAKING_SRAM3_CAG_OFFSET (PEAKING_SRAM2_CAG_OFFSET + PEAKING_SRAM2_CAG_COUNT)
#define PEAKING_SRAM4_CAG_OFFSET (PEAKING_SRAM3_CAG_OFFSET + PEAKING_SRAM3_CAG_COUNT)

#define PEAKING_SRAM_CAG_TOTAL_COUNT PEAKING_SRAM1_CAG_COUNT + PEAKING_SRAM2_CAG_COUNT + PEAKING_SRAM3_CAG_COUNT + PEAKING_SRAM4_CAG_COUNT

void _Hal_PQ_set_sram_peaking_cag_table(MS_U16 *pBuf, MS_U8 u8SRAM_Idx, MS_U16 u16Cnt)
{
    MS_U16 i;
    if(u8SRAM_Idx > 3)
    {
    ...
    MApi_XC_W2BYTEMSK(REG_SC_BK76_5C_L, BIT(0), BIT(0)); // io_en disable
    MApi_XC_W2BYTEMSK(REG_SC_BK76_5C_L, u8SRAM_Idx<<1, BIT(2)|BIT(1)); // sram select

    for(i=0; i<u16Cnt; i++)
    {
        MApi_XC_W2BYTEMSK(REG_SC_BK76_5D_L, i, 0x01FF); // address[8:0]
        MApi_XC_W2BYTEMSK(REG_SC_BK76_5E_L, pBuf[i], 0x01FF); //data[8:0]
        MApi_XC_W2BYTEMSK(REG_SC_BK76_5E_L, BIT(15), BIT(15)); // io_w enable
    }
    MApi_XC_W2BYTEMSK(REG_SC_BK76_5C_L, 0, BIT(0)); // io_en enable
}
void Hal_PQ_set_sram_peaking_cag_table(void *pInstance,MS_U8 u8sramtype, void *pData)
{
    MS_U16 Peaking_SRAM1_CAG[PEAKING_CAG_MAX_SRAM_SIZE] = {0};
    MS_U16 Peaking_SRAM2_CAG[PEAKING_CAG_MAX_SRAM_SIZE] = {0};
    MS_U16 Peaking_SRAM3_CAG[PEAKING_CAG_MAX_SRAM_SIZE] = {0};
    MS_U16 Peaking_SRAM4_CAG[PEAKING_CAG_MAX_SRAM_SIZE] = {0};

    MS_U16* u32Addr = (MS_U16*)pData;
    MS_U16 u16Index = 0 ;
    MS_U16 cont1=0, cont2=0, cont3=0, cont4=0;
    if(u8sramtype != SC_FILTER_SRAM_PEAKING_CAG)
    {
        return;
    }
//    printf("[%s]SRAM_IHC_TOTAL_COUNT=%u,SRAM2_IHC_OFFSET=%u,SRAM3_IHC_OFFSET=%u,SRAM4_IHC_OFFSET=%u\n",
//        __FUNCTION__,SRAM_IHC_TOTAL_COUNT,SRAM2_IHC_OFFSET,SRAM3_IHC_OFFSET,SRAM4_IHC_OFFSET);
    ...
    for (;u16Index < PEAKING_SRAM_CAG_TOTAL_COUNT ; u16Index++)
    {
        if (u16Index < PEAKING_SRAM2_CAG_OFFSET)
        {
            Peaking_SRAM1_CAG[cont1] = (u32Addr[u16Index]);
            //printf("@@SRAM1_CAG[%u]=%x\n",cont1,(u32Addr[u16Index]));
            cont1++;
        }
        else if (u16Index < PEAKING_SRAM3_CAG_OFFSET)
        {
            Peaking_SRAM2_CAG[cont2] = (u32Addr[u16Index]);
            //printf("@@SRAM2_CAG[%u]=%x\n",cont2,(u32Addr[u16Index]));
            cont2++;

        }
        else if (u16Index < PEAKING_SRAM4_CAG_OFFSET)
        {
            Peaking_SRAM3_CAG[cont3] = (u32Addr[u16Index]);
            //printf("@@SRAM3_CAG[%u]=%x\n",cont3,(u32Addr[u16Index]));
            cont3++;
        }
        else
        {
            Peaking_SRAM4_CAG[cont4] = (u32Addr[u16Index]);
            //printf("@@SRAM4_CAG[%u]=%x\n",cont4,(u32Addr[u16Index]));
            cont4++;
        }
    }

    _Hal_PQ_set_sram_peaking_cag_table(&Peaking_SRAM1_CAG[0], 0, PEAKING_SRAM1_CAG_COUNT);
    _Hal_PQ_set_sram_peaking_cag_table(&Peaking_SRAM2_CAG[0], 1, PEAKING_SRAM2_CAG_COUNT);
    _Hal_PQ_set_sram_peaking_cag_table(&Peaking_SRAM3_CAG[0], 2, PEAKING_SRAM3_CAG_COUNT);
    _Hal_PQ_set_sram_peaking_cag_table(&Peaking_SRAM4_CAG[0], 3, PEAKING_SRAM4_CAG_COUNT);
}
