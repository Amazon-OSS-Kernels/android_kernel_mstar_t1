...
MS_BOOL MDrv_TSensor_SetTemperatureMointerRange(MS_S16 s16UpBound_C, MS_S16 s16LowBound_C, TSENSOR_CB_FUNC pTSensor)
{
    ...
    MS_BOOL bRet = TRUE;
    bRet &= HAL_TSensorIRQDisable(); //set tempertaure low bound
    bRet &= HAL_TSensorSetUpBound(s16UpBound_C);
    bRet &= HAL_TSensorSetLowBound(s16LowBound_C);
    if(pTSensor)
    {
    ...
    {
        bRet &= HAL_TSensorIRQClear(); //clear interrupt in SAR bank
    }
    ISRIsAttched = TRUE;
    bRet &= MsOS_EnableInterrupt(E_INT_IRQ_KEYPAD); //enable interrupt in INTR bank
    bRet &= HAL_TSensorIRQEnable();
    return bRet;
}
