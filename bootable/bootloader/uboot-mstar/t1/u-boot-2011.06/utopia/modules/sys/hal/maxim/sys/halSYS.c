        ...
        if((g_SysIoProc.SysSwI2CReadBytes != 0) && (g_SysIoProc.SysSwI2CWriteBytes != 0))
        {
            ...
            else
            {
                while(1)
                {
                    ...
                    {
                        result = SysDvfsCpuPowerAdjustment(hMstarDvfsReadInfo.dwVoltage, hMstarDvfsReadInfo.dwCluster);
                    }
                    else if(hMstarDvfsReadInfo.dwVoltageType == CONFIG_DVFS_CORE_POWER)
                    {
                        result = SysDvfsCorePowerAdjustment(hMstarDvfsReadInfo.dwVoltage, hMstarDvfsReadInfo.dwCluster);
                    }
                    ...
                    hMstarDvfsWriteInfo.dwDataExchangeResult = result;
...
MS_BOOL SysDvfsCpuPowerAdjustment(MS_U32 dwCpuPowerVoltage, MS_U8 byCluster)
{
    ...
    if(hMstarDvfsInfo.dwPowerChipId == CONFIG_DVFS_CHIP_ID_PRADO)
    {
        ...
        if ((dwOriginalCpuPowerVoltage < 85) || (dwOriginalCpuPowerVoltage > 130))
        {
            dwOriginalCpuPowerVoltage = 130;
            printf("[DVFS][ERROR] error original cpu voltage \n");
        }
