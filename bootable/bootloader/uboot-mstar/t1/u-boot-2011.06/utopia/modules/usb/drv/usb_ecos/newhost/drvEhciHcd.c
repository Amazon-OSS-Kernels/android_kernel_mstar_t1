...
) 
{
  ...
  case Req_ClearPortFeature:
    ...
    case USB_PORT_FEAT_ENABLE:
      ...
      {
          ms_debug_err("Not only cantain one qHD in asyn. scheduler (%p)\n", pEhci->stAsync);
          ms_debug_err("async hw_next_qh %x\n", pEhci->stAsync->hw_next_qh);
          struct ehci_qh *pQh = pEhci->stAsync->qh_next.qh;
          do {
            ms_debug_err("qh %p\n============\n", pQh);
            ms_debug_err("[UM] status %d\n", pQh->qh_status);
            ms_debug_err("[UM] dummy qtd %x\n", pQh->pDummyQtd);
            ms_debug_err("[UM] qtd_list %x, %s\n", pQh->qtd_list, !ms_is_empty_list (&pQh->qtd_list) ? "there": "none");
            ms_debug_err("[UM] refcnt %d\n", pQh->tRefCnt);
            pQh = pQh->qh_next.qh;
          } while(pQh);
      }
      hcd_reg_writel (u32Temp & ~PORTSC_PE, (U32)&pEhci->op_regs->portsc [u16Index]);
      //¸Ë¸m³Q²¾°£ªº®É­Ô °±¤îscheduler
  ...
  case Req_GetPortStatus:
    ...
    u32Status = 0;
    u32Temp = hcd_reg_readl ((U32)&pEhci->op_regs->portsc [u16Index]);
    #if 0 // old flow to patch hub polling
...
)
{
  ...
  if (u32Token & QTD_STS_HALT) 
  {
    ...
    else if (u32Token & QTD_STS_XACTERR) 
    {
      ...
      if (QTD_CERR (u32Token))    /* ¿ù»~«¬ºAtimeout, bad crc, wrong PID */
    ...
    /* ¨S¦³error bit ¦ý¬O¦³halt, will stall */
    } 
    ...
    /* Stall ªí¥Ü»Ý­n°µerror recovery */
...
) 
{
  ...
  /* EP Control/bulk ¸g¹L TT ¤£»Ý­n scheduling */   
  if (pQh != 0) 
  {
    if (pQh->qh_status == QH_STS_IDLE)
...
//      || !spin_is_locked (&pEhci->tHcdLock)
  ...
  {    
    ...
    ms_debug_err("--[%s] HCD halt, end unlink directly\n", __FUNCTION__);
    return;
  }
...
int ehci_port_not_change_cnt = 0;
...
void ms_ehci_irq (struct usb_hcd *pHcd, struct stPtRegs *pRegs)
{
  ...
  {      
      ...
      #if 0 // debug
      if (ehci_port_not_change_cnt == 0)
          ms_debug_err("<irq> portsc[0x%4x], done[0] = 0x%x, cmd[%x], RW_lock[%x]\n", iPsc, (unsigned int)pEhci->u32ResetEnd[0], u32Cmd, USB_RW_lock);                    
      #endif
      
      /* new patch, root hub not run */
      if (iPsc == saved_ehci_state.iPsc &&
            pEhci->u32ResetEnd[0] == saved_ehci_state.reset_done &&
            u32Cmd == saved_ehci_state.usbcmd &&
            (u32Cmd & USBCMD_RUN) == 0)
        {
                ++ehci_port_not_change_cnt;
                if (ehci_port_not_change_cnt > 7)
                    isHalt = 1;
        }
      else
        ehci_port_not_change_cnt = 0;
      saved_ehci_state.iPsc = iPsc;
      saved_ehci_state.reset_done = pEhci->u32ResetEnd[0];
      saved_ehci_state.usbcmd = u32Cmd;
      if (isHalt)
      {
      ...
      /* Patch for hun+device hot plug frequency then lost disconnect event */
    ...
    switch (pQh->qh_status) {
        ...
        case QH_STS_COMPLETING:
            ...
            else if (pEhci->stReclaim)
            {
                ...
                diag_printf("[urb_dequeue] set QH_STS_UNLINK_WAIT\n");
            }
            else
                ms_start_unlink_async (pEhci, pQh); 
            break;
...
#if 0 // NUSED, not proper hardware patch
...
rescan:
  ...
  switch (pQh->qh_status) 
  {
      case QH_STS_UNLINK:    
        ...
        if (--numRescan)
        	goto rescan;
        diag_printf("[UM-DEP] rescan 7 times\n");
        //ms_ehci_stoprun_setting(HOST20_Disable, pEhci); // non-sense
        osapi_spin_lock_irqsave (&pEhci->tHcdLock, u32Flags);
      case QH_STS_IDLE:    
        if (ms_is_empty_list (&pQh->qtd_list)) 
        {
...
void ms_ehci_shutdown (struct usb_hcd *pHcd)
{
    struct ehci_hcd  *pEhci = hcd_to_ehci (pHcd);
...
void ms_qh_ehci_reinit(struct usb_hcd *pHcd, int not_fix)
{
    ...
    if( pEhci->stAsync->qh_next.qh != 0 )
    {
        ms_debug_err("[reinit] Not only cantain one qHD in asyn. scheduler (%p)\n", pEhci->stAsync);
