...
static int ms_hub_probe(struct usb_interface *pIntf, const struct usb_device_id *id)
{
    ...
    {
        diag_printf("hub nested too deep\n");
        return -MS_ERR_2BIG;
    }
    ...
    {
descriptor_error:
        diag_printf ("bad descriptor, ignoring hub\n");
        return -EIO;
    }
    ...
    if (!pHub) 
    {
        diag_printf("couldn't kmalloc hub struct\n");
        return -ENOMEM;
    }
          ...
          U16 portstatus, U16 portchange)
{
  ...
  for (i = 0; i < HUB_PROBE_TRIES; i++) {
    ...
    if (!pUdev) {
      diag_printf ("couldn't allocate usb_device\n");
      break;
    }
    ...
    //Clean qh buffer
    diag_printf("[HPCG] usb new device failed (%d)\n", i);
  ...
  if (i >= HUB_PROBE_TRIES) // 10 times reset retries
    pHcd->isBadDevice = TRUE;
...
int ms_usb_hub_init(void)
{
    ...
    {
        diag_printf("Unable to register USB hub driver\n");
        return -1;
    }
...
int ms_usb_physical_reset_device(struct usb_device *ms_dev)
{
    ...
    if (!parent) 
    {
        diag_printf("attempting to reset root hub!\n");
        return -EINVAL;
    }
    ...
    if (s32Err < 0) 
    {
        diag_printf("USB device not accepting new address (error=%d)\n", (int)s32Err);
        ms_hub_port_disable(parent, port);
        kfree(d_desc);
    ...
    if (s32Err < 0) 
    {
        diag_printf("USB device not getting device descriptor (error=%d)\n", (int)s32Err);
        kfree(d_desc);
        return s32Err;
    }
...
/**
     ...
     * @return          none
     */
extern struct st_saved_ehci_state saved_ehci_state;
static int ms_hub_events(struct s_gVar4UsbPort *pRootHub)
{
    ...
    while (1)
    {
        ...
        if (hub->error) 
        {
            ...
            if (ms_hub_reset(hub)) 
            {
                ms_usbhost_debug ("can't reset; disconnecting\n");
        ...
        if (ms_dev->parent == NULL)
            diag_printf("<saved irq> portsc[0x%4x], done[0] = 0x%x, cmd[%x]\n", saved_ehci_state.iPsc, saved_ehci_state.reset_done, saved_ehci_state.usbcmd);
        ...
        {
            diag_printf ("get_hub_status failed\n");
        }
        else 
        {
