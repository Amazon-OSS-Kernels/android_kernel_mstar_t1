...
#if 0 // move the code to ms_usb_hcd_flush_endpoint()
...
urb_rescan:
  ...
  )
  {
    ...
    diag_printf ("[UM-DEP]pre pUrb %p pipe %08x ep%d%s, ep type = %d, status = %d\n",
    ...
    if (tmp == -EINPROGRESS)
    {
      ms_debug_err("ms_hcd_endpoint_disable return ESHUTDOWN !!!\n");    
    ...
    if (tmp == -EINPROGRESS) 
    {
      ...
      ms_debug_err ("shutdown pUrb %p pipe %08x ep%d%s, ep type = %d\n",
        pUrb, tmp, usb_pipeendpoint (tmp),
...
urb_rescan:
    ...
    )
    {
        ...
        diag_printf ("[UM-FLUSH]%s pUrb %p pipe %08x ep%d%s, ep type = %d, status = %d\n",
        ...
        if (tmp == -EINPROGRESS)
        {
            ms_debug_err("<%s> return ESHUTDOWN !!!\n", __FUNCTION__);    
            pUrb->s32Status = -ESHUTDOWN;
        }
        osapi_spin_unlock (&pUrb->lock);
        ...
        if (tmp == -EINPROGRESS) 
        {
            tmp = pUrb->u32Pipe;
            ms_unlink1 (pHcd, pUrb);
            ms_debug_err ("<%s> shutdown pUrb %p pipe %08x ep%d%s, ep type = %d\n",
                __FUNCTION__, pUrb, tmp, usb_pipeendpoint (tmp),
                (tmp & USB_DIR_IN) ? "in" : "out", usb_pipetype (tmp));
        }
        usb_put_urb (pUrb);
