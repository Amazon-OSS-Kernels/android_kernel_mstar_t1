...
VDEC_EX_Result _MApi_VDEC_EX_V2_GetNextDispFrameInfoExt(VDEC_StreamId *pStreamId, void *param)
{
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        ...
        case E_VDEC_EX_DECODER_HVD:
        {
            MS_U32 *pVerCtl = (MS_U32 *)param;

            if((pVerCtl[0] == 0)
            && (pVerCtl[1] == sizeof(VDEC_EX_FrameInfoExt_version0)))
            {
            ...
            && (pVerCtl[1] == sizeof(VDEC_EX_FrameInfoExt)))
            {
                ...
                pFrmInfoEx->maxLuminance             = info.u32MaxLuminance;
                pFrmInfoEx->minLuminance             = info.u32MinLuminance;
                pFrmInfoEx->primaries[0][0]          = info.u16Primaries[0][0];
                pFrmInfoEx->primaries[0][1]          = info.u16Primaries[0][1];
                pFrmInfoEx->primaries[1][0]          = info.u16Primaries[1][0];
                pFrmInfoEx->primaries[1][1]          = info.u16Primaries[1][1];
                pFrmInfoEx->primaries[2][0]          = info.u16Primaries[2][0];
                pFrmInfoEx->primaries[2][1]          = info.u16Primaries[2][1];
                pFrmInfoEx->whitePoint[0]            = info.u16WhitePoint[0];
                pFrmInfoEx->whitePoint[1]            = info.u16WhitePoint[1];
                pFrmInfoEx->Frm_Info_Ext_avail       = info.u8Frm_Info_Ext_avail;  //bit1: SEI_Enabled,  bit0=colur_description_present_flag
                pFrmInfoEx->colour_primaries         = info.u8Colour_primaries;
                pFrmInfoEx->transfer_characteristics = info.u8Transfer_characteristics;
                pFrmInfoEx->matrix_coefficients      = info.u8Matrix_coefficients;

                /*
                printf("VUI info\n");
                printf("%d,%d,%d,%d\n",info.u8Frm_Info_Ext_avail,info.u8Colour_primaries,info.u8Transfer_characteristics,info.u8Matrix_coefficients);
                printf("Color info\n");
                printf("%d,%d,%d,%d\n",info.u32MaxLuminance,info.u32MinLuminance,info.u16Primaries[0][0],info.u16Primaries[0][1]);
                printf("%d,%d,%d,%d\n",info.u16Primaries[1][0],info.u16Primaries[1][1],info.u16Primaries[2][0],info.u16Primaries[2][1]);
                printf("%d,%d\n",info.u16WhitePoint[0],info.u16WhitePoint[1]);
                printf("end\n");
                */
                eRet = E_VDEC_EX_OK;
            }
#elif defined(VDEC_CAP_HEVC_HDR_V2)
            else if((pVerCtl[0] == 2)
            && (pVerCtl[1] == sizeof(VDEC_EX_FrameInfoExt_v2)))
            {
                ...
                pFrmInfoEx2->sFrameInfoExt.sFrameInfo.u32LumaAddr      = info.u32LumaAddr;
                pFrmInfoEx2->sFrameInfoExt.sFrameInfo.u32ChromaAddr    = info.u32ChromaAddr;
                pFrmInfoEx2->sFrameInfoExt.sFrameInfo.u32TimeStamp     = info.u32TimeStamp;
                pFrmInfoEx2->sFrameInfoExt.sFrameInfo.u32ID_L          = info.u32ID_L;
                pFrmInfoEx2->sFrameInfoExt.sFrameInfo.u32ID_H          = info.u32ID_H;
                pFrmInfoEx2->sFrameInfoExt.sFrameInfo.u16Pitch         = info.u16Pitch;
                pFrmInfoEx2->sFrameInfoExt.sFrameInfo.u16Width         = info.u16Width;
                pFrmInfoEx2->sFrameInfoExt.sFrameInfo.u16Height        = info.u16Height;
                pFrmInfoEx2->sFrameInfoExt.sFrameInfo.eFrameType       = info.eFrmType;
                pFrmInfoEx2->sFrameInfoExt.sFrameInfo.eFieldType       = info.eFieldType;

                pFrmInfoEx2->sFrameInfoExt.u32LumaAddr_2bit            = info.u32LumaAddr_2bit;
                pFrmInfoEx2->sFrameInfoExt.u32ChromaAddr_2bit          = info.u32ChromaAddr_2bit;
                pFrmInfoEx2->sFrameInfoExt.u32LumaAddrI                = info.u32LumaAddrI;
                pFrmInfoEx2->sFrameInfoExt.u32LumaAddrI_2bit           = info.u32LumaAddrI_2bit;
                pFrmInfoEx2->sFrameInfoExt.u32ChromaAddrI              = info.u32ChromaAddrI;
                pFrmInfoEx2->sFrameInfoExt.u32ChromaAddrI_2bit         = info.u32ChromaAddrI_2bit;
                pFrmInfoEx2->sFrameInfoExt.u32MFCodecInfo              = info.u32MFCodecInfo;
                pFrmInfoEx2->sFrameInfoExt.u32LumaMFCbitlen            = info.u32LumaMFCbitlen;
                pFrmInfoEx2->sFrameInfoExt.u32ChromaMFCbitlen          = info.u32ChromaMFCbitlen;
                pFrmInfoEx2->sFrameInfoExt.u16Pitch_2bit               = info.u16Pitch_2bit;
                pFrmInfoEx2->sFrameInfoExt.u8LumaBitdepth              = info.u8LumaBitdepth;
                pFrmInfoEx2->sFrameInfoExt.u8ChromaBitdepth            = info.u8ChromaBitdepth;
    ...
    while (1)
    {
        ...
        switch(u8Index)
        {
            case E_VDEC_EX_REE_TO_TEE_MBX_MSG_FW_LoadCode:
                PRINTF("VDEC receive load f/w code in R2,%d\n");
