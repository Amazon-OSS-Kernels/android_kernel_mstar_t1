...
static MS_U32 _VDEC_Map2HVDErrCode(MS_U32 u32ErrCode)
{
    ...
    switch (u32ErrCode)
    {
        ...
        case E_HVD_EX_ERRCODE_DIVX_PLUS_UNSUPPORTED:
            u32Ret = E_VDEC_EX_ERR_CODE_DIVX_PLUS_UNSUPPORTED;
            break;
        default:
            break;
    }
...
#endif
        case E_VDEC_EX_USER_CMD_SET_AR_CHANGE_MODE          : eEXUserCmd = E_VDEC_EX_V2_USER_CMD_SET_AR_CHANGE_MODE ;break;
    ...
    VDEC_EX_ALLOCATOR_BUFFER_TYPE eBufType, VDEC_EX_NDecBufRangeParam *pstBufRgn, VDEC_EX_NDecBufferProperty *pstProperty)
{
    ...
    else
    {
        ...
        {
            ALLOCATOR_PRINTF_ERR("[ALLOC][ERR][%s][%d] CMAInit failed, Not Support CMA\n", __FUNCTION__, __LINE__);
            goto exit;
        }
#endif
    }
    ...
    if(pstProperty->bNDecode == TRUE)
    {
        ...
        if(pstProperty->bMonopoly == FALSE)
        {
            ...
            if(eBufType == E_VDEC_EX_ALLOCATOR_BUFFER_FB1)
            {
                ...
                if(pstAllocatorSub->szTotalSize == pstAllocatorSub->szUnusedSize)
                {
                    // if first stream
                    stCriterion.szSize = MEGA(49);
                }
                else
                {
                    stCriterion.szSize = MEGA(24);
                }
            }
...
static void _VDEC_EX_InitLocalVar(MS_U32 u8Idx)
{
    ...
    //init TspMinData value
    pVdecExContext->u32MVDMinTspDataSize[u8Idx] = 0;//byte
...
static VDEC_EX_Result _VDEC_EX_SetMinTspDataSize(VDEC_StreamId *pStreamId, MS_U32 u32Size)
{
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        case E_VDEC_EX_DECODER_MVD:
            ...
            {
               pVdecExContext->u32MVDMinTspDataSize[u8Idx] = u32Size;
               eRet = E_VDEC_EX_OK;
            }
            break;
        case E_VDEC_EX_DECODER_HVD:
...
static VDEC_EX_Result _VDEC_EX_GetMinTspDataSize(VDEC_StreamId *pStreamId, MS_U32 *param)
{
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        ...
        case E_VDEC_EX_DECODER_MVD:
            ...
            if(pVdecExContext->u32MVDMinTspDataSize[u8Idx] > 0)//set value
            {
                *param = pVdecExContext->u32MVDMinTspDataSize[u8Idx];
            }
            else
            {
                *param = 0x16000;//byte
                if(MApi_VDEC_EX_GetDispInfo(pStreamId, &DisplayInfo) == TRUE)
                {
                    if(DisplayInfo.u16HorSize >= 1920 && DisplayInfo.u16VerSize >= 1080)
                    {
                        *param = 0x69500;//byte
                    }
                }
...
#define SIZE_DSINFOBUF 0x100
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        case E_VDEC_EX_DECODER_MVD:
        {
            ...
            stMVDExternalDSBuf.u32DSBufSize = pExternalDSBuf->u32DSBufSize - SIZE_DSINFOBUF;
        ...
        case E_VDEC_EX_DECODER_HVD:
        {
            ...
            stExternalDSBuf.u32DSBufSize = pExternalDSBuf->u32DSBufSize - SIZE_DSINFOBUF;
...
VDEC_EX_Result _MApi_VDEC_EX_V2_Flush(VDEC_StreamId *pStreamId, VDEC_EX_FreezePicSelect eFreezePic)
{
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        case E_VDEC_EX_DECODER_MVD:
        {
            if(eFreezePic == E_VDEC_EX_FAST_FLUSH)
                _MVD_RET_TO_VDEC_RESULT(eRet, MDrv_MVD_FlushQueue(u32Id,TRUE));
            else
                _MVD_RET_TO_VDEC_RESULT(eRet, MDrv_MVD_FlushQueue(u32Id,FALSE));
            break;
        }
        case E_VDEC_EX_DECODER_HVD:
        {
...
VDEC_EX_Result _MApi_VDEC_EX_V2_SetARChangeMode(VDEC_StreamId *pStreamId,MS_U32 u32ARChangeMode)
{
    VDEC_EX_Result eRet = E_VDEC_EX_FAIL;
    MS_U8  u8Idx = _VDEC_GET_IDX(pStreamId->u32Id);
    MS_U32 u32Id = _VDEC_GET_DRV_ID(pStreamId->u32Id);
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        ...
        case E_VDEC_EX_DECODER_HVD:
            ...
            _HVD_RET_TO_VDEC_RESULT(eRet,MDrv_HVD_EX_SetARChangeMode(u32Id,u32ARChangeMode));
            break;
        case E_VDEC_EX_DECODER_MJPEG:
            eRet = E_VDEC_EX_RET_UNSUPPORTED;
            break;
        ...
        switch (pVdecExContext->_Attr[u8Idx].eDecoder)
        {
            case E_VDEC_EX_DECODER_MVD:
            {
                ...
                if (E_MVD_ERR_UNKNOWN == errCode)
                {
                    ...
                    if ( (bIsSupportDivx == FALSE) && (MDrv_MVD_GetDivxVer(u32Id) != 0) )
                    {
...
VDEC_EX_Result _MApi_VDEC_EX_V2_GetNextDispFrameInfoExt(VDEC_StreamId *pStreamId, void *param)
{
...
#endif
#if defined(VDEC_CAP_FRAME_INFO_EXT_V7)
     || (version == 7 && size == sizeof(VDEC_EX_FrameInfoExt_v7))
#endif
    )
    {
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        case E_VDEC_EX_DECODER_MVD:
        {
            ...
            if(version >= 2)
            {
                ...
                if(version == 2)
                    ...
                    memset(param, 0, sizeof(VDEC_EX_FrameInfoExt_v7));
#endif
        ...
        case E_VDEC_EX_DECODER_HVD:
        {
            ...
            if (version >= 7)
            {
                VDEC_EX_FrameInfoExt_v7 *pFrmInfoEx7 = (VDEC_EX_FrameInfoExt_v7 *)param;

                pFrmInfoEx7->u16MaxContentLightLevel    = info.u16MaxContentLightLevel;
                pFrmInfoEx7->u16MaxPicAverageLightLevel = info.u16MaxPicAverageLightLevel;
...
//    MS_U32 u32Id = _VDEC_GET_DRV_ID(pStreamId->u32Id);
    ...
    char task_name[24];
    snprintf(task_name,sizeof(task_name),"VdecPowerTask_%d",u8Idx);
    ...
    else if (u16PowerState == E_POWER_RESUME)
    {
        if (pVdecExContext->_prev_u16PowerState == E_POWER_SUSPEND)
        {
            ...
            if(pVdecExContext->u32ResumeStream[0] == TRUE)
            {
                ...
                MApi_VDEC_EX_V2_GetFreeStream(&(pVdecExContext->streamId[0]),
                                  sizeof(VDEC_StreamId),
                                  E_VDEC_EX_MAIN_STREAM,
                                  pVdecExContext->eCodecType[0]);
                MDrv_HVD_EX_ForceSwRst();
                MApi_VDEC_EX_V2_Init(&(pVdecExContext->streamId[0]),&(pVdecExContext->_Attr[0].vdecExInitParam));

                // to do, set control
                pVdecExContext->store_set_cmd_read_pointer[_VDEC_GET_IDX((pVdecExContext->streamId[0]).u32Id)] = 0;
                u8Idx = _VDEC_GET_IDX((pVdecExContext->streamId[0]).u32Id);
                for(i=0;i<pVdecExContext->store_set_cmd_write_pointer[u8Idx];i++)
                {
                    MApi_VDEC_EX_V2_SetControl(&(pVdecExContext->streamId[0]),
                ...
                _MApi_VDEC_EX_V2_Play(&(pVdecExContext->streamId[0]));
                pVdecExContext->u32ResumeStream[0] = FALSE;
                pVdecExContext->_power_state[0] = E_VDEC_EX_POWER_RESUME_DONE;
                _MApi_VDEC_EX_Power_Proc(&(pVdecExContext->streamId[0]));
            }

            if(pVdecExContext->u32ResumeStream[1] == TRUE)
            {
                pVdecExContext->_power_state[1] = E_VDEC_EX_POWER_RESUME_ING;
                MApi_VDEC_EX_V2_GetFreeStream(&(pVdecExContext->streamId[1]),
                                  sizeof(VDEC_StreamId),
                                  E_VDEC_EX_SUB_STREAM,
                                  pVdecExContext->eCodecType[1]);
                MDrv_HVD_EX_ForceSwRst();
                MApi_VDEC_EX_V2_Init(&(pVdecExContext->streamId[1]),&(pVdecExContext->_Attr[1].vdecExInitParam));

                // to do, set control
                pVdecExContext->store_set_cmd_read_pointer[_VDEC_GET_IDX((pVdecExContext->streamId[1]).u32Id)] = 0;
                u8Idx = _VDEC_GET_IDX((pVdecExContext->streamId[1]).u32Id);
                for(i=0;i<pVdecExContext->store_set_cmd_write_pointer[u8Idx];i++)
                {
                    MApi_VDEC_EX_V2_SetControl(&(pVdecExContext->streamId[1]),
                                               pVdecExContext->store_set_cmd[pVdecExContext->store_set_cmd_read_pointer[u8Idx]].cmd,
                                               (void**)(&param),
                                               TRUE);
                }
                ...
                _MApi_VDEC_EX_V2_Play(&(pVdecExContext->streamId[1]));
                pVdecExContext->u32ResumeStream[1] = FALSE;
                pVdecExContext->_power_state[1] = E_VDEC_EX_POWER_RESUME_DONE;
                _MApi_VDEC_EX_Power_Proc(&(pVdecExContext->streamId[1]));
            }
//            pVdecExContext->_prev_u16PowerState = u16PowerState;
            ret = E_VDEC_EX_OK;
         }
...
VDEC_EX_Result MApi_VDEC_EX_V2_SetControl(VDEC_StreamId* pStreamId, VDEC_EX_V2_User_Cmd cmd_id, void** param,MS_BOOL IsResume)
{
    ...
    switch (cmd_id)
    {
        ...
        case E_VDEC_EX_V2_USER_CMD_SET_CODEC_CONFIG:
            ...
            break;
        case E_VDEC_EX_V2_USER_CMD_SET_AR_CHANGE_MODE:
            ret = _MApi_VDEC_EX_V2_SetARChangeMode(pStreamId,(*((MS_U32*)(param[0]))));
            break;
        default:
...
MS_BOOL _VDEC_EX_OPTEE_SET_SecureBuffer(VDEC_EX_OPTEE_SecureBuffer *buffer)
{
    ...
    printf("[VDEC][OPTEE]%s,stream_id=%d\n",__FUNCTION__,stream_ID);
...
MS_U32 MDrv_VDEC_GetResourceByPipeID(MS_U64 u64PipeID, MS_U32*  U32MaxNRetResNum, VDEC_EX_RESOURCE* pResouce)
{
    ...
    {
        printf("[VDEC][OPTEE]Resource Number too less=%d\n",(int)*U32MaxNRetResNum);
        return UTOPIA_STATUS_FAIL;
    }
...
MS_U32 MDrv_VDEC_ConfigPipe(MS_U64 u64PipeID, MS_U32 u32SecureDMA, MS_U32 u32OperationMode)
{
    ...
    Secure_Status_Sub=0;
    printf("[VDEC][OPTEE]%s,stream_id=%d\n",__FUNCTION__,stream_ID);
    printf("[VDEC][OPTEE]secureDMA=%d,opeation=%d\n",u32SecureDMA,u32OperationMode);
...
MS_U32 MDrv_VDEC_CheckPipe(MS_U64 u64PipeID, MS_U32 u32SecureDMA, MS_U32 u32OperationMode)
{
    ...
    {
        printf("[VDEC][OPTEE]stream0 check pipe fail\n");
        return UTOPIA_STATUS_FAIL;
    }
    else if(stream_ID==1 && (!Secure_Status_Sub))
    {
        printf("[VDEC][OPTEE]stream1 check pipe fail\n");
        return UTOPIA_STATUS_FAIL;
    }
