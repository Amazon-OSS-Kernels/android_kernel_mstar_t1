...
#ifdef STELLAR
#define DIP_STACK_SIZE 4096
typedef struct
{
    MS_BOOL        bInstall[MAX_DIP_WINDOW];
    MS_S32         s32TaskId[MAX_DIP_WINDOW];
    InterruptCb dip_callback[MAX_DIP_WINDOW];
}DIP_CbData;

DIP_CbData _stDIPCB = {.bInstall={0,0,0},.s32TaskId = {-1,-1,-1}};
MS_U8  _u8DIP_CallbackStack[MAX_DIP_WINDOW][DIP_STACK_SIZE];
static char* DipTaskName[MAX_DIP_WINDOW]={"DIP_WINDOW_TASK","DWIN0_WINDOW_TASK","DWIN1_WINDOW_TASK"};
#endif
...
//-------------------------------------------------------------------------------------------------
#ifdef STELLAR
void _DIP_Callback_Proc(SCALER_DIP_WIN eWindow)
{
    if(eWindow < MAX_DIP_WINDOW)
    {
        DIP_INTER_STUS* pIntrStus = (DIP_INTER_STUS*)utopia_malloc(sizeof(DIP_INTER_STUS));
        while(1)
        {
            if(pInstantDip[eWindow] && pIntrStus)
            {
                pIntrStus->IntrStautus = 0;
                pIntrStus->eFlag = DIP_INTR_QUERY;
                pIntrStus->eWindow = eWindow;
                if(UtopiaIoctl(pInstantDip[eWindow],MDrv_CMD_DIP_INTERRUPT,(void*)pIntrStus) == UTOPIA_STATUS_SUCCESS)
                {
                    _stDIPCB.dip_callback[eWindow](pIntrStus->IntrStautus);
                }
            }
            else
            {
...
void _DIP_Callback_task(InterruptCb pIntCb,SCALER_DIP_WIN eWindow)
{
    if(eWindow < MAX_DIP_WINDOW)
    {
        if(_stDIPCB.bInstall[eWindow] == FALSE && _stDIPCB.s32TaskId[eWindow] == -1)
        {
            _stDIPCB.s32TaskId[eWindow] = MsOS_CreateTask((TaskEntry)_DIP_Callback_Proc,
                                              (MS_U32)(eWindow),
                                              E_TASK_PRI_SYS,
                                              TRUE,
                                              (void*)_u8DIP_CallbackStack[eWindow],
                                              DIP_STACK_SIZE,
                                              DipTaskName[eWindow]);

            if (_stDIPCB.s32TaskId[eWindow] >= 0)
            {
                _stDIPCB.bInstall[eWindow] = TRUE;
            }
        }
        _stDIPCB.dip_callback[eWindow] = pIntCb;
    }
}
#endif
...
E_APIXC_ReturnValue MApi_XC_DIP_InterruptAttach(InterruptCb pIntCb,SCALER_DIP_WIN eWindow)
{
...
#if (!defined(DIP_UTOPIA2K) || defined(STELLAR))
    ...
    utopia_free(pIntrCb);
    #ifdef STELLAR
    _DIP_Callback_task(pIntCb,eWindow);
    #endif
#else
...
//******************************************************************************
/// API for MApi_XC_DIP_SetConfig
/// @return E_APIXC_RET_OK - Success
//******************************************************************************
E_APIXC_ReturnValue MApi_XC_DIP_SetConfig(EN_DIP_CONFIG_TYPE enType, void *plist,SCALER_DIP_WIN eWindow)
{
#ifndef DISABLE_DIP_FUNCTION
    ...
    pSetConfig = (DIP_SETCONFIG_PARAM*)utopia_malloc(sizeof(DIP_SETCONFIG_PARAM));
    if (pSetConfig == NULL)
    {
        XC_ERR("%s alloc fail\n",__FUNCTION__);
        return E_APIXC_RET_FAIL;
    ...
    pSetConfig->cfg_type = enType;
    pSetConfig->pCfg = plist;
    pSetConfig->eWindow = eWindow;

    if(UtopiaIoctl(pInstantDip[eWindow],MDrv_CMD_DIP_SET_CONFIG,(void*)pSetConfig) != UTOPIA_STATUS_SUCCESS)
    {
        XC_ERR("Ioctl MDrv_CMD_DIP_SET_CONFIG fail\n");
		utopia_free(pSetConfig);
        return E_APIXC_RET_FAIL;
    }
    utopia_free(pSetConfig);
