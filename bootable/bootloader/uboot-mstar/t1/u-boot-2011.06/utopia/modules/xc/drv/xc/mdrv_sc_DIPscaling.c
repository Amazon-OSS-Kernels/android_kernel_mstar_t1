...
#ifdef STELLAR
#define DIP_EVENT_ISR_EVENT_CHANGE   0x40000000
#define DIP_U32_MAX                  0xffffffffUL
#define DIP_EVENT_BASE           0x1000
MS_S32 _s32DIPEventId[MAX_DIP_WINDOW] = {-1,-1,-1};
MS_U32 _u32DIPWaitEventFlag[MAX_DIP_WINDOW] = {DIP_EVENT_ISR_EVENT_CHANGE,DIP_EVENT_ISR_EVENT_CHANGE,DIP_EVENT_ISR_EVENT_CHANGE};
MS_U16 _u16DIPISRStatus[MAX_DIP_WINDOW] = {0,0,0};
#endif
...

{
    ...
    if((fmt == XC_DWIN_DATA_FMT_YUV420_H265) || (fmt == XC_DWIN_DATA_FMT_YUV420_H265_10BITS))
    {
        u16NewWidth = ALIGN_CHECK(u16Width,DIP_H265_TILE_BLOCK_H);
        u16NewLineOft = ALIGN_CHECK(u16LineOft,DIP_H265_TILE_BLOCK_H);
    }
    else
    {
    ...
    if( u64TotalLen > (u64BufEnd - u64BufStart))
    {
        ...
        DIP_D_ERR("\033[31m[%s:%d] Buffer size is not correct,need = %lx \033[0m\n",__FUNCTION__,__LINE__,u64TotalLen);
#else
        DIP_D_ERR("\033[31m[%s:%d] Buffer size is not correct,need = %tx \033[0m\n",__FUNCTION__,__LINE__,(ptrdiff_t)u64TotalLen);
#endif
        //Recalculate the hight, total length, offset.

        u64TotalLen = u64BufEnd - u64BufStart;
        u64_result = u64TotalLen;
        u64Offset = do_div(u64_result, (u8BufCnt*u16Bpp));

        if(bYUV420 == TRUE)
        {
            u64Offset = u64Offset*u64YLen/(u64YLen + u64CLen);
            u16NewHeight = do_div(u64Offset, u16NewLineOft);
            u64_result = (MS_PHY) u16NewHeight;
            u16NewHeight &= ~(DIP_HVD_TILE_BLOCK_V-1);
            u64YLen = (MS_PHY)u16NewLineOft*(MS_PHY)u16NewHeight;

            u64_result = u16NewLineOft*(u64_result & ~((DIP_HVD_TILE_BLOCK_V<<1) -1));
            do_div(u64_result,2);
            u64CLen = u64_result;
            u64Offset = u64YLen+u64CLen;
        }
...
{
    MS_U8 u8YBufMIUSel = 0,u8CBufMIUSel = 0;
    MS_U8 u8MIUSelTMP = 0;
    ...
    _phy_to_miu_offset(u8YBufMIUSel, pstDIPRProperty->u32YBufAddr, pstDIPRProperty->u32YBufAddr);
    _phy_to_miu_offset(u8CBufMIUSel, pstDIPRProperty->u32CBufAddr, pstDIPRProperty->u32CBufAddr);
    ...
    HAL_XC_DIP_SetDIPRMiuSel(pInstance, u8YBufMIUSel, u8CBufMIUSel,eWindow);
...
void MDrv_XC_DIP_SetDIPRProperty_EX(void* pInstance,ST_XC_DIPR_PROPERTY_EX *pstDIPRProperty_ex, SCALER_DIP_WIN eWindow)
{
    ...
    //DIPr = DIPw MIU selection, so change DIPW MIU selection
    _phy_to_miu_offset(u8YBufMIUSel, pstDIPRProperty_ex->stDIPRProperty.u32YBufAddr, pstDIPRProperty_ex->stDIPRProperty.u32YBufAddr);
    _phy_to_miu_offset(u8CBufMIUSel, pstDIPRProperty_ex->stDIPRProperty.u32CBufAddr, pstDIPRProperty_ex->stDIPRProperty.u32CBufAddr);
...
#ifdef STELLAR
void DIP_WINDOW_SetEvent(InterruptNum eIntNum)
{
    if(_s32DIPEventId[DIP_WINDOW] != -1)
    {
        MsOS_SetEvent(_s32DIPEventId[DIP_WINDOW], (DIP_EVENT_ISR_EVENT_CHANGE |(DIP_EVENT_BASE<<DIP_WINDOW)));
        _u16DIPISRStatus[DIP_WINDOW] = (eIntNum);
    }
}

void DWIN0_WINDOW_SetEvent(InterruptNum eIntNum)
{
    if(_s32DIPEventId[DWIN0_WINDOW] != -1)
    {
        MsOS_SetEvent(_s32DIPEventId[DWIN0_WINDOW], (DIP_EVENT_ISR_EVENT_CHANGE |(DIP_EVENT_BASE<<DWIN0_WINDOW)));
        _u16DIPISRStatus[DWIN0_WINDOW] = (eIntNum);
    }
}

void DWIN1_WINDOW_SetEvent(InterruptNum eIntNum)
{
    if(_s32DIPEventId[DWIN1_WINDOW] != -1)
    {
        MsOS_SetEvent(_s32DIPEventId[DWIN1_WINDOW], (DIP_EVENT_ISR_EVENT_CHANGE |(DIP_EVENT_BASE<<DWIN1_WINDOW)));
        _u16DIPISRStatus[DWIN1_WINDOW] = (eIntNum);
    }
}
#endif
...
void MDrv_XC_DIP_InterruptAttach(void* pInstance,InterruptCb pIntCb,SCALER_DIP_WIN eWindow)
{
	_XC_ENTRY(pInstance);
#ifdef STELLAR
    InterruptCb dip_cb = pIntCb;
    if(dip_cb == NULL)
    {
        char pu8DIPEvent[40];
        if(eWindow == DIP_WINDOW)
        {
            strncpy(pu8DIPEvent, "DIP_WINDOW_Event", sizeof(pu8DIPEvent));
            dip_cb = DIP_WINDOW_SetEvent;
        }
        else if(eWindow == DWIN0_WINDOW)
        {
            strncpy(pu8DIPEvent, "DWIN0_WINDOW_Event", sizeof(pu8DIPEvent));
            dip_cb = DWIN0_WINDOW_SetEvent;
        }
        else if(eWindow == DWIN1_WINDOW)
        {
            strncpy(pu8DIPEvent, "DWIN1_WINDOW_Event", sizeof(pu8DIPEvent));
            dip_cb = DWIN1_WINDOW_SetEvent;
        }
        printf("%s  create event %s and register cb:%p\n",__FUNCTION__,pu8DIPEvent,dip_cb);
        if(_s32DIPEventId[eWindow] == -1)
        {
            _s32DIPEventId[eWindow] = MsOS_CreateEventGroup(pu8DIPEvent);
        }
        if(_s32DIPEventId[eWindow] >= 0)
        {
            printf("%s  create event id:%d\n",__FUNCTION__,_s32DIPEventId[eWindow]);
            MsOS_ClearEvent(_s32DIPEventId[eWindow], DIP_U32_MAX);
            _u32DIPWaitEventFlag[eWindow] = (DIP_EVENT_ISR_EVENT_CHANGE |(DIP_EVENT_BASE<<eWindow));
            _u16DIPISRStatus[eWindow] = 0;
        }
    }
    HAL_XC_DIP_InterruptAttach(pInstance, dip_cb,eWindow);
#else
...
void MDrv_XC_DIP_InterruptDetach(void* pInstance,SCALER_DIP_WIN eWindow)
{
    ...
    if(_s32DIPEventId[eWindow] >= 0)
    {
        printf("%s  delete event id:%d\n",__FUNCTION__,_s32DIPEventId[eWindow]);
        MsOS_ClearEvent(_s32DIPEventId[eWindow], DIP_U32_MAX);
        _u32DIPWaitEventFlag[eWindow] = DIP_U32_MAX;
    }
    #endif
...
{
    MS_U32 u32DipEventFlag = 0;
    MS_U16 u16Status = 0;
    if(_s32DIPEventId[eWindow] >= 0)
    {
        MsOS_WaitEvent_Interrupt(_s32DIPEventId[eWindow],
            _u32DIPWaitEventFlag[eWindow],
            &u32DipEventFlag,
            E_OR_CLEAR,
            MSOS_WAIT_FOREVER);
        u16Status = _u16DIPISRStatus[eWindow];
    }
    return u16Status;
}
#endif
...
{
    _XC_ENTRY(pInstance);
    HAL_XC_DIP_IMI_ENABLE(pInstance, bEnalbe, eWindow);
    _XC_RETURN(pInstance);
}
