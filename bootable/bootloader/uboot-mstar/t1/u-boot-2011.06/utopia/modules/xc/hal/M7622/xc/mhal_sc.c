...
void Hal_SC_init_riu_base(MS_VIRT u32riu_base, MS_VIRT u32PMriu_base, MS_VIRT u32EXTriu_base)
{
    ...
    #ifdef SUPPORT_XC_EXT_REG
    _XC_EXT_RIU_BASE = u32EXTriu_base;
    #endif
}
...
#ifndef DONT_USE_CMA
    if (pXCResourcePrivate->sthal_SC.bIsConfigCMA == TRUE)
    {
...
void Hal_SC_3D_CopyDispWinToSTGEN(void *pInstance, SCALER_WIN eWindow)
{
    ...
    if(eWindow == MAIN_WINDOW)
    {
        ...
        u16VideoHeight = u16End - u16Start + 1;
        ...
        if(MHal_FRC_IsEnableFRC3D(pInstance, MAIN_WINDOW) && (pXCResourcePrivate->stdrvXC_3D._bLAToTB) && IS_OUTPUT_4K2K_60HZ_PANEL())
        {
            ...
            {
                u16End = (u16Start -pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart)/2
                    + pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart + (u16End-u16Start +1)/2 -1;
                u16Start = (u16Start -pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart)/2
            ...
            else
            {
                u16Start = (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height
                    - (u16Start -pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart)
                    - u16VideoHeight) / 2
                    + u16Start + u16VideoHeight / 2;

                u16End = u16Start + u16VideoHeight/2 -1;
            }
        }
...
static void _Hal_SC_fill_main_sw_db_burst(void *pInstance, P_SC_SWDB_INFO pDBreg)
{
    ...
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_01_L, (MS_U16)(pDBreg->u32H_PostScalingIniFactor>>00), 0xFFFF);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_02_L, (MS_U16)(pDBreg->u32H_PostScalingIniFactor>>16), 0xFFFF);
    //H post-scaling Sub1 Mode
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_08_L, (pDBreg->bIsHScalingUp == TRUE ? BIT(9) : 0), BIT(9));
    //V post-scaling Phase
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_03_L, (MS_U16)(pDBreg->u32V_PostScalingIniFactor>>00), 0xFFFF);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_04_L, (MS_U16)(pDBreg->u32V_PostScalingIniFactor>>16), 0xFFFF);
    //V post-scaling Sub1 Mode
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_0A_L, (pDBreg->bIsVScalingUp == TRUE ? BIT(9) : 0), BIT(9));
#endif
    ...
    if (gSrcInfo[MAIN_WINDOW].bR_FBL)
    {
        ...
        if (gSrcInfo[MAIN_WINDOW].bInterlace)
        {
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK79_09_L, (MS_U16)(gSrcInfo[MAIN_WINDOW].stCropWin.height>>01), 0x0FFF);
        }
        else
        {
    ...
    else
    {
        ...
        if (gSrcInfo[MAIN_WINDOW].bInterlace)
        {
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK79_09_L, (MS_U16)(pDBreg->u16V_CapSize>>01), 0x0FFF);
        }
        else
        {
...
#if DLC_CaptureRange_SIX_OVER_EIGHT
    ////DLC Capture SIX OVER EIGHT Range
    if(gSrcInfo[MAIN_WINDOW].bInterlace)
    {
        MDrv_XC_MLoad_WriteCmd(pInstance,REG_SC_BK5C_06_L,((gSrcInfo[MAIN_WINDOW].stCapWin.height/2)*1/8 -1),0x0FFF);
        MDrv_XC_MLoad_WriteCmd(pInstance,REG_SC_BK5C_07_L,((gSrcInfo[MAIN_WINDOW].stCapWin.height/2)*7/8 -1),0x0FFF);
    }
    else
    {
...
#else
    ////DLC Capture FULL Range
    if(gSrcInfo[MAIN_WINDOW].bInterlace)
    {
        MDrv_XC_MLoad_WriteCmd(pInstance,REG_SC_BK5C_06_L, 0, 0x0FFF);
        MDrv_XC_MLoad_WriteCmd(pInstance,REG_SC_BK5C_07_L,((gSrcInfo[MAIN_WINDOW].stCapWin.height/2)-2),0x0FFF);
    }
    else
    {
        MDrv_XC_MLoad_WriteCmd(pInstance,REG_SC_BK5C_06_L, 0, 0x0FFF);
        MDrv_XC_MLoad_WriteCmd(pInstance,REG_SC_BK5C_07_L,(gSrcInfo[MAIN_WINDOW].stCapWin.height-2),0x0FFF);
    }
#endif
    ...
    if (psXCInstPri->u32DeviceID == 1)
    {
        ...
        //Display window
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_08_L, u16HDEStart/2, VOP_DISPLAY_HSTART_MASK);
        ...
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_04_L, u16HDEStart/2, VOP_DISPLAY_HSTART_MASK);
    ...
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK12_20_L, 0x1840,0xFFFF);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK12_21_L, 0x1840,0xFFFF);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK12_22_L, 0x4040,0xFFFF);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK12_23_L, 0x1040,0xFFFF);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK12_24_L, 0x0040,0xFFFF);
...
#ifdef SUPPORT_Scaling_Sub1_Mode
    //H post-scaling Phase
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_21_L, (MS_U16)(pDBreg->u32H_PostScalingIniFactor>>00), 0xFFFF);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_22_L, (MS_U16)(pDBreg->u32H_PostScalingIniFactor>>16), 0xFFFF);
    //H post-scaling Sub1 Mode
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_28_L, (pDBreg->bIsHScalingUp == TRUE ? BIT(9) : 0), BIT(9));
    //V post-scaling Phase
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_23_L, (MS_U16)(pDBreg->u32V_PostScalingIniFactor>>00), 0xFFFF);
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_24_L, (MS_U16)(pDBreg->u32V_PostScalingIniFactor>>16), 0xFFFF);
    //V post-scaling Sub1 Mode
    MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_2A_L, (pDBreg->bIsVScalingUp == TRUE ? BIT(9) : 0), BIT(9));
#endif
...
void Hal_SC_sw_db(void *pInstance, P_SC_SWDB_INFO pDBreg, SCALER_WIN eWindow )
{
    ...
    XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"[%s,%5d] [DS] bDynamicScalingEnable = %s\n",__func__,__LINE__,pXCResourcePrivate->sthal_SC.bDynamicScalingEnable?"True":"False");
    ...
    if( eWindow == MAIN_WINDOW )
    {
...
#ifdef SUPPORT_Scaling_Sub1_Mode
        SC_W4BYTE(psXCInstPri->u32DeviceID, REG_SC_BK23_01_L, pDBreg->u32H_PostScalingIniFactor);   // H pre-scaling Phase
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK23_08_L, (pDBreg->bIsHScalingUp == TRUE ? BIT(9) : 0), BIT(9)); //H post-scaling Sub1 Mode
        SC_W4BYTE(psXCInstPri->u32DeviceID, REG_SC_BK23_03_L, pDBreg->u32V_PostScalingIniFactor);   // V pre-scaling Phase
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK23_0A_L, (pDBreg->bIsVScalingUp == TRUE ? BIT(9) : 0), BIT(9)); //V post-scaling Sub1 Mode
#endif
        ...
        SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK12_1C_L, 0x0);         //always disable IP H mirror line offset note:ds will enable this register at vdec firmware
...
//      SC_W4BYTE(psXCInstPri->u32DeviceID, REG_SC_BK12_14_L, pDBreg->u32OPMBase2); // output address2
        ...
        if (gSrcInfo[MAIN_WINDOW].bR_FBL)
        {
            ...
            if (gSrcInfo[MAIN_WINDOW].bInterlace)
            {
                SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK79_09_L, (MS_U16)(gSrcInfo[MAIN_WINDOW].stCropWin.height>>01), 0x0FFF);
            }
            else
            {
        ...
        else
        {
            ...
            if (gSrcInfo[MAIN_WINDOW].bInterlace)
            {
                SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK79_09_L, (MS_U16)(pDBreg->u16V_CapSize>>01), 0x0FFF);
            }
            else
            {
        ...
        if(gSrcInfo[MAIN_WINDOW].bInterlace)
        {
            SC_W2BYTE(psXCInstPri->u32DeviceID,REG_SC_BK5C_06_L,((gSrcInfo[MAIN_WINDOW].stCapWin.height/2)*1/8)-1);
            SC_W2BYTE(psXCInstPri->u32DeviceID,REG_SC_BK5C_07_L,((gSrcInfo[MAIN_WINDOW].stCapWin.height/2)*7/8)-1);
        }
        else
        {
        ...
        if(gSrcInfo[MAIN_WINDOW].bInterlace)
        {
            SC_W2BYTE(psXCInstPri->u32DeviceID,REG_SC_BK5C_06_L, 0);
            SC_W2BYTE(psXCInstPri->u32DeviceID,REG_SC_BK5C_07_L,(gSrcInfo[MAIN_WINDOW].stCapWin.height/2)-2);
        }
        else
        {
            SC_W2BYTE(psXCInstPri->u32DeviceID,REG_SC_BK5C_06_L, 0);
            SC_W2BYTE(psXCInstPri->u32DeviceID,REG_SC_BK5C_07_L,gSrcInfo[MAIN_WINDOW].stCapWin.height-2);
        }
#endif
#if PIP_PATCH_USING_SC1_MAIN_AS_SC0_SUB
        //Display window
        if (psXCInstPri->u32DeviceID == 1)
        {
                ...
                )
            {
                ...
                SC_W2BYTE(0, REG_SC_BK68_61_L, (u32Sc1TGEN_Dly%u32Sc1OneLineDly)*2);// 2p mode
                SC_W2BYTE(0, REG_SC_BK68_62_L, u32Sc1TGEN_Dly/u32Sc1OneLineDly);
            }
            else
            {
                MS_U32 u32STGEN_Dly = VIP_SC0_USER_MODE_V_DLY*u32Sc1OneLineDly+VIP_SC0_USER_MODE_H_DLY;
                ...
                u32Sc1TGEN_Dly = u32Sc0TGEN_Dly - u32Sc1VIP_Dly - SC1_PRE_RUN_DLY + u32HComp;
                SC_W2BYTE(0, REG_SC_BK68_61_L, (u32Sc1TGEN_Dly%u32Sc1OneLineDly)*2);
                SC_W2BYTE(0, REG_SC_BK68_62_L, u32Sc1TGEN_Dly/u32Sc1OneLineDly);
                SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK10_0A_L, pDBreg->u16V_DisStart);
                SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK10_0B_L, pDBreg->u16V_DisEnd);
                // Maxim: Copy MDE to FDE for SC1 case
...
// 2_frame_mirror setting
...
#ifdef SUPPORT_Scaling_Sub1_Mode
        SC_W4BYTE(psXCInstPri->u32DeviceID, REG_SC_BK23_21_L, pDBreg->u32H_PostScalingIniFactor);   // H pre-scaling Phase
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK23_28_L, (pDBreg->bIsHScalingUp == TRUE ? BIT(9) : 0), BIT(9)); //H post-scaling Sub1 Mode
        SC_W4BYTE(psXCInstPri->u32DeviceID, REG_SC_BK23_23_L, pDBreg->u32V_PostScalingIniFactor);   // V pre-scaling Phase
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK23_2A_L, (pDBreg->bIsVScalingUp == TRUE ? BIT(9) : 0), BIT(9)); //V post-scaling Sub1 Mode
#endif
    ...
    // by HW RD's request, for main video BW issue, adjust R/W line buffer length
    SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK12_20_L, 0x1840);
    SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK12_21_L, 0x1840);
    SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK12_22_L, 0x4040);
    SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK12_23_L, 0x1040);
    SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK12_24_L, 0x0040);
...
void Hal_SC_SWDS_AddCmd(void *pInstance, P_SC_SWDB_INFO pDBreg, SCALER_WIN eWindow)
{
    ...
    if(eWindow == MAIN_WINDOW)
    {
        if(pXCResourcePrivate->sthal_SC.bDynamicScalingEnable[eWindow])
        {
            ...
            if(gSrcInfo[eWindow].Status2.bEnable_ForceP)
            {
                //MVOP //0x1014 0x11[7]  reg_fld_md
                MS_U16 u16Reg_Fld_Md = MDrv_Read2Byte(0x101422);
                MS_U16 u16Reg_SW_Interlance_Mode = SC_R2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK01_21_L);
                u16Reg_SW_Interlance_Mode |= BIT(0); // SW interlace/progressive mode Enable
                MS_U16 u16XC_DI = SC_R2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK20_01_L);
                MS_U16 u16XC_OP_Read_Fields = SC_R2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK12_03_L);

                if(gSrcInfo[eWindow].bInterlace)
                {
                    Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,0x101422,(MS_U16)(u16Reg_Fld_Md | BIT(7)),DS_IP,DS_MVOP,&PSTXC_DS_CMDCNT);
                    Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK12_01_L,FORCEP_I_MODE_MADI_MODE,DS_OP,DS_XC,&PSTXC_DS_CMDCNT);

                    // SW interlace mode
                    Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK01_21_L,(MS_U16)(u16Reg_SW_Interlance_Mode | BIT(1)),DS_IP,DS_XC,&PSTXC_DS_CMDCNT);

                    u16XC_DI &= (~ BIT(1)); // Open DI
                    Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK20_01_L,(MS_U16)(u16XC_DI),DS_OP,DS_XC,&PSTXC_DS_CMDCNT);
                    u16XC_OP_Read_Fields |= BIT(3); // F2 OP read 4 fields enable
                    Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK12_03_L,(MS_U16)(u16XC_OP_Read_Fields),DS_OP,DS_XC,&PSTXC_DS_CMDCNT);
                }
                else
                {
                    Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,0x101422,(MS_U16)(u16Reg_Fld_Md & (0xFF7F)),DS_IP,DS_MVOP,&PSTXC_DS_CMDCNT);
                    Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK12_01_L,FORCEP_P_MODE_MADI_MODE,DS_OP,DS_XC,&PSTXC_DS_CMDCNT);
                    ...
                    u16XC_DI |= BIT(1); // Open DI
                    Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK20_01_L,(MS_U16)(u16XC_DI),DS_OP,DS_XC,&PSTXC_DS_CMDCNT);
                    u16XC_OP_Read_Fields &= (~ BIT(3)); // F2 OP read 4 fields disable
                    Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK12_03_L,(MS_U16)(u16XC_OP_Read_Fields),DS_OP,DS_XC,&PSTXC_DS_CMDCNT);
                }
            }
            ...
            if (gSrcInfo[MAIN_WINDOW].bR_FBL)
            {
                ...
                if (gSrcInfo[MAIN_WINDOW].bInterlace)
                {
                    Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK79_09_L,(MS_U16)(gSrcInfo[MAIN_WINDOW].stCropWin.height>>01),DS_IP,DS_XC,&PSTXC_DS_CMDCNT);
                }
                else
                {
            ...
            else
            {
                ...
                if (gSrcInfo[MAIN_WINDOW].bInterlace)
                {
                    Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK79_09_L,(MS_U16)(pDBreg->u16V_CapSize>>01),DS_IP,DS_XC,&PSTXC_DS_CMDCNT);
                }
                else
                {
            ...
            {
                Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK06_21_L,gSrcInfo[eWindow].Status2.u16DNREnableValue,DS_IP,DS_XC,&PSTXC_DS_CMDCNT);
                if((gSrcInfo[eWindow].Status2.bEnable_ForceP) && (gSrcInfo[eWindow].bInterlace))
                {
                    //Disable force BOB
                    Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK22_78_L,(MS_U16)(0x0),DS_OP,DS_XC,&PSTXC_DS_CMDCNT);
                }
            }
            else
            {
                gSrcInfo[eWindow].Status2.u16DNREnableValue = SC_R2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK06_21_L);
                Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK06_21_L,gSrcInfo[eWindow].Status2.u16DNREnableValue & 0xFFFD,DS_IP,DS_XC,&PSTXC_DS_CMDCNT);
                if(gSrcInfo[eWindow].Status2.bEnable_ForceP)
                {
                    //Enable force BOB
                    Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK22_78_L,(MS_U16)(0x8F8F),DS_OP,DS_XC,&PSTXC_DS_CMDCNT);
                }
            }
#endif
            ...
            if(gSrcInfo[MAIN_WINDOW].bInterlace)
            {
                Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK5C_06_L,((gSrcInfo[MAIN_WINDOW].stCapWin.height/2)*1/8 -1),DS_IP,DS_XC,&PSTXC_DS_CMDCNT);
                Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK5C_07_L,((gSrcInfo[MAIN_WINDOW].stCapWin.height/2)*7/8 -1),DS_IP,DS_XC,&PSTXC_DS_CMDCNT);
            }
            else
            {
            ...
            if(gSrcInfo[MAIN_WINDOW].bInterlace)
            {
                Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK5C_06_L, 0,DS_IP,DS_XC,&PSTXC_DS_CMDCNT);
                Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK5C_07_L,((gSrcInfo[MAIN_WINDOW].stCapWin.height/2)-2),DS_IP,DS_XC,&PSTXC_DS_CMDCNT);
            }
            else
            {
                Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK5C_06_L, 0,DS_IP,DS_XC,&PSTXC_DS_CMDCNT);
                Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK5C_07_L,(gSrcInfo[MAIN_WINDOW].stCapWin.height-2),DS_IP,DS_XC,&PSTXC_DS_CMDCNT);
            }
#endif
...
#ifdef SUPPORT_Scaling_Sub1_Mode
                //H post-scaling Phase
                Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK23_01_L,(MS_U16)(pDBreg->u32H_PostScalingIniFactor>>00),DS_OP,DS_XC,&PSTXC_DS_CMDCNT);//0xFFFF0009 918AXXXX
                Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK23_02_L,(MS_U16)(pDBreg->u32H_PostScalingIniFactor>>16),DS_OP,DS_XC,&PSTXC_DS_CMDCNT);//0xFFFF0009 918AXXXX
                //H post-scaling Sub1 Mode
                Hal_SC_WriteSWDSCommand_Mask(pInstance,MAIN_WINDOW,REG_SC_BK23_08_L,(pDBreg->bIsHScalingUp == TRUE ? BIT(9) : 0),DS_OP,DS_XC,&PSTXC_DS_CMDCNT,BIT(9));
                //V post-scaling Phase
                Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK23_03_L,(MS_U16)(pDBreg->u32V_PostScalingIniFactor>>00),DS_OP,DS_XC,&PSTXC_DS_CMDCNT);//0xFFFF0009 918AXXXX
                Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK23_04_L,(MS_U16)(pDBreg->u32V_PostScalingIniFactor>>16),DS_OP,DS_XC,&PSTXC_DS_CMDCNT);//0xFFFF0009 918AXXXX
                //V post-scaling Sub1 Mode
                Hal_SC_WriteSWDSCommand_Mask(pInstance,MAIN_WINDOW,REG_SC_BK23_0A_L,(pDBreg->bIsVScalingUp == TRUE ? BIT(9) : 0),DS_OP,DS_XC,&PSTXC_DS_CMDCNT,BIT(9));
#endif
...
#ifdef SUPPORT_Scaling_Sub1_Mode
                    //H post-scaling Phase
                    Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK23_21_L,(MS_U16)(pDBreg->u32H_PostScalingIniFactor>>00),DS_OP,DS_XC,&PSTXC_DS_CMDCNT);//0xFFFF0009 918AXXXX
                    Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK23_22_L,(MS_U16)(pDBreg->u32H_PostScalingIniFactor>>16),DS_OP,DS_XC,&PSTXC_DS_CMDCNT);//0xFFFF0009 918AXXXX
                    //H post-scaling Sub1 Mode
                    Hal_SC_WriteSWDSCommand_Mask(pInstance,MAIN_WINDOW,REG_SC_BK23_28_L,(pDBreg->bIsHScalingUp == TRUE ? BIT(9) : 0),DS_OP,DS_XC,&PSTXC_DS_CMDCNT,BIT(9));
                    //V post-scaling Phase
                    Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK23_23_L,(MS_U16)(pDBreg->u32V_PostScalingIniFactor>>00),DS_OP,DS_XC,&PSTXC_DS_CMDCNT);//0xFFFF0009 918AXXXX
                    Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK23_24_L,(MS_U16)(pDBreg->u32V_PostScalingIniFactor>>16),DS_OP,DS_XC,&PSTXC_DS_CMDCNT);//0xFFFF0009 918AXXXX
                    //V post-scaling Sub1 Mode
                    Hal_SC_WriteSWDSCommand_Mask(pInstance,MAIN_WINDOW,REG_SC_BK23_2A_L,(pDBreg->bIsVScalingUp == TRUE ? BIT(9) : 0),DS_OP,DS_XC,&PSTXC_DS_CMDCNT,BIT(9));
#endif
                    //Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK12_10_L, (MS_U16)(pDBreg->u32OPMBase0>>00),DS_OP,DS_XC,&PSTXC_DS_CMDCNT); // OPM base0  //0xFFFF0009 8910XXXX
                    //Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK12_11_L,(MS_U16)(pDBreg->u32OPMBase0>>16),DS_OP,DS_XC,&PSTXC_DS_CMDCNT); // OPM base0   //0xFFFF0009 8911XXXX
            ...
            if(IS_SUPPORT_DS_SR(gSrcInfo[eWindow].u16InputVFreq))
            {
                ...
                else
                {
                    ...
                    {
                        u16VSyncStart += (FRC_PIPE_DELAY_VCNT_FSC_FHD_HDMITX - FRC_PIPE_DELAY_VCNT_FSC_4K);
                        u16VSyncEnd += (FRC_PIPE_DELAY_VCNT_FSC_FHD_HDMITX - FRC_PIPE_DELAY_VCNT_FSC_4K);
                    }
           ...
           //STGEN setting
                ...
                )
            {
                ...
                else
                {
                    ...
                    if((pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Init_Misc.u32MISC_B & E_XC_INIT_MISC_B_HDMITX_ENABLE)
                        ...
                        &&(pXCResourcePrivate->stdrvXC_MVideo.gu16VPixelShiftRange == 0)) //Check if PIXEL FEATURE is ENABLE or NOT.
                    {
                        u16MdeVstart += (FRC_PIPE_DELAY_VCNT_FSC_FHD_HDMITX - FRC_PIPE_DELAY_VCNT_FSC_4K);
                        u16MdeVend += (FRC_PIPE_DELAY_VCNT_FSC_FHD_HDMITX - FRC_PIPE_DELAY_VCNT_FSC_4K);
                    }
                    ...
                    if((pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Init_Misc.u32MISC_B & E_XC_INIT_MISC_B_HDMITX_ENABLE)
                        ...
                        &&(pXCResourcePrivate->stdrvXC_MVideo.gu16VPixelShiftRange == 0)) //Check if PIXEL FEATURE is ENABLE or NOT.
                    {
                        u16FdeVstart += (FRC_PIPE_DELAY_VCNT_FSC_FHD_HDMITX - FRC_PIPE_DELAY_VCNT_FSC_4K);
                        u16FdeVend += (FRC_PIPE_DELAY_VCNT_FSC_FHD_HDMITX - FRC_PIPE_DELAY_VCNT_FSC_4K);
                    }
            ...
            MS_U32 u32MaxCmdCnt = pXCResourcePrivate->sthal_SC.u16DynamicScalingIndexDepth_Ext * (Hal_XC_MLoad_Get_64Bits_MIU_Bus_Sel(pInstance)+1);
...
void Hal_SC_Enable_MiuMask(void *pInstance)
{
    ...
    {
        pXCResourcePrivate->sthal_SC.Miu0Mask.u16MiuG6Mask |= MIU_SC0_G6REQUEST_MASK;
    }
    else
    {
        pXCResourcePrivate->sthal_SC.Miu0Mask.u16MiuG6Mask |= MIU_SC1_G6REQUEST_MASK;
    }
    ...
    if(psXCInstPri->u32DeviceID == E_XC_DEVICE0)
    {
        pXCResourcePrivate->sthal_SC.Miu1Mask.u16MiuG6Mask |= MIU_SC0_G6REQUEST_MASK;
    }
    else
    {
        pXCResourcePrivate->sthal_SC.Miu1Mask.u16MiuG6Mask |= MIU_SC1_G6REQUEST_MASK;
    }
...
void Hal_SC_set_miusel(void *pInstance, MS_U8 u8MIUSel)
{
    ...
    if(psXCInstPri->u32DeviceID == 0)
    {
        ...
        {
            case 0:
                MDrv_MIU_SelMIU(MIU_CLIENT_SC_IPMAIN_RW, MIU_SELTYPE_MIU0);
                MDrv_MIU_SelMIU(MIU_CLIENT_SC_OPMAIN_RW, MIU_SELTYPE_MIU0);
                MDrv_MIU_SelMIU(MIU_CLIENT_HDR_L_RW, MIU_SELTYPE_MIU0);
                MDrv_MIU_SelMIU(MIU_CLIENT_HDR_R_RW, MIU_SELTYPE_MIU0);
                break;
            case 1:
                MDrv_MIU_SelMIU(MIU_CLIENT_SC_IPMAIN_RW, MIU_SELTYPE_MIU1);
                MDrv_MIU_SelMIU(MIU_CLIENT_SC_OPMAIN_RW, MIU_SELTYPE_MIU1);
                MDrv_MIU_SelMIU(MIU_CLIENT_HDR_L_RW, MIU_SELTYPE_MIU1);
                MDrv_MIU_SelMIU(MIU_CLIENT_HDR_R_RW, MIU_SELTYPE_MIU1);
                break;
            /*case 2:
                        MDrv_MIU_SelMIU(MIU_CLIENT_SC_IPMAIN_RW, MIU_SELTYPE_MIU2);
                        MDrv_MIU_SelMIU(MIU_CLIENT_SC_OPMAIN_RW, MIU_SELTYPE_MIU2);
                        MDrv_MIU_SelMIU(MIU_CLIENT_HDR_L_RW, MIU_SELTYPE_MIU2);
                        MDrv_MIU_SelMIU(MIU_CLIENT_HDR_R_RW, MIU_SELTYPE_MIU2);
                        break;*/
            default:
                printf("[XC]MIU SELECT Out of MIU0/1\n");
                MDrv_MIU_SelMIU(MIU_CLIENT_SC_IPMAIN_RW, MIU_SELTYPE_MIU0);
                MDrv_MIU_SelMIU(MIU_CLIENT_SC_OPMAIN_RW, MIU_SELTYPE_MIU0);
                MDrv_MIU_SelMIU(MIU_CLIENT_HDR_L_RW, MIU_SELTYPE_MIU0);
                MDrv_MIU_SelMIU(MIU_CLIENT_HDR_R_RW, MIU_SELTYPE_MIU0);
    ...
    else
    {
        ...
        {
            case 0:
                MDrv_MIU_SelMIU(MIU_CLIENT_SC1_OPM_R, MIU_SELTYPE_MIU0);
                MDrv_MIU_SelMIU(MIU_CLIENT_SC1_IPMAIN_RW, MIU_SELTYPE_MIU0);
                break;
            case 1:
                MDrv_MIU_SelMIU(MIU_CLIENT_SC1_OPM_R, MIU_SELTYPE_MIU1);
                MDrv_MIU_SelMIU(MIU_CLIENT_SC1_IPMAIN_RW, MIU_SELTYPE_MIU1);
                break;
            default:
                MDrv_MIU_SelMIU(MIU_CLIENT_SC1_OPM_R, MIU_SELTYPE_MIU0);
                MDrv_MIU_SelMIU(MIU_CLIENT_SC1_IPMAIN_RW, MIU_SELTYPE_MIU0);
                printf("[XC]Device 1 MIU SELECT Out of MIU0/1\n");
                break;
        }
    }
...
void Hal_SC_set_disp_window(void *pInstance, SCALER_WIN eWindow, MS_WINDOW_TYPE *pdspwin)
{
    ...
    if (MDrv_XC_MLoad_GetStatus(pInstance) == E_MLOAD_ENABLED)
    {
        ...
        if(eWindow == MAIN_WINDOW)
        {
            ...
            if (psXCInstPri->u32DeviceID == 1)
            {
                //Display window
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_08_L, u16HDEStart/2, VOP_DISPLAY_HSTART_MASK);
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_09_L, (u16HDEStart/2+pdspwin->width-1), VOP_DISPLAY_HEND_MASK);

                // Maxim: Copy MDE to FDE for SC1 case
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_04_L, u16HDEStart/2, VOP_DISPLAY_HSTART_MASK);
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_05_L, (u16HDEStart/2+pdspwin->width-1), VOP_DISPLAY_HEND_MASK);
            }
            else
            {
                //Display window
    ...
    else
    {
        if(eWindow == MAIN_WINDOW)
        {
            ...
            if (psXCInstPri->u32DeviceID == 1)
            {
                SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK10_08_L, u16HDEStart/2);
                SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK10_09_L, (u16HDEStart/2+ pdspwin->width-1));

                // Maxim: Copy MDE to FDE for SC1 case
                SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK10_04_L, u16HDEStart/2);
                SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK10_05_L, (u16HDEStart/2+ pdspwin->width-1));
            }
            else
            {
...
void Hal_SC_set_mrq_miusel(void *pInstance, MS_U8 u8MIUSel)
{
    ...
    // Scaler control MRQ MIU and MCDI MIU sel by itself
    // So MIU1 IP-select is set to 1. At this moment, MIU0 Miu select is not working.
    if (u8MIUSel==0)
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK0D_07_L, 0, BIT(8)|BIT(9));     // reg_mrq_miu_sel to MIU0
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK17_04_L, 0, BIT(0)|BIT(1));     // reg_mcdi_arb_sel0 to MIU0
    }
    else if (u8MIUSel==1)
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK0D_07_L, BIT(8), BIT(8)|BIT(9));     // reg_mrq_miu_sel to MIU1
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK17_04_L, BIT(0), BIT(0)|BIT(1));     // reg_mcdi_arb_sel0 to MIU1
    }
    else if (u8MIUSel==2)
    {
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK0D_07_L, BIT(9), BIT(8)|BIT(9));     // reg_mrq_miu_sel to MIU2
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK17_04_L, BIT(1), BIT(0)|BIT(1));     // reg_mcdi_arb_sel0 to MIU2
    }
...
MS_BOOL MHal_XC_Init_Patch(void *pInstance, MS_BOOL bEnable, SCALER_WIN eWindow)
{
    ...
    // bug fix for abc123 1031600: AC on/off see all pink video, reproduce rate about 1/20
    ...
    //Patch for M7622
    //HW default sets DS req length to 0x88, but if each index contains DS commands less than 0x88
    //ext DS req length (0x131F_51[7:0])should be set to 0.
    //At initial stage, set ext DS req length to 0 as default.
    SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK1F_51_L, 0x00, 0x00FF);
...
MS_BOOL Hal_SC_Init(void *pInstance)
{
    ...
    if(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Init_Misc.u32MISC_B & E_XC_INIT_MISC_B_HDMITX_ENABLE)
    {
         MHal_FRC_Set_Pipe_Delay_Value(pInstance, FRC_PIPE_DELAY_HCNT_FRC, FRC_PIPE_DELAY_VCNT_FSC_FHD_HDMITX);
    }
    else
    {
...
#if PIP_PATCH_USING_SC1_MAIN_AS_SC0_SUB
        SCALER_WIN eCma_Win = WINDOW_NUM;
        if(psXCInstPri->u32DeviceID==0)
        {
...
MS_BOOL MHal_XC_Get_CMA_UsingCondition(void *pInstance, MS_BOOL bIsGetCMABuff[], MS_U32 au32CMAMemSCMSize[],MS_U32 au32CMAMemFRCMSize[], MS_U32* pu32DualMiuMemSize, MS_U32 u32DataSize, SCALER_WIN eWindow)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                u32CMAMemSCMSize = XC_CMA_48MB + XC_CMA_8MB;   //DNR 256,  It needs 51M(2048 X 1080 x 3 X 8) when the source is 1080
            }
    }
...
void Hal_SC_3D_enable_black_edge(void *pInstance, MS_BOOL b2DTo3D)
{
    ...
    if(b2DTo3D)
    {
        u16TempGarbageWidth = pXCResourcePrivate->stdrvXC_3D._st3DHw2DTo3DPara.u16Offset>>3; //offset
        u16TempGarbageWidth = (u16TempGarbageWidth > 16)?(u16TempGarbageWidth - 16):(16 - u16TempGarbageWidth);
        u16TempGarbageWidth = u16TempGarbageWidth + (pXCResourcePrivate->stdrvXC_3D._st3DHw2DTo3DPara.u16Gain + 1) / 2;//gain
    }
    else
    {
        if(0 == pXCResourcePrivate->stdrvXC_3D._u163DHShift)
        {
            u16TempGarbageWidth = pXCResourcePrivate->stdrvXC_3D._u163DHShift;
        }
        else if(pXCResourcePrivate->stdrvXC_3D._u163DHShift > 112)
        {
            u16TempGarbageWidth = pXCResourcePrivate->stdrvXC_3D._u163DHShift - 112;//_u163DHShift = value on UI + 112
        }
        else
        {
            u16TempGarbageWidth = 0;
            printf("[%s,%5d] ERROR: invalid value of  gain\n",__FUNCTION__,__LINE__);
        }

        if((0 == u16TempGarbageWidth) || (16 == u16TempGarbageWidth))
        {
            u16TempGarbageWidth =  0;
        }
        else
    ...
    {
        u16GarbageWidth = u16TempGarbageWidth* 2;
    }
    else if(u16DispWinWidth > 960)
    {
        u16GarbageWidth = u16TempGarbageWidth;
    }
    ...
    {
        u16GarbageWidth = u16TempGarbageWidth / 4;
    }
...
E_APIXC_ReturnValue MHal_XC_AutoDownload_Write(void* pInstance, EN_XC_AUTODOWNLOAD_CLIENT enClient, MS_U8* pu8Data, MS_U32 u32Size, void* pParam)
{
    ...
    switch(enClient)
    {
        ...
        case E_XC_AUTODOWNLOAD_CLIENT_FRCSPTPOPM:
        case E_XC_AUTODOWNLOAD_CLIENT_FOOPM:
        case E_XC_AUTODOWNLOAD_CLIENT_HVSP:
        case E_XC_AUTODOWNLOAD_CLIENT_MAX:
        default:
        {
            return E_APIXC_RET_FAIL;
...
MS_BOOL MHal_XC_PixelShiftStatusChange(void* pInstance, XC_PIXEL_SHIFT_STATUS enStatus)
{
    ...
    XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "enStatus: %d, psXCInstPri->u32DeviceID: %ld.\n", enStatus, psXCInstPri->u32DeviceID);
...
void Hal_SC_3D_Set_DualView(void *pInstance, MS_BOOL bEnable)
{
    ...
    if (bEnable)
    {
        ...
        {
            SC_W2BYTE(0, REG_SC_BK23_2C_L, 0);
        }
        else
        {
...
#ifdef CONFIG_UTOPIA_PROC_DBG_SUPPORT
static void _MHal_XCMdbloctl_Cat_OPTEE(void *pInstance, MDBCMD_GETINFO_PARAMETER *paraGetInfo)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
    ...
    MdbPrint(paraGetInfo->u64ReqHdl,"OPTEE REE enable                   : %s\n", pXCResourcePrivate->sthal_Optee.op_tee_xc[MAIN_WINDOW].isEnable==TRUE?"TRUE":"FALSE");
...
static void _MHal_XCMdbloctl_Cat_3D(void *pInstance, MDBCMD_GETINFO_PARAMETER *paraGetInfo)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
...
static void _MHal_XCMdbloctl_Cat_TGEN(void *pInstance, MDBCMD_GETINFO_PARAMETER *paraGetInfo)
{
    MdbPrint(paraGetInfo->u64ReqHdl,"----------------  Mstar Scaler TGEN Info -----------------\n");
    MdbPrint(paraGetInfo->u64ReqHdl,"Hsync width                        : %d\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_01_L, 0xFF));
    MdbPrint(paraGetInfo->u64ReqHdl,"Vsync position                     : (%d,%d)\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_02_L, 0x7FF)+(SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_02_L, 0x1000)>>1), SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_03_L, 0x1FFF));
    MdbPrint(paraGetInfo->u64ReqHdl,"HDE position                       : (%d,%d)\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_04_L, 0x3FFF), SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_05_L, 0x3FFF));
    MdbPrint(paraGetInfo->u64ReqHdl,"VDE position                       : (%d,%d)\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_06_L, 0x1FFF), SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_07_L, 0x1FFF));
    MdbPrint(paraGetInfo->u64ReqHdl,"Vsync control mode                 : %s\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_05_L, 0x01)==0x00?"Auto":"Manual");
}
static void _MHal_XCMdbloctl_Cat_MOD(void *pInstance, MDBCMD_GETINFO_PARAMETER *paraGetInfo)
{
    MdbPrint(paraGetInfo->u64ReqHdl,"----------------  Mstar Scaler MOD Info -----------------\n");

    MdbPrint(paraGetInfo->u64ReqHdl,"Output config                      : 0x%x, 0x%x\n", R2BYTE(0x111e00UL), R2BYTE(0x111e02UL));
    MdbPrint(paraGetInfo->u64ReqHdl,"Panel clock                        : %d\n", R2BYTEMSK(0x111e00UL, 0x1F));
    MdbPrint(paraGetInfo->u64ReqHdl,"Panel free swap                    : 0x%x, 0x%x, 0x%x, 0x%x\n", R2BYTE(0x103210UL), R2BYTE(0x103212UL), R2BYTE(0x103214UL), R2BYTE(0x103216UL));
    MdbPrint(paraGetInfo->u64ReqHdl,"VB1 lock status                    : %s\n", R2BYTEMSK(0x1031C2UL, BIT(14))==BIT(14)?"UNLOCK":"LOCK");
    MdbPrint(paraGetInfo->u64ReqHdl,"VB1 unlock count                   : %d\n", R2BYTEMSK(0x1031ECUL, 0x7F));
}
static void _MHal_XCMdbloctl_Cat_HDR(void *pInstance, MDBCMD_GETINFO_PARAMETER *paraGetInfo)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
    ...
    if(stCfdStatus.u8VideoHdrType == 0)
    {
        MdbPrint(paraGetInfo->u64ReqHdl,"HDR type                           : SDR\n");
    }
    else if(stCfdStatus.u8VideoHdrType == BIT(0))
    {
        MdbPrint(paraGetInfo->u64ReqHdl,"HDR type                           : Dolby HDR\n");
    }
    else if(stCfdStatus.u8VideoHdrType == BIT(1))
    {
        MdbPrint(paraGetInfo->u64ReqHdl,"HDR type                           : Open HDR\n");
    }
    else if(stCfdStatus.u8VideoHdrType == BIT(2))
    {
        MdbPrint(paraGetInfo->u64ReqHdl,"HDR type                           : TCH\n");
    }
    else if(stCfdStatus.u8VideoHdrType == BIT(3))
    {
        MdbPrint(paraGetInfo->u64ReqHdl,"HDR type                           : HLG\n");
    }

    if((stCfdStatus.u8VideoHdrType == BIT(1)) || (stCfdStatus.u8VideoHdrType == BIT(3)))
    {
        MdbPrint(paraGetInfo->u64ReqHdl,"Input color range is               : %s\n", stCfdStatus.bIsFullRange==0?"limit range":"full range");

        if(pXCResourcePrivate->stdrvXC_Display._enInputSourceType == E_XC_INPUT_SOURCE_TYPE_HDMI)
        {
            MdbPrint(paraGetInfo->u64ReqHdl,"Input Pixel format                 : %d\n", stCfdStatus.HDRMetadata.stHdmiInfoCFD.u8PixelFormat);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input Colorimetry                  : %d\n", stCfdStatus.HDRMetadata.stHdmiInfoCFD.u8Colorimetry);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input ExtendedColorimetry          : %d\n", stCfdStatus.HDRMetadata.stHdmiInfoCFD.u8ExtendedColorimetry);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input HDR Info                     : %s exit\n", stCfdStatus.HDRMetadata.stHdmiInfoCFD.bHDRInfoFrameValid==0?"is not":"is");
            MdbPrint(paraGetInfo->u64ReqHdl,"Input Display Primaries_x[0]       : 0x%x\n", stCfdStatus.HDRMetadata.stHdmiInfoCFD.u16Display_Primaries_x[0]);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input Display Primaries_x[1]       : 0x%x\n", stCfdStatus.HDRMetadata.stHdmiInfoCFD.u16Display_Primaries_x[1]);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input Display Primaries_x[2]       : 0x%x\n", stCfdStatus.HDRMetadata.stHdmiInfoCFD.u16Display_Primaries_x[2]);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input Display Primaries_y[0]       : 0x%x\n", stCfdStatus.HDRMetadata.stHdmiInfoCFD.u16Display_Primaries_y[0]);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input Display Primaries_y[1]       : 0x%x\n", stCfdStatus.HDRMetadata.stHdmiInfoCFD.u16Display_Primaries_y[1]);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input Display Primaries_y[2]       : 0x%x\n", stCfdStatus.HDRMetadata.stHdmiInfoCFD.u16Display_Primaries_y[2]);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input White point_x                : 0x%x\n", stCfdStatus.HDRMetadata.stHdmiInfoCFD.u16White_point_x);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input White point_y                : 0x%x\n", stCfdStatus.HDRMetadata.stHdmiInfoCFD.u16White_point_y);
        }
        else if(pXCResourcePrivate->stdrvXC_Display._enInputSourceType == E_XC_INPUT_SOURCE_TYPE_OTT)
        {
            MdbPrint(paraGetInfo->u64ReqHdl,"Input format                       : %d\n", stCfdStatus.HDRMetadata.stHDRMemFormatCFD.u8InputFormat);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input data format                  : %d\n", stCfdStatus.HDRMetadata.stHDRMemFormatCFD.u8InputDataFormat);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input UVI                          : %s exit\n", stCfdStatus.HDRMetadata.stHDRMemFormatCFD.bVUIValid==0?"is not":"is");
            MdbPrint(paraGetInfo->u64ReqHdl,"Input Colour primaries             : %d\n", stCfdStatus.HDRMetadata.stHDRMemFormatCFD.u8Colour_primaries);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input Transfer characteristics     : %d\n", stCfdStatus.HDRMetadata.stHDRMemFormatCFD.u8Transfer_Characteristics);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input Matrix coeffs                : %d\n", stCfdStatus.HDRMetadata.stHDRMemFormatCFD.u8Matrix_Coeffs);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input SEI                          : %s exit\n", stCfdStatus.HDRMetadata.stHDRMemFormatCFD.bSEIValid==0?"is not":"is");
            MdbPrint(paraGetInfo->u64ReqHdl,"Input Display primaries_x[0]       : 0x%x\n", stCfdStatus.HDRMetadata.stHDRMemFormatCFD.u16Display_Primaries_x[0]);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input Display primaries_x[1]       : 0x%x\n", stCfdStatus.HDRMetadata.stHDRMemFormatCFD.u16Display_Primaries_x[1]);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input Display primaries_x[2]       : 0x%x\n", stCfdStatus.HDRMetadata.stHDRMemFormatCFD.u16Display_Primaries_x[2]);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input Display primaries_y[0]       : 0x%x\n", stCfdStatus.HDRMetadata.stHDRMemFormatCFD.u16Display_Primaries_y[0]);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input Display primaries_y[1]       : 0x%x\n", stCfdStatus.HDRMetadata.stHDRMemFormatCFD.u16Display_Primaries_y[1]);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input Display primaries_y[2]       : 0x%x\n", stCfdStatus.HDRMetadata.stHDRMemFormatCFD.u16Display_Primaries_y[2]);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input White point_x                : 0x%x\n", stCfdStatus.HDRMetadata.stHDRMemFormatCFD.u16White_point_x);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input uWhite point_y               : 0x%x\n", stCfdStatus.HDRMetadata.stHDRMemFormatCFD.u16White_point_y);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input Panel_Max_Luminance          : 0x%x\n", stCfdStatus.HDRMetadata.stHDRMemFormatCFD.u32Master_Panel_Max_Luminance);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input Panel_Min_Luminance          : 0x%x\n", stCfdStatus.HDRMetadata.stHDRMemFormatCFD.u32Master_Panel_Min_Luminance);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input ContentLightLevelEnabled     : %s exit\n", stCfdStatus.HDRMetadata.stHDRMemFormatCFD.bContentLightLevelEnabled==0?"is not":"is");
            MdbPrint(paraGetInfo->u64ReqHdl,"Input MaxContentLightLevel         : %d\n", stCfdStatus.HDRMetadata.stHDRMemFormatCFD.u16maxContentLightLevel);
            MdbPrint(paraGetInfo->u64ReqHdl,"Input MaxPicAverageLightLevel      : %d\n", stCfdStatus.HDRMetadata.stHDRMemFormatCFD.u16maxPicAverageLightLevel);
        }
    }
    MdbPrint(paraGetInfo->u64ReqHdl,"TCH                                : %s\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK71_01_L, BIT(0))==BIT(0)?"On":"Off");
    MdbPrint(paraGetInfo->u64ReqHdl,"Gamma autodownload status          : %s\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK7A_01_L, BIT(3))==0?"On":"Off");
    MdbPrint(paraGetInfo->u64ReqHdl,"Degamma autodownload status        : %s\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK7A_01_L, BIT(5))==0?"On":"Off");
    MdbPrint(paraGetInfo->u64ReqHdl,"TMO autodownload status            : %s\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK7A_02_L, BIT(2))==0?"On":"Off");
    MdbPrint(paraGetInfo->u64ReqHdl,"3D LUT autodownload status         : %s\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK7A_02_L, BIT(4))==0?"On":"Off");
    u32address = (MS_U32)((SC_R2BYTE(MAIN_WINDOW,REG_SC_BK67_2A_L))<<16) | (MS_U32)(SC_R2BYTE(MAIN_WINDOW,REG_SC_BK67_29_L));
    MdbPrint(paraGetInfo->u64ReqHdl,"autodownload address               : 0x%lx\n", u32address);
    _phy_to_miu_offset(u8sel, u8temp, u32address);
    MdbPrint(paraGetInfo->u64ReqHdl,"autodownload MIU number            : MIU%d\n", u8sel);
    MdbPrint(paraGetInfo->u64ReqHdl,"XC handshake status                : %s\n", SC_R2BYTE(MAIN_WINDOW,REG_SC_BK02_10_L)==0x25?"Off":"On");
    MdbPrint(paraGetInfo->u64ReqHdl,"CRC status                         : %s\n", stCfdStatus.bCRCStatus==TRUE?"TRUE":"FALSE");
    MdbPrint(paraGetInfo->u64ReqHdl,"R2Y status                         : %s\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK02_2B_L, BIT(4))==BIT(4)?"TRUE":"FALSE");
    MdbPrint(paraGetInfo->u64ReqHdl,"Y2R status                         : %s\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_2F_L, BIT(4))==BIT(4)?"TRUE":"FALSE");
    MdbPrint(paraGetInfo->u64ReqHdl,"MM HDR vdec version number         : %d\n", stCfdStatus.u8VdecVersion);
    MdbPrint(paraGetInfo->u64ReqHdl,"Autodownload fail bit              : %s\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK67_28_L, BIT(0))==BIT(0)?"TRUE":"FALSE");
    //MdbPrint(paraGetInfo->u64ReqHdl,"Dolby Global dimming value         : \n");
    //MdbPrint(paraGetInfo->u64ReqHdl,"Dolby Global dimming delay count   : \n");
    MdbPrint(paraGetInfo->u64ReqHdl,"set_HDRDS_info average time        : %dms\n", stCfdStatus.u16SetDSAverageTime);
    MdbPrint(paraGetInfo->u64ReqHdl,"set_HDRDS_info peak time           : %dms (%d)\n", stCfdStatus.u16SetDSPeakTime, stCfdStatus.u64SetDSPeakFrame);
    MdbPrint(paraGetInfo->u64ReqHdl,"HDR HSize                          : %d\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK79_08_L, 0x1FFF));
    MdbPrint(paraGetInfo->u64ReqHdl,"HDR VSize                          : %d\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK79_09_L, 0x1FFF));
    MdbPrint(paraGetInfo->u64ReqHdl,"LSB/MSB                            : %s\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK79_06_L, BIT(8))==BIT(8)?"LSB":"MSB");
    if((SC_R2BYTE(MAIN_WINDOW,REG_SC_BK7A_42_L)==0) && (SC_R2BYTE(MAIN_WINDOW,REG_SC_BK7A_54_L)==0) && (SC_R2BYTE(MAIN_WINDOW,REG_SC_BK7A_55_L)==0) &&
        (SC_R2BYTE(MAIN_WINDOW,REG_SC_BK7A_59_L)==BIT(11)) && (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK02_10_L, BIT(4))==BIT(4)))
    {
        MdbPrint(paraGetInfo->u64ReqHdl,"HDR mute                           : TRUE\n");
    }
    else
    {
        MdbPrint(paraGetInfo->u64ReqHdl,"HDR mute                           : FALSE\n");
    }
    MdbPrint(paraGetInfo->u64ReqHdl,"Dolby efuse status                 : FALSE\n");
    MdbPrint(paraGetInfo->u64ReqHdl,"Dolby HW bond status               : %s\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK79_3F_L, BIT(14))?"TRUE":"FALSE");
}
static void _MHal_XCMdbloctl_Cat_MEMC(void *pInstance, MDBCMD_GETINFO_PARAMETER *paraGetInfo)
{
    ...
    MdbPrint(paraGetInfo->u64ReqHdl,"FRC enable                         : %s\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK68_40_L, BIT(2))==BIT(2)?"On":"Off");
    ...
    MdbPrint(paraGetInfo->u64ReqHdl,"MEMC in/out frame rate             : %d:%d \n", gSrcInfo[MAIN_WINDOW].u16InputVFreq, gSrcInfo[MAIN_WINDOW].Status2.u16OP1_OutputVFreqAfterFRC);
#endif
}
static void _MHal_XCMdbloctl_Cat_FrameLock(void *pInstance, MDBCMD_GETINFO_PARAMETER *paraGetInfo)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
    ...
    MdbPrint(paraGetInfo->u64ReqHdl,"Phase offset limit                 : %x%x\n", R2BYTEMSK(0x103120UL,0x00FF), R2BYTEMSK(0x10311EUL,0xFFFF));
    ...
    MdbPrint(paraGetInfo->u64ReqHdl,"Lock from IP                       : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK68_28_L,BIT(2))==BIT(2))?"FALSE":"TRUE");
}
static void _MHal_XCMdbloctl_Cat_SetWindow(void *pInstance, MDBCMD_GETINFO_PARAMETER *paraGetInfo)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
    ...
    MdbPrint(paraGetInfo->u64ReqHdl,"Capture window                     : (%d,%d) %dx%d\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK01_05_L, 0x1FFF),SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK01_04_L, 0x1FFF),SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK01_07_L, 0x3FFF),SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK01_06_L, 0x1FFF));
    ...
    MdbPrint(paraGetInfo->u64ReqHdl,"Display window                     : (%d,%d) %dx%d\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_08_L, 0x3FFF),SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_0A_L, 0x1FFF),SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_09_L, 0x3FFF),SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_0B_L, 0x1FFF));
    MdbPrint(paraGetInfo->u64ReqHdl,"Sub Capture window                 : (%d,%d) %dx%d\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK03_05_L, 0x1FFF),SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK03_04_L, 0x1FFF),SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK03_07_L, 0x3FFF),SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK03_06_L, 0x1FFF));
    MdbPrint(paraGetInfo->u64ReqHdl,"Sub Crop window                    : (%d,%d) %dx%d\n", gSrcInfo[SUB_WINDOW].stCropWin.x, gSrcInfo[SUB_WINDOW].stCropWin.y, gSrcInfo[SUB_WINDOW].stCropWin.width, gSrcInfo[SUB_WINDOW].stCropWin.height);
    MdbPrint(paraGetInfo->u64ReqHdl,"Sub Display window                 : (%d,%d) %dx%d\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK0F_07_L, 0x3FFF),SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK0F_09_L, 0x1FFF),SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK0F_08_L, 0x3FFF),SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK0F_0A_L, 0x1FFF));
    ...
    if (pXCResourcePrivate->sthal_SC.bIsConfigCMA == TRUE)
    {
        ...
        {
            MdbPrint(paraGetInfo->u64ReqHdl,"XC_CMA_Self                        : Start:0x%x Size:0x%x @ MIU%d  use for %s\n",CMA_Pool_Init_PARAM[CMA_XC_SELF_MEM].heap_miu_start_offset,gSrcInfo[MAIN_WINDOW].u32PreCMAMemSCMSize[CMA_XC_SELF_MEM],CMA_Pool_Init_PARAM[CMA_XC_SELF_MEM].miu,"SCMI0");
        }
        else
        {
            MdbPrint(paraGetInfo->u64ReqHdl,"XC_CMA_Self                        : Start:0x%x Size:0x%x @ MIU%d %s\n",CMA_Pool_Init_PARAM[CMA_XC_SELF_MEM].heap_miu_start_offset,0,CMA_Pool_Init_PARAM[CMA_XC_SELF_MEM].miu,"XC no use");
        }

        if (gSrcInfo[MAIN_WINDOW].u32PreCMAMemSCMSize[CMA_XC_COBUFF_MEM] != 0)
        {
            MdbPrint(paraGetInfo->u64ReqHdl,"XC_CMA_Cobuffer                    : Start:0x%x Size:0x%x @ MIU%d  use for %s\n",CMA_Pool_Init_PARAM[CMA_XC_COBUFF_MEM].heap_miu_start_offset,gSrcInfo[MAIN_WINDOW].u32PreCMAMemSCMSize[CMA_XC_SELF_MEM],CMA_Pool_Init_PARAM[CMA_XC_COBUFF_MEM].miu,"SCMI0");
        }
        else if(gSrcInfo[MAIN_WINDOW].Status2.u32PreCMAMemDualSCMSize != 0)
        {
            MdbPrint(paraGetInfo->u64ReqHdl,"XC_CMA_Cobuffer                    : Start:0x%x Size:0x%x @ MIU%d  use for %s\n",CMA_Pool_Init_PARAM[CMA_XC_COBUFF_MEM].heap_miu_start_offset,gSrcInfo[MAIN_WINDOW].Status2.u32PreCMAMemDualSCMSize,CMA_Pool_Init_PARAM[CMA_XC_COBUFF_MEM].miu,"SCMI1");
        }
        else
        {
            MdbPrint(paraGetInfo->u64ReqHdl,"XC_CMA_Cobuffer                    : Start:0x%x Size:0x%x @ MIU%d %s\n",CMA_Pool_Init_PARAM[CMA_XC_COBUFF_MEM].heap_miu_start_offset,0,CMA_Pool_Init_PARAM[CMA_XC_COBUFF_MEM].miu,"XC no use");
        }
    }
    else
    {
        MdbPrint(paraGetInfo->u64ReqHdl,"XC_CMA_Self                        : NOT SUPPORTED\n ");
        MdbPrint(paraGetInfo->u64ReqHdl,"XC_CMA_Cobuffer                    : NOT SUPPORTED\n ");
    }
#else
    MdbPrint(paraGetInfo->u64ReqHdl,"XC_CMA_Self                        : NOT SUPPORTED\n ");
    MdbPrint(paraGetInfo->u64ReqHdl,"XC_CMA_Cobuffer                    : NOT SUPPORTED\n ");
#endif
    MdbPrint(paraGetInfo->u64ReqHdl,"Frame count                        : %d\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK12_19_L, 0x000F));
    MdbPrint(paraGetInfo->u64ReqHdl,"RW diff count                      : %d\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK12_07_L, 0xE000)>>13));
    MdbPrint(paraGetInfo->u64ReqHdl,"2-steps scaling status             : %s\n", (MApi_XC_IsSupport2StepScaling()==TRUE)?"TRUE":"FALSE");
    u32address = (MS_U32)((SC_R2BYTE(MAIN_WINDOW,REG_SC_BK12_09_L))<<16) | (MS_U32)(SC_R2BYTE(MAIN_WINDOW,REG_SC_BK12_08_L));
    _phy_to_miu_offset(u8sel, u8temp, u32address);
    MdbPrint(paraGetInfo->u64ReqHdl,"Scaler MIU number                  : MIU%d\n", u8sel);
    MdbPrint(paraGetInfo->u64ReqHdl,"Memory IPM address                 : 0x%lx\n", u32address);
    u32address = (MS_U32)((SC_R2BYTE(MAIN_WINDOW,REG_SC_BK12_11_L))<<16) | (MS_U32)(SC_R2BYTE(MAIN_WINDOW,REG_SC_BK12_10_L));
    MdbPrint(paraGetInfo->u64ReqHdl,"Memory OPM address                 : 0x%lx\n", u32address);
    MdbPrint(paraGetInfo->u64ReqHdl,"IPM Offset                         : %d\n", SC_R2BYTE(MAIN_WINDOW,REG_SC_BK12_0E_L));
    MdbPrint(paraGetInfo->u64ReqHdl,"IPM Fetch                          : %d\n", SC_R2BYTE(MAIN_WINDOW,REG_SC_BK12_0F_L));
    MdbPrint(paraGetInfo->u64ReqHdl,"OPM Offset                         : %d\n", SC_R2BYTE(MAIN_WINDOW,REG_SC_BK12_16_L));
    MdbPrint(paraGetInfo->u64ReqHdl,"OPM Fetch                          : %d\n", SC_R2BYTE(MAIN_WINDOW,REG_SC_BK12_17_L));
#ifdef UFO_XC_SUPPORT_DUAL_MIU
    SC_MIUSEL_t stMIUSel;
    memset(&stMIUSel, 0, sizeof(SC_MIUSEL_t));
    u32address = (MS_U32)((SC_R2BYTE(MAIN_WINDOW,REG_SC_BK52_09_L))<<16) | (MS_U32)(SC_R2BYTE(MAIN_WINDOW,REG_SC_BK52_08_L));
    _phy_to_miu_offset(u8sel, u8temp, u32address);
    Hal_SC_get_dual_miusel(pInstance,&stMIUSel);
    u8sel = stMIUSel.u8MainFBSel;
    MdbPrint(paraGetInfo->u64ReqHdl,"Scaler Dual MIU number             : MIU%d\n", u8sel);
    MdbPrint(paraGetInfo->u64ReqHdl,"Dual Memory IPM address            : 0x%lx\n", u32address);
    u32address = (MS_U32)((SC_R2BYTE(MAIN_WINDOW,REG_SC_BK52_11_L))<<16) | (MS_U32)(SC_R2BYTE(MAIN_WINDOW,REG_SC_BK52_10_L));
    MdbPrint(paraGetInfo->u64ReqHdl,"Dual Memory OPM address            : 0x%lx\n", u32address);
    MdbPrint(paraGetInfo->u64ReqHdl,"Dual IPM Offset                    : %d\n", SC_R2BYTE(MAIN_WINDOW,REG_SC_BK52_0E_L));
    MdbPrint(paraGetInfo->u64ReqHdl,"Dual IPM Fetch                     : %d\n", SC_R2BYTE(MAIN_WINDOW,REG_SC_BK52_0F_L));
    MdbPrint(paraGetInfo->u64ReqHdl,"Dual OPM Offset                    : %d\n", SC_R2BYTE(MAIN_WINDOW,REG_SC_BK52_16_L));
    MdbPrint(paraGetInfo->u64ReqHdl,"DUal OPM Fetch                     : %d\n", SC_R2BYTE(MAIN_WINDOW,REG_SC_BK52_17_L));
#else
    MdbPrint(paraGetInfo->u64ReqHdl,"Scaler Dual MIU number             : NOT SUPPORTED\n");
    MdbPrint(paraGetInfo->u64ReqHdl,"Dual Memory IPM address            : NOT SUPPORTED\n");
    MdbPrint(paraGetInfo->u64ReqHdl,"Dual Memory OPM address            : NOT SUPPORTED\n");
    MdbPrint(paraGetInfo->u64ReqHdl,"Dual IPM Offset                    : NOT SUPPORTED\n");
    MdbPrint(paraGetInfo->u64ReqHdl,"Dual IPM Fetch                     : NOT SUPPORTED\n");
    MdbPrint(paraGetInfo->u64ReqHdl,"Dual OPM Offset                    : NOT SUPPORTED\n");
    MdbPrint(paraGetInfo->u64ReqHdl,"DUal OPM Fetch                     : NOT SUPPORTED\n");
#endif
    ...
    MdbPrint(paraGetInfo->u64ReqHdl,"Scart CVBS/RGB setting             : %s\n", ((MDrv_ReadByte(0x10258B) & 0x50)==0x10)?"RGB":"CVBS");
    MdbPrint(paraGetInfo->u64ReqHdl,"FastBlanking Input                 : 0x%x\n",R2BYTEMSK(0x102580,0xF0)>>4);
    MdbPrint(paraGetInfo->u64ReqHdl,"Enable TCON Panel RGBW             : %s\n", (R2BYTEMSK(0x331020,BIT(0))?"TRUE":"FALSE"));
    MdbPrint(paraGetInfo->u64ReqHdl,"Enable Panel Gamma                 : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK24_6D_L,BIT(12))>>12?"TRUE":"FALSE"));
    MdbPrint(paraGetInfo->u64ReqHdl,"Enable PQ Gamma                    : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_50_L,BIT(0))?"TRUE":"FALSE"));
    MdbPrint(paraGetInfo->u64ReqHdl,"H Mirror mode                      : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK12_03_L,BIT(12))?"TRUE":"FALSE"));
    MdbPrint(paraGetInfo->u64ReqHdl,"V Mirror mode                      : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK12_03_L,BIT(13))?"TRUE":"FALSE"));
    MdbPrint(paraGetInfo->u64ReqHdl,"Display Trigger Pointer            :Auto tune area: %d Display area:%d \n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK20_1A_L, 0x0FFF),SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK20_1B_L, 0x0FFF));
    MdbPrint(paraGetInfo->u64ReqHdl,"ADC ISOG Mode                      : %s\n", (R2BYTEMSK(0x1025D8,BIT(12))==BIT(12))?"NORMAL MODE":"STANDBY MODE");
    MdbPrint(paraGetInfo->u64ReqHdl,"Scaling Filter Status              : %x\n", SC_R2BYTE(MAIN_WINDOW,REG_SC_BK23_0B_L));

}
static void _MHal_XCMdbloctl_Cat_ACE(MDBCMD_GETINFO_PARAMETER *paraGetInfo)
{
    MS_U16 u16HStart, u16HEnd, u16VStart, u16VEnd;

    MdbPrint(paraGetInfo->u64ReqHdl,"----------------  Mstar Scaler ACE Info -----------------\n");

    MdbPrint(paraGetInfo->u64ReqHdl,"Brightness_R                       : %d\n", MApi_XC_ACE_GetACEInfo(MAIN_WINDOW, E_XC_ACE_INFO_TYPE_BRIGHTNESS_R));
    MdbPrint(paraGetInfo->u64ReqHdl,"Brightness_G                       : %d\n", MApi_XC_ACE_GetACEInfo(MAIN_WINDOW, E_XC_ACE_INFO_TYPE_BRIGHTNESS_G));
    MdbPrint(paraGetInfo->u64ReqHdl,"Brightness_B                       : %d\n", MApi_XC_ACE_GetACEInfo(MAIN_WINDOW, E_XC_ACE_INFO_TYPE_BRIGHTNESS_B));
    MdbPrint(paraGetInfo->u64ReqHdl,"Contrast                           : %d\n", MApi_XC_ACE_GetACEInfo(MAIN_WINDOW, E_XC_ACE_INFO_TYPE_CONTRAST));
    MdbPrint(paraGetInfo->u64ReqHdl,"Saturation                         : %d\n", MApi_XC_ACE_GetACEInfo(MAIN_WINDOW, E_XC_ACE_INFO_TYPE_SATURATION));
    MdbPrint(paraGetInfo->u64ReqHdl,"Hue                                : %d\n", MApi_XC_ACE_GetACEInfo(MAIN_WINDOW, E_XC_ACE_INFO_TYPE_HUE));

    MdbPrint(paraGetInfo->u64ReqHdl,"DLC status                         : %s\n", (R2BYTEMSK(0x141A08UL,BIT(7))==BIT(7))?"TRUE":"FALSE");
    u16HStart = SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK5C_04_L, 0x0FFF);
    u16HEnd = SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK5C_04_L, 0x0FFF)+1;
    u16VStart = (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK5C_04_L, 0x0FFF)+1)*8;
    u16VEnd = (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK5C_04_L, 0x0FFF)+1)*8/7;
    MdbPrint(paraGetInfo->u64ReqHdl,"DLC window                         : (%d,%d) %dx%d\n", u16HStart, u16VStart, u16HEnd-u16HStart, u16VEnd-u16VStart);
}
static void _MHal_XCMdbloctl_Cat_DISP(void *pInstance, MDBCMD_GETINFO_PARAMETER *paraGetInfo)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
    ...
    MdbPrint(paraGetInfo->u64ReqHdl,"Window enable                      : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK20_10_L,BIT(0))==BIT(0))?"TRUE":"FALSE"); //main\n");
    MdbPrint(paraGetInfo->u64ReqHdl,"Mute enable                        : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_19_L, BIT(1))==BIT(1))?"TRUE":"FALSE");
    MdbPrint(paraGetInfo->u64ReqHdl,"SubWindow Mute enable              : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_19_L, BIT(5))==BIT(5))?"TRUE":"FALSE");
    ...
    if((int)pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.eLPLL_Type == (int)E_XC_PNL_LPLL_VBY1_10BIT_4LANE ||
        (int)pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.eLPLL_Type == (int)E_XC_PNL_LPLL_VBY1_8BIT_4LANE)
    {
        MdbPrint(paraGetInfo->u64ReqHdl,"Output type                        : VB1_4lane\n");
    }
    else if((int)pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.eLPLL_Type == (int)E_XC_PNL_LPLL_VBY1_10BIT_8LANE ||
        (int)pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.eLPLL_Type == (int)E_XC_PNL_LPLL_VBY1_8BIT_8LANE)
    {
        MdbPrint(paraGetInfo->u64ReqHdl,"Output type                        : VB1_8lane\n");
    }
    else if((int)pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.eLPLL_Type == (int)E_XC_PNL_LPLL_VBY1_10BIT_2LANE ||
        (int)pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.eLPLL_Type == (int)E_XC_PNL_LPLL_VBY1_8BIT_2LANE)
    {
        MdbPrint(paraGetInfo->u64ReqHdl,"Output type                        : VB1_2lane\n");
    }
    else if((int)pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.eLPLL_Type == (int)E_XC_PNL_LPLL_LVDS)
    {
    ...
    else if(IsSrcTypeATV(gSrcInfo[MAIN_WINDOW].enInputSourceType))
    {
        MdbPrint(paraGetInfo->u64ReqHdl,"Input source type                  : ATV\n");
    }
    else if(IsSrcTypeDTV(gSrcInfo[MAIN_WINDOW].enInputSourceType))
    {
        MdbPrint(paraGetInfo->u64ReqHdl,"Input source type                  : DTV\n");
    }
    else if(IsSrcTypeStorage(gSrcInfo[MAIN_WINDOW].enInputSourceType))
    {
        MdbPrint(paraGetInfo->u64ReqHdl,"Input source type                  : MM\n");
    }
    else if(IsSrcTypeVga(gSrcInfo[MAIN_WINDOW].enInputSourceType))
    {
    ...
    MdbPrint(paraGetInfo->u64ReqHdl,"Source resolution                  : %d_%d_%d_%c\n", gSrcInfo[MAIN_WINDOW].stCapWin.width, gSrcInfo[MAIN_WINDOW].stCapWin.height
        , gSrcInfo[MAIN_WINDOW].u16InputVFreq, (gSrcInfo[MAIN_WINDOW].bInterlace==TRUE)?'I':'P');
    MdbPrint(paraGetInfo->u64ReqHdl,"Input I/P mode status              : %s\n", (gSrcInfo[MAIN_WINDOW].bInterlace==TRUE)?"Interlace":"Progressive");
    MdbPrint(paraGetInfo->u64ReqHdl,"Memory color space                 : %s\n", (gSrcInfo[MAIN_WINDOW].bUseYUVSpace==TRUE)?"YUV":"RGB");
    MdbPrint(paraGetInfo->u64ReqHdl,"Display size                       : %d_%d\n", SC_R2BYTE(MAIN_WINDOW,REG_SC_BK10_09_L)-SC_R2BYTE(MAIN_WINDOW,REG_SC_BK10_08_L)+1,
    SC_R2BYTE(MAIN_WINDOW,REG_SC_BK10_0B_L)-SC_R2BYTE(MAIN_WINDOW,REG_SC_BK10_0A_L)+1);

     if(gSrcInfo[MAIN_WINDOW].Status2.eMirrorMode == MIRROR_NORMAL)
    {
         MdbPrint(paraGetInfo->u64ReqHdl,"Mirror mode                        : FALSE\n");
    }
    else if(gSrcInfo[MAIN_WINDOW].Status2.eMirrorMode == MIRROR_H_ONLY)
    {
         MdbPrint(paraGetInfo->u64ReqHdl,"Mirror mode                        : H-Mirror\n");
    }
    else if(gSrcInfo[MAIN_WINDOW].Status2.eMirrorMode == MIRROR_V_ONLY)
    {
         MdbPrint(paraGetInfo->u64ReqHdl,"Mirror mode                        : V-Mirror\n");
    }
    else if(gSrcInfo[MAIN_WINDOW].Status2.eMirrorMode == MIRROR_HV)
    {
         MdbPrint(paraGetInfo->u64ReqHdl,"Mirror mode                        : HV-Mirror\n");
    }
    else
    {
         MdbPrint(paraGetInfo->u64ReqHdl,"Mirror mode                        : Not Support\n");
    }
    ...
    MdbPrint(paraGetInfo->u64ReqHdl,"Scaler dual miu                    : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK11_29_L,BIT(15))==BIT(15))?"TRUE":"FALSE");
    MdbPrint(paraGetInfo->u64ReqHdl,"Scaler BWD                         : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK46_01_L, 0xFFFF) != 0)?"TRUE":"FALSE");
    if (pXCResourcePrivate->sthal_SC.bIsConfigCMA == TRUE)
    {
    ...
    MdbPrint(paraGetInfo->u64ReqHdl,"Scaler auto no signal status       : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK01_02_L,BIT(7))==BIT(7))?"TRUE":"FALSE");
    MdbPrint(paraGetInfo->u64ReqHdl,"DS status                          : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK1F_10_L,BIT(9))==BIT(9) && SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK1F_10_L,BIT(11))==BIT(11))?"TRUE":"FALSE");
    u32address = (MS_U32)((SC_R2BYTE(MAIN_WINDOW,REG_SC_BK1F_12_L))<<16) | (MS_U32)(SC_R2BYTE(MAIN_WINDOW,REG_SC_BK1F_11_L));
    MdbPrint(paraGetInfo->u64ReqHdl,"DS address                         : 0x%lx\n", u32address);
    _phy_to_miu_offset(u8sel, u8temp, pXCResourcePrivate->sthal_SC.u32DynamicScalingBaseAddr);
    MdbPrint(paraGetInfo->u64ReqHdl,"DS MIU number                      : MIU%d\n", u8sel);
    MdbPrint(paraGetInfo->u64ReqHdl,"DS MIU from register              : MIU%d\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK1F_10_L, BIT(13))>>13);
    MdbPrint(paraGetInfo->u64ReqHdl,"DS buffer size                     : 0x%x\n", pXCResourcePrivate->sthal_SC.u32DynamicScalingBufSize);
    u32address = (MS_U32)((SC_R2BYTE(MAIN_WINDOW,REG_SC_BK1F_0F_L))<<16) | (MS_U32)(SC_R2BYTE(MAIN_WINDOW,REG_SC_BK1F_0E_L));
    MdbPrint(paraGetInfo->u64ReqHdl,"DS OP address                      : 0x%lx\n", u32address);
    MdbPrint(paraGetInfo->u64ReqHdl,"DS depth                           : %d\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK1F_13_L, 0x00FF));
    MdbPrint(paraGetInfo->u64ReqHdl,"MVOP Crop(x:0x%x y:0x%x width:0x%x height:0x%x)\n", R2BYTE(0x101480), R2BYTE(0x101482), R2BYTE(0x101484), R2BYTE(0x101486));
    //DS toggle CMD
    MdbPrint(paraGetInfo->u64ReqHdl,"DS toggle CMD                      : 0x%x\n", SC_R2BYTE(MAIN_WINDOW,REG_SC_BK1F_17_L));
    MdbPrint(paraGetInfo->u64ReqHdl,"BIT MASK                           : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK1F_07_L,BIT(15))==BIT(15))?"ON":"OFF");
    //ForceP
#ifdef XC_DS_VERSION
    MdbPrint(paraGetInfo->u64ReqHdl,"DS version                         : %d\n", XC_DS_VERSION);
#endif
    MdbPrint(paraGetInfo->u64ReqHdl,"ForceP enable(main window)         : %s\n", (gSrcInfo[MAIN_WINDOW].Status2.bEnable_ForceP)?"ON":"OFF");
    MdbPrint(paraGetInfo->u64ReqHdl,"Field array                        : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK11_07_L,BIT(12))==BIT(12))?"ON":"OFF");
    MdbPrint(paraGetInfo->u64ReqHdl,"Field direct mode                  : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK02_10_L,BIT(5))==BIT(5))?"ON":"OFF");
    // Get DS Info
    XC_GET_DS_INFO stGetDSInfo;
    MApi_XC_GetDSInfo(&stGetDSInfo, sizeof(XC_GET_DS_INFO), MAIN_WINDOW);
    MdbPrint(paraGetInfo->u64ReqHdl,"DynamicScalingEnable(main)         : %d\n", stGetDSInfo.bDynamicScalingEnable);
    MdbPrint(paraGetInfo->u64ReqHdl,"bUpdate_DS_CMD(main)               : %d\n", stGetDSInfo.bUpdate_DS_CMD);
    MApi_XC_GetDSInfo(&stGetDSInfo, sizeof(XC_GET_DS_INFO), SUB_WINDOW);
    MdbPrint(paraGetInfo->u64ReqHdl,"DynamicScalingEnable(sub)          : %d\n", stGetDSInfo.bDynamicScalingEnable);
    MdbPrint(paraGetInfo->u64ReqHdl,"bUpdate_DS_CMD(sub)                : %d\n", stGetDSInfo.bUpdate_DS_CMD);
    MdbPrint(paraGetInfo->u64ReqHdl,"Main DS get Index from             : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK1F_13_L,BIT(9))==BIT(9))?"sub":"main");
    MdbPrint(paraGetInfo->u64ReqHdl,"Sub DS get Index from              : %s\n", (SC_R2BYTEMSK(SUB_WINDOW,REG_SC_BK1F_13_L,BIT(9))==BIT(9))?"sub":"main");
    MdbPrint(paraGetInfo->u64ReqHdl,"SC0 IP MUX                         : %x\n", SC_R2BYTE(MAIN_WINDOW,REG_IPMUX_01_L));
    MdbPrint(paraGetInfo->u64ReqHdl,"SC2 IP MUX                         : %x\n", SC_R2BYTE(MAIN_WINDOW,REG_IPMUX_61_L));

    //BWS:
    char BWSType[20] = {0};

    if(SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK08_01_L,BIT(0)) == 1)
    {
        strcpy(BWSType,"OFF");
    }
    else
    {
        if(SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK08_10_L,BIT(0)) == 1)
        {
              strcpy(BWSType,"8BIT");
        }
        else
    ...
    MdbPrint(paraGetInfo->u64ReqHdl,"BWS         : %s\n", BWSType);
    MdbPrint(paraGetInfo->u64ReqHdl,"Menuload status                    : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK1F_02_L,BIT(15))==BIT(15))?"TRUE":"FALSE");
    u32address = (MS_U32)((SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK1F_04_L, 0x3FF))<<16) | (MS_U32)(SC_R2BYTE(MAIN_WINDOW,REG_SC_BK1F_03_L));
    MdbPrint(paraGetInfo->u64ReqHdl,"Menuload address                   : 0x%lx\n", u32address);
    _phy_to_miu_offset(u8sel, u8temp, u32address);
    MdbPrint(paraGetInfo->u64ReqHdl,"Menuload MIU number                : MIU%d\n", u8sel);
    MdbPrint(paraGetInfo->u64ReqHdl,"2P mode status                     : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK01_07_L,BIT(15))==BIT(15))?"TRUE":"FALSE");
    MdbPrint(paraGetInfo->u64ReqHdl,"Sub 2P mode status                 : %s\n", (SC_R2BYTEMSK(SUB_WINDOW, REG_SC_BK01_07_L , BIT(15))==BIT(15))?"TRUE":"FALSE");
    ...
    MdbPrint(paraGetInfo->u64ReqHdl,"Freeze                             : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK12_01_L,BIT(11))==BIT(11))?"TRUE":"FALSE");
    MdbPrint(paraGetInfo->u64ReqHdl,"Offline detect                     : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,0x257EUL,BIT(1))==0)?"TRUE":"FALSE");
    ...
    MdbPrint(paraGetInfo->u64ReqHdl,"Local dimming                      : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK2E_01_L,BIT(0))==BIT(0))?"TRUE":"FALSE");
    MdbPrint(paraGetInfo->u64ReqHdl,"Disable input source               : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK01_02_L,BIT(7))==BIT(7))?"TRUE":"FALSE");
    MdbPrint(paraGetInfo->u64ReqHdl,"Scaler IRQ status (Bit 4)          : %s (OP Vsync)\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK00_14_L,BIT(4))==BIT(4))?"TRUE":"FALSE");
    MdbPrint(paraGetInfo->u64ReqHdl,"Scaler IRQ status (Bit 11)         : %s (IP1 Vsync)\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK00_14_L,BIT(11))==BIT(11))?"TRUE":"FALSE");
    MdbPrint(paraGetInfo->u64ReqHdl,"Scaler IRQ status (Bit 21)         : %s (HDR DMA)\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK00_15_L,BIT(5))==BIT(5))?"TRUE":"FALSE");
    MdbPrint(paraGetInfo->u64ReqHdl,"Scaler IRQ status (Bit 22)         : %s (HDR Descramble)\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK00_15_L,BIT(6))==BIT(6))?"TRUE":"FALSE");
    MdbPrint(paraGetInfo->u64ReqHdl,"Scaler DI status                   : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK20_01_L,BIT(1))==BIT(1))?"TRUE":"FALSE");
    MdbPrint(paraGetInfo->u64ReqHdl,"HDMI 444 to 422                    : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK01_61_L,BIT(0))==BIT(0))?"ON":"BYPASS");
    MdbPrint(paraGetInfo->u64ReqHdl,"VOP Color matrix                   : %x %x %x\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_26_L,0xFFFF),SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_27_L,0xFFFF),SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_28_L,0xFFFF));
    MdbPrint(paraGetInfo->u64ReqHdl,"                                   : %x %x %x\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_29_L,0xFFFF),SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_2A_L,0xFFFF),SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_2B_L,0xFFFF));
    MdbPrint(paraGetInfo->u64ReqHdl,"                                   : %x %x %x\n", SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_2C_L,0xFFFF),SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_2D_L,0xFFFF),SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_2E_L,0xFFFF));
    if((pXCResourcePrivate->stdrvXC_MVideo.gu16HPixelShiftRange !=0) && (pXCResourcePrivate->stdrvXC_MVideo.gu16VPixelShiftRange !=0))
    {
       MdbPrint(paraGetInfo->u64ReqHdl,"Scaler PixelShift status           : TRUE H:%d V:%d\n",(int)pXCResourcePrivate->stdrvXC_MVideo.s8HPixelShift,(int)pXCResourcePrivate->stdrvXC_MVideo.s8VPixelShift);
    }
    else
    {
       MdbPrint(paraGetInfo->u64ReqHdl,"Scaler PixelShift status           : FALSE\n");
    }
    MdbPrint(paraGetInfo->u64ReqHdl,"OP write                        : %s\n", (SC_R2BYTEMSK(MAIN_WINDOW,REG_SC_BK12_27_L,BIT(0))==BIT(0))?"DISABLE":"ENABLE");
}

static void _XCMdbloctl_TestPattern(MS_U8 u8IP, MS_BOOL bEnable, MS_U8 u8window, MS_U16 BValue, MS_U16 GValue, MS_U16 RValue)
{
    ...
    if(u8IP == 0)
    {
        ...
        ipmux_test_pattern.u16R_CR_Data = RValue;
        ipmux_test_pattern.u16G_Y_Data  = GValue;
        ipmux_test_pattern.u16B_CB_Data = BValue;
    ...
    else if(u8IP == 4)
    {
        ...
        vop2_test_pattern.u16B_CB_Data = BValue;
        vop2_test_pattern.u16G_Y_Data= GValue;
        vop2_test_pattern.u16R_CR_Data= RValue;
    ...
    else if(u8IP == 6)
    {
        ...
        mod_test_pattern.u16B_CB_Data = BValue;
        mod_test_pattern.u16G_Y_Data= GValue;
        mod_test_pattern.u16R_CR_Data= RValue;
        MApi_XC_GenerateTestPattern(E_XC_MOD_PATTERN_MODE,(void*)&mod_test_pattern,sizeof(XC_SET_MOD_TESTPATTERN_t));
    }
#endif
}
static void _MHal_XCMdbIoctl_EchoCmd(MDBCMD_CMDLINE_PARAMETER *paraCmdLine)
{
    char pch[] = " =";
    ...
    if(strcmp("pattern", psep) == 0)
    {
        ...
        if(psep == NULL)
        {
            _XCMdbloctl_TestPattern(u8IP, bEnable, u8window, 0, 0, 0);
            return;
        }
        psep = strsep(&pcCmdLine,pch);
        if(psep == NULL)
        ...
        RValue = (MS_U16)atoi(psep);
        _XCMdbloctl_TestPattern(u8IP, bEnable, u8window, BValue, GValue, RValue);
    }
    else if(strcmp("panel_gamma", psep) == 0)
    {
        psep = strsep(&pcCmdLine,pch);
        if(psep == NULL)
        {
        ...
        if(strcmp("ON", psep) == 0)
        {
            SC_W2BYTEMSK(MAIN_WINDOW,REG_SC_BK24_6D_L,BIT(12),BIT(12));
        }
        else if(strcmp("OFF", psep) == 0)
        {
            SC_W2BYTEMSK(MAIN_WINDOW,REG_SC_BK24_6D_L,0,BIT(12));
        }
    }
    else if(strcmp("pq_gamma", psep) == 0)
    {
        psep = strsep(&pcCmdLine,pch);
        if(psep == NULL)
        {
        ...
        if(strcmp("ON", psep) == 0)
        {
            SC_W2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_50_L,BIT(0),BIT(0));
        }
        else if(strcmp("OFF", psep) == 0)
        {
            SC_W2BYTEMSK(MAIN_WINDOW,REG_SC_BK10_50_L,0,BIT(0));
        }
    }
    else if(strcmp("ReadRequest", psep) == 0) //ReadRequest
    {
        psep = strsep(&pcCmdLine,pch);
        if(psep == NULL)
        {
        ...
        if(strcmp("ON", psep) == 0)
        {
            SC_W2BYTEMSK(MAIN_WINDOW,REG_SC_BK12_07_L,0,BIT(1));
        }
        else if(strcmp("OFF", psep) == 0)
        {
            SC_W2BYTEMSK(MAIN_WINDOW,REG_SC_BK12_07_L,BIT(1),BIT(1));
        }
    }
    else if(strcmp("WriteRequest", psep) == 0) //WriteRequest
    {
        psep = strsep(&pcCmdLine,pch);
        if(psep == NULL)
        {
        ...
        if(strcmp("ON", psep) == 0)
        {
            SC_W2BYTEMSK(MAIN_WINDOW,REG_SC_BK12_07_L,0,BIT(0));
        }
        else if(strcmp("OFF", psep) == 0)
        {
            SC_W2BYTEMSK(MAIN_WINDOW,REG_SC_BK12_07_L,BIT(0),BIT(0));
        }
    }
    else if(strcmp("Framecount", psep) == 0) //Framecount
    {
        psep = strsep(&pcCmdLine,pch);
        if(psep == NULL)
        {
        ...
        MS_U8 u8FrameCount = (MS_U8)atoi(psep);
        SC_W2BYTEMSK(MAIN_WINDOW,REG_SC_BK12_19_L,u8FrameCount,0x1F);
    }
    else if(strcmp("R/W diff count", psep) == 0) //R/W diff count
    {
        psep = strsep(&pcCmdLine,pch);
        if(psep == NULL)
        {
        ...
        MS_U32 u32diffcount = (MS_U32)atoi(psep);
        SC_W2BYTEMSK(MAIN_WINDOW,REG_SC_BK12_07_L,u32diffcount<<13,0xE000);
    }
    else if(strcmp("forceread", psep) == 0) //forceread
    {
        psep = strsep(&pcCmdLine,pch);
        if(psep == NULL)
        {
        ...
        if(strcmp("ON", psep) == 0)
        {
            SC_W2BYTEMSK(MAIN_WINDOW,REG_SC_BK12_07_L,BIT(11),BIT(11));
        }
        else if(strcmp("OFF", psep) == 0)
        {
            SC_W2BYTEMSK(MAIN_WINDOW,REG_SC_BK12_07_L,0,BIT(11));
        }
    }
    else if(strcmp("forcereadframe", psep) == 0) //forcereadframe
    {
        psep = strsep(&pcCmdLine,pch);
        if(psep == NULL)
        {
        ...
        MS_U32 u32forcereadframe = (MS_U32)atoi(psep);
        SC_W2BYTEMSK(MAIN_WINDOW,REG_SC_BK12_07_L,u32forcereadframe<<8,0x700);
    }
    else if(strcmp("FDmask", psep) == 0) //FDmask
    {
        psep = strsep(&pcCmdLine,pch);
        if(psep == NULL)
        {
        ...
        if(strcmp("ON", psep) == 0)
        {
            SC_W2BYTEMSK(MAIN_WINDOW,REG_SC_BK01_26_L,BIT(8),BIT(8));
        }
        else if(strcmp("OFF", psep) == 0)
        {
            SC_W2BYTEMSK(MAIN_WINDOW,REG_SC_BK01_26_L,0,BIT(8));
        }
    }
    else if(strcmp("UCNR", psep) == 0) //UCNR
    {
        psep = strsep(&pcCmdLine,pch);
        if(psep == NULL)
        {
        ...
        if(strcmp("ON", psep) == 0)
        {
            SC_W2BYTEMSK(MAIN_WINDOW,REG_SC_BK12_27_L,0,BIT(0));
        }
        else if(strcmp("OFF", psep) == 0)
        {
            SC_W2BYTEMSK(MAIN_WINDOW,REG_SC_BK12_27_L,BIT(0),BIT(0));
        }
    }
    else if(strcmp("Motion", psep) == 0) //Motion
    {
        psep = strsep(&pcCmdLine,pch);
        if(psep == NULL)
        {
        ...
        if(strcmp("ON", psep) == 0)
        {
            SC_W2BYTE(MAIN_WINDOW, REG_SC_BK22_78_L,0x8F8F);
        }
        else if(strcmp("OFF", psep) == 0)
        {
            SC_W2BYTE(MAIN_WINDOW, REG_SC_BK22_78_L,0x8080);
        }
    }
    else if(strcmp("LPLL", psep) == 0) //LPLL setting
    {
        psep = strsep(&pcCmdLine,pch);
        if(psep == NULL)
        {
        ...
        MS_U32 u32LPLLSet = (MS_U32)atoi(psep);
        W4BYTE(L_BK_LPLL(0x0F),u32LPLLSet);
    }
    else if(strcmp("GOPOn", psep) == 0) //GOP
    {
        MS_U8 u8GOPNum = 0;
        psep = strsep(&pcCmdLine,pch);
        if(psep == NULL)
        ...
        u8GOPNum = (MS_U8)atoi(psep);
        u8GOPNum = 15-u8GOPNum;
        psep = strsep(&pcCmdLine,pch);
        if(psep == NULL)
        {
            return;
        ...
        MS_BOOL bEnable = (MS_BOOL)atoi(psep);
        if(bEnable)
        {
            SC_W2BYTEMSK(MAIN_WINDOW,REG_SC_BK00_06_L,BIT(u8GOPNum),BIT(u8GOPNum));
        }
        else
        {
            SC_W2BYTEMSK(MAIN_WINDOW,REG_SC_BK00_06_L,0,BIT(u8GOPNum));
        }
    }
    else if(strcmp("DebugLevel", psep) == 0) //DebugLevel
    {
        static MS_U16 u16Level = 0;
        psep = strsep(&pcCmdLine,pch);
        if(psep == NULL)
        {
            return;
        ...
        MS_U8 u16templevel = (MS_U8)atoi(psep);
        psep = strsep(&pcCmdLine,pch);
        if(psep == NULL)
        {
            return;
        ...
        if(bEnable)
        {
            u16Level = u16Level | BIT(u16templevel);
        }
        else
        {
            u16Level = u16Level & (~BIT(u16templevel));
        }
        MApi_XC_SetDbgLevel(u16Level);
    }
    else if(strcmp("help", psep) == 0) //help
    {
        MdbPrint(paraCmdLine->u64ReqHdl, "------------------- Mstar XC help --------------------\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "==================Test Pattern==================\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "ex: echo pattern=2 enable=1 window=0\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "pattern=0~6, enable=0/1, window=0/1\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "====================PQ Gamma====================\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "ex: echo pq_gamma=ON\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "pq_gamma=ON/OFF\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "===================Panel Gamma==================\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "ex: echo panel_gamma=ON\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "panel_gamma=ON/OFF\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "===============Read/Write Request===============\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "ex: echo ReadRequest=ON\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "ex: echo WriteRequest=ON\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "ReadRequest=ON/OFF, WriteRequest=ON/OFF\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "===================Frame Count==================\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "ex: echo Framecount=4\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "Framecount=3/4/5/6/7/8\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "==================R/W diff count================\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "ex: echo RWcount=4\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "RWcount=0/1/2/3/4\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "====================Force Read==================\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "ex: echo forceread=ON\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "ex: echo forcereadframe=4\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "forceread=ON/OFF, forcereadframe=0/1/2/3/4\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "======================FDMask====================\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "ex: echo FDMask=ON\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "FDMask=ON/OFF\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "======================UCNR======================\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "ex: echo UCNR=ON\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "UCNR=ON/OFF\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "======================Motion======================\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "ex: echo Motion=ON\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "Motion=ON/OFF\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "======================GOP======================\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "ex: echo GOPOn num=2 enable=ON\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "num=0/1/2/3\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "enable=0/1\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "======================Debug Level======================\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "ex: echo DebugLevel switch=2 enable=1\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "switch : 0~14");
        MdbPrint(paraCmdLine->u64ReqHdl, "enable=0/1\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "======================LPLL======================\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "ex: echo LPLL=0x251254\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "LPLL=0x251254\n");
        MdbPrint(paraCmdLine->u64ReqHdl, "------------------------------------------------------\n");
    }
}
void SYMBOL_WEAK MHal_XCMdbIoctl_EchoCmd(MDBCMD_CMDLINE_PARAMETER *paraCmdLine)
{
    _MHal_XCMdbIoctl_EchoCmd(paraCmdLine);
}
void SYMBOL_WEAK MHal_XCMdbIoctl_Cat(MDBCMD_GETINFO_PARAMETER *paraGetInfo)
{
    ...
    _MHal_XCMdbloctl_Cat_DISP(pInstance, paraGetInfo);
    _MHal_XCMdbloctl_Cat_ACE(paraGetInfo);
    _MHal_XCMdbloctl_Cat_SetWindow(pInstance, paraGetInfo);
    _MHal_XCMdbloctl_Cat_FrameLock(pInstance, paraGetInfo);
    _MHal_XCMdbloctl_Cat_MEMC(pInstance, paraGetInfo);
    _MHal_XCMdbloctl_Cat_HDR(pInstance, paraGetInfo);
    _MHal_XCMdbloctl_Cat_TGEN(pInstance, paraGetInfo);
    _MHal_XCMdbloctl_Cat_MOD(pInstance, paraGetInfo);
    _MHal_XCMdbloctl_Cat_3D(pInstance, paraGetInfo);
    _MHal_XCMdbloctl_Cat_OPTEE(pInstance, paraGetInfo);
}
#endif

void Hal_SC_HDR_CLK_Setting(void *pInstance)
{
    MDrv_WriteByteMask(REG_CKG_S2_MECLK_ext, 0, 0x3F);  // REG 0x100A_40[13:8] set to 0
    MDrv_WriteByteMask(REG_CKG_S2_MGCLK_ext, 0, 0x3F); // REG 0x100A_41[13:8] set to 0
    MDrv_WriteByteMask(REG_CKG_S2_GOP_HDR_ext, 0, 0x3F); // REG 0x100A_42[13:8] set to 0
}

void Hal_SC_MVOP_HandShake_Control(void * pInstance,SCALER_WIN eWindow)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
    ...
    if((psXCInstPri->u32DeviceID == 0) && (eWindow == MAIN_WINDOW))
    {
        ...
        //Always enable HSK for M7622 (MVOP source)
        if(IsSrcTypeStorage(gSrcInfo[eWindow].enInputSourceType))
        {
            bEnable = TRUE;
        }

        //printk("[%s,%5d] IsSrcTypeStorage:%d bEnable:%d  \n",__func__,__LINE__,IsSrcTypeStorage(gSrcInfo[eWindow].enInputSourceType),bEnable);

        //Set XC HSK
        // Bit0: Enable HSK mode
        // Bit2: reg_ip2_vs_sel 0:ip1 1:mvop
        // Bit5: reg_ip2_fd_sel 0:use ip1_field 1: use mvop_field
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK02_10_L,bEnable?(BIT(0)|BIT(2)|BIT(5)):0 ,BIT(0)|BIT(2)|BIT(5));
    ...
    {
        printf("Not support HSK mode with sub window ! \n");
    }
}

void Hal_SC_MVOP_Gen_Timing_From_XC_Control(void * pInstance,SCALER_WIN eWindow)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
    ...
    if((psXCInstPri->u32DeviceID == 0) && (eWindow == MAIN_WINDOW))
    {
        MS_BOOL bGenTimingFromXC = FALSE;

        //Always GenTiming from MVOP for M7622 (MVOP source)
        gSrcInfo[eWindow].Status2.bMVOPHSKMode = bGenTimingFromXC; //This variable represents bGenTimingFromXC.....

        SC_W2BYTE(psXCInstPri->u32DeviceID, REG_SC_BK10_5D_L, 0x1018);//return to HW default value
        //printk(" [%s,%5d] bGenTimingFromXC:%d  bMVOPHSKMode:%d  \n",__func__,__LINE__,bGenTimingFromXC,gSrcInfo[MAIN_WINDOW].Status2.bMVOPHSKMode);

        //Set MVOP Gen timing from XC
        if(IsSrcTypeStorage(gSrcInfo[eWindow].enInputSourceType) || IsSrcTypeDTV(gSrcInfo[eWindow].enInputSourceType))
        {
            MVOP_Handle stMvopHd = { E_MVOP_MODULE_MAIN };
            MDrv_MVOP_SetCommand(&stMvopHd, E_MVOP_CMD_SET_XC_GEN_TIMING,&bGenTimingFromXC);
        }
    }
    else
    {
        printf("Not support gen timing from XC  with sub window ! \n");
    }
}
...
void MHal_XC_SetMemoryType_ForceP(void *pInstance, MS_BOOL bInterlace)
{
}

void MHal_XC_SetVirtualBox(void *pInstance, MS_U16 *pu16VBWidth, MS_U16 *pu16VBHeight)
{
}

void MHal_XC_SetFrameNum(void *pInstance, MS_U8 *u8FrameCount)
{
}

void MHal_XC_SetForcepConfig(void *pInstance, E_XC_SCAN_TYPE eScanType)
{
}
#endif
#undef  MHAL_SC_C
