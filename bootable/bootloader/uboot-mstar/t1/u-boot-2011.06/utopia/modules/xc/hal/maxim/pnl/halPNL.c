...
#define OVERDRIVER_TABLE_COL_17 17
#define OVERDRIVER_TABLE_COL_16 16
#define OVERDRIVER_TABLE_ROW_17 17
#define OVERDRIVER_TABLE_ROW_16 16
...
void MHal_PNL_OverDriver_Init(void *pInstance, MS_PHY u32OD_MSB_Addr, MS_PHY u32OD_MSB_limit, MS_PHY u32OD_LSB_Addr, MS_PHY u32OD_LSB_limit, MS_U8 u8MIUSel)
{
    ...
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_16_L, (MS_U16)((u32OD_MSB_Addr >> 16) & 0xFFFF), 0xFFFF); // OD MSB request base address
    SC_W2BYTE(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_17_L, (MS_U16)(u32OD_MSB_limit & 0xFFFF)); // OD MSB request address limit
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_18_L, (MS_U16)((u32OD_MSB_limit >> 16) & 0xFFFF), 0xFFFF); // OD MSB request address limit

    SC_W2BYTE(pPNLInstancePrivate->u32DeviceID,REG_SC_BK16_6C_L,0x2E01);

    SC_W2BYTE(pPNLInstancePrivate->u32DeviceID,REG_SC_BK16_11_L,0x40); //OD Weight
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_12_L,0x08,0x00FF); //OD Active threshold

    SC_W2BYTE(pPNLInstancePrivate->u32DeviceID,REG_SC_BK16_2D_L,0x0303); //OD Target bit rate

    MS_PHY u32OD_MSB_PG_Addr = u32OD_MSB_Addr + ((pPNLResourcePrivate->stdrvPNL._stPnlInitData.u16Width *pPNLResourcePrivate->stdrvPNL._stPnlInitData.u16Height *24)/(8 * BYTE_PER_WORD));
    MS_U16 u16Width = pPNLResourcePrivate->stdrvPNL._stPnlInitData.u16Width / 2;
    SC_W2BYTE(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_2F_L, (MS_U16)(u16Width & 0x0FFF)); // OD horizontal size of a frame
    SC_W2BYTE(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_47_L, (MS_U16)(u32OD_MSB_PG_Addr & 0xFFFF)); // OD PG Base request base address
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_48_L, (MS_U16)((u32OD_MSB_PG_Addr >> 16) & 0xFFFF), 0xFFFF); //OD PG Base request base address
    SC_W2BYTE(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_27_L,0x3F); // OD PG Base request base address
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID,REG_SC_BK16_27_L,BIT(9),BIT(9)); //OD PG Address Limit Enable

    SC_W2BYTE(pPNLInstancePrivate->u32DeviceID,REG_SC_BK16_29_L,0x0F);
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID,REG_SC_BK16_29_L,BIT(8)|BIT(9)|BIT(10)|BIT(11),0x3F00); //OD one index containts

    SC_W2BYTE(pPNLInstancePrivate->u32DeviceID,REG_SC_BK16_19_L,0x10);
    SC_W2BYTE(pPNLInstancePrivate->u32DeviceID,REG_SC_BK16_1A_L,0x3F);
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID,REG_SC_BK16_1A_L,(0x00 << 8),(0xFF << 8));
    SC_W2BYTE(pPNLInstancePrivate->u32DeviceID,REG_SC_BK16_1B_L,0x0A);
    SC_W2BYTE(pPNLInstancePrivate->u32DeviceID,REG_SC_BK16_1C_L,0x3F);
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID,REG_SC_BK16_1C_L,(0x00 << 8),(0xFF << 8));
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID,REG_SC_BK16_28_L,0x3F,0xFF);

    SC_W2BYTE(pPNLInstancePrivate->u32DeviceID,REG_SC_BK16_55_L,0x0000);

    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID,REG_SC_BK16_32_L,FALSE,BIT(0));
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID,REG_SC_BK16_32_L,FALSE,BIT(1));

    SC_W2BYTE(pPNLInstancePrivate->u32DeviceID,REG_SC_BK16_3A_L,0x2EFF);
    SC_W2BYTE(pPNLInstancePrivate->u32DeviceID,REG_SC_BK16_39_L,0xFFFF);
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID,REG_SC_BK16_3C_L,0xFF,0xFF);
    SC_W2BYTE(pPNLInstancePrivate->u32DeviceID,REG_SC_BK16_3B_L,0xFFFF);
...
void MHal_PNL_OverDriver_Enable(void *pInstance, MS_BOOL bEnable)
{
    ...
    if (bEnable)
    {
        SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_10_L, 0x2B, 0x2F);
    }
    else
    {
        SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_10_L, 0x2A, 0x2F);
    }
}
...
void MHal_PNL_OverDriver_Setting_TBL(void *pInstance, MS_U16* pu16ODTbl,MS_U32 u32ODTabSize,MS_U16 u16SepatateMode)
{
    ...
    MS_U16 u16CodeTableIndex = 0;
    MS_U16 u16CodeCurrentTableIndex = 0;
    ...
    bEnable = SC_R2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_10_L, BIT(0));
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_10_L, 0x00, BIT(0)); // OD enable
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_01_L, u16SepatateMode, 0x0E); // OD table SRAM enable, RGB channel
    if(u32ODTabSize == OVERDRIVER_TABLE_SIZE_33X32)
    {
        u16TargetIndex = 9;
        u16Target= pu16ODTbl[u16TargetIndex];
        for(u16CodeTableIndex=0;u16CodeTableIndex<OVERDRIVER_TABLE_COL_17*OVERDRIVER_TABLE_ROW_16;u16CodeTableIndex++)
        {
                SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_03_L, (u16CodeTableIndex == u16TargetIndex)?u16Target:(u16Target ^ pu16ODTbl[u16CodeTableIndex]), 0x00FF);
                SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_02_L, (u16CodeTableIndex|0x8000), 0x81FF);
                while(SC_R2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_02_L, BIT(15)));
        }
        u16CodeCurrentTableIndex += OVERDRIVER_TABLE_COL_17*OVERDRIVER_TABLE_ROW_16;
        u16TargetIndex += 10;
        u16Target= pu16ODTbl[(u16CodeCurrentTableIndex+u16TargetIndex)];
        for (u16CodeTableIndex=0; u16CodeTableIndex<OVERDRIVER_TABLE_COL_17*OVERDRIVER_TABLE_ROW_16; u16CodeTableIndex++)
        {
            SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_06_L, (u16CodeTableIndex == u16TargetIndex)?u16Target:(u16Target ^ pu16ODTbl[(u16CodeCurrentTableIndex+u16CodeTableIndex)]), 0x00FF);
            SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_05_L, (u16CodeTableIndex|0x8000), 0x81FF);
            while(SC_R2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_05_L, BIT(15)));
        }
        u16CodeCurrentTableIndex += OVERDRIVER_TABLE_COL_17*OVERDRIVER_TABLE_ROW_16;
        u16TargetIndex += 10;
        u16Target= pu16ODTbl[(u16CodeCurrentTableIndex+u16TargetIndex)];
        for (u16CodeTableIndex=0; u16CodeTableIndex<OVERDRIVER_TABLE_COL_16*OVERDRIVER_TABLE_ROW_16; u16CodeTableIndex++)
        {
            SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_09_L, (u16CodeTableIndex == u16TargetIndex)?u16Target:(u16Target ^ pu16ODTbl[(u16CodeCurrentTableIndex+u16CodeTableIndex)]), 0x00FF);
            SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_08_L, (u16CodeTableIndex|0x8000), 0x81FF);
            while(SC_R2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_08_L, BIT(15)));
        }
        u16CodeCurrentTableIndex += OVERDRIVER_TABLE_COL_16*OVERDRIVER_TABLE_ROW_16;
        u16TargetIndex += 10;
        u16Target= pu16ODTbl[(u16CodeCurrentTableIndex+u16TargetIndex)];
        for (u16CodeTableIndex=0; u16CodeTableIndex<OVERDRIVER_TABLE_COL_16*OVERDRIVER_TABLE_ROW_16; u16CodeTableIndex++)
        {
            SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_0C_L, (u16CodeTableIndex == u16TargetIndex)?u16Target:(u16Target ^ pu16ODTbl[(u16CodeCurrentTableIndex+u16CodeTableIndex)]), 0x00FF);
            SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_0B_L, (u16CodeTableIndex|0x8000), 0x81FF);
            while(SC_R2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_0D_L, BIT(15)));
        }
    }
    else if(u32ODTabSize == OVERDRIVER_TABLE_SIZE_33X33)
    {
        u16TargetIndex = 9;
        u16Target= pu16ODTbl[u16TargetIndex];
        for(u16CodeTableIndex=0;u16CodeTableIndex<OVERDRIVER_TABLE_COL_17*OVERDRIVER_TABLE_ROW_17;u16CodeTableIndex++)
        {
            if(u16CodeTableIndex < OVERDRIVER_TABLE_COL_17*OVERDRIVER_TABLE_ROW_16)
            {
               SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_03_L, (u16CodeTableIndex == u16TargetIndex)?u16Target:(u16Target ^ pu16ODTbl[u16CodeTableIndex]), 0x00FF);
            }
            else
            {
               SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_03_L,0x0000,0x00FF);
            }
            SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_02_L, (u16CodeTableIndex|0x8000), 0x81FF);
            while(SC_R2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_02_L, BIT(15)));
        }

        u16CodeCurrentTableIndex += OVERDRIVER_TABLE_COL_17*OVERDRIVER_TABLE_ROW_17;
        u16TargetIndex += 10;
        u16Target= pu16ODTbl[(u16CodeCurrentTableIndex+u16TargetIndex)];
        for (u16CodeTableIndex=0; u16CodeTableIndex<OVERDRIVER_TABLE_COL_17*OVERDRIVER_TABLE_ROW_16; u16CodeTableIndex++)
        {
        ...
        for (u16CodeTableIndex=0; u16CodeTableIndex<OVERDRIVER_TABLE_COL_17*OVERDRIVER_TABLE_ROW_16; u16CodeTableIndex++)
        {
                if(u16CodeTableIndex < OVERDRIVER_TABLE_COL_16*OVERDRIVER_TABLE_ROW_16)
                {
                    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK16_09_L, (u16CodeTableIndex == u16TargetIndex)?u16Target:(u16Target ^ pu16ODTbl[(u16CodeCurrentTableIndex+u16CodeTableIndex)]), 0x00FF);
                }
                else
...
void MHal_PNL_Init_XC_Clk(void *pInstance, PNL_InitData *pstPanelInitData)
{
    ...
    else if ((pstPanelInitData->eLPLL_Type == E_PNL_TYPE_DAC_P) || (pstPanelInitData->eLPLL_Type == E_PNL_TYPE_DAC_I))
    {
        W2BYTEMSK(L_CLKGEN0(0x53), DISABLE, CKG_ODCLK_GATED);                          // [0] disable clock
        W2BYTEMSK(L_CLKGEN0(0x53), DISABLE, CKG_ODCLK_INVERT);                         // [1] invert clock
        W2BYTEMSK(L_CLKGEN0(0x53), CKG_ODCLK_SEL_LPLL, CKG_ODCLK_SEL_SOURCE);          // [2] select source tobe LPLL clock, 0:synthetic clock out, 1:LPLL clock out
        ...
        // For abc123 1208252: HDMI Tx frame lock issue, HW RD said 0x100A_44 should be 0x1C for HDMI Tx case
    ...
    MS_U32 u32PanelLinkType = 0;
    if(stTCONPanelConfig.bUsingTCON)
    {
        u32PanelLinkType = stTCONPanelConfig.stPanelInfo.u32PanelLinkType;
    }
    else
    {
        u32PanelLinkType = pstPanelInitData->eLPLL_Type;
    }
    if(  IsVBY1(u32PanelLinkType) )
    {
    ...
    else
    {
        ...
        if (u32PanelLinkType == E_PNL_TYPE_TTL)
        {
        ...
        else if(u32PanelLinkType == E_PNL_LPLL_ISP_8BIT_12P)
        {
        ...
        if((u32PanelLinkType == E_PNL_TYPE_HS_LVDS)&&(pstPanelInitData->eLPLL_Mode == E_PNL_MODE_SINGLE))
        {
    ...
    if( (E_PNL_TYPE_TTL == u32PanelLinkType)||
       ((E_PNL_TYPE_LVDS == u32PanelLinkType)&&(E_PNL_MODE_SINGLE==pstPanelInitData->eLPLL_Mode))||
       ((E_PNL_TYPE_HS_LVDS == u32PanelLinkType)&&(E_PNL_MODE_SINGLE==pstPanelInitData->eLPLL_Mode)))
    {
        //set LPLL mux
    ...
    else if ((E_PNL_TYPE_DAC_P == u32PanelLinkType) ||
            (E_PNL_TYPE_DAC_I == u32PanelLinkType))
    {
        //set LPLL mux
    ...
    if(u32PanelLinkType == E_PNL_LPLL_ISP_8BIT_12P)
    {
...
void MHal_PNL_Init_MOD(void *pInstance, PNL_InitData *pstPanelInitData)
{
    ...
    if(pstPanelInitData->eLPLL_Type == E_PNL_TYPE_TTL)
    {
        if(pstPanelInitData->u16MOD_CTRLA & BIT(4))
        {
            MOD_A_W2BYTEMSK(REG_MOD_A_BK00_69_L,BIT(12),BIT(12));
        }
        else
        {
            MOD_A_W2BYTEMSK(REG_MOD_A_BK00_69_L,FALSE,BIT(12));
        }
    }
    ...
    // To avoid abc123 1082875: boot logo flash issue, remove  SRAM PD for LD from XC_init to PNL_init
    if(pPNLInstancePrivate->u32DeviceID == 0)
    {
...
void MHal_PNL_PowerDownLPLL(void *pInstance, MS_BOOL bEnable)
{
    if(bEnable)
    {
