...
void HAL_XC_DIP_SetMux(void *pInstance, MS_U8 u8Data_Mux, MS_U8 u8Clk_Mux,SCALER_DIP_WIN eWindow)
{
    if( eWindow == DIP_WINDOW )
    {
        ...
        MDrv_DIP_WriteByteMask(REG_CKG_DIP, u8Clk_Mux, CKG_DIP_CLK_MASK);
    }
    else if( eWindow == DWIN0_WINDOW )
    {
        DIP_W2BYTEMSK(0, REG_SC_BK34_7E_L,u8Data_Mux << 8,BMASK(11:8));
        MDrv_DIP_WriteByteMask(REG_CKG_DWIN0, u8Clk_Mux, CKG_DWIN0_CLK_MASK);
    }
    else if( eWindow == DWIN1_WINDOW )
    {
        DIP_W2BYTEMSK(0, REG_SC_BK34_7E_L,u8Data_Mux << 12,BMASK(15:12));
        MDrv_DIP_WriteByteMask(REG_CKG_DWIN1, u8Clk_Mux, CKG_DWIN1_CLK_MASK);
    }
    else
    {
        return;
...
void HAL_XC_DIP_MuxDispatch(void *pInstance, SCALER_DIP_SOURCE_TYPE eSource,SCALER_DIP_WIN eWindow)
{
    ...
    if( eWindow == DIP_WINDOW )
    {
        ...
        {
            HAL_XC_DIP_SetMux(pInstance, DIP_DIP_MUX_IP_MAIN,CKG_DIP_CLK_IP_MAIN,eWindow);
            DIP_W2BYTEMSK(0, REG_SC_BK34_50_L, 0,BIT(15));
        }
        else if(eSource == SCALER_DIP_SOURCE_TYPE_SUB)
        {
            HAL_XC_DIP_SetMux(pInstance, DIP_DIP_MUX_IP_SUB,CKG_DIP_CLK_IP_SUB,eWindow);
            DIP_W2BYTEMSK(0, REG_SC_BK34_50_L, 0,BIT(15));
        }
        ...
        else if(eSource == SCALER_DIP_SOURCE_TYPE_OP_CAPTURE)
        {
            ...
            {
                HAL_XC_DIP_SetMux(pInstance, DIP_DIP_MUX_OP,CKG_DIP_CLK_OP,eWindow);
            }
            else
            {
                HAL_XC_DIP_SetMux(pInstance, DIP_DIP_MUX_OP,CKG_DIP_CLK_HDMI,eWindow);
            }
        }
        else if(eSource == SCALER_DIP_SOURCE_TYPE_SC2_OP_CAPTURE)
        {
            MDrv_DIP_WriteByteMask(REG_CKG_DIP_GATE, CKG_DWIN0_GATE_EN, CKG_DWIN0_GATE_EN);
            DIP_W2BYTEMSK(0, REG_SC_BK3B_70_L, BIT(5),BIT(5));
            HAL_XC_DIP_SetMux(pInstance, DIP_DIP_MUX_SC2_OP,CKG_DIP_CLK_SC2_OP,eWindow);
        }
        else if(eSource == SCALER_DIP_SOURCE_TYPE_OSD)
        {
            HAL_XC_DIP_SetMux(pInstance, DIP_DIP_MUX_OSD,CKG_DIP_CLK_OSD,eWindow);
        }
        else
    ...
    else if( eWindow == DWIN0_WINDOW )
    {
        if(eSource == SCALER_DIP_SOURCE_TYPE_MAIN)
        {
            HAL_XC_DIP_SetMux(pInstance, DIP_DWIN0_MUX_IP_MAIN,CKG_DWIN0_CLK_IP_MAIN,eWindow);
            DIP_W2BYTEMSK(0, REG_SC_BK3B_7A_L, 0,BIT(15));
            DIP_W2BYTEMSK(0, REG_SC_BK3B_70_L, 0,BMASK(4:1));
        }
        else if(eSource == SCALER_DIP_SOURCE_TYPE_SUB)
        {
            HAL_XC_DIP_SetMux(pInstance, DIP_DWIN0_MUX_IP_SUB,CKG_DWIN0_CLK_IP_SUB,eWindow);
        ...
        {
            HAL_XC_DIP_SetMux(pInstance, DIP_DWIN0_MUX_OP,CKG_DWIN0_CLK_OP,eWindow);
        }
        else if(eSource == SCALER_DIP_SOURCE_TYPE_SC2_OP_CAPTURE)
        {
            MDrv_DIP_WriteByteMask(REG_CKG_DIP_GATE, CKG_DWIN1_GATE_EN, CKG_DWIN1_GATE_EN);
            DIP_W2BYTEMSK(0, REG_SC_BK3C_70_L, BIT(5),BIT(5));
            HAL_XC_DIP_SetMux(pInstance, DIP_DWIN0_MUX_SC2_OP,CKG_DWIN0_CLK_SC2_OP,eWindow);
        }
        else
            DIP_H_DBUG("MApi_XC_DIP_SetDIPWinProperty eSource = %d is not correct\n",eSource);
    }
    else if( eWindow == DWIN1_WINDOW )
    {
        if(eSource == SCALER_DIP_SOURCE_TYPE_MAIN)
        {
            HAL_XC_DIP_SetMux(pInstance, DIP_DWIN1_MUX_IP_MAIN,CKG_DWIN1_CLK_IP_MAIN,eWindow);
            DIP_W2BYTEMSK(0, REG_SC_BK3C_7A_L, 0,BIT(15));
            DIP_W2BYTEMSK(0, REG_SC_BK3C_70_L, 0,BMASK(4:1));
        }
        else if(eSource == SCALER_DIP_SOURCE_TYPE_SUB)
        {
            HAL_XC_DIP_SetMux(pInstance, DIP_DWIN1_MUX_IP_SUB,CKG_DWIN1_CLK_IP_SUB,eWindow);
        ...
        {
            HAL_XC_DIP_SetMux(pInstance, DIP_DWIN1_MUX_OP,CKG_DWIN1_CLK_OP,eWindow);
        }
        else if(eSource == SCALER_DIP_SOURCE_TYPE_SC2_OP_CAPTURE)
        {
            HAL_XC_DIP_SetMux(pInstance, DIP_DWIN1_MUX_SC2_OP,CKG_DWIN1_CLK_OP,eWindow);
        }
        else
            DIP_H_DBUG("MApi_XC_DIP_SetDIPWinProperty eSource = %d is not correct\n",eSource);
    }
...
void HAL_XC_DIP_SetWinProperty(void *pInstance, MS_U8 u8BufCnt,MS_U16 u16Width,MS_U16 u16LineOft,MS_U16 u16Height,MS_PHY u64OffSet, SCALER_DIP_SOURCE_TYPE eSource,MS_BOOL bPIP, MS_BOOL b2P_Enable, SCALER_DIP_WIN eWindow)
{
    ...
    {
        u16MaxWith = (DIP_R2BYTEMSK(0, REG_SC_BK90_05_L,BMASK(13:0),eWindow) - DIP_R2BYTEMSK(0, REG_SC_BK90_04_L,BMASK(13:0),eWindow)+1);
    }
    else if(eSource == SCALER_DIP_SOURCE_TYPE_DRAM)
    {
...
void HAL_XC_DIP_SetMiuSel(void *pInstance, MS_U8 u8MIUSel,SCALER_DIP_WIN eWindow)
{
    ...
    {
        //miu hw sel
        MDrv_Write2ByteMask(MIU0_G6_REQUEST_MASK, BIT(11),BIT(11));
        MDrv_Write2ByteMask(MIU1_G6_REQUEST_MASK, BIT(11),BIT(11));
        DIP_W2BYTEMSK(0, REG_SC_BK7F_11_L, BIT(11), BIT(11));
        MDrv_Write2ByteMask(MIU0_G6_REQUEST_MASK, 0, BIT(11));
        MDrv_Write2ByteMask(MIU1_G6_REQUEST_MASK, 0, BIT(11));
    ...
    {
        //miu hw sel
        MDrv_Write2ByteMask(MIU0_G6_REQUEST_MASK, BIT(8),BIT(8));
        MDrv_Write2ByteMask(MIU1_G6_REQUEST_MASK, BIT(8),BIT(8));
        DIP_W2BYTEMSK(0, REG_SC_BK7F_11_L, BIT(8), BIT(8));
        MDrv_Write2ByteMask(MIU0_G6_REQUEST_MASK, 0, BIT(8));
        MDrv_Write2ByteMask(MIU1_G6_REQUEST_MASK, 0, BIT(8));
        if( ((CHIP_MIU_ID)u8MIUSel) == E_CHIP_MIU_0 )
        {
...
static void HAL_XC_DIP_CallBack(InterruptNum eIntNum)
{
    ...
    if( (SC_R2BYTEMSK(0, REG_SC_BK36_0A_L,BMASK(7:0)) > 0) && (pDipIntCb != NULL) )
        (void)(pDipIntCb)(SC_R2BYTEMSK(0, REG_SC_BK36_0A_L,BMASK(7:0)));
    if( (SC_R2BYTEMSK(0, REG_SC_BK3B_0A_L,BMASK(7:0)) > 0) && (pDwin0IntCb != NULL) )
        (void)(pDwin0IntCb)(SC_R2BYTEMSK(0, REG_SC_BK3B_0A_L,BMASK(7:0)));
    if( (SC_R2BYTEMSK(0, REG_SC_BK3C_0A_L,BMASK(7:0)) > 0) && (pDwin1IntCb != NULL) )
        (void)(pDwin1IntCb)(SC_R2BYTEMSK(0, REG_SC_BK3C_0A_L,BMASK(7:0)));
#else
    if( (SC_R2BYTEMSK(0, REG_SC_BK36_0A_L,BMASK(7:0)) > 0) && (pDipIntCb != NULL) )
...
void HAL_XC_DIP_2P_Width_Check(void *pInstance, XC_SETWIN_INFO *pstXC_SetWin_Info, SCALER_DIP_WIN eWindow)
{
    ...
    else if (g_DIPSrcInfo[eWindow].enDIPSourceType == SCALER_DIP_SOURCE_TYPE_OP_CAPTURE)
    {
        u16MaxWith = (DIP_R2BYTEMSK(0, REG_SC_BK10_05_L,BMASK(13:0),eWindow) - DIP_R2BYTEMSK(0, REG_SC_BK10_04_L,BMASK(13:0),eWindow)+1);
        if(u16MaxWith > FHD_WIDTH)
        {
            if(pstXC_SetWin_Info->u16PreHCusScalingDst > (pstXC_SetWin_Info->u16PreHCusScalingSrc >> 1))
            {
                pstXC_SetWin_Info->u16PreHCusScalingDst = (pstXC_SetWin_Info->u16PreHCusScalingSrc >> 1);
            }
            u64_result = pstXC_SetWin_Info->u16PreHCusScalingDst;
            do_div(u64_result,u16alignment);
            u16tmp = u64_result;
