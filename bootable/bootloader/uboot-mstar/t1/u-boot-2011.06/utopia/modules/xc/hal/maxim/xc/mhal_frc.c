...
MS_BOOL bTimingStable = FALSE;

#define FRC_STABLE_TIMEOUT_COUNTER 10
#define FRC_STABLE_COUNTER_THRESHOLD 2
...
void MHal_FRC_TGEN_SetVSyncStartEndY(void *pInstance, MS_U16 u16Start, MS_U16 u16End)
{
    ...
    {
        u16Start += (FRC_PIPE_DELAY_VCNT_FSC_FHD_HDMITX - FRC_PIPE_DELAY_VCNT_FSC_4K);
        u16End += (FRC_PIPE_DELAY_VCNT_FSC_FHD_HDMITX - FRC_PIPE_DELAY_VCNT_FSC_4K);
    }
...
void MHal_FRC_TGEN_SetFdeStartEndY(void *pInstance, MS_U16 u16Start, MS_U16 u16End)
{
    ...
    if((MHal_FRC_IsFHDToFSC(pInstance,MAIN_WINDOW))
        ...
        && (pXCResourcePrivate->stdrvXC_MVideo.gu16VPixelShiftRange == 0)) //Check if PIXEL FEATURE is ENABLE or NOT.
    {
        u16Start += VSTART_OFFSET_FHD;
        u16End += VSTART_OFFSET_FHD;
    }
...
void MHal_FRC_SetYTrig(void *pInstance, MS_U16 u16Trig1, MS_U16 u16Trig2)
{
    ...
    if(MDrv_ReadByte(REG_CHIP_REVISION) < HW_VTT_LIMITATION_CHIPREV)
        ...
        if (!gSrcInfo[MAIN_WINDOW].bForceRGBin)
        {
            ...
            else if (IS_OUTPUT_1280_800(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width, pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height)) // 1280X800 out
            {
                u16Trig1 = YTRIG_1280_800;
                u16Trig2 = YTRIG_1280_800 + 1;
            }
            else
            {
...
#endif
        if(u16BK10_VTotal != 0 &&
           !IS_OUTPUT_1280_800(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width, pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height))
        {
...
#define MIU_FRC_G5REQUEST_NO_PQ_MASK   (0x03FC)
void MHal_FRC_Disable_MiuMask(void *pInstance)
{
    ...
    // The memory address will be updated to HW until IP Vsync is stable
    // For FRC client 0,1,10~14, it would access incorrect memory address if IP Vsync is not stable, and cause coredump 
    // HW RD suggest FRC client 0,1,10~14 need to be masked untail IP Vsync is stable
    if (bTimingStable)
    {
        pXCResourcePrivate->sthal_FRC.FRC_Miu0Mask.u16MiuG5Mask &= ~MIU_FRC_G5REQUEST_MASK;
    ...
    {
        pXCResourcePrivate->sthal_FRC.FRC_Miu0Mask.u16MiuG5Mask &= ~MIU_FRC_G5REQUEST_NO_PQ_MASK;

        pXCResourcePrivate->sthal_FRC.FRC_Miu1Mask.u16MiuG5Mask &= ~MIU_FRC_G5REQUEST_NO_PQ_MASK;
    }
...
void MHal_FRC_Init(void *pInstance, PMST_PANEL_INFO_t pPanelInfo, PFRC_INFO_t pFRCInfo)
{
    ...
    MS_U16 u16FB_YcountLinePitch = 0, u16LineLimit = 0;
    ...
    //Before FRC Address setting, mask all ip first
    MHal_FRC_Enable_MiuMask(pInstance);

    // internal mask
    W2BYTEMSK(REG_FRC_BK23A_1F_L, BIT(10), BIT(10));
    W2BYTEMSK(REG_FRC_BK218_70_L, BIT(7), BIT(7));
    W2BYTEMSK(REG_FRC_BK218_71_L, BIT(7), BIT(7));
    W2BYTEMSK(REG_FRC_BK22C_62_L, (BIT(14)|BIT(15)), (BIT(14)|BIT(15)));
    W2BYTEMSK(REG_FRC_BK22C_53_L, (BIT(13)|BIT(14)), (BIT(13)|BIT(14)));
    ...
    {
        MFC_3D_2D_4K2K_2D_4K2K_YUV();
    }
    else
    {
        MFC_3D_2D_FHD_2D_FHD_YUV();
    }

    // wait stable count changes
    MS_U32 u32FRC_VsyncCounter = MDrv_Read2Byte(REG_FRC_BK253_E0);
    MS_U16 u16timeoutCounter = 0;
    while(abs(MDrv_Read2Byte(REG_FRC_BK253_E0)-u32FRC_VsyncCounter)< FRC_STABLE_COUNTER_THRESHOLD)
    {
        MsOS_DelayTask(5);
        if(u16timeoutCounter++ > FRC_STABLE_TIMEOUT_COUNTER)
        {
            u16timeoutCounter = 0;
            break;
        }
    ...
    //After FRC Address setting, unmask
    ...
    // wait counter + N
    u32FRC_VsyncCounter = MDrv_Read2Byte(REG_FRC_BK253_E0);
    u16timeoutCounter = 0;
    while(abs(MDrv_Read2Byte(REG_FRC_BK253_E0)-u32FRC_VsyncCounter)< FRC_STABLE_COUNTER_THRESHOLD)
    {
        MsOS_DelayTask(5);
    ...
    MHal_FRC_Disable_MiuMask(pInstance);
    // internal unmask
    W2BYTEMSK(REG_FRC_BK23A_1F_L, 0, BIT(10));
    W2BYTEMSK(REG_FRC_BK218_70_L, 0, BIT(7));
    W2BYTEMSK(REG_FRC_BK218_71_L, 0, BIT(7));
    W2BYTEMSK(REG_FRC_BK22C_62_L, 0, (BIT(14)|BIT(15)));
    W2BYTEMSK(REG_FRC_BK22C_53_L, 0, (BIT(13)|BIT(14)));
...
void MHal_FRC_Set_3D_QMap(void *pInstance, MS_U8 u8FRC3DPanelType, MS_U16 u16sc_in_3dformat, MS_U16 u16sc_out_3dformat, MS_U8 u83D_FI_out)
{
        ...
        (psXCInstPri->u32DeviceID != 1))
    {
                             ...
                             pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height)) /// 4k0.5k out
        {
            ...
            else if(IS_OUTPUT_1280_800(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width,
                             pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height)) //1280 X 800
            {
                if (!gSrcInfo[MAIN_WINDOW].bForceRGBin)
                {
                    MFC_3D_2D_1280_2D_1280_YUV(); // load FRC 3D-table as 1280X800 in
                }
                else
                {
                    MFC_3D_2D_1280_2D_1280_RGB_BYPASS();   // for by pass MEMC test
                }
        }
            else
            {
...
#else
        ( (gSrcInfo[eWindow].stCapWin.width < 1440)
...
void MHal_FRC_Set_TimingStable(void *pInstance, MS_BOOL bStable)
{
    bTimingStable = bStable;
}
