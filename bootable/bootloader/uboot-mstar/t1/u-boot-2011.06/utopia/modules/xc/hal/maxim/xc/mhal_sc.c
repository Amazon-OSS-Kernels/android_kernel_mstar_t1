...
void Hal_SC_set_write_limit(void *pInstance, MS_PHY u32WritelimitAddrBase, SCALER_WIN eWindow)
{
    ...
    else if ((pXCResourcePrivate->sthal_SC.bDynamicScalingEnable[eWindow] || pXCResourcePrivate->stdrvXC_Dynamicscaling._bDSForceIndexEnable[eWindow] ) &&
...
static void _Hal_SC_fill_main_sw_db_burst(void *pInstance, P_SC_SWDB_INFO pDBreg)
{
    ...
    else
    {
        ...
        {
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_08_L, (MS_U16)(stTCONPanelConfig.stPanelInfo.u32HDEStart>>8), VOP_DISPLAY_HSTART_MASK);
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_09_L, (MS_U16)(stTCONPanelConfig.stPanelInfo.u32HDEEnd>>8), VOP_DISPLAY_HEND_MASK);
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_0A_L, (MS_U16)(stTCONPanelConfig.stPanelInfo.u32VDEStart>>8), 0xFFFF);
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_0B_L, (MS_U16)(stTCONPanelConfig.stPanelInfo.u32VDEEnd>>8), 0xFFFF);
        }
        else
        {
...
void Hal_SC_sw_db(void *pInstance, P_SC_SWDB_INFO pDBreg, SCALER_WIN eWindow )
{
    ...
    XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"[%s,%5d] [DS] bDynamicScalingEnable%d  \n",__func__,__LINE__,pXCResourcePrivate->sthal_SC.bDynamicScalingEnable[eWindow]);
        ...
        if(pXCResourcePrivate->sthal_SC.bDynamicScalingEnable[eWindow])
        {
           ...
           //STGEN setting
            ...
            for(cnt_x = 0 ;cnt_x < DS_REG_CNT; cnt_x++)
            {
                ...
                #endif
                memset(&DS_REG[cnt_x],0,sizeof(stDS_PQ_REG));
            }
            DS_REG_CNT = 0;
#endif
...
void Hal_SC_set_miusel(void *pInstance, MS_U8 u8MIUSel)
{
    ...
    if (psXCInstPri->u32DeviceID == 0)
    {
        ...
        SC_W2BYTEMSK(psXCInstPri->u32DeviceID, REG_SC_BK7F_11_L, BIT(0) | BIT(1) | BIT(9) | BIT(13) | BIT(15), BIT(0) | BIT(1) | BIT(9) | BIT(13) | BIT(15));
    }
    else
    {
    ...
    else
    {
        if ((gSrcInfo[(eWindow)].stCapWin.width>= 3840) && (gSrcInfo[(eWindow)].stCapWin.height>= 2160))
        {
            if (gSrcInfo[eWindow].Status2.stXCConfigCMA[CMA_XC_COBUFF_MEM].u32HeapID != XC_INVALID_HEAP_ID && IsEnableDualMode(eWindow))
            {
                ...
                if(pXCResourcePrivate->stdrvXC_MVideo.bUseDdr4DramForXc)
                {
                    u32CMAMemSCMSize = XC_CMA_72MB/2;
                }
                else
                {
...
#ifdef STELLAR
void Hal_SC_enable_PIP(void *pInstance, MS_BOOL bEn)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
...
void Hal_SC_enable_PIP_burst(void *pInstance, MS_BOOL bEn)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
