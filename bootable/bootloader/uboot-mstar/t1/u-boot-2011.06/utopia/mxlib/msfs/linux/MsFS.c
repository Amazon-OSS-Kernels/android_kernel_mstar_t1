...
static int _MsFS_UTF8StrToUnicodeStr(const unsigned char* utf8_str, unsigned short* unicode_str);
...
struct dirent * MsFS_ReadDir( DIR *dirp )
{
    ...
    unsigned short unicode_d_name[256] = {0};

    if(_bUnicode && direntp)
    {
        unicode_len = _MsFS_UTF8StrToUnicodeStr((const unsigned char*)(direntp->d_name), unicode_d_name);
        if(unicode_len == -1)
            return NULL;
        memcpy(direntp->d_name, unicode_d_name, sizeof(unsigned short)*unicode_len);
    }
...
static int _MsFS_UTF8ToUnicode(const unsigned char* pUTF8, unsigned int* pUnic, int len)
{
    unsigned char b1, b2, b3, b4, b5, b6; //b1~b6: high~low byte(eg: 0x1234 ---- b1 = 0x12, b2 = 0x34).
    unsigned char *p = (unsigned char *)pUnic;

    if((pUTF8!=NULL) && (pUnic!=NULL))
    {
        *pUnic = 0x0;
...
static int _MsFS_UTF8StrToUnicodeStr(const unsigned char* utf8_str, unsigned short* unicode_str)
{
    int i=0, j=1;
    int utf8_len = 0;
    unsigned short* s = unicode_str;

    if((utf8_str == NULL) || (unicode_str == NULL))
        return -1;

    utf8_len = strlen((const char *)utf8_str);

    for(i=0,j=1; i<utf8_len; i+=j,s++)
    {
        if((utf8_str[i]&0x80) == 0) //0000 0000 - 0000 007F
            j=1;
        else   //0000 0080 - 7FFF FFFF
        {
            for(j=2; i+j<utf8_len && (utf8_str[i+j]&0x80) != 0 && (utf8_str[i+j]&0xc0) != 0xc0; j++)
            {
                if(j>6)
                    return -1;
            }
        ...
        if(_MsFS_UTF8ToUnicode(utf8_str+i, (unsigned int*)s, j))
            return -1;
    }

    *s = '\0';
    return (s - unicode_str); //return the size of unicode_str
}
