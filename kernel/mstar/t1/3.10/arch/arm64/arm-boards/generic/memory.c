...
static int RECOVERY_MODE_setup(char *str)
{
	if (str == NULL) {
		printk("Warning: recovery_mode setup error \n");
		return 0;
	}
	if (!strcmp(str, "true"))
		RECOVERY_MODE = 1;
...
early_param("recovery_mode", RECOVERY_MODE_setup);
#ifdef CONFIG_MP_R2_STR_ENABLE
...
//the lx_mem info is parising in function "early_init_dt_scan_memory"
    ...
    if ((linux_memory_address | linux_memory_length | linux_memory2_address | linux_memory2_length
                | linux_memory3_address | linux_memory3_length | linux_memory4_address | linux_memory4_length) & (PMD_SIZE-1))
    {
