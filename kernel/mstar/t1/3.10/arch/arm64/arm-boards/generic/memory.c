...
{
	if (cmdline == NULL)
		pr_debug("Warning: tee_mode setup error\n");
	else
...
{
	pr_debug("MALI_RESERVE_IN_MIU0_SIZE = %s\n", str);
    if( str != NULL )
    {
    	int ret = -1;
	...
	pr_debug("MALI_RESERVE_IN_MIU0_SIZE = %s, mali_reserve_size_miu0 = %lx\n", str, mali_reserve_size_miu0);
...
static int MALI_MIU1_RESERVE_setup(char *str)
{
	...
	pr_debug("ret=%d,mali_reserve_size_miu1 = %ld\n", ret, mali_reserve_size_miu1);
	...
	//mali_reserve_size_miu0 = 0x6400000;
	pr_debug("MALI_RESERVE_IN_MIU1_SIZE = %s, mali_reserve_size_miu1 = %lx\n", str, mali_reserve_size_miu1);
...
void get_boot_mem_info(BOOT_MEM_INFO type, u64 *addr, u64 *len)
{
	...
	pr_debug("!!!!!!!!!!SBOOT memory type=%x addr=%x, len=%x!!!!!!!!!!\n", type, *addr, *len);
    return;
#endif//CONFIG_MSTAR_OFFSET_FOR_SBOOT
    ...
    else
    {
        ...
        case LINUX_MEM2:
            ...
            else
            {
                ...
                *len = LINUX_MEM2_LEN;
		pr_debug("LINUX_MEM2 addr %08llx, len %08llx\n", *addr, *len);
                #else
                *addr = 0;
                *len = 0;
                #endif
...
//the lx_mem is added in  function "early_init_dt_scan_memory"
	...
	pr_debug("\n");
	pr_debug("LX_MEM  = 0x%llX, 0x%llX\n", linux_memory_address, linux_memory_length);
	pr_debug("LX_MEM2 = 0x%llX, 0x%llX\n", linux_memory2_address, linux_memory2_length);
	pr_debug("LX_MEM3 = 0x%llX, 0x%llX\n", linux_memory3_address, linux_memory3_length);
	pr_debug("EMAC_LEN= 0x%lX\n", EMACmem);
	pr_debug("DRAM_LEN= 0x%lX\n", DRAMlen);
