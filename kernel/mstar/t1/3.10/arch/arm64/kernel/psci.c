			    ...
			    unsigned long entry_point)
{
	...
	if (TEEINFO_TYPTE == SECURITY_TEEINFO_OSTYPE_OPTEE) {
		...
		if (!is_mstar_str()) {
			/* Kernel mode Utopia */
			err = invoke_psci_fn(fn, power_state, entry_point, PSCI_MSTAR_ARMv8_64BIT | PSCI_MSTAR_KERNEL_DRIVER);
			}
		else{
			/* User mode Utopia */
			err = invoke_psci_fn(fn, power_state, entry_point, PSCI_MSTAR_ARMv8_64BIT | PSCI_MSTAR_USER_DRIVER);
			}
#endif
	} else {
...
{
	pr_debug("\033[1;33;31m [CPU_PSCI] %s %d\033[m\n", __func__, __LINE__);
	invoke_psci_fn = __invoke_psci_fn_smc;
...
#ifdef CONFIG_MP_TRUSTZONE_PATCH
	state[index].type = 1;
	return psci_ops.cpu_suspend(state[index], 0x20280000);
#else
