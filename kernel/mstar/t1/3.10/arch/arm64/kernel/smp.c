...
extern ptrdiff_t magic_number_address;
#endif
...
//clear magic number to fix smp failed issue 
#if defined(CONFIG_MP_PLATFORM_ARM_64bit_PORTING)
	if (magic_number_address != NULL)
...
void handle_IPI(int ipinr, struct pt_regs *regs)
{
	...
	irq_abnormal_check(NULL, 1, ipinr - IPI_RESCHEDULE);
#endif
	...
	case IPI_CPU_STOP:
		...
		extern struct pt_regs ramdump_cpu_regs[NR_CPUS];
		memcpy(&ramdump_cpu_regs[smp_processor_id()], regs, sizeof(struct pt_regs));
