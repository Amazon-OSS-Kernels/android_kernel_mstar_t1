...
#ifdef CONFIG_MSTAR_RAM_CONSOLE
extern mstar_ramoops_init();
#endif
...
void __init arm64_memblock_init(void)
{
	...
	pr_debug("\033[35mFunction = %s, Line = %d, find cma_default buffer at whole lowmem\033[m\n", __PRETTY_FUNCTION__, __LINE__);
	dma_contiguous_reserve(dma_phys_limit);
#else
	// this is to limit cma default buffer at LX_MEM(LX0)
	pr_debug("\033[35mFunction = %s, Line = %d, find cma_default buffer at only LX0\033[m\n", __PRETTY_FUNCTION__, __LINE__);
	dma_contiguous_reserve(min((lx_mem_addr+lx_mem_size), (unsigned long)dma_phys_limit));
#endif
...
void __init mem_init(void)
{
	...
	for_each_memblock(memory, reg) {
		...
		num_physpages += pages;
		pr_debug(" %ldMB", pages >> (20 - PAGE_SHIFT));
	}
