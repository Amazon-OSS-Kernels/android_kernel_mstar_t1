...
int zram_cma_page_cnt[MSTAR_MAX_MIU_CNT] = {0};	/* to get to cma_buffer by dev->cma */
	...
	while(dma_declare_index < mstar_driver_boot_cma_buffer_num)
	{
	    ...
	    */
		...
		dma_contiguous_reserve_area(cma_config[dma_declare_index].size, cma_config[dma_declare_index].start, limit,
										&mstar_cma_device_area[dma_declare_index]);
...
static int __init cma_activate_area(struct cma *cma)
{
	...
	printk(CMA_DEBUG "\033[35mFunction = %s, cma_start pfn: 0x%X length0x%X is in zone %s\033[m\n", __PRETTY_FUNCTION__, (unsigned int)pfn, (unsigned int)cma->count, zone->name);
				       ...
				       phys_addr_t limit, struct cma **res_cma)
{
	...
	*res_cma = cma;
	...
	pr_debug("CMA: reserved %ld MiB at %08lx\n", (unsigned long)size / SZ_1M,
		(unsigned long)base);
...
#ifdef CONFIG_ARM64
	/* 32-bit highmem will not get va to do __dma_remap */
    /* __dma_remap result in garbage, todo: find the root cause*/
