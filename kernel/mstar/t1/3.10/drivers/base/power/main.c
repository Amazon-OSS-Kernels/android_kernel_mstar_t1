...
extern unsigned int if_show_cpu_workqueue_info;
...
#ifdef CONFIG_MP_DEBUG_TOOL_SHOW_CPU_WORKQUEUE_INFO
	int print_count = 0;
	printk("1. cpu %d hang @dpm_wd_handler, task is %s\n", get_cpu(), tsk->comm);
	if_show_cpu_workqueue_info = 0;
#endif
...
#ifdef CONFIG_MP_DEBUG_TOOL_SHOW_CPU_WORKQUEUE_INFO
	print_cpu_workqueue_info(0);
	dev_emerg(dev, "**** DPM device timeout print_cpu_workqueue_info done****\n");
#endif
