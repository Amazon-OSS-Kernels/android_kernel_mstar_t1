...
/* When BCM chip is connected to PMU, enable the below flag
    to read GPIO from device tree config */
/*
...
#if (BTRFKILL_DBG)
#define BTRFKILLDBG(fmt, arg...) pr_err(" ** [ BTRFKILL :  %s ( %d ) ] " fmt "\n" , __func__, __LINE__, ## arg)
#else
...
#define PR(msg, ...) pr_info("####"msg, ##__VA_ARGS__)
...
static int bluetooth_set_power(void *data, bool blocked)
{
	...
	pr_info("bluetooth_set_power set blocked=%d\n", blocked);

	if (!blocked) {
#ifdef CONFIG_BCM4335BT
		if (bcm_bt_lock(lock_cookie_bt) != 0)
			pr_err("** BT rfkill: timeout in acquiring bt lock**\n");
#endif /* CONFIG_BCM4335BT */

		MDrv_GPIO_Set_Low(bt_reset_gpio);
		msleep(75);
		MDrv_GPIO_Set_High(bt_reset_gpio);

		pr_debug("Bluetooth RESET HIGH, 30ms-low 150ms-high turnON!! gpio: %d\n", bt_reset_gpio);
		msleep(150);
	} else {
		MDrv_GPIO_Set_Low(bt_reset_gpio);
		pr_debug("Bluetooth RESET LOW- turnOFF!!\n");
		msleep(50);
	}
	/* Workaround : TODO : Identify why RFKILL needs this 50 ms sleep. - BRCM : 01062014 */
	/* Need to sleep for btusb enumeration */
	msleep(800);
...
static int bluetooth_rfkill_probe(struct platform_device *pdev)
{
	...
	pr_debug("Bluetooth bluetooth_rfkill_probe gpio: %d\n", bt_rfkill_dev->reset_gpio);
...
static int __init bluetooth_rfkill_init(void)
{
	...
	if (ret)
		pr_err("Fail to register rfkill platform driver (%d)\n", ret);
...
{
	pr_debug("bluetooth_rfkill_exit\n");
