...
arch_timer_detect_rate(void __iomem *cntbase, struct device_node *np)
{
	...
	if (of_property_read_u32(np, "clock-frequency", &arch_timer_rate)) {
		/* 0 means we get arch_timer_rate from dts, do not go into here */
		if (cntbase)
...
{
	printk("Architected %s%s%s timer(s) running at %lu.%02luMHz (%s%s%s).\n",
...
static void __init arch_timer_init(struct device_node *np)
{
	...
	pr_debug("\033[31mFunction = %s, arch_timer_rate is %d\033[m\n", __PRETTY_FUNCTION__, arch_timer_rate);
