...
static void od_check_cpu(int cpu, unsigned int load)
{
	...
	if (cpu == ondemand_timer[cpu].cluster_m) {
	/* for cluster_head, use max_cpu_load_freq among all online cpus of same cluster */
		mutex_lock(&mstar_cpuload_lock);
		for_each_online_cpu(i)
		{
		if (cpu != ondemand_timer[i].cluster_m)
			continue;
	...
	if (load > od_tuners->up_threshold) {
		 ...
		 * Here means the sampling_delay time can be increased or decreased (delay = sampling_rate * rate_mult)
