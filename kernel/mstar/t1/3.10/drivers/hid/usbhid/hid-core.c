				   ...
				   unsigned char dir)
{
	...
	if (usbhid->urbout && dir == USB_DIR_OUT && report->type == HID_OUTPUT_REPORT) {
		...
		kfree(usbhid->out[usbhid->outhead].raw_report);
	...
	if (dir == USB_DIR_OUT) {
		kfree(usbhid->ctrl[usbhid->ctrlhead].raw_report);
...
static void usbhid_disconnect(struct usb_interface *intf)
{
	...
	for (i = 0; i < HID_CONTROL_FIFO_SIZE; i++)
		kfree(usbhid->ctrl[i].raw_report);
	for (i = 0; i < HID_CONTROL_FIFO_SIZE; i++)
		kfree(usbhid->out[i].raw_report);
	kfree(usbhid);
}
