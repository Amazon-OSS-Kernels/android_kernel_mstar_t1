...
static void ir_boost_func(void)
{
    del_timer(&ir_boost_timer);
    ir_boost_flag = 0;
}
#endif
...
#if defined(CONFIG_SCHED_HMP) && defined(CONFIG_MP_HMP_GTS_SCHEDULER_CUSTOMIZED_RULES)
            // when a key is depressed, we enable rule 7 flag
            if(value == 1) {
                if(proc_rule_applied[RULE_IR_MIGRATION]) {
                    if (!ir_boost_flag) {
                        ir_boost_flag = 1;
                        init_timer(&ir_boost_timer);
                        ir_boost_timer.function = ir_boost_func;
                        ir_boost_timer.data = ((unsigned long) 0);
                        ir_boost_timer.expires = jiffies + msecs_to_jiffies(RULE7_IR_INTERVAL_ON_BIG_CORE * 1000);
                        add_timer(&ir_boost_timer);
                    } else {
                        mod_timer(&ir_boost_timer, jiffies + msecs_to_jiffies(RULE7_IR_INTERVAL_ON_BIG_CORE * 1000));
                    }
                }
            }
#endif
...
#if defined(CONFIG_MSTAR_CPU_CLUSTER_CALIBRATING)
    if (sscanf(buffer,"%d %d %d", &set_cpu, &cpu_freq_in_khz, &garbage) == 3)
    {
       return -EINVAL;
    }
    else if (sscanf(buffer,"%d %d", &set_cpu, &cpu_freq_in_khz) == 2)
    {
...
static int __init input_proc_init(void)
{
    ...
    for (i = 0; i < CONFIG_NR_CPUS; i ++) {
	    ...
	    write_ir_boost_cpu_freq(CONFIG_DVFS_CPU_IRBOOST_CLOCK(i), i);
    }
#elif defined(CONFIG_MSTAR_CPU_calibrating)
