...
volatile unsigned long joe_test[10] = {0};
volatile int joe_test_index = 0;
...
extern int arm_set_wake(struct irq_data *d, unsigned int on);
			   ...
			   u32 percpu_offset, struct device_node *node)
{
	...
	// irq_start is 16 while irq_start = 27, hwirq_base = 16
	joe_test[joe_test_index] |= irq_start << 15;

	/*
	 * Find out how many interrupts are supported.
	 * The GIC only supports up to 1020 interrupt sources.
	 */
	gic_irqs = readl_relaxed(gic_data_dist_base(gic) + GIC_DIST_CTR) & 0x1f;	// this is 7 sets
	gic_irqs = (gic_irqs + 1) * 32;												// 8 * 32 = 256
	if (gic_irqs > 1020)
		gic_irqs = 1020;
	gic->gic_irqs = gic_irqs;

	gic_irqs -= hwirq_base; /* calculate # of irqs to allocate */				// 256 -16 = 240

	if (of_property_read_u32(node, "arm,routable-irqs",
				 &nr_routable_irqs)) {
		joe_test[joe_test_index] |= 1 << 2;
		...
		if (IS_ERR_VALUE(irq_base)) {
			WARN(1, "Cannot allocate irq_descs @ IRQ%d, assuming pre-allocated\n",
			     irq_start);
			irq_base = irq_start;
			joe_test[joe_test_index] |= 1 << 7;
		}
		else
		{
			joe_test[joe_test_index] |= 1 << 8;
		}

		gic->domain = irq_domain_add_legacy(node, gic_irqs, irq_base,
					hwirq_base, &gic_irq_domain_ops, gic);
	} else {
		joe_test[joe_test_index] |= 1 << 3;
...
#endif
		joe_test[joe_test_index] |= 1 << 5;
		set_handle_irq(gic_handle_irq);
	}
	else
		joe_test[joe_test_index] |= 1 << 6;
