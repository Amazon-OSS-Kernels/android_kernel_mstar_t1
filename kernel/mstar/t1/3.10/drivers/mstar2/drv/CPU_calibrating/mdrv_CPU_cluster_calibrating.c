...
atomic_t dvfs_finish = ATOMIC_INIT(1);
...
bool voltage_change_result[CONFIG_NR_CPUS] = {0};
...
static int boost_duration_thread(void *arg)
{
	...
	while (!kthread_should_stop()) {
		msleep_interruptible(BOOST_DURATION_CHECK_PERIOD);
		try_to_freeze();
		for_each_online_cpu(i)
		{
			if (i != ondemand_timer[i].cluster_m)
