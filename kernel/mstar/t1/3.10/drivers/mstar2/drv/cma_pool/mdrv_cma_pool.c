...
static void free_buffer_list_and_release_special_buf(struct cma_memory_info *heap_info, struct cma_buffer **release_buf_front , struct cma_buffer **release_buf_back, bool delay_cma_free);
...
struct cma_buffer_free_list {
	struct list_head list_free_info;
	struct mutex lock;
};

struct cma_buffer_free {
...
static struct task_struct *g_cma_free_tsk;
static struct cma_buffer_free_list cma_free_list;
static bool cma_free_loop_wait;
static DECLARE_WAIT_QUEUE_HEAD(cma_free_wait);
#endif
...
static int CMA_Pool_Release_Heap(unsigned int heap_id, filp_private * pdev)
{
	...
	printk(CMA_DEBUG "\033[31mFunction = %s, Line = %d, pid is %d, current->pid is %d\033[m\n", __PRETTY_FUNCTION__, __LINE__, pid, current->pid);
	printk(CMA_DEBUG "\033[31mRelease for heap_id %u\033[m\n", heap_id);
	...
	free_buffer_list_and_release_special_buf(heap_info, &release_buf_front, &release_buf_back, false);
...
static int CMA_Pool_Release(struct inode *inode, struct file *filp)
{
	...
	printk(CMA_DEBUG "\033[31mFunction = %s, Line = %d, pid is %d, current->pid is %d\033[m\n", __PRETTY_FUNCTION__, __LINE__, pid, current->pid);
	printk(CMA_DEBUG "\033[31mFunction = %s, Line = %d, [%s] do CMA_Pool_Release for /dev/cmapool close\033[m\n", __PRETTY_FUNCTION__, __LINE__, current->comm);
	...
	list_for_each_entry_safe(heap_info, next_heap, &heap_info_list, list_node)	// search for all heap_info(each heap_id has a heap_info)
	{
		...
		free_buffer_list_and_release_special_buf(heap_info, &release_buf_front, &release_buf_back, true);
		/*for this heap, check if having free buffer*/
...
ALLOC_RETRY:
		...
		cma_measurement_data[heap_id].total_alloc_size_kb += length;
...
#ifdef CONFIG_TEE_SUPPORT
static unsigned long cma_ba_to_pa(unsigned long ba)
{
	unsigned long pa = 0x0UL;

	/* pa = ba - offset */
	if ((ba >= ARM_MIU0_BUS_BASE) && (ba < ARM_MIU1_BUS_BASE))	/* MIU0*/
		pa = ba - ARM_MIU0_BUS_BASE + ARM_MIU0_BASE_ADDR;
	else if ((ba >= ARM_MIU1_BUS_BASE) && (ba < ARM_MIU2_BUS_BASE))	/* MIU1*/
		pa = ba - ARM_MIU1_BUS_BASE + ARM_MIU1_BASE_ADDR;
	else
		pa = ba - ARM_MIU2_BUS_BASE + ARM_MIU2_BASE_ADDR;	/* MIU2*/

	return pa;
}

static bool cma_check_secure_range(unsigned long addr, unsigned long size)
{
...
static void cma_free_job(struct cma_buffer_free *cma_free)
{
	unsigned long pa = 0;
	int retry_time = 0;

	pa = cma_ba_to_pa(cma_free->ba);

	if (cma_check_secure_range(pa, cma_free->length) == true) {
		if (retry_time < 5)
			cma_free->retry_time++;
		else
			goto retry_error;
	} else {
		printk(CMA_DEBUG "\33[0;36m   %s:%d   pa = %x size = %x free success\33[m \n", __func__, __LINE__, pa, cma_free->length);
		dma_release_from_contiguous(cma_free->dev, cma_free->pages, cma_free->length >> PAGE_SHIFT);
		cma_free->job_done = 1;
	}
	return;

retry_error:
	printk(CMA_ERR "\33[0;36m   %s:%d    pa = %x size = %x free error\33[m \n", __func__, __LINE__, pa, cma_free->length);
	cma_free->job_done = 1;
}

static void cma_free_loop(void *arg)
{
	struct cma_buffer_free *pos = NULL, *n = NULL;

	while (1) {
		mutex_lock(&cma_free_list.lock);
		if (list_empty(&cma_free_list.list_free_info)) {
			mutex_unlock(&cma_free_list.lock);
			wait_event_timeout(cma_free_wait, (cma_free_loop_wait == 1), msecs_to_jiffies(2000));
			cma_free_loop_wait = 0;
		} else {
			mutex_unlock(&cma_free_list.lock);
			msleep(200);
		}
		...
		mutex_lock(&cma_free_list.lock);
		list_for_each_entry_safe(pos, n, &cma_free_list.list_free_info, list_node) {
			cma_free_job(pos);
			if (pos->job_done) {
...
#ifdef CONFIG_TEE_SUPPORT
		if (delay_cma_free == true) {
			if (cma_check_secure_range(cma_ba_to_pa(front->start_pa), front->length) == true) {
				struct cma_buffer_free *cma_free = kmalloc(sizeof(struct cma_buffer_free), GFP_KERNEL);
				if (!cma_free) {
					printk(CMA_ERR "\33[0;36m   %s:%d    pa = %x size = %x kmalloc fail\33[m \n", __func__, __LINE__, cma_ba_to_pa(front->start_pa), front->length);
				} else {
					INIT_LIST_HEAD(&cma_free->list_node);
					cma_free->dev = heap_info->dev;
					cma_free->pages = front->page;
					cma_free->ba = front->start_pa;
					cma_free->length = front->length;
					cma_free->retry_time = 0;
					cma_free->job_done = 0;
					mutex_lock(&cma_free_list.lock);
					list_add(&cma_free->list_node, &cma_free_list.list_free_info);
					mutex_unlock(&cma_free_list.lock);
					cma_free_loop_wait = 1;
					wake_up(&cma_free_wait);
				}
			} else {
...
#ifdef CONFIG_TEE_SUPPORT
		if (delay_cma_free == true) {
			if (cma_check_secure_range(cma_ba_to_pa(back->start_pa), back->length) == true) {
				struct cma_buffer_free *cma_free = kmalloc(sizeof(struct cma_buffer_free), GFP_KERNEL);
				if (!cma_free) {
					printk(CMA_ERR "\33[0;36m   %s:%d    pa = %x size = %x kmalloc fail\33[m \n", __func__, __LINE__, cma_ba_to_pa(back->start_pa), back->length);
				} else {
					INIT_LIST_HEAD(&cma_free->list_node);
					cma_free->dev = heap_info->dev;
					cma_free->pages = back->page;
					cma_free->ba = back->start_pa;
					cma_free->length = back->length;
					cma_free->retry_time = 0;
					cma_free->job_done = 0;
					mutex_lock(&cma_free_list.lock);
					list_add(&cma_free->list_node, &cma_free_list.list_free_info);
...
#if CMA_POOL_DUMP
    printk(CMA_ERR "%s:%d heapid %u, [%d,%d,name=%s]\n", __FUNCTION__,__LINE__, heap_id, current->pid, current->tgid,current->comm);
#endif
...
static ssize_t delay_free_size_read(struct file *file, char __user *buf, size_t size, loff_t *ppos){
	...
	len = snprintf(msg, sizeof(msg), "Delay Free Size Limite : %d byte\n", delay_free_size_limite);
	if (len < 0)
		return 0;
	if (len >= sizeof(msg))
		len = sizeof(msg) - 1;
	...
	if (copy_to_user(buf, msg, size_to_copy))
		return -EFAULT;

	*ppos = size_to_copy - 1;

	return size_to_copy - 1;
}
...
static ssize_t delay_free_timeout_read(struct file *file, char __user *buf, size_t size, loff_t *ppos){
	...
	len = snprintf(msg, sizeof(msg), "Delay Free time out : %d sec\n", delay_free_time_limite);
	if (len < 0)
		return 0;
	if (len >= sizeof(msg))
		len = sizeof(msg) - 1;
...
static ssize_t delay_free_lowmem_minfree_read(struct file *file, char __user *buf, size_t size, loff_t *ppos){
	...
	len = snprintf(msg, sizeof(msg), "Delay Free lowmem minfree : %d byte\n", delay_free_lowmem_minfree);
	if (len < 0)
		return 0;
	if (len >= sizeof(msg))
		len = sizeof(msg) - 1;
...
void str_reserve_mboot_cma_buffer()
{
	...
	list_for_each_entry_safe(still_allocated_mboot_str_cma_buffer, next, &mboot_str_cma_heap_alloc_list->list_head, list)
	{
		...
		if(still_allocated_mboot_str_cma_buffer->start_pa != mboot_str_ask_cma_start_ba)
		{
			...
			{
				int length = still_allocated_mboot_str_cma_buffer->start_pa
						- mboot_str_ask_cma_start_ba;
				...
				mboot_str_ask_cma_page = dma_alloc_at_from_contiguous(mboot_str_heap->dev,
					length >> PAGE_SHIFT, 1, mboot_str_ask_cma_start_ba);
...
#ifdef CONFIG_TEE_SUPPORT
	INIT_LIST_HEAD(&cma_free_list.list_free_info);
	mutex_init(&cma_free_list.lock);
	g_cma_free_tsk = kthread_run(cma_free_loop, NULL, "cma_free_loop");
#endif
	pr_debug("\033[35mFunction = %s, Line = %d, lx0 from %lx to %lx\033[m\n", __func__, __LINE__, __phys_to_pfn(lx_mem_addr), __phys_to_pfn(lx_mem_addr + lx_mem_size - PAGE_SIZE));
	pr_debug("\033[35mFunction = %s, Line = %d, lx1 from %lx to %lx\033[m\n", __func__, __LINE__, __phys_to_pfn(lx_mem2_addr), __phys_to_pfn(lx_mem2_addr + lx_mem2_size - PAGE_SIZE));
	return 0;
}
