...
static int MDev_EMAC_update_linkspeed (struct net_device *dev)
{
	...
	static unsigned int phy_restart_cnt;
...
static int MDev_EMAC_open (struct net_device *dev)
{
    ...
    if (-1 == ret)
    {
		...
		ThisBCE.connected = 0;
		EMAC_DBG("link down in open\n");
    }
    else if(0 == ret)
    {
        netif_carrier_on(dev);
        ThisBCE.connected = 1;
		EMAC_DBG("link up in open\n");
    }
    return 0;
}
...
static void MDev_EMAC_timer_callback(unsigned long value)
{
    ...
    {
		if ((is_still_an == TRUE) && (ret == -1)) {
			EMAC_DBG("link down after an in timer cb\n");
			is_still_an = FALSE;
		}

		if (ThisBCE.connected) {
			EMAC_DBG("link down in timer cb\n");
            ThisBCE.connected = 0;
        }
...
static int MDev_EMAC_init(struct platform_device *pdev)
{
    ...
    SET_NETDEV_DEV(emac_dev, &(pdev->dev));
#if TX_THROUGHPUT_TEST
