...
{
	// GPIO18 share for JP1 & JP2
	// Let GPIO18 use for FCIE JP2 for compatible with MST095D
	// MST224AB & MST095D use GPIO18 as CDZ
	// 2016.2.1 MST098D also use GPIO18
	FCIE_RIU_16_ON(CHIP_GPIO_09, GPIO18_OEN);

	if (FCIE_RIU_R16(CHIP_GPIO_09) & GPIO18_IN)
	{
		return 0;
	}
	else
...
#if 0
    // GPIO27 224A
    // high means write protected
    FCIE_RIU_16_ON(CHIP_GPIO_0D, GPIO27_OEN);

    if(FCIE_RIU_R16(CHIP_GPIO_0D) & GPIO27_IN)
    {
...
#else
    // GPIO17 is Amazon abc123 project WP
    // low should be write protected
    FCIE_RIU_16_ON(CHIP_GPIO_08, GPIO17_OEN);

    if(FCIE_RIU_R16(CHIP_GPIO_08) & GPIO17_IN)
    {
        return 0;
    }
    else
