...
static int mmc_fcie_suspend_notifier(struct notifier_block *nb,
				unsigned long event,
				void *dummy)
{
	(void)nb;
...
static struct notifier_block mmc_fcie_notif_block = {
	.notifier_call = mmc_fcie_suspend_notifier,
};
...
static int mmc_fcie_probe(struct platform_device *pdev)
{
	...
	ret = register_pm_notifier(&mmc_fcie_notif_block);
	if (ret)
		pr_err("mmc_fcie: failed to register pm notifier\n");
...
{
	unregister_pm_notifier(&mmc_fcie_notif_block);
...
int mmc_fcie_hotplug(void *data)
{
	...
	while(1)
	{
		if (try_to_freeze()) {
			...
			pr_debug("mmc_fcie: sleep for 5s on resume\n");
			msleep(5000);
			card_det = 0;
