...
#define     GFLIPIO_KDBG(_fmt, _args...)        pr_debug(KERN_WARNING _fmt, ## _args)
#define     GFLIPIO_ASSERT(_con)   do {\
                                                            if (!(_con)) {\
								GFLIPIO_KDBG(KERN_CRIT "BUG at %s:%d assert(%s)\n", __FILE__, __LINE__, #_con);\
                                                                BUG();\
                                                            }\
                                                          } while (0)
#else
...
int _MDrv_GFLIPIO_IOC_RESUME_GOP(struct file *filp, unsigned long arg);
...
int _MDrv_GFLIPIO_IOC_RESUME_GOP(struct file *filp, unsigned long arg)
{
	...
	return MDrv_GFLIP_Resume_GOP(u32GopIdx) ? 0 : (-EFAULT);
}
/********************************************************
...
{
	GFLIPIO_KDBG("This ioctl is obsolete now\n");
    return -1;
}
...
int _MDrv_GFLIPIO_IOC_GetVsync(struct file *filp, unsigned long arg)
{
    ...
    {
		GFLIPIO_KDBG("GFLIPIO_IOC_GetVsync GOP = %td is out of range\n", (ptrdiff_t)u32GopIdx);
        return EFAULT;
    }
    u64VsyncTs = MDrv_GFLIP_WaitForVsync(u32GopIdx);
    __put_user(u64VsyncTs, &(((GFLIP_GETVSYNC_DATA __user *)arg)->vsync_ts));
    return 0;
}
...
long _MDrv_GFLIPIO_IOCtl(struct file *filp, U32 u32Cmd, unsigned long u32Arg)
{
    ...
    switch(u32Cmd)
    {
        ...
        case MDRV_GFLIP_IOC_DLCSETHDRINFO:
            ...
            break;
	case MDRV_GFLIP_IOC_SETLOGOGOP1:
		retval = _MDrv_GFLIPIO_IOC_RESUME_GOP(filp , u32Arg);
	break;
