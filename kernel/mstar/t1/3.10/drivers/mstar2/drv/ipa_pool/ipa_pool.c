...
struct ipa_pool_allocation
{
    ...
    u64 timestamp;                 //jiffies when pool allocation is inserted
};
...
struct IPA_space_heap_man IPASpcHeapMan;
...
#ifdef PA_INFO_DEBUG
#define IPA_PA_INFO_DEBUG(fmt, args...)  \
    printk(KERN_ERR "%s %d: " fmt, __FUNCTION__, __LINE__, ## args);
#else
#define IPA_PA_INFO_DEBUG(fmt, args...)
#endif
             ...
             && pool_allocation->offset_in_heap+pool_allocation->length-1 >= start)
          {
             ...
             if(pool_allocation->pool !=  MSTAR_IPA_STR_POOL)
             {
                ...
                if(MAX_CONFLICT_WAKE_UP_COUNTS == i)//if not find 
                {
                    ...
                    IPA_HEAP_ALLOC_DEBUG("client_filp=%p,offset_in_heap 0x%lx ,length=0x%lx  conflict_counts=0x%lx\n",pool_allocation->client_filp,pool_allocation->offset_in_heap,pool_allocation->length,conflict_info->conflict_counts);
                }
             }
          }
       }
                                                  ...
                                                  unsigned long start, unsigned long length, struct IPA_pool_conflict *conflict_info,  unsigned long timeout)
{
         ...
         &heap->pool_allocation_tree_root, merge_check_start, merge_check_last)
   {
      if((pool_allocation->pool != pool) 
        ...
        && !(pool_allocation->pool->flags & IPA_POOL_FLAG_NO_CONFLICT))
      {
         //the required range is holded by other co-buffered pools, reject it
         if(pool_allocation->offset_in_heap <= last
		 	  && pool_allocation->offset_in_heap+pool_allocation->length-1 >= start)
...
SKIP_MERGE:
   pool_allocation2->timestamp = jiffies;
       ...
       if(pool_allocation->client_filp == client_filp)
       {
                ...
                && last>=pool_allocation->offset_in_heap)
            {
                ...
                if(start>pool_allocation->offset_in_heap && last<pool_allocation->offset_in_heap+pool_allocation->length-1)
                {
                   ...
                   //Use old time stamp
                   pool_allocation2->timestamp = pool_allocation->timestamp;
                   pool_allocation2->client_filp = client_filp;
                   ...
                   //Do not change time stamp of pool_allocation
...
int pool_allocation_timeout_test(void *param1, void *param2)
{
    ...
    struct ipa_pool_allocation *pool_allocation = (struct ipa_pool_allocation *)param2;
    
    u64 timeout = pool_allocation->timestamp;
    timeout += (u64)msecs_to_jiffies(heap->delay_free_timelimit);
    
    if(time_after64(jiffies, timeout))
        return 1;
    return 0;
}
	                           ...
	                           unsigned long start, unsigned long length, struct list_head *head, test_t test)
{
    ...
    mutex_lock(&heap->lock);
    heap_pool_allocation_interval_tree_foreach(pool_allocation,
         &heap->pool_allocation_tree_root, start, start + length -1) {

        if(pool_allocation->pool != pool || pool_allocation->client_filp != client_filp)
            continue;

        if(test && !test(heap, pool_allocation))
            continue;

        node = (struct pool_range_node *)kzalloc(sizeof(struct pool_range_node), GFP_KERNEL);
        if(!node) {
            IPA_HEAP_ERROR("Alloc range node fail\n");
            goto error;
        }

        node->start = start > pool_allocation->offset_in_heap ? start : pool_allocation->offset_in_heap;
        node->length = start + length < pool_allocation->offset_in_heap + pool_allocation->length ? 
            start + length - node->start : 
            pool_allocation->offset_in_heap + pool_allocation->length - node->start;
	                           ...
	                           struct file *client_filp, struct list_head *head, test_t test)
{
    ...
    list_for_each_entry_safe(node, next, head, list) {
        if(test && !test(heap, node))
            continue;
        ret = pool_alloc(pool, client_filp, node->start, node->length, &conflict_info, 0);
        if(ret) {
            IPA_HEAP_ALLOC_DEBUG("Fail to alloc for pool %d, (%llx, %llx)\n", pool->pool_id, 
                node->start, node->length);
            goto out;;
        }
        list_move(&node->list, &alloc_list);
    }
    
out:
    //Free the nodes that alloc success
    if(ret) {
        int ret2 = 0;
        
        list_for_each_entry_safe(node, next, &alloc_list, list) {
            ret2 = pool_free(pool, client_filp, node->start, node->length);
            if(ret2) {
                IPA_HEAP_ERROR("Fail to free for pool %d(%llx, %llx)\n", pool->id, 
                    node->start, node->length);
                BUG();
            }
            list_move(&node->list, head);
        }  
    }
    else {
        list_for_each_entry_safe(node, next, &alloc_list, list) {
...
int pa_miu_get_heap(signed int miu,unsigned long pa_offset_in_miu,struct IPA_heap_attr *info,bool *in_heap,signed int *heap_id)
{
    ...
    *in_heap = false;
    mutex_lock(&IPASpcHeapMan.lock);
    IPA_PA_INFO_DEBUG("debug %s:%d  miu=%d,pa_offset_in_miu=0x%lx\n",__FUNCTION__,__LINE__,miu,pa_offset_in_miu);
    list_for_each_entry(heap, &IPASpcHeapMan.heaps, node)
    {  
        ret = heap->heap_op->heap_get_attr(info, heap->priv);
        if(0 == ret)
        {
            BUG_ON(info == NULL);
            if((info->miu == miu)
                &&(pa_offset_in_miu >= info->miu_offset)
                &&(pa_offset_in_miu <= info->miu_offset + info->heap_len -1))
            {
                *in_heap = true;
                *heap_id = heap->hid;
                IPA_PA_INFO_DEBUG("%s:%d  *heap_id=%d  info->miu=%d,info->miu_offset=0x%lx,info->heap_len=0x%lx\n",__FUNCTION__,__LINE__,*heap_id,info->miu,info->miu_offset,info->heap_len);
                break;
            }
        }
...
//before call this function,should already get info about heap,
//and this func is used for get whether heap_off is allocated,and allocated by who.
...
{
    struct ipa_pool_allocation *pool_allocation=NULL;
    struct IPA_heap* heap = NULL;
    IPA_PA_INFO_DEBUG("debug %s:%d   space_id=%d   heap[0 0x%lx],pa_offset_in_heap=0x%lx\n",__FUNCTION__,__LINE__,space_id,space_length,pa_offset_in_heap);
    *p_allocated = false;//initial out value wiht invalid value
    mutex_lock(&IPASpcHeapMan.lock);
    heap = _IPASpcMan_find_heap(space_id);
    mutex_unlock(&IPASpcHeapMan.lock);
    mutex_lock(&heap->lock); 
    if(heap)
    {
        heap_pool_allocation_interval_tree_foreach(pool_allocation,
              &heap->pool_allocation_tree_root, 0
              , 0 + space_length -1)
        {   
            IPA_PA_INFO_DEBUG("debug %s:%d   space_id=%d   pool_allocation[0x%lx 0x%lx]\n",__FUNCTION__,__LINE__,space_id,pool_allocation->offset_in_heap,pool_allocation->length);             
             if(( pa_offset_in_heap >= pool_allocation->offset_in_heap)
                 &&(pa_offset_in_heap <= pool_allocation->offset_in_heap + pool_allocation->length -1))
             {
                 //allocated
                 *p_allocated =true;
                 IPA_PA_INFO_DEBUG("debug %s:%d   space_id=%d   heap[0 0x%lx],pa_offset_in_heap=0x%lx allocated\n",__FUNCTION__,__LINE__,space_id,space_length,pa_offset_in_heap);       
                 IPA_PA_INFO_DEBUG("%s:%d  pool_allocation->pool   hid=%d,heap_off=0x%lx,len=0x%lx\n",__FUNCTION__,__LINE__,pool_allocation->pool->hid,pool_allocation->pool->heap_off,pool_allocation->pool->len);
                 return pool_allocation->pool;
             }
        }    
