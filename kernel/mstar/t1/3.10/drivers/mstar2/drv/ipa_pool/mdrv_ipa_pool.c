...
#if 0
#define IPAPOOLDEBUG(fmt,...) printk("[IPAPool Debug][%s %d]: " fmt, __FUNCTION__, __LINE__,##__VA_ARGS__)
#else
...
#define IPAPOOL_REGION_VMA(reg) (reg->data)
                           ...
                           struct file **pfp,struct IPA_pool_ref **ppr,struct IPA_pool **ppool)
{
    ...
    fp=fget(pool_id);
    if(!fp){
        IPAPOOLERR("%s:%d  no pool %d  ,current->comm=%s\n",__FUNCTION__, __LINE__, pool_id,current->comm);
        ret=-IPAERROR_NO_POOL;
        return ret;
    }
    if(!_IPAPool_IsValidFile(fp)){
        ...
        if(conflict_info.conflict_counts > 0)
        {
            for(i=0;i<conflict_info.conflict_counts;i++)
            {
                ...
                if(conflict_info.conflict_detail[i].conflict_client_filp)
                {
                    ...
                    IPAPOOLDEBUG("%s:%d, wake_pr=%p  conflict_info.conflict_detail[%d].conflict_client_filp=%p\n", __FUNCTION__, __LINE__,wake_pr, i, conflict_info.conflict_detail[i].conflict_client_filp);
                    ...
                    if(wake_pr->having_polling_thread)
                    {
                        ...
                        {  
                            ...
                            IPAPOOLDEBUG("%s:%d  wake_pr=%p   event %d  start_length[0x%llx,   0x%llx]\n",__FUNCTION__,__LINE__,wake_pr,event_node->pool_event.event,event_node->pool_event.start,event_node->pool_event.length);
...
static inline void phy_to_MiuOffset(unsigned long phy_addr, unsigned int *miu, unsigned long *offset)
{
    *miu = INVALID_MIU;
    ...
    IPAPOOLDEBUG("debug %s:%d  , phy_addr=0x%lx,*miu=%u,*offset=0x%lx,ARM_MIU1_BUS_BASE=0x%x\n",__FUNCTION__,__LINE__,phy_addr,*miu,*offset,ARM_MIU1_BUS_BASE);
}
...
{    
    ...
    IPAPOOLDEBUG("debug %s:%d  , *phy_addr=0x%lx,miu=%u,offset=0x%lx\n",__FUNCTION__,__LINE__,*phy_addr,miu,offset);
}
...
static void PA_in_IPA_POOL_info(struct PA_In_IPA_Pool_Args *in_ipa_pool_args)
{
    ...
    if(INVALID_MIU == miu)
    {
        ...
        IPAPOOLDEBUG("%s:%d error ,fail get miu phy_addr=0x%lx\n",__FUNCTION__,__LINE__,phy_addr);
        in_ipa_pool_args->pa_state = PA_NOT_IN_MIU;
        return;
    }

    BUG_ON((miu + 1) > MIU_NUM);//shoud not exceed total miu num
    in_ipa_pool_args->miu = miu;
    IPAPOOLDEBUG("debug %s:%d phy_addr=0x%lx miu=%d  miu=%d\n",__FUNCTION__,__LINE__,phy_addr,miu,in_ipa_pool_args->miu);
        
    //get which heap and which pool.

    ret = pa_miu_get_heap(in_ipa_pool_args->miu,pa_offset_in_miu,&info,&in_ipa_pool_args->in_heap,&in_ipa_pool_args->space_id);
    
    if(FALSE == in_ipa_pool_args->in_heap)
        {
        IPAPOOLDEBUG("%s:%d not in heap!!!\n",__FUNCTION__,__LINE__);
        in_ipa_pool_args->pa_state = PA_NOT_IN_HEAP;
        return;
        }

    in_ipa_pool_args->space_type =info.type;
    in_ipa_pool_args->space_miu_start_offset =info.miu_offset;
    in_ipa_pool_args->space_length = info.heap_len;
    in_ipa_pool_args->pa_offset_in_heap = (unsigned long)(pa_offset_in_miu - info.miu_offset);
    IPAPOOLDEBUG("%s:%d   in_ipa_pool_args->space_length=%lx\n",__FUNCTION__,__LINE__,in_ipa_pool_args->space_length);
    IPAPOOLDEBUG("%s:%d   info.heap_len=%lx\n",__FUNCTION__,__LINE__,info.heap_len);
    IPAPOOLDEBUG("debug %s:%d heap_id=%d,[0x%lx, 0x%lx]\n",__FUNCTION__,__LINE__
                                  ,in_ipa_pool_args->space_id,(unsigned long)in_ipa_pool_args->space_miu_start_offset,(unsigned long)in_ipa_pool_args->space_length);
    IPAPOOLDEBUG("%s:%d   in_ipa_pool_args->pa_offset_in_heap=0x%lx\n",__FUNCTION__,__LINE__,(unsigned long)in_ipa_pool_args->pa_offset_in_heap);
    pool = heap_check_allocation(in_ipa_pool_args->space_id,(unsigned long)in_ipa_pool_args->space_length,(unsigned long)in_ipa_pool_args->pa_offset_in_heap
                                                        ,&in_ipa_pool_args->allocated);

    if(FALSE == in_ipa_pool_args->allocated || (!pool))
    {
        IPAPOOLDEBUG("%s:%d not allocated!!!\n",__FUNCTION__,__LINE__);
        in_ipa_pool_args->pa_state = PA_NOT_IN_POOL;
        return;
    }
    IPAPOOLDEBUG("debug %s:%d   \n",__FUNCTION__,__LINE__);
   ...
   // in_ipa_pool_args->pool_handle_id = xxx; //pool_handle_id not easy to get
    ...
    IPAPOOLDEBUG("debug %s:%d   allocated in pool.\n",__FUNCTION__,__LINE__);
    in_ipa_pool_args->pool_len = pool->len;       //out: pool length in  space 
    in_ipa_pool_args->pool_offset_in_heap =pool->heap_off;    //out: pool location in heap
    IPAPOOLDEBUG("%s:%d   in_ipa_pool_args->pa_offset_in_heap=0x%lx,  pool_in_heap[0x%lx,0x%lx]\n",__FUNCTION__,__LINE__,(unsigned long)in_ipa_pool_args->pa_offset_in_heap,(unsigned long)in_ipa_pool_args->pool_offset_in_heap,(unsigned long)in_ipa_pool_args->pool_len);

    if(in_ipa_pool_args->pa_offset_in_heap < in_ipa_pool_args->pool_offset_in_heap)
    {
        IPAPOOLDEBUG("%s:%d error fail!!!  pa_offset_in_heap=0x%lx,pool_offset_in_heap=0x%lx\n",__FUNCTION__,__LINE__,(unsigned long)in_ipa_pool_args->pa_offset_in_heap,(unsigned long)in_ipa_pool_args->pool_offset_in_heap);
        in_ipa_pool_args->pa_state = PA_NOT_IN_POOL;
        return;
    }
    ...
    IPAPOOLDEBUG("debug %s:%d   allocated in pool. in_ipa_pool_args->pa_offset_in_pool=0x%lx\n",__FUNCTION__,__LINE__,in_ipa_pool_args->pa_offset_in_pool);
...
static void POOL_TO_PA_CALC(struct Pool_To_PA_Args *pool_to_pa_args)
{
    ...
    IPAPOOLDEBUG("debug %s:%d  hid[0x%x] \n",__FUNCTION__,__LINE__,pool->hid);
    pool_to_pa_args->heap_id = pool->hid;
    
    IPAPOOLDEBUG("debug %s:%d  heap_id[0x%x] \n",__FUNCTION__,__LINE__,pool_to_pa_args->heap_id);
    ...
    if(heap_get_attr(pool_to_pa_args->heap_id, &heap_attr)>=0)
    {
        ...
        IPAPOOLDEBUG("debug %s:%d  heap_id[0x%x] \n",__FUNCTION__,__LINE__,pool_to_pa_args->heap_id);
        IPAPOOLDEBUG("debug %s:%d  miu[0x%x] \n",__FUNCTION__,__LINE__,heap_attr.miu);
        pool_to_pa_args->miu = heap_attr.miu;
        IPAPOOLDEBUG("debug %s:%d  heap_id[0x%x] \n",__FUNCTION__,__LINE__,pool_to_pa_args->heap_id);
        IPAPOOLDEBUG("debug %s:%d  miu[0x%x] \n",__FUNCTION__,__LINE__,pool_to_pa_args->miu);
        
        offset = (unsigned long)pool_to_pa_args->offset_in_pool + pool->heap_off + heap_attr.miu_offset;
    }
    else
    {
        IPAPOOLDEBUG("%s:%d  heap_get_attr fail  heap_id=0x%x\n",__FUNCTION__,__LINE__,pool_to_pa_args->heap_id);
        pool_to_pa_args->error_code = -1;
        goto OUT;
    }
    ...
    IPAPOOLDEBUG("debug %s:%d  heap_id[0x%x] \n",__FUNCTION__,__LINE__,pool_to_pa_args->heap_id);
    IPAPOOLDEBUG("debug %s:%d  miu[0x%x] \n",__FUNCTION__,__LINE__,pool_to_pa_args->miu);
    IPAPOOLDEBUG("debug %s:%d  phy_addr[0x%lx] \n",__FUNCTION__,__LINE__,phy_addr);
    pool_to_pa_args->PA = phy_addr;
    IPAPOOLDEBUG("debug %s:%d  heap_id[0x%x] \n",__FUNCTION__,__LINE__,pool_to_pa_args->heap_id);
    IPAPOOLDEBUG("debug %s:%d  miu[0x%x] \n",__FUNCTION__,__LINE__,pool_to_pa_args->miu);    
    IPAPOOLDEBUG("debug %s:%d  PA[0x%lx] \n",__FUNCTION__,__LINE__,(unsigned long)pool_to_pa_args->PA);    
    
OUT:    
    mutex_unlock(&IPAPoolDev.apilock);

    //For 32bits chip printk right,MS_U64 var must use (unsigned long)var .
    IPAPOOLDEBUG("debug %s  [0x%lx ,0x%x,0x%x] \n",__FUNCTION__,(unsigned long)pool_to_pa_args->PA,pool_to_pa_args->miu,pool_to_pa_args->heap_id);
    return ;
}
...
void str_reserve_mboot_ipa_str_pool_buffer()
{
    ...
    if(MSTAR_IPA_STR_POOL == NULL)
    {
        ...
        printk("[%s:%d] will create pool heap_id=%d,heap_off=0x%lx,pool_len=%lx,name=%s\n"
        ...
        printk("[%s:%d] after create pool heap_id=%d,heap_off=0x%lx,pool_len=%lx,name=%s\n"
                 ,__FUNCTION__,__LINE__,ipa_str_pool_config.ipa_id,ipa_str_pool_config.start,ipa_str_pool_config.size, ipa_str_pool_config.name);//notify reserve process.
        mutex_unlock(&IPAPoolDev.apilock);
        if(ret <0)
        {
...
static long IPAPoolDev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case IPA_POOL_IOC_HEAP_ATTR:
     ...
     case IPA_POOL_IOC_PA_INFO:
     {
        ...
        data.in_ipa_pool_args.space_id = in_ipa_pool_args.space_id;
        data.in_ipa_pool_args.space_type = in_ipa_pool_args.space_type;
        data.in_ipa_pool_args.space_miu_start_offset = in_ipa_pool_args.space_miu_start_offset;
        data.in_ipa_pool_args.space_length = in_ipa_pool_args.space_length;
        ...
        {
           IPAPOOLDEBUG("space_id=[%d  %d]",in_ipa_pool_args.space_id,data.in_ipa_pool_args.space_id);
     ...
     {
        struct Pool_To_PA_Args pool_to_pa_args;
        IPAPOOLDEBUG("%s:%d  case IPA_POOL_IOC_POOL_TO_PA\n",__FUNCTION__,__LINE__);
        pool_to_pa_args.error_code = 0;
