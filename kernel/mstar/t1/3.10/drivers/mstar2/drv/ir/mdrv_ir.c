...
#ifndef IR3_HEADER_CODE0
#define IR3_HEADER_CODE0 0xffUL
#endif
#ifndef IR3_HEADER_CODE1
#define IR3_HEADER_CODE1 0xffUL
#endif

static U8 _u8IR3HeaderCode0 = IR3_HEADER_CODE0;
static U8 _u8IR3HeaderCode1 = IR3_HEADER_CODE1;
#endif
...
#elif (IR_TYPE_SEL == IR_TYPE_MSTAR_DTV)
static U8 _u8FactoryIRHeaderCode_CV_0 = IR_FHEADER_CV_CODE0;
static U8 _u8FactoryIRHeaderCode_CV_1 = IR_FHEADER_CV_CODE1;
static U8 _u8FactoryIRHeaderCode_TS_0 = 0xFF;
static u8 _u8FactoryIRHeaderCode_TS_1 = 0xFF;
static u8 _u8FactoryIRHeaderCode_IN_0 = 0xFF;
static u8 _u8FactoryIRHeaderCode_IN_1 = 0xFF;
static U8 _u8PCCommandHeaderCode0 = FCTORY_HEADER_RECEIVE;
static U8 _u8PCCommandHeaderCode1 = PCCOMMAND_HEADER_RECEIVE;
#define IR_REPEAT_SKIP_COUNT    0
#else
static U8 _u8FactoryIRHeaderCode0 = 0x00UL;
static U8 _u8FactoryIRHeaderCode1 = 0x00UL;
#endif

#ifndef IR_REPEAT_SKIP_COUNT
#define IR_REPEAT_SKIP_COUNT 5
#endif
...
struct ir_remote_wake_map {
	u32 ircode;
	u32 keycode;
	u32 toggle_backlight;
	char keyname[32];
};

static struct ir_remote_wake_map keira_ir_wake_map[] = {
	{ 0x46,	KEY_POWER,	OTA_TOGGLE_BACKLIGHT,	"AMZN_KEY_POWER"	},
	{ 0xA0, KEY_POWER,	OTA_TURN_ON_BACKLIGHT,	"AMZN_VOICE"		},
	{ 0xA1,	KEY_CUSTOM_2,	OTA_TURN_ON_BACKLIGHT,	"AMZN_KEY_CUSTOM_2"	},
	{ 0x5F,	KEY_CUSTOM_1,	OTA_TURN_ON_BACKLIGHT,	"AMZN_KEY_CUSTOM_1"	},
	{ 0xA2,	KEY_CUSTOM_3,	OTA_TURN_ON_BACKLIGHT,	"AMZN_KEY_CUSTOM_3"	},
	{ 0xA3, KEY_CUSTOM_4,	OTA_TURN_ON_BACKLIGHT,	"AMZN_KEY_CUSTOM_4"	},
	{ 0xA4, KEY_BUTTON_1,	OTA_TURN_ON_BACKLIGHT,	"AMZN_KEY_BUTTON_1"	},
	{ 0xA5, KEY_BUTTON_2,	OTA_TURN_ON_BACKLIGHT,	"AMZN_KEY_BUTTON_2"	},
	{ 0x9F, KEY_POWER,	OTA_TURN_ON_BACKLIGHT,	"AMZN_KEY_HOME"		},
	{ 0x4A, KEY_POWER, 	OTA_TURN_ON_BACKLIGHT,	"AMZN_KEY_ENTER"	},
	{ 0x12,	KEY_POWER,	OTA_TOGGLE_BACKLIGHT,	"TS_KEY_POWER"		},
	{ 0x15, KEY_SLEEP,	OTA_TURN_OFF_BACKLIGHT, "TS_KEY_SLEEP"		},
	{ 0x7E, KEY_POWER,	OTA_TURN_ON_BACKLIGHT,  "TS_KEY_WAKEUP"		},
	{ 0x7F, KEY_SLEEP,	OTA_TURN_OFF_BACKLIGHT, "TS_KEY_SLEEP"		},
	{ 0x0F,	KEY_POWER,	OTA_TOGGLE_BACKLIGHT,	"IN_KEY_POWER"		},
	{ 0x13, KEY_SLEEP,	OTA_TURN_OFF_BACKLIGHT, "IN_KEY_SLEEP"		},
	{ 0x61, KEY_POWER, 	OTA_TURN_ON_BACKLIGHT,  "IN_KEY_WAKEUP"		},
	{ 0x60, KEY_SLEEP,	OTA_TURN_OFF_BACKLIGHT, "IN_KEY_SLEEP"		},
	{ 0xAF,	KEY_LIVETV,	OTA_TURN_ON_BACKLIGHT,	"IN_KEY_LIVETV"		},
	{ 0x20,	KEY_CUSTOM_1,	OTA_TURN_ON_BACKLIGHT,	"IN_KEY_CUSTOM_1"	},
	{ 0x27,	KEY_CUSTOM_2,	OTA_TURN_ON_BACKLIGHT,	"IN_KEY_CUSTOM_2"	},
	{ 0x22,	KEY_CUSTOM_3,	OTA_TURN_ON_BACKLIGHT,	"IN_KEY_CUSTOM_3"	},
	{ 0x23, KEY_CUSTOM_4,	OTA_TURN_ON_BACKLIGHT,	"IN_KEY_CUSTOM_4"	},
	{ 0x61, KEY_POWER,	OTA_TURN_ON_BACKLIGHT,	"IN_KEY_POWER_ON"	},
};
int mstar_get_wake_key_name(u32 ir_code, char *key_name, u32 *key_code, u32 *toggle)
{
	int size = ARRAY_SIZE(keira_ir_wake_map);
	int i = 0;

	for (i = 0; i < size; i++) {
		if (keira_ir_wake_map[i].ircode == ir_code) {
			if (key_name)
				strcpy(key_name, keira_ir_wake_map[i].keyname);
			if (key_code)
				*key_code = keira_ir_wake_map[i].keycode;
			if (toggle)
				*toggle = keira_ir_wake_map[i].toggle_backlight;
			return 0;
		}
	}
...
static void print_keyname_and_set_backlight(u32 scancode)
{
	u32 key_code = 0;
	u32 toggle = 0;
	char keyname[32] = "";
	if (mstar_get_wake_key_name(scancode, keyname, &key_code, &toggle) >= 0) {
		pr_info("IR %s is pressed \n", keyname);
#ifdef CONFIG_AMAZON_SILENT_OTA_BACKLIGHT
		if (toggle)
			toggle_backlight(toggle);
#endif
	}
}
//-------------------------------------------------------------------------------------------------
...
static U8 _MDrv_IR_CUS22T_Factory_ParseKey(U32 u32KeyData)
{
	...
	pr_debug("Linux: %s:%s:%d u32KeyData:%x\n", __FILE__, __func__, __LINE__, u32KeyData);
...
void enqueue(u32 data) {
	...
	DEBUG_IR_INPUT_DEVICE(pr_debug("queue is full\n"));
    }  else {
...
u32 dequeue(void) {
    ...
    if (q.front == q.rear) {
	DEBUG_IR_INPUT_DEVICE(pr_debug("queue is empty\n"));
    } else {
...
int key_dispatch(void *p) {
	...
	DEBUG_IR_INPUT_DEVICE(pr_debug("key_dispatch thread start\n"));
	...
	DEBUG_IR_INPUT_DEVICE(pr_debug("xxxxx 0\n"));
        if (prev_scancode == 0xFFFFUL) {
		DEBUG_IR_INPUT_DEVICE(pr_debug("xxxxx 1\n"));
            ret = wait_for_completion_interruptible(&key_completion);
        } else {
		DEBUG_IR_INPUT_DEVICE(pr_debug("xxxxx 2\n"));
        ...
        if (ret < 0) {
		DEBUG_IR_INPUT_DEVICE(pr_debug("completion interruptible\n"));
            continue;
        }
	scancode = dequeue();
        if ((prev_scancode != 0xFFFFUL) && (scancode == 0xFFFFUL)) {
		DEBUG_IR_INPUT_DEVICE(pr_debug("xxxxx 3\n"));
            rc_keyup(ir->dev);
		print_keyname_and_set_backlight(prev_scancode & 0xFF);
        } else if ((prev_scancode != 0xFFFFUL) && (scancode != 0xFFFFUL)) {
		DEBUG_IR_INPUT_DEVICE(pr_debug("xxxxx 4\n"));

           if ((scancode != prev_scancode)) {
		DEBUG_IR_INPUT_DEVICE(pr_debug("xxxxx 5\n"));
               rc_keyup(ir->dev);
		print_keyname_and_set_backlight(prev_scancode & 0xFF);
		...
		DEBUG_IR_INPUT_DEVICE(pr_debug("xxxxx 6\n"));
            rc_keydown_notimeout(ir->dev, scancode, 0);
        }

	DEBUG_IR_INPUT_DEVICE(pr_debug("xxxxx 7, scancode=%d\n", scancode));
        prev_scancode = scancode;
    }

	DEBUG_IR_INPUT_DEVICE(pr_debug("key_dispatch thread end\n"));
    return 0;
}
...
static void _MDrv_IR_MailBoxHandler(MBX_Class eMsgClass, MBX_MSGQ_Status *pMsgQueueStatus)
{
    ...
    {
	pr_debug(KERN_EMERG "Handler Timeout!\n");
        return;
    }
    ...
    if (gMBXResult == E_MBX_SUCCESS)
    {
        ...
        {
            gIRResult = E_IR_FAIL;
		pr_debug(KERN_EMERG "Not Implemented!\n");
            return;
        }
	gIRResult = E_IR_FAIL;
...
static irqreturn_t  MDrv_IR_ISRParseKey(void)
{
    ...
    if(_eCurentProtocol!=E_IR_PROTOCOL_NONE)
    {
        ...
        else if(eResult == E_IR_DECODE_DATA_OK)
        {
            ...
            while(down_trylock(&fantasy_protocol.sem)!=0)
            {
		pr_debug("busy wait semaphore\n");
            }

            if(fantasy_protocol.data!=0)
            {
		pr_debug("unprocessed fantasy protocol data!!!!\n");
            }
            else
            {
    ...
    if (bHaveKey = _MDrv_IR_GetKey(&u8Key, &u8System, &u8RepeatFlag)) {
        if (down_trylock(&fantasy_protocol.sem) == 0) {
            if (fantasy_protocol.data == 0) {
                ...
                #elif (IR_TYPE_SEL == IR_TYPE_MSTAR_DTV)
                U32 cultraview_factory_ir=0;
                if(u8RepeatFlag&FCTORY_HEADER_RECEIVE)//sec ir
                {
                    u8RepeatFlag &= (~FCTORY_HEADER_RECEIVE);
                    cultraview_factory_ir=(0x04 << 28);
                }
                else if(u8RepeatFlag&PCCOMMAND_HEADER_RECEIVE)//third ir
                {
                    u8RepeatFlag &= (~PCCOMMAND_HEADER_RECEIVE);
                    cultraview_factory_ir=(0x08 << 28);
                }
                else//first ir
                {
                    cultraview_factory_ir=(0x01 << 28);
                }
                fantasy_protocol.data = (u8Key<<8 | u8RepeatFlag);
                fantasy_protocol.data |= cultraview_factory_ir;
#elif defined(IR_TYPE_TCL) && (IR_TYPE_SEL == IR_TYPE_TCL)
                fantasy_protocol.data = (u8Key << 8 | u8RepeatFlag);
...
static BOOL _MDrv_IR_GetKey(U8 *pu8Key, U8 *pu8System, U8 *pu8Flag)
{
    ...
    *pu8System = 0;
	...
	if(_bskyworth_normal_code || _bskyworth_shuttle_code)
	{
		if(_brepeat_flag)
		{
			...
			*pu8Flag = 1;
			...
			_bskyworth_normal_code = FALSE;
			_bskyworth_shuttle_code = FALSE;
			DEBUG_IR(pr_debug("Repeat input key is 0x%x\n", *pu8Key));
			return TRUE;
		}

		if(_bskyworth_normal_code)
		{
			DEBUG_IR(pr_debug("Use skyworth normal code\n"));
			if (_u32IRCount < 34)  //multiple 8
			{
				DEBUG_IR(pr_debug("Get normal IRCount error, _u32IRCount=%d\n", _u32IRCount));
			...
			if(_u32IRData[1]< (IR_HDC_LOB)||_u32IRData[1] > (IR_HDC_UPB))
			{
				...
				_bskyworth_normal_code = FALSE;
				_bIRStart = FALSE;
				DEBUG_IR(pr_debug("No normal IR header\n"));
				return FALSE;
			}
		...
		{
			DEBUG_IR(pr_debug("Use skyworth shuttle code\n"));
			if(_u32IRCount < (10))
			{
				DEBUG_IR(pr_debug("Get shuttle IRCount error, _u32IRCount=%d\n", _u32IRCount));
				return FALSE; //not complete yet
			}
			if(_u32IRData[1]< (IR_OFC_LOB2)||_u32IRData[1] > (IR_OFC_UPB2))
			{
				...
				_bskyworth_shuttle_code = FALSE;
				_bIRStart = FALSE;
				DEBUG_IR(pr_debug("No shuttle IR header\n"));
				return FALSE;
			}
			u8IRSwModeByte = 0;
			pu32IRData = &_u32IRData[2];
			...
			for( u8Byte=0; u8Byte<IR_RAW_DATA_NUM; u8Byte++)
			{
				for( u8Bit=0; u8Bit<8; u8Bit++)
				{
					...
					else
					{
						...
						_bskyworth_normal_code = FALSE;
						_bIRStart = FALSE;
						DEBUG_IR(pr_debug("Invalid waveform,0x%x\n", u32BitLen));
						return FALSE;
					}
				}
				if((u8IRSwModeBuf[0]!=_u8IRHeaderCode0)||(u8Byte==1&&u8IRSwModeBuf[1]!=_u8IRHeaderCode1))
				{
					...
					_bskyworth_normal_code = FALSE;
					_bIRStart = FALSE;
					DEBUG_IR(pr_debug("Invalid header,0x%x,0x%x\n", u8IRSwModeBuf[0], u8IRSwModeBuf[1]));
					return FALSE;
				}
				else if(u8Byte==3)
				{
					if(u8IRSwModeBuf[2] == (u8)~u8IRSwModeBuf[3])
					{
						...
						*pu8Flag = 0;
						...
						_bskyworth_normal_code = FALSE;
						_bIRStart = FALSE;
						DEBUG_IR(pr_debug("Input key is 0x%x\n", *pu8Key));
						return TRUE;
					}
				}
			}
		...
		else if(_bskyworth_shuttle_code) //we have to handle shuttle key
		{
			for( u8Bit=0; u8Bit<8; u8Bit++)
			{
				...
				else
				{
					...
					_bskyworth_shuttle_code = FALSE;
					_bIRStart = FALSE;
					DEBUG_IR(pr_debug("Invalid waveform,0x%x,u32BitLen[%d]=ERROR\n", u32BitLen, u8Bit));
					return FALSE;
				}
			}
			...
			if((u8IRSwModeByte&0x03UL) != 0x01UL ) // Customer_Code(01)
			{
				...
				_bskyworth_shuttle_code = FALSE;
				_bIRStart = FALSE;
				DEBUG_IR(pr_debug("Invalid header\n"));
				return FALSE;
			}
    ...
    if (_u32IRData[1] > IR_HDC_LOB && _u32IRData[1] < IR_HDC_UPB) {
		...
		pr_debug(KERN_EMERG "Invalid leader code\n");
        bRet = FALSE;
        goto done;
    }
	...
	DEBUG_IR(pr_debug("Invalid data\n"));
    bRet = FALSE;
	...
	if (((u8IRSwModeBuf[0]&0xf0UL)>>4) == _u8FactoryIRHeaderCode_CV_0 || (((u8IRSwModeBuf[0]&0xf0UL)>>4) == _u8FactoryIRHeaderCode_TS_0)
		|| (((u8IRSwModeBuf[0]&0xf0UL)>>4) == _u8FactoryIRHeaderCode_IN_0)) {
                ...
                if((u8IRSwModeBuf[2]+u8IRSwModeBuf[3])==0x0fUL)
                {
                    ...
                    _u8PrevKeyCode=*pu8Key;
			DEBUG_IR(pr_debug("slide key 0x%x\n", *pu8Key));
			pr_debug("slide key 0x%x\n", *pu8Key);
                    *pu8Flag = 0;
                    bRet = TRUE;
                }
            }
	...
	DEBUG_IR(pr_debug("_u32IRCount=%d", _u32IRCount));
    for( u8Byte=0; u8Byte<_u32IRCount; u8Byte++)
    {
	DEBUG_IR(pr_debug(" 0x%x", _u32IRData[u8Byte]));
    }
	pr_debug("\n");
	pr_debug("_u16IRKeyMap:0x%x\n", _u16IRKeyMap);

    if(_bKeyValueHit == FALSE)
    {
        *pu8Key = _MDrv_IR_CUS21SH_ParseKey(_u16IRKeyMap);    // translate for SH

	pr_debug("_u16IRKeyMap:0x%x ==> *pu8Key:%x\n", _u16IRKeyMap, *pu8Key);
    ...
    {
        _u8IRRepeatedNum++;
	if (_u8IRRepeatedNum < IR_REPEAT_SKIP_COUNT)
        {
            return FALSE;
        }
        _u8IRRepeated = 0;
        ...
        *pu8Flag = 1;
        bRet = TRUE;
        #if (IR_TYPE_SEL == IR_TYPE_HISENSE || IR_TYPE_MSTAR_DTV)
	...
	DEBUG_IR(pr_debug(" H2 "));
    }
    else
    {
	DEBUG_IR(pr_debug(" invalid leader code\n"));
        bRet = FALSE;
        goto done;
    }
    ...
    *pu8System = u8IRSwModeBuf[1];//timl
    ...
    if(u8IRSwModeBuf[0] == _u8IR3HeaderCode0)
    {
        if(u8IRSwModeBuf[1] == _u8IR3HeaderCode1)
        {
            if(u8IRSwModeBuf[2] == (u8)~u8IRSwModeBuf[3])
            {
                *pu8Key = u8IRSwModeBuf[2];
                *pu8Flag = 0;
                bRet = TRUE;
                #if (IR_TYPE_SEL == IR_TYPE_HISENSE || IR_TYPE_SEL == IR_TYPE_MSTAR_DTV || \
                        IR_TYPE_SEL == IR_TYPE_CHANGHONG || IR_TYPE_SEL == IR_TYPE_MSTAR_DTV)
                _u8PrevKeyCode = *pu8Key;
                _u8PrevSystemCode = *pu8System;
                _u8IRRepeateDetect = 1;
                _u8IRHeadReceived = 0;
    ...
    #if (IR_TYPE_SEL == IR_TYPE_HISENSE || IR_TYPE_SEL == IR_TYPE_MSTAR_DTV)
    if ((u8IRSwModeBuf[0] == _u8FactoryIRHeaderCode_CV_0) || (u8IRSwModeBuf[0] == _u8FactoryIRHeaderCode_TS_0) || (u8IRSwModeBuf[0] == _u8FactoryIRHeaderCode_IN_0))
    {
		if ((u8IRSwModeBuf[1] == _u8FactoryIRHeaderCode_CV_1) || (u8IRSwModeBuf[1] == _u8FactoryIRHeaderCode_TS_1) || (u8IRSwModeBuf[1] == _u8FactoryIRHeaderCode_IN_1))
        {
            if(u8IRSwModeBuf[2] == (u8)~u8IRSwModeBuf[3])
            {
                *pu8Key = u8IRSwModeBuf[2];
...
void MDrv_IR_Init(int bResumeInit)
{
	...
	if (model_name) {
		remote_name = strstr(model_name, "_");
		if (remote_name) {
			if ((*(remote_name+1) == 'F') || (*(remote_name+1) == 'B') || (*(remote_name+1) == 'E')) {
				_u8FactoryIRHeaderCode_TS_0 = IR_FHEADER_TS_CODE0;
				_u8FactoryIRHeaderCode_TS_1 = IR_FHEADER_TS_CODE1;
			}
		}
	}

	if (_u8FactoryIRHeaderCode_TS_0 == IR_FHEADER_TS_CODE0) {
		pr_info("Toshiba remote is supported \n");
	} else {
		pr_info("Insignia remote is supported \n");
		_u8FactoryIRHeaderCode_IN_0 = IR_FHEADER_IN_CODE0;
		_u8FactoryIRHeaderCode_IN_1 = IR_FHEADER_IN_CODE1;
	}
    ...
    ir_irq_depth = 0;
    if (result) {
	pr_debug(KERN_EMERG "IR IRQ registartion ERROR\n");
    } else{
	pr_debug("IR IRQ registartion OK\n");
    }
}
...
BOOL MDrv_IR_InitCfg(MS_IR_InitCfg* pIRInitCfg)
{
    ...
    {
		_u8FactoryIRHeaderCode_CV_0 = pIRInitCfg->u8Ctrl0;
		_u8FactoryIRHeaderCode_CV_1 = pIRInitCfg->u8Ctrl1;
		_u8FactoryIRHeaderCode_TS_0 = pIRInitCfg->u8Ctrl0;
		_u8FactoryIRHeaderCode_TS_1 = pIRInitCfg->u8Ctrl1;
		_u8FactoryIRHeaderCode_IN_0 = pIRInitCfg->u8Ctrl0;
		_u8FactoryIRHeaderCode_IN_1 = pIRInitCfg->u8Ctrl1;
...
BOOL MDrv_IR_SetProtocol(MS_MultiProtocolCfg *pstProtocolCfg)
{
    ...
    return TRUE;
#else
	pr_debug("Multiple protocol not support");
    return FALSE;
#endif
}
