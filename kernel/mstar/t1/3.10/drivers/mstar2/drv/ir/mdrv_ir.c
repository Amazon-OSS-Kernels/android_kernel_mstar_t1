...
#elif (IR_TYPE_SEL == IR_TYPE_MSTAR_DTV)
static U8 _u8FactoryIRHeaderCode0 = IR_FHEADER_CV_CODE0;
static U8 _u8FactoryIRHeaderCode1 = IR_FHEADER_CV_CODE1;
...
#ifdef CONFIG_MSTAR_IR_INPUT_DEVICE
		if (u8Key == IRDA_KEY_MAPPING_POWER) {
			pr_info("IR Power key is pressed\n");
		} else if (u8Key == IRKEY_CUSTOM_1) {
			pr_info("IR Netflix key is pressed \n");
		} else if (u8Key == IRKEY_CUSTOM_2) {
			pr_info("IR AIV key is pressed \n");
		} else if (u8Key == IRKEY_CUSTOM_3) {
			pr_info("IR Custom 3 key is pressed \n");
		}

extern int get_state_entering(void);
extern void clear_state_entering(void);
	if (u8Key == IRDA_KEY_MAPPING_POWER) {
		if (get_state_entering())
