...
#define     MBXIO_KDBG(_fmt, _args...)        pr_debug(_fmt, ## _args)
#define     MBXIO_ASSERT(_con)   do {\
                                                            if (!(_con)) {\
								pr_debug(KERN_CRIT "BUG at %s:%d assert(%s)\n", __FILE__, __LINE__, #_con);\
                                                                BUG();\
                                                            }\
                                                          } while (0)
#else
...
int _MDrv_MBXIO_IOC_Init(struct file *filp, unsigned long arg)
{
    ...
    if (stInitInfo.eHostRole >= E_MBX_ROLE_MAX)
	return EFAULT;
...
int _MDrv_MBXIO_IOC_RegisterMsg(struct file *filp, unsigned long arg)
{
    ...
    if (stRegisterMsg.eMsgClass >= E_MBX_CLASS_MAX)
	return EFAULT;
...
int _MDrv_MBXIO_IOC_UnRegisterMsg(struct file *filp, unsigned long arg)
{
    ...
    if (stUnRegisterMsg.eMsgClass >= E_MBX_CLASS_MAX)
	return EFAULT;
...
int _MDrv_MBXIO_IOC_ClearMsg(struct file *filp, unsigned long arg)
{
    ...
    if (stClearMsg.eMsgClass >= E_MBX_CLASS_MAX)
	return EFAULT;
...
int _MDrv_MBXIO_IOC_SendMsg(struct file *filp, unsigned long arg)
{
    ...
    if (mbxMsg.u8MsgClass >= E_MBX_CLASS_MAX)
	return EFAULT;
...
int _MDrv_MBXIO_IOC_RecvMsg(struct file *filp, unsigned long arg)
{
    ...
    if (stRecvMsg.eTargetClass >= E_MBX_CLASS_MAX)
	return EFAULT;
...
int _MDrv_MBXIO_IOC_GetMsgQStatus(struct file *filp, unsigned long arg)
{
    ...
    if (stGetMsgQStatus.eTargetClass >= E_MBX_CLASS_MAX)
	return EFAULT;
...
int _MDrv_MBXIO_IOC_SetInformation(struct file *filp, unsigned long arg)
{
    ...
    if (stSetInfo.u8Size > MAX_MBX_INFORMATION_SIZE || stSetInfo.eTargetRole >= E_MBX_ROLE_MAX)
		return EFAULT;
...
int _MDrv_MBXIO_IOC_GetInformation(struct file *filp, unsigned long arg)
{
    ...
    if (stGetInfo.u8Size > MAX_MBX_INFORMATION_SIZE || stGetInfo.eTargetRole >= E_MBX_ROLE_MAX)
	return EFAULT;
