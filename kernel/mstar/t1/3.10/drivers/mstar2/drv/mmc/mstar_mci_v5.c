...
static int mstar_mci_config_ecsd(struct mmc_data *pData_st)
{
    ...
    if(0 ==(g_eMMCDrv.u32_DrvFlag & DRV_FLAG_INIT_DONE))
    {
		...
		uRPMB = pBuf[168]; /* RPMB Multi */
		uPART = pBuf[179]; /* Part_config */
		WARN_ON((0 == uRPMB) || (0 == uPART));

		Chip_Inv_Cache_Range_VA_PA((uintptr_t)pBuf, (uintptr_t)dmaaddr, 512);

        for(i=0; i<512; i++)
        {
		if ((i&0xF) == 0x0)
			eMMC_debug(eMMC_DEBUG_LEVEL_LOW, 0, "%03X: ", i);
...
static void mstar_mci_send_command(struct mstar_mci_host *pMStarHost_st, struct mmc_command *pCmd_st)
{
	...
	#if 1  /* check BootMode */
	if (6 == pCmd_st->opcode && (pCmd_st->arg & 0xff0000) == 0xB30000) {
		U8 val = ((pCmd_st->arg & 0xff00) >> 8) & 0x78;
		if (0x08 != val) {
			eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "clr BootMode: %Xh\n", pCmd_st->arg);
			pCmd_st->arg &= ~(val<<8);
			pCmd_st->arg |= 0x08<<8;
			eMMC_debug(eMMC_DEBUG_LEVEL, 1, "fix BootMode: %Xh\n", pCmd_st->arg);
		}
	}
	#endif
