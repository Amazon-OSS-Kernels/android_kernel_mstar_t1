...
#define eMMC_CMD13_RETRY_CNT   0x5
...
U32 eMMC_FCIE_Reset(void)
{
	...
	eMMC_clock_setting(BIT_FCIE_CLK_12M);
...
)
{
	...
	#if 1 /* check BootMode */
	if (6 == u8_CmdIdx && (u32_Arg & 0xff0000) == 0xB30000) {
		U8 val = ((u32_Arg & 0xff00) >> 8) & 0x78;
		if (0x08 != val) {
...
U32 eMMC_ModifyExtCSD(U8 u8_AccessMode, U8 u8_ByteIdx, U8 u8_Value)
{
	...
	do {
		if (u8_retry_prg > eMMC_CMD13_RETRY_CNT)
			break;
