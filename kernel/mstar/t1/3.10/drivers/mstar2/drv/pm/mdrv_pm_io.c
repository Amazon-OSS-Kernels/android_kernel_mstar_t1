...
int _MDrv_PM_SetCodeDramAddr(struct file *filp, unsigned long arg)
{
	...
	pr_debug("\033[32m [%s, %s, %d] address: %x size: %x \033[0m\n", __FILE__, __func__, __LINE__, stPM_temp.Address, stPM_temp.Size);
    return 0;
}
...
MSYSTEM_STATIC int _MDrv_PMIO_ModuleInit(void)
{
    ...
    if (0 > s32Ret)
    {
	pr_debug("Unable to get major %d\n", _PMDev.s32Major);
    ...
    {
	pr_debug("Unable add a character device\n");
...
*/	
    device_create(pm_class, NULL, dev, NULL, MDRV_NAME_PM);
