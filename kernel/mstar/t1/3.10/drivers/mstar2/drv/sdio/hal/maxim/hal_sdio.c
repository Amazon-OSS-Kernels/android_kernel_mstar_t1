...
U8 HalSdio_CmdFifoGet(U8 u8addr)
{
	U16 u16Tmp;
	if (u8addr > 16) {
		printk("SD Err: Cmd FIFO addr overflow in get %d\n", u8addr);
	}
...
void HalSdio_CmdFifoSet(U8 u8addr, U8 u8value)
{
	U16 u16Tmp;
	if (u8addr > 16) {
		printk("SD Err: Cmd FIFO addr overflow to set %d\n", u8addr);
	}
	FCIE_RIU_16_ON(FCIE_SD_CTRL, BIT_CHK_CMD);
	u16Tmp = FCIE_RIU_R16(FCIE_CMDFIFO_ADDR(u8addr >> 1));
	if (u8addr & 1) {
...
E_IO_STS HalSdio_ParsingErrStatus(U16 u16_reg)
{
	...
	if (u16_reg & BITS_ERROR) {
		...
		} else if (u16_reg & BIT_DAT_WR_CERR) {
			IoSts = IO_W_DATA_STS_ERROR;
		} else if (u16_reg & BIT_DAT_WR_TOUT) {
			IoSts = IO_TIME_OUT;
		} else if (u16_reg & BIT_CMD_NO_RSP) {
			IoSts = IO_CMD_NO_RSP;
		} else if (u16_reg & BIT_CMD_RSP_CERR) {
			IoSts = IO_RSP_CRC_ERROR;
		} else if (u16_reg & BIT_DAT_RD_TOUT) {
			IoSts = IO_TIME_OUT;
		}
		return IoSts;
	} else {
...
{
	/* U32	u32DescptAddr; */
	static uintptr_t *DescriptorVirtAddr;
