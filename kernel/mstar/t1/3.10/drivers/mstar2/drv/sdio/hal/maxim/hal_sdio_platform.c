...
U32 HalSdio_SlectBestSkew4(U32 u32_Candidate, SKEWER *pSkewer)
{
	...
	if ((st_skewer1.u8_edge == 2) || (st_skewer2.u8_edge == 2)) { /* only 1 continuous workable skew4 group */
		if (st_skewer1.u8_edge == 2) {
			...
			if (u32_LatchBestSelect == 18) {
				u32_LatchBestSelect = 0;
			}
			/* swap start & end */
			u32_Temp = st_skewer1.u32LatchOKStart;
			st_skewer1.u32LatchOKStart = st_skewer1.u32LatchOKEnd;
			st_skewer1.u32LatchOKEnd = u32_Temp;
		}
...
static int CDZ_option;
static int CdzGpioBank;
static int CdzGpioOffset;
static int CdzGpioBits;

static int pwr_option;
static int pwrGpioBank;
static int pwrGpioOffset;
static int pwrGpioBits;

static int wp_option;
static int wpGpioBank;
static int wpGpioOffset;
static int wpGpioBits;
