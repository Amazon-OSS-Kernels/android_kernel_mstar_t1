...
typedef struct
{
    ...
    int                 locked_count;
    struct semaphore    *sem;
	pid_t               locked_pid;             /*the pid this semaphore locked by*/
	pid_t               locked_tgid;            /*the tgid this semaphore locked by*/
	struct file         *locked_file;
	char                procname[64];
}semaphore_info;
...
return_index0:
		...
		SEMUTEX_DPRINTK("[SEMUTEX] index %d name %s lock timeout\n",
				lockarg.index,
				dev->mutex_info[lockarg.index-MUTEX_INDEX_BEGIN].name);

		lockarg.index -= (int)MUTEX_INDEX_BEGIN;
		if ((lockarg.index < 0) ||
		    (lockarg.index >= MUTEX_SUPPORTED)) {
			printk(KERN_WARNING"[SEMUTEX] LOCK_WITHTIMEOUT mutex index %d error\n", lockarg.index);
			return -EINVAL;
		}

		if (dev->mutex_info[lockarg.index].sem == NULL) {
			printk(KERN_WARNING"[SEMUTEX] LOCK_WITHTIMEOUT mutex [%d] is NULL\n", lockarg.index);
			return -EINVAL;
		}
		...
		if ((index < 0) ||
		    (index >= MUTEX_SUPPORTED)) {
			printk(KERN_WARNING"[SEMUTEX] TRY_LOCK mutex index %d error\n", index);
			return -EINVAL;
		}

		if (dev->mutex_info[index].sem == NULL) {
			printk(KERN_WARNING"[SEMUTEX] TRY_LOCK mutex [%d] is NULL\n", index);
			return -EINVAL;
		}
		...
		if ((index < 0) ||
		    (index >= MUTEX_SUPPORTED)) {
			printk(KERN_WARNING"[SEMUTEX] LOCK mutex index %d error\n", index);
			return -EINVAL;
		}

		if (dev->mutex_info[index].sem == NULL) {
			printk(KERN_WARNING"[SEMUTEX] LOCK mutex [%d] is NULL\n", index);
			return -EINVAL;
		}
		...
		while ((ret = down_timeout_interruptible(
			dev->mutex_info[index].sem, msecs_to_jiffies(1000 * 10))) == -ETIME) {
			...
			/*dumpMutexStatus(dev);*/
			/*msleep(30*1000);*/
			if (dump_tombstone == true) {
				printk(KERN_EMERG"Send SIGUSR1 to dump backtrace : locked pid %d\n",
					dev->mutex_info[index].locked_pid);
				struct siginfo info;
				...
				kill_proc_info(SIGUSR1, &info, dev->mutex_info[index].locked_pid);
				dump_tombstone = false;
			}
		};
				if(ret)
					return ret;
		...
		if ((index < 0) ||
		    (index >= MUTEX_SUPPORTED)) {
			printk(KERN_WARNING"[SEMUTEX] UNLOCK mutex index %d error\n", index);
			return -EINVAL;
		}

		if (dev->mutex_info[index].sem == NULL) {
			printk(KERN_WARNING"[SEMUTEX] UNLOCK mutex [%d] is NULL\n", index);
			return -EINVAL;
		}
	...
	case MDRV_SEMUTEX_SET_CROSS_THREAD_UNLOCK:
	{
		...
		if ((unlock_info.index < 0) ||
		(unlock_info.index >= MUTEX_SUPPORTED)) {
			printk(KERN_WARNING"[SEMUTEX] CROSS THREAD UNLOCK mutex index %d error\n", unlock_info.index);
			return -EINVAL;
		}

		if (dev->mutex_info[unlock_info.index].sem == NULL) {
			printk(KERN_WARNING"[SEMUTEX] CROSS THREAD UNLOCK mutex [%d] is NULL\n", unlock_info.index);
			return -EINVAL;
		}
	...
	break;
		...
		if ((index < 0) ||
		    (index >= MUTEX_SUPPORTED)) {
			printk(KERN_WARNING"[SEMUTEX] DEL_MUTEX index %d error\n", index);
			return -EINVAL;
		}

		if (dev->mutex_info[index].sem == NULL) {
			printk(KERN_WARNING"[SEMUTEX] DEL_MUTEX [%d] is NULL\n", index);
			return -EINVAL;
		}
		...
		/* Null terminated */
		semaarg.semaname[sizeof(semaarg.semaname)-1] = '\0';
...
create_semaphore_end:
		...
		bool dump_tombstone = true;
				...
				if (index >= MUTEX_SUPPORTED) {
					printk("[SEMUTEX] LOCK semaphore index error:%d.\n", index);
					return -EINVAL;
				}
				...
				{
					printk(KERN_EMERG "~!~ semaphore %s timeout ,index = %d ,locked pid=%d, tgid=%d, procname=%s, current->pid=%d, tgid=%d, procname=%s~!~\n",
						dev->sema_info[index].sema_name, index, dev->sema_info[index].locked_pid, dev->sema_info[index].locked_tgid,
						dev->sema_info[index].procname, current->pid, current->tgid, current->comm);
					if (dump_tombstone == true) {
						printk(KERN_EMERG"Send SIGUSR1 to dump backtrace : locked pid %d\n",
							dev->sema_info[index].locked_pid);
						struct siginfo info;
						info.si_signo = SIGUSR1;
						info.si_errno = 0;
						info.si_code = SI_USER;
						info.si_pid = get_current()->pid;
						info.si_uid = current_uid();
						kill_proc_info(SIGUSR1, &info, dev->sema_info[index].locked_pid);
						dump_tombstone = false;
					}
	            };
		...
		mutex_lock(&semutex_semaphore);
		dev->sema_info[index].locked_pid = current->pid;
		dev->sema_info[index].locked_tgid = current->tgid;
		dev->sema_info[index].locked_file = filp;
		memset(dev->sema_info[index].procname, 0, sizeof(dev->sema_info[index].procname));
		strncpy(dev->sema_info[index].procname, current->comm, sizeof(dev->sema_info[index].procname));
		mutex_unlock(&semutex_semaphore);
            }
            break;
        case MDRV_SEMUTEX_SEMA_TRY_LOCK:
            {
				...
				if (index >= MUTEX_SUPPORTED) {
					printk("[SEMUTEX] TRY_LOCK semaphore index error:%d.\n", index);
					return -EINVAL;
				}
        ...
        case MDRV_SEMUTEX_SEMA_UNLOCK:
            {
				...
				if (index >= MUTEX_SUPPORTED) {
					printk("[SEMUTEX] UNLOCK semaphore index error:%d.\n", index);
					return -EINVAL;
				}
...
#endif
		mutex_lock(&semutex_semaphore);
		dev->sema_info[index].locked_file = NULL;
		dev->sema_info[index].locked_pid = 0;
		dev->sema_info[index].locked_tgid = 0;
		memset(dev->sema_info[index].procname, 0, sizeof(dev->sema_info[index].procname));
		mutex_unlock(&semutex_semaphore);
        ...
        case MDRV_SEMUTEX_SEMA_RESET:
            {
				...
				if (index >= MUTEX_SUPPORTED) {
					printk("[SEMUTEX] RESET semaphore index error:%d.\n", index);
					return -EINVAL;
				}
		...
		if ((len <= 0) || (len > dev->shared_memorysize)) {
			printk("required length %d is not valid, available len is %u\n", len, dev->shared_memorysize);
			return -1;
		}
