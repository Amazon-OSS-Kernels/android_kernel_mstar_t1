...
int MDrv_SYS_IsSupportDolbyVison(bool * const pbSupportDolbyVison)
{
	U16 u16ReadResult = 0;
	unsigned long u32RetryCount = 0;
	unsigned long u32Timeout = 1000000;
	unsigned long u32CheckDoneDelayUs = 5;

	if (pbSupportDolbyVison == NULL) {
		return -EINVAL;
	}

	/* address 0x6E *4 = 0x1B8 */
	REG_PM_ADDR(0x2050) = (U16)0x21B8;
	while ((REG_PM_ADDR(0x2050) & (U16)0x2000UL)) {
		u32RetryCount++;
	...
	u16ReadResult = REG_PM_ADDR(0x2058);
	if (u16ReadResult & (U16)0x0040) {
		*pbSupportDolbyVison = FALSE;
	} else {
		*pbSupportDolbyVison = TRUE;
	}

    return 0;
}
EXPORT_SYMBOL(MDrv_SYS_IsSupportDolbyVison);

int MDrv_SYS_IsSupportMEMC(bool * const pbSupportMEMC)
{
	U16 u16ReadResult = 0;
	bool bDisableMEMC = FALSE;
	bool bIgnoreDisableMEMC = FALSE;
	unsigned long u32RetryCount = 0;
	unsigned long u32Timeout = 1000000;
	unsigned long u32CheckDoneDelayUs = 5;

	if (pbSupportMEMC == NULL) {
		return -EINVAL;
	}

	/* address 0x6F *4 = 0x1BC */
	REG_PM_ADDR(0x2050) = 0x21BC;
	while (REG_PM_ADDR(0x2050) & (U16)0x2000UL) {
		u32RetryCount++;
		if ((u32RetryCount * u32CheckDoneDelayUs) >= u32Timeout) {
			return -EBUSY;
	...
	u16ReadResult = REG_PM_ADDR(0x2058);
	/* 6F[4]: 1: disable MEMC */
	bDisableMEMC = ((u16ReadResult & 0x0010) >> 4);
	/* 6F[0]: 1: ignore previous bit for disable MEMC */
	bIgnoreDisableMEMC = (u16ReadResult & 0x0001);
	if (bIgnoreDisableMEMC || (bDisableMEMC == FALSE)) {
		*pbSupportMEMC = TRUE;
	} else {
		*pbSupportMEMC = FALSE;
	}
...
EXPORT_SYMBOL(MDrv_SYS_IsSupportMEMC);

int MDrv_SYS_GetChipPartName(char * const pChipPartName)
{
	bool bSupportMEMC = FALSE;
	bool bSupportDolbyVison = FALSE;

	if (pChipPartName == NULL) {
		return -EINVAL;
	}
	if (MDrv_SYS_IsSupportMEMC(&bSupportMEMC)) {
		printk(KERN_ERR "[%s:%d]Error: _MDrv_SYS_IsSupportMEMC fail\n", __FUNCTION__, __LINE__);
	}
	if (MDrv_SYS_IsSupportDolbyVison(&bSupportDolbyVison)) {
		printk(KERN_ERR "[%s:%d]Error: _MDrv_SYS_IsSupportDolbyVison fail\n", __FUNCTION__, __LINE__);
	}
	if (bSupportMEMC && !bSupportDolbyVison) {
		strncpy(pChipPartName, "MSDUAUV704-A-00DM", 18);
	} else if (bSupportMEMC && bSupportDolbyVison) {
		strncpy(pChipPartName, "MSDUAUV705-A-00DM", 18);
	} else if (!bSupportMEMC && !bSupportDolbyVison) {
		strncpy(pChipPartName, "MSDUAUV707-A-00DM", 18);
	} else if (!bSupportMEMC && bSupportDolbyVison) {
		strncpy(pChipPartName, "MSDUAUV708-A-00DM", 18);
	} else {
		strncpy(pChipPartName, "unknown", 8);
	}
	return 0;
}
EXPORT_SYMBOL(MDrv_SYS_GetChipPartName);
