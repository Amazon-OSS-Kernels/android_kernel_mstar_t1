...
/**
 * struct kbase_uk_mem_umm_export - User/Kernel space data exchange structure
 * @header:         UK structure header
 * @base:           the address of the memory to export
 * @size:           the size of the memory to export
 * @is_bus_address: is the address a bus address
 * @fd:             file descriptor of the exported memory
 *
 * This structure is used when performing a call to export memory as UMM.
 */

#if MSTAR_UMM_EXPORT
struct kbase_uk_mem_umm_export {
	union uk_header header;
	/* IN */
	u32 base;
	u32 size;
...
#if MSTAR_UMM_EXPORT
	KBASE_FUNC_MEM_UMM_EXPORT = (UK_FUNC_ID + 39),
#endif
