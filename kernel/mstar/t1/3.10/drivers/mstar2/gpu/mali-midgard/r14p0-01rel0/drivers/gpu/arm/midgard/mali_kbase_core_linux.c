...
#include <mali_kbase_mali_debug_debugfs.h>
...
#else
int mali_gpu_utilization = 99; /* Set initial value as 99 to represent Max GPU frequnce when disable DVFS. */
#endif
...
#if MSTAR_UMM_EXPORT
	case KBASE_FUNC_MEM_UMM_EXPORT:
		{
			struct kbase_uk_mem_umm_export *export = (struct kbase_uk_mem_umm_export *)args;

			if (sizeof(*export) != args_size)
				goto bad_size;

#ifdef CONFIG_DMA_SHARED_BUFFER
			export->fd = mem_block_export_dma_buf(export->base,
												  export->size,
												  export->is_bus_address);
#else
			export->fd = -1;
			ukh->ret = MALI_ERROR_FUNCTION_FAILED;
#endif
...
#ifdef MSTAR_DVFS_FREQ_ADJUSTABLE
/**
...
#endif /* MSTAR_DVFS_FREQ_ADJUSTABLE */
#endif /* defined(MSTAR) && defined(CONFIG_MALI_MIDGARD_DVFS) */
...
static int kbase_device_debugfs_init(struct kbase_device *kbdev)
{
	...
	kbasep_gpu_clock_debugfs_init(kbdev);
	kbasep_mali_debug_debugfs_init(kbdev);
