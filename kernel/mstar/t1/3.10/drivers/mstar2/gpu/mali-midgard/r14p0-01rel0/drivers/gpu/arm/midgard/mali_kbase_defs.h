...
#ifdef MSTAR
	/* When executing protected content, there is often a mixture of protected
	 * and non-protected jobs running on the GPU (eg due to Android SurfaceFlinger).
	 * Due to a bug in the state machine which manages the transition between protected
	 * and non-protected mode on the GPU, it is possible for a protected job and
	 * a non-protected job to block each other from submission, causing a deadlock.
	 * This will usually be seen as a fence timeout and cancelled jobs.
	 */
...
struct mstar_dvfs
{
	...
	bool (*clock_up)(struct kbase_device *);
	bool (*clock_down)(struct kbase_device *);
...
struct kbase_device {
	...
	/* MStar: the following data should be kept in the above "platform_context",
