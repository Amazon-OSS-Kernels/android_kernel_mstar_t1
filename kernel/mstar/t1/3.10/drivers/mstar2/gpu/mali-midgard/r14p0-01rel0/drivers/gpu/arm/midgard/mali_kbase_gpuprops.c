...
static void kbase_gpuprops_get_props(base_gpu_props * const gpu_props, struct kbase_device *kbdev)
{
	...
	if (NULL != kbdev->get_num_disabled_cores)
	{
		int num_cores = hweight64(gpu_props->raw_props.shader_present);
		int num_disabled_cores = kbdev->get_num_disabled_cores();

		if (num_disabled_cores > num_cores)
		{
			num_disabled_cores = num_cores;
		}

		if (num_disabled_cores > 0)
		{
			u64 shader_present = gpu_props->raw_props.shader_present;
			int i;

			for (i = 0; i < num_disabled_cores; i++);
			{
				/* mask higher bits first */
				shader_present &= ~(1 << (num_cores - i - 1));
			}
