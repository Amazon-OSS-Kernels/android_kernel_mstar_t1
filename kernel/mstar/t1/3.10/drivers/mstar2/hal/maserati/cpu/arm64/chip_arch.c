...
static int __init Mstar_chip_post_init(void){
	...
	pr_debug("\033[0;32;31m Function = %s Line = %d uncached_buf : 0x%lx\033[m\n", __func__, __LINE__, (unsigned long)uncached_buf);
	BUG_ON(!uncached_buf);
...
unsigned int query_frequency(unsigned int cpu_id)
{
	...
	{
		pr_debug("\033[0;32;31m [Kernel] %s %d frequency : 216M \033[m\n", __func__, __LINE__);
		return 216;
	}
	else
	{
		pr_debug("\033[0;32;31m [Kernel] %s %d frequency : 12M \033[m\n", __func__, __LINE__);
		return 12;
	}
#endif
}
