...
void mstar_str_power_off(void)
{
    ...
    else
    {
		...
		MDrv_PM_CopyBin2PhysicalDram();
		SerPrintf("\Switching from RT-PM51 to STR-PM51...");
		if (MDrv_PM_SetSRAMOffsetForMCU() != E_PM_OK) {
				SerPrintf("Error, Failed!!!!\n");
				BUG();
		}
		SerPrintf("Success\n");
		SerPrintf("\nIn STR mode.... Total Times [%d]\n", mstr_cnt);
#endif
        }
    }
}
...
static int mstar_pm_enter(suspend_state_t state)
{
	...
	"mov x0, #0\n"
	"STR x0, %0\n"
	: "=m"(ret) : : "r0", "r1", "r2", "r3", "r4", "r5", "r6", "r7", "r8", "r9", "r10", "r11", "r12", "r13", "r14", "r15", "memory", "cc"
    );
Exit:
