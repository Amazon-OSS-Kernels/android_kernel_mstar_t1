...
extern DoVi_TargetDisplayConfig_t *MHal_XC_GetTargetDisplayConfig(void);
...
MsHdr_Comp_Regtable_t* MsHdr_CompAllocRegTable(void)
{
	...
	if (pRegTable == NULL) {
		printk(KERN_WARNING "[%s %d]memory allocate error!", __func__, __LINE__);
		return NULL;
	}
...
int DoVi_Replace_3DLut(MsHdr_3D_RegTable_t* p3DLutReg, const MS_U8* pu8Array, MS_U32 u32Size)
{
	...
	if (bNewPQBCMS_109727 == TRUE) {
		DoVi_TargetDisplayConfig_t *_pstDoVi_TargetDisplayConfig;

		_pstDoVi_TargetDisplayConfig = MHal_XC_GetTargetDisplayConfig();
		if (!_pstDoVi_TargetDisplayConfig) {
			printk(KERN_WARNING, "failed to get TargetDisplayConfig\n");
			return -1;
		}
		...
		DoVi_SetDisplayProperty(_pstDoVi_TargetDisplayConfig,
				_pstDoVi_TargetDisplayConfig->BackLightUI);
