...
void MHal_LD_SetHWTriggerMode(BOOL bEnable)
{
    MHal_LD_WriteRegBit(REG_LD_HWTRIMODE, bEnable, REG_LD_HWTRIMODE_BIT);
}


void MHal_LD_SetEdge2dSymmetryEn(BOOL bEnable)
{
    MHal_LD_WriteRegBit(REG_LD_EDGE2DSYMMETRYEN, bEnable, REG_LD_EDGE2DSYMMETRYEN_BIT);
}

  // Judgement to RT51 running
BOOL MHal_LD_GetRt51Run(void)
{
  U16 tmp = MHal_LD_Read2Byte(0x0EAA);
  if ( (0xBABE==tmp) || (0xAAAA ==tmp) )
  {
      return FALSE;
  }
  return TRUE;
