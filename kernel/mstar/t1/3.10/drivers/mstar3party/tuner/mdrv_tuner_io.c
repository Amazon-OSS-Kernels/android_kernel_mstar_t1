...
#endif
{
    ...
    switch(cmd)
    {
	    ...
	    EN_FREQ_STEP *eFreqStep = kzalloc(sizeof(EN_FREQ_STEP), GFP_KERNEL);
	    if (eFreqStep == NULL)
		return -EFAULT;
	    if (copy_from_user(eFreqStep, ExtendCommend_Info.pvoidParam3, sizeof(EN_FREQ_STEP))) {
		kfree(eFreqStep);
		return -EFAULT;
	    }

	    retval = MDrv_TUNER_ExtendCommand(minor, ExtendCommend_Info.u8SubCmd, ExtendCommend_Info.u32Param1, ExtendCommend_Info.u32Param2, eFreqStep);
        ...
        case MDRV_TUNER_TunerInit:
            ...
            retval=MDrv_TUNER_TunerInit(minor);
            if (copy_to_user((int *) arg, &retval, sizeof(int)))
            {
 	            return -EFAULT;
            }
