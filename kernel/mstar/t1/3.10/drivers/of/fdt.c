...
void __init early_init_dt_check_for_initrd(unsigned long node)
{
	...
	pr_debug("__ramdisk_start = 0x%lx, __ramdisk_end = 0x%lx\n", __ramdisk_start, (__ramdisk_start + __ramdisk_len));

	pr_debug("\033[35mFunction = %s, Line = %d, PHYS_OFFSET is 0x%lX, PAGE_OFFSET is 0x%lX\033[m\n", __PRETTY_FUNCTION__, __LINE__, PHYS_OFFSET, PAGE_OFFSET);
	...
	early_init_dt_setup_initrd_arch( (unsigned long long)__ramdisk_start, (unsigned long long)(__ramdisk_start+__ramdisk_len));
#endif

	prop = of_get_flat_dt_prop(node, "linux,initrd-start", &len);
	if (!prop)
	{
		pr_debug("\033[35mFunction = %s, Line = %d, no \"linux,initrd-start\" setting @dts\033[m\n", __PRETTY_FUNCTION__, __LINE__);
		return;
	}
