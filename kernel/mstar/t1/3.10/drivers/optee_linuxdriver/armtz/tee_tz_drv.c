...
#ifdef CONFIG_MP_TRUSTZONE_PATCH
#define tee_ramlog_write_rpoint(value)	(*(unsigned int *)(ctx->buffer_addr + 4) = (value - ctx->buffer_addr))
#define tee_ramlog_read_wpoint		((*(unsigned int *)(ctx->buffer_addr)) + ctx->buffer_addr)
#define tee_ramlog_read_rpoint		((*(unsigned int *)(ctx->buffer_addr + 4)) + ctx->buffer_addr)
#define MAX_PRINT_SIZE      256

static int tee_ramlog_init_addr(struct tee_ramlog_context *ctx, unsigned long buf_adr, unsigned long buf_len)
{
	if ((buf_adr == 0) || (buf_len == 0))
		return -EINVAL;

	BUG_ON(!ctx);
	ctx->buffer_addr = buf_adr;
	ctx->buffer_len = buf_len;
	mutex_init(&(ctx->lock));
	return 0;
}

static void tee_ramlog_dump(struct tee_ramlog_context *ctx)
{
	...
	while ((unsigned long)log_buff_read_point != log_buff_write_point) {
		if (isascii(*(log_buff_read_point)) && *(log_buff_read_point) != '\0') {
			if (log_count >= MAX_PRINT_SIZE) {
				pr_info_ratelimited("%s %d buf is over MAX_PRINT_SIZE\n", __func__, __LINE__);
				log_count = 0;
			} else {
		...
		log_buff_read_point++;
		tmp_point = (char *)(ctx->buffer_addr + ctx->buffer_len);
		if (log_buff_read_point == tmp_point) {
			tmp_point = (char *)(ctx->buffer_addr + 8);
...
{
	struct tee_ramlog_context *ctx = (struct tee_ramlog_context *)p;
...
static int configure_ramlog(struct tee_tz *ptee)
{
	...
	ptee->ramlog_ctx = kmalloc(sizeof(struct tee_ramlog_context), GFP_KERNEL);
	if (!ptee->ramlog_ctx) {
		dev_err(DEV, "ramlog tee_ramlog_ctx kmalloc failed\n");
		ret = -ENOMEM;
		goto out_fail_alloc;
	}

	ret = tee_ramlog_init_addr(ptee->ramlog_ctx, (unsigned long)ramlog_vaddr, param.a2);
	if (ret) {
		dev_err(DEV, "ram log have been taken\n");
		goto out_init;
	}
	ptee->ramlog_ctx->cleanup_task = kthread_run(tee_ramlog_loop, ptee->ramlog_ctx, "tee_ramlog_loop");

	mutex_unlock(&ptee->mutex);
	return ret;
out_init:
	kfree(ptee->ramlog_ctx);
out_fail_alloc:
...
#ifdef CONFIG_MP_TRUSTZONE_PATCH
	if (ptee->ramlog_ctx) {
		kthread_stop(ptee->ramlog_ctx->cleanup_task);
		iounmap((void *)ptee->ramlog_ctx->buffer_addr);
		kfree(ptee->ramlog_ctx);
		ptee->ramlog_ctx = NULL;
	}
#endif
	ptee->started = false;
