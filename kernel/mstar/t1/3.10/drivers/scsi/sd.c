...
static int sd_probe(struct device *dev)
{
	...
	if (sdp->removable){
		...
		sd_printk(KERN_INFO, sdkp, "%s: scsi-polling pid=%d\n", __func__, sdkp->polling.pid);
	}
#endif
...
#if (MP_SCSI_MSTAR_SD_CARD_HOTPLUG == 1)
static int sd_polling_suspend_notifier(struct notifier_block *nb,
...
static struct notifier_block sd_polling_notif_block = {
    .notifier_call = sd_polling_suspend_notifier,
};
...
//#endif
    ...
    while(1)
    {
		...
		if (try_to_freeze()) {
			/* resume path, add 5 seconds to debounce frequent suspend/resume.*/
			sd_printk(KERN_INFO, sdkp, "%s: sleep for 50ms on resume\n", __func__);
			msleep(50);
			continue;
		}

		if (atomic_read(&suspending)) {
			sd_printk(KERN_INFO, sdkp, "%s: suspending\n", __func__);
			msleep(50);
			continue;
		}
...
#if (MP_SCSI_MSTAR_SD_CARD_HOTPLUG == 1)
		err = register_pm_notifier(&sd_polling_notif_block);
	if (err)
		printk(KERN_ERR "%s: failed to register pm notifier\n", __func__);
#endif
...
#if (MP_SCSI_MSTAR_SD_CARD_HOTPLUG == 1)
	unregister_pm_notifier(&sd_polling_notif_block);
#endif
