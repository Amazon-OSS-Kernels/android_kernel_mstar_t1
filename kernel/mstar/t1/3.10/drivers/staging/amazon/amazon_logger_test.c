...
static int metrics_test(void)
{
	...
	snprintf(metrics_log, METRICS_STR_LEN, "%s:%s:100:VALUE_A=%d;IN;1,VALUE_B=%d;IN;1:NR",
		KERNEL_METRIC_TEST_GROUP_ID, KERNEL_METRICS_TEST_SCHEMA_ID, 1, 0);
#else
	snprintf(metrics_log, METRICS_STR_LEN, "VALUE_A=%d;IN;1,VALUE_B=%d;IN;1:NR", 1, 0);
#endif
...
#ifdef CONFIG_AMAZON_MINERVA_METRICS_LOG
	snprintf(metrics_log, METRICS_STR_LEN, "%s:%s:100:V A L U E A=%d;IN;1,V A L U E B=%d;IN;1:NR"
		KERNEL_METRIC_TEST_GROUP_ID, KERNEL_METRICS_TEST_SPACE_SCHEMA_ID, 1, 0);
#else
	snprintf(metrics_log, METRICS_STR_LEN, "V A L U E A=%d;IN;1,V A L U E B=%d;IN;1:NR", 1, 0);
#endif
...
static int vital_test(void)
{
...
#ifdef CONFIG_AMAZON_MINERVA_METRICS_LOG
		/* key=NULL case */
		ret = log_timer_to_vitals_v2(ANDROID_LOG_INFO, KERNEL_METRIC_TEST_GROUP_ID,
					     KERNEL_METRICS_VITALS_TIMER_TEST_SCHEMA_ID, "Kernel timer", "TEST",
					     "time_in_test", NULL, now.tv_sec, NULL, VITALS_TIME_BUCKET, NULL, NULL);
		if (ret)
			pr_err("log_timer_to_vitals: fails in key=NULL case %d\n", ret);
		/* normal case */
		ret = log_timer_to_vitals_v2(ANDROID_LOG_INFO, KERNEL_METRIC_TEST_GROUP_ID,
					     KERNEL_METRICS_VITALS_TIMER_TEST_SCHEMA_ID, "Kernel timer", "TEST",
					     "time_in_test", "Test", now.tv_sec, "s", VITALS_TIME_BUCKET, NULL, NULL);
		if (ret)
	...
	for (i = 0; i < loop; i++) {
#ifdef CONFIG_AMAZON_MINERVA_METRICS_LOG
		/* nomral case */
		ret = log_counter_to_vitals_v2(ANDROID_LOG_INFO, KERNEL_METRIC_TEST_GROUP_ID,
					       KERNEL_METRICS_VITALS_COUNTER_TEST_SCHEMA_ID, "Kernel vitals", "TEST",
					       "VITALS", "test_value", (u32)1, "count", "s", VITALS_NORMAL, NULL, NULL);
		if (ret)
			pr_err("log_counter_to_vitals fails in nomral case %d\n", ret);
		/* metadata=NULL case */
		ret = log_counter_to_vitals_v2(ANDROID_LOG_INFO, KERNEL_METRIC_TEST_GROUP_ID,
						KERNEL_METRICS_VITALS_COUNTER_TEST_SCHEMA_ID, "Kernel vitals", "TEST",
						"VITALS", "test_value", (u32)1, "count", NULL, VITALS_NORMAL, NULL, NULL);
		if (ret)
			pr_err("log_counter_to_vitals: fails in metadata=NULL %d\n", ret);
		/* key=NULL case */
		ret = log_counter_to_vitals_v2(ANDROID_LOG_INFO, KERNEL_METRIC_TEST_GROUP_ID,
					       KERNEL_METRICS_VITALS_COUNTER_TEST_SCHEMA_ID,
					       "Kernel vitals", "TEST", "VITALS",
						NULL, (u32)1, "count", "s", VITALS_NORMAL, NULL, NULL);
		if (ret)
		...
		ret = log_counter_to_vitals_v2(ANDROID_LOG_INFO, KERNEL_METRIC_TEST_GROUP_ID,
						KERNEL_METRICS_VITALS_COUNTER_TEST_SCHEMA_ID,
						"Kernel vitals", "TEST", "VITALS",
						NULL, (u32)1, "count", NULL, VITALS_NORMAL, NULL, NULL);
		if (ret)
		...
		if (ret)
			pr_err("log_counter_to_vitals: fails in metadata=NULL %d\n", ret);
		/* key=NULL case */
		ret = log_counter_to_vitals_v2(ANDROID_LOG_INFO, "Kernel vitals", "TEST", "VITALS",
