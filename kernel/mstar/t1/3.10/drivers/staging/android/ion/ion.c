...
static int add_to_lmk_buffer;
static int print_to_console;
static DEFINE_MUTEX(ion_print_lock);
static struct work_struct work;
static bool work_inited;
static struct work_struct work_lmk;
static bool work_lmk_inited;

#define _ion_print(fmt, ...) \
{ \
	if (add_to_lmk_buffer) \
		lmk_add_to_buffer(fmt, ##__VA_ARGS__); \
	if (print_to_console) \
...
static void _ion_mm_heap_memory_detail(void)
{
	int i;
	...
	_ion_print("%16.s(%16.s) %16.s %16.s\n", "client", "heap_name", "pid", "size");
	_ion_print("-----------------------------------------------------\n");
	...
	_ion_print("-----------------------------------------------------\n");
	_ion_print("orphaned allocations (info is from last known client):\n");
	if (dev) {
		mutex_lock(&dev->buffer_lock);
		for (n = rb_first(&dev->buffers); n; n = rb_next(n)) {
			...
			total_size += buffer->size;
			if (!buffer->handle_count) {
				_ion_print("%16.s(%16.s) %16u %16zu %d %d\n",
					buffer->task_comm,
					"orphan",
					buffer->pid,
					buffer->size,
...
static void ion_mm_heap_memory_detail_lmk_work(void)
{
	mutex_lock(&ion_print_lock);
	add_to_lmk_buffer = 1;
	print_to_console = 1;
	_ion_mm_heap_memory_detail();
	mutex_unlock(&ion_print_lock);
}

void ion_mm_heap_memory_detail_lmk(void)
{

	if (false == work_lmk_inited) {
		INIT_WORK(&work_lmk, ion_mm_heap_memory_detail_lmk_work);
		work_lmk_inited = true;
	}
	schedule_work(&work_lmk);
}

static void ion_mm_heap_memory_detail_work(void)
{
	mutex_lock(&ion_print_lock);
	add_to_lmk_buffer = 0;
	print_to_console = 1;
...
void ion_mm_heap_memory_detail(void)
{
	...
	if (false == work_inited) {
		INIT_WORK(&work, ion_mm_heap_memory_detail_work);
		work_inited = true;
	}
	schedule_work(&work);
}
