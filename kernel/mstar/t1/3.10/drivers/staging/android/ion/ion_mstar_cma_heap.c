...
#define FB_PAGE_COUNT (PAGE_ALIGN(1920 * 1080 * 4) >> PAGE_SHIFT)
/*
 * totally 24576 pages (96MB) in  MALI_GOP (total available pages may be changed over time)
 * FB pool: 12150 pages
 * COMMON pool: 12426 pages
 * reserve 6 FB buffers for supporting UI (triple FB buffer) + Overlay (triple FB buffer)
 * Any size other than FB size is allocated from common pool
 * This help reduce possibility of FB allocation failure
 */
#define COMMON_POOL_OFFSET (FB_PAGE_COUNT * 6)
...
retry:
	...
	if (ptr) {
		...
		__dma_flush_range(ptr, ptr + pool->size);
...
static void *gralloc_alloc_from_pool(size_t size, struct page **ret_page)
{
	...
	spin_lock(&pool->lock);
	if (count == FB_PAGE_COUNT)
		pageno = bitmap_find_next_zero_area(pool->bitmap, pool->nr_pages,
					    0, count, 0);
	else {
		unsigned int pageno_fb0 = 0;
		unsigned int pageno_fb1 = 0;
		unsigned int pageno_fb2 = 0;

		pageno = bitmap_find_next_zero_area(pool->bitmap, pool->nr_pages,
					    COMMON_POOL_OFFSET, count, 0);
		if (pageno < pool->nr_pages) {
			/* make sure launcher always have 3 MALI_GOP backed framebuffer */
			pageno_fb0 = bitmap_find_next_zero_area(pool->bitmap, pool->nr_pages,
							0, count, 0);
			pageno_fb1 = bitmap_find_next_zero_area(pool->bitmap, pool->nr_pages,
							pageno_fb0 + FB_PAGE_COUNT, count, 0);
			pageno_fb2 = bitmap_find_next_zero_area(pool->bitmap, pool->nr_pages,
							pageno_fb1 + FB_PAGE_COUNT, count, 0);
			if (pageno_fb2 >= pool->nr_pages) {
				pr_info("MALI_GOP: no enough buffer to serve 3 FB after allocate %d pages, reject the request\n", count);
				pageno = pageno_fb2; /* reject this allocation */
			}
		}
	}
	if (pageno < pool->nr_pages) {
		...
		*ret_page = pool->pages[pageno];
		pr_info("MALI_GOP: allocate %d pages from page index %d\n", count, pageno);
	} else {
...
static int gralloc_free_from_pool(void *start, size_t size)
{
	...
	pr_info("MALI_GOP: free %d pages from page index %d\n", count, pageno);
	spin_lock(&pool->lock);
...
ALLOC:
	...
	if (!buffer_info->page) {
		if (retry < CMA_CONTIG_RTYCNT)
		{
			...
			if (freepg >= len>>PAGE_SHIFT)
			{
				...
				pr_debug("CMA contig alloc retry by shrinking %d pages!\n", freepg);
	...
	{
		mutex_unlock(&mstar_cma->contig_rtylock);
		pr_debug("CMA contig alloc retry pass! retry %d\n", retry);
	}
		...
		unsigned long flags)
{
	...
	pr_debug("ion_cma_alloc: allocate %d kB, buffer flag is [0x%x]\n",
...
ALLOC:
	...
	if (!buffer_info->page) {
		if (retry < CMA_CONTIG_RTYCNT)
		{
			...
			if (freepg >= len>>PAGE_SHIFT)
			{
				...
				retry++;
				freepg = ion_mstar_cma_shrink_lock(heap);
				pr_debug("CMA contig alloc retry with addr by shrinking %d pages!\n", freepg);
	...
	{
		mutex_unlock(&mstar_cma->contig_rtylock);
		pr_debug("CMA contig alloc retry with addr pass! retry %d\n", retry);
	}
			    ...
			    unsigned long flags)
{
	...
	if (flags & ION_FLAG_CONTIGUOUS) {
		...
		pr_debug("\033[31m grAlloc use ION_FLAG_STARTADDR!!!!!!!!!!!!!\033[m\n");
...
struct ion_heap *ion_mstar_cma_heap_create(struct ion_platform_heap *data)
{
	...
	pr_debug("\033[31mFunction = %s, dev = %s, cma = %p, cma from 0x%lX to 0x%lX\033[m\n",
...
void get_cma_heap_info(struct ion_heap *heap, int *mali_heap_info, char *name, size_t len)
{
	struct ion_mstar_cma_heap *mstar_cma_heap = to_mstar_cma_heap(heap);
	int freebits;

	if (!strcmp("MALI_GOP", dev_name(mstar_cma_heap->dev)) && (mali_gop_init == 0)) {
		mali_heap_info[0] = 0;
		mali_heap_info[1] = 0;
		mali_heap_info[2] = 0;
		mali_heap_info[3] = 0;
		strlcpy(name, dev_name(mstar_cma_heap->dev), len);
		return;
	}
