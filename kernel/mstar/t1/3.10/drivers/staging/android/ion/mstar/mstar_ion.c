...
static DEFINE_MUTEX(cma_info_mutex);
//-------------------------------------------------------------------------------------------------
// Macros
//-------------------------------------------------------------------------------------------------
...
static struct platform_device *common_ion_devices[]  = {
	&ion_dev,
};
...
void get_cma_status(char *info, size_t count)
{
	int i;
	int j = 0;
	int ret;
	struct ion_heap *heap;
	mutex_lock(&cma_info_mutex);
	memset(info, 0, count);
	for (i = 0; i < num_heaps; i++) {
		int cma_status[4] = {0};
		char name[16] = {0};
		...
		if(heap->type != ION_HEAP_TYPE_MSTAR_CMA && heap->type != ION_HEAP_TYPE_SYSTEM)
			continue;
		if (count  <= j) {
			pr_err("Error, cma_info is not big enough !!!\n");
			break;
		}
		if(heap->type == ION_HEAP_TYPE_MSTAR_CMA)
		{
			get_cma_heap_info(heap, cma_status, name, 16);
			ret = snprintf(info+j, MIN(128, count-j), "\n%20s (%8d kB %8d kB %8d kB %8d kB)", name,
				cma_status[0]*4, cma_status[1]*4, cma_status[2]*4, cma_status[3]*4);
		}
		else
		{
			get_system_heap_info(dma_contiguous_default_area, cma_status);
			ret = snprintf(info+j, MIN(128, count-j), "\n%20s (%8d kB %8d kB %8d kB %8d kB)", " DEFAULT_CMA_BUFFER",
				cma_status[0]*4, cma_status[1]*4, cma_status[2]*4, cma_status[3]*4);
		}
		if (ret <= 0) {
			pr_err("Error cma_info could not be copied correctly !!!!\n");
			break;
		}
		j += ret;
	}
	mutex_unlock(&cma_info_mutex);
}
...
static int cmarunlist_proc_open(struct inode *inode, struct file *file)
{
	...
	if(cma){
		...
		pr_debug("cma %s, total page %lu, free page %lu\n", heap->name, cma->count, freebits);
...
static ssize_t cma_measurement_data_read(struct file *file, char __user *buf, size_t size, loff_t *ppos)
{
	...
	pr_debug("\033[31mthis show_cma_measurement is %s\033[m\n", show_cma_measurement->cma_heap_name);
	pr_debug("\033[31mtotal_alloc_size_kb is %lu kb, total_alloc_time_cost_ms is %lu ms\033[m\n", show_cma_measurement->total_alloc_size_kb, show_cma_measurement->total_alloc_time_cost_ms);
	pr_debug("\033[31mtotal_migration_size_kb is %lu kb, total_migration_time_cost_ms is %lu ms\033[m\n", show_cma_measurement->total_migration_size_kb, show_cma_measurement->total_migration_time_cost_ms);
...
static int mstar_ion_probe(struct platform_device *pdev)
{
	...
	num_heaps = pdata->nr;
	pr_debug("\033[31mFunction = %s, Line = %d, doing mstar_ion_probe for %s, having %d heaps\033[m\n", __PRETTY_FUNCTION__, __LINE__, pdev->name, num_heaps);
	...
	for (i = 0; i < num_heaps; i++) {
		...
		{
			pr_debug("\033[31mFunction = %s, Line = %d, STR_MBOOT co-buffer heap is %d\033[m\n", __PRETTY_FUNCTION__, __LINE__, heap_data->id);
		...
		if(heaps[i]->type == ION_HEAP_TYPE_MSTAR_CMA)
		{
			...
			pr_debug("\033[35mFunction = %s, Line = %d, cma_measurement_data name is %s, id is %d\033[m\n",
