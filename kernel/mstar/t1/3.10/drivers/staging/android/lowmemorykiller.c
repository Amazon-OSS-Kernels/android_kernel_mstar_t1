...
static void lowmem_dump_gpu_info(struct work_struct *work)
{
	typedef void (*kbase_gpu_callback)();
	kbase_gpu_callback gpu_info_dump;
	gpu_info_dump = (kbase_gpu_callback)kallsyms_lookup_name("kbasep_gpu_memory_lmk");
	if (gpu_info_dump == 0) {
...
static int lowmem_shrink(struct shrinker *s, struct shrink_control *sc)
{
	...
	if (foreground_kill) {
		if (false == work_inited) {
			INIT_WORK(&work, lowmem_dump_gpu_info);
			work_inited = true;
		}
		schedule_work(&work);
