	...
	if (!kdebugd_running) {
		...
		if (c == sched_serial[sched_serial_idx]) {
			...
			if (sched_serial_idx == strlen(sched_serial)) {
				...
				if (!kdbg_mode) {
#endif
					pr_debug("[SP Kernel Debugger] ");
				pr_debug("Enable Debug Mode!\n");
				kdebugd_start();
				kdebugd_running = 1;
#ifdef CONFIG_KDEBUGD_BG
				} else {
					pr_debug("Kdebugd already running in Background !!!\n");
					ret = 1;
				}
#endif
			}
...
static inline void n_tty_receive_char(struct tty_struct *tty, unsigned char c)
{
	...
	if(tty == INPUT_tty) {
		...
		if(c == enable_serial[enable_index]) {
			...
			pr_debug("\n");
			pr_debug("[SERIAL INPUT MANAGE] %d-th ENABLE Magic serial input match!\n",
				enable_index);
#endif
...
#ifdef CONFIG_SERIAL_INPUT_ENABLE_HELP_MSG
				pr_debug(KERN_ALERT "\n");
				pr_debug(KERN_ALERT "[SERIAL INPUT MANAGE] serial input ENABLE!!!!!\n");
#endif
			}
			return;
		} else
...
#ifdef CONFIG_SERIAL_INPUT_ENABLE_HELP_MSG
			pr_debug("\n");
			pr_debug("[SERIAL INPUT MANAGE] %d-th DISABLE Magic serial input match!\n",
				disable_index);
#endif
...
#ifdef CONFIG_SERIAL_INPUT_ENABLE_HELP_MSG
				pr_debug("\n");
				pr_debug("[SERIAL INPUT MANAGE] serial input DISABLE!!!!!\n");
#endif
				return;
			}
		} else
...
#ifdef CONFIG_SERIAL_INPUT_ENABLE_HELP_MSG
		pr_debug("[SERIAL INPUT MANAGE] disable_serial : %s(len:%d)\n",
			disable_serial, disable_string_size);
		pr_debug("[SERIAL INPUT MANAGE] enable_serial : %s(len: %d)\n",
			enable_serial, enable_string_size);
#endif

#ifdef KDEBUGD_STRING
		sprintf(sched_serial, "%s", KDEBUGD_STRING);
		pr_debug("[KDEBUGD] enter : %s\n\n", KDEBUGD_STRING);
#endif
