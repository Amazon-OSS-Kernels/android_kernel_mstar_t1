...
static int usb_start_wait_urb(struct urb *urb, int timeout, int *actual_length)
{
...
#if (MP_USB_MSTAR == 1)
	{
		...
		/* Lower Bound for preventing false alarm. */
		if (unlikely(timeout < TIMEOUT_SETP_MS*10))
			timeout = TIMEOUT_SETP_MS*10;

		/* Upper Bound for preventing DPM timeout during device suspend.*/
		if (unlikely(timeout > TIMEOUT_SETP_MS*50))
			timeout = TIMEOUT_SETP_MS*50;
		...
		do {
			if (!wait_for_completion_timeout(&ctx.done, timeout_step)) {
				...
				dev_info_ratelimited(&urb->dev->dev, "usb_start_wait_urb exceed %d ms\n",
									jiffies_to_msecs(total_time));

				/* device on roothub; CSC happen on ehci port */
				if  ((urb->dev->parent != NULL) && (urb->dev->parent->parent == NULL)
					&& (hcd->ehc_base != 0)	&& (hcd_readb(hcd, 0x30) & BIT1))
					csc_flag = 1;

				if  ((csc_flag == 1) || (total_time >= expire))
				{
					dev_err(&urb->dev->dev, "is_ehci_quick_resume_port(hcd) =%d, hcd->port_index=%d\n",
							(is_ehci_quick_resume_port(hcd) ? 1 : 0), hcd->port_index);
					#if defined(CONFIG_SUSPEND)
					dev_err(&urb->dev->dev, "get_state_entering() =%d, expire = %djiffies\n",
							get_state_entering(), expire);
					#endif

					dev_err(&urb->dev->dev, "usb_kill_urb\n");
					usb_kill_urb(urb);
					retval = (ctx.status == -ENOENT ? -ETIMEDOUT : ctx.status);
					dev_err(&urb->dev->dev, "ctx.status = %d\n", ctx.status);

					if  (total_time >= expire)
						dev_err(&urb->dev->dev,
							"[USB] %s %dms timedout happen on ep%d%s len=%u/%u\n",
							current->comm,
							timeout,
