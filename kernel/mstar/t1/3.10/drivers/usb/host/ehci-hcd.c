...
#if (MP_USB_MSTAR == 1)
	/*
	 * 1) Mstar USB only has ehci, no uhci & ohci, all HS/FS/LS would be
	 * supported by ehci; 2) for Mstar STR failure, all HS/FS/LS USB device
	 * would do resume flow.
	 */
	if (!hibernated) {
#else
...
#ifdef CONFIG_MP_EHCI_PORT_INIT_SEQ
#define	PLATFORM_DRIVER		ehci_hcd_mstar_driver
#define	SECOND_PLATFORM_DRIVER	second_ehci_hcd_mstar_driver
#else
#define	PLATFORM_DRIVER		second_ehci_hcd_mstar_driver
#define	SECOND_PLATFORM_DRIVER	ehci_hcd_mstar_driver
#endif
