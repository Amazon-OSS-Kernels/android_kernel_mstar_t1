...
static int ehci_bus_suspend (struct usb_hcd *hcd)
{
	...
	if (retval)
		ehci_err(ehci, "[%s] ehci_halt fail:%d\n", __func__, retval);
#endif
...
#if (MP_USB_MSTAR == 1)
			/* Stop run bit before set suspend */
			temp1 = ehci_readl(ehci, &ehci->regs->command);
			temp2 = temp1;
			temp1 &= ~CMD_RUN;
			ehci_writel(ehci, temp1, &ehci->regs->command);

			retval =  handshake(ehci, &ehci->regs->status, STS_HALT, STS_HALT, 16 * 125);
			if (retval) {
				ehci_writel(ehci, temp2, &ehci->regs->command);
				ehci_err(ehci, "set port %d suspend error %d\n",  hcd->port_index, retval);
				goto error;
			}
#endif
			ehci_writel(ehci, temp | PORT_SUSPEND, status_reg);
#if (MP_USB_MSTAR == 1)
			ehci_writel(ehci, temp2, &ehci->regs->command);		/* restore ehci command reg */
#endif
			if (ehci->has_hostpc) {
				spin_unlock_irqrestore(&ehci->lock, flags);
