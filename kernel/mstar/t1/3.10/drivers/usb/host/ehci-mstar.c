...
void Titania3_series_start_ehc(uintptr_t UTMI_base, uintptr_t USBC_base, uintptr_t UHC_base, unsigned int flag)
{
	...
	if (UTMI_DISCON_LEVEL_2A & (BIT3|BIT2|BIT1|BIT0)) {
		...
		printk(KERN_INFO "[USB] init squelch level 0x%x\n", readb((void *)(UTMI_base+0x2a*2)));
	}
...
static int ehci_hcd_mstar_drv_resume(struct platform_device *pdev)
{
	...
	printk("ehci_hcd_mstar_platform_resume...port %d, state %d\n",
	       hcd->port_index, get_state_value());

#if defined(CONFIG_MP_MSTAR_STR_BASE)
	if (STENT_RESUME_FROM_SUSPEND != get_state_value()) {
		dev_dbg(&pdev->dev, "ehci_resume(hcd, false) when Suspend fail\n");
