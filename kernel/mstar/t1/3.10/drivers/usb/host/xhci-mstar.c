...
void  XHCI_enable_testbus(uintptr_t CHIPTOP_base, uintptr_t U3TOP_base, uintptr_t XHCI_base)
{
	...
	/* [5] = reg_debug_mask to 1'b0
	 * [4:0] = 0x06= rrdy & wrdy */
	writeb(0x09,   (void *)(U3TOP_base+0x0C*2));
#endif
...
void Mstar_U2utmi_init(uintptr_t UTMI_base, uintptr_t U3TOP_base, unsigned int flag)
{
	...
	/* default value 0x7; don't change it. */
	...
	if ((0xFFF0 == (readw((void *)(UTMI_base+0x3C*2)) & 0xFFF0)) ||
		...
		printk(KERN_WARNING "WARNING: CA Fail!!\n");
...
void Mstar_U3phy_MS28_eFuse_2p(uintptr_t U3PHY_P0_A_base, uintptr_t U3PHY_P1_A_base)
{
	...
	if ((value & 0x80008000) == 0x80008000) {
		printk(KERN_INFO "[XHC: get efuse value: 0x%x]\n", value);
...
static int xhci_mstar_plat_probe(struct platform_device *pdev)
{
	...
	*((struct xhci_hcd **) xhci->shared_hcd->hcd_priv) = xhci;
	...
	XHCI_enable_PPC(_MSTAR_U3TOP_BASE);
#endif
...
static int xhci_hcd_mstar_drv_resume(struct platform_device *pdev)
{
	...
	if (get_state_value() != STENT_RESUME_FROM_SUSPEND) {
		dev_info(&pdev->dev,
			 "xhci_resume(hcd, false) when Suspend fail\n");
		xhci = hcd_to_xhci(hcd);
		retval = xhci_resume(xhci, false);
		if (retval) {
			dev_err(&pdev->dev,
				"xhci_resume FAIL: -0x%x\n", -retval);
			return retval;
		}
		return 0;
	}
	...
	xhci = hcd_to_xhci(hcd);
	retval = xhci_resume(xhci, true);
	if (retval) {
		dev_err(&pdev->dev, "xhci_resume FAIL: -0x%x", -retval);
		return retval;
	}
