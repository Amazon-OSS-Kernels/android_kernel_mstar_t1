...
int xhci_reset(struct xhci_hcd *xhci)
{
	...
	for (i = 0; i< 1000; i++)
	{
		if (0 == xhci_readl(xhci, &xhci->cap_regs->hcc_params))
		{
			udelay(1000);
			if (999==i)
				printk(KERN_ERR"[xHCI ERROR] : hw reset doesn't end !!\n");
		}
		else
			break;
	}
...
#if (MP_USB_MSTAR == 0)
	/*
	 * If power is off when STR success, doesn't need restore register.
	 * If power is on when STR fail, doesn't need restore register.
	 */

	/* step 3: save registers */
	xhci_save_registers(xhci);

	/* MSTAR XHCI hardware design doesn't support CSS flag */
...
#if (MP_USB_MSTAR == 0)
	/* MSTAR XHCI hardware design doesn't support restore function */
	if (!hibernated) {
