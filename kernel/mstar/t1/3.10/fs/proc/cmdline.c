...
/* console is configured to off */
	...
	if (!(idme_get_fos_flags_value() & FOS_FLAGS_CONSOLE_ON)) {
		char *console_cmdline = strstr(cmdline, CONFIG_ANDROID_CONSOLE_CMDLINE);
		char *temp = console_cmdline;
		...
		memmove((void *)console_cmdline, (const void *)(console_cmdline + console_len), (len - console_len));
		console_cmdline[strlen(cmdline) - console_len + 1] = '\0';
	}
out:
#endif
	return 0;
