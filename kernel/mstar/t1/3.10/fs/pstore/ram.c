	...
	if (size + ecc_notice_size + uboot_log_size + tvmsg_log_size >= 0) {
		*buf = kmalloc(size + ecc_notice_size + uboot_log_size + tvmsg_log_size + strlen(tvmsg_log_mark)
		...
		if (is_console) {
			...
			persistent_uboot_log(*buf + size + tvmsg_log_size + strlen(tvmsg_log_mark) + 5 + ecc_notice_size);
			*id = 0;
			*type = PSTORE_TYPE_CONSOLE;
		}
	}

	return size + ecc_notice_size + uboot_log_size + tvmsg_log_size + strlen(tvmsg_log_mark) + 5;
#else
