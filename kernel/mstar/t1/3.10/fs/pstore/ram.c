...
#ifdef CONFIG_PSTORE_TVOS_PMSG
const char *pmsg_log_mark =  "===================================  PMSG LOG ========================================";
#endif
...
static ssize_t get_uboot_log_size(void)
{
	...
	if (root) {
		...
		if (pp_value && pp_value->value && (pp_value->length > 0))
			ret_value = pp_value->length + strlen(uboot_log_mark) + 3;
		of_node_put(root);
	}
...
{
	ssize_t size = 0, ecc_notice_size = 0, uboot_log_size = 0;
	struct ramoops_context *cxt = psi->data;
	struct persistent_ram_zone *prz;
	static int console_done;
	int is_console = 0;
#ifdef CONFIG_PSTORE_TVOS_PMSG
	ssize_t  pmsg_log_size = 0;
	struct persistent_ram_zone *prz_pmsg;
#endif
...
#ifndef CONFIG_PSTORE_TVOS_PMSG
	if (!prz_ok(prz))
...
#ifdef CONFIG_PSTORE_TVOS_PMSG
		prz_pmsg = ramoops_get_next_prz(&cxt->mprz, &cxt->pmsg_read_cnt,
					   1, id, type, PSTORE_TYPE_PMSG, 0);

		if (prz_ok(prz_pmsg))
			pmsg_log_size = persistent_ram_old_size(prz_pmsg);
#endif
	}

#ifdef CONFIG_PSTORE_TVOS_PMSG
	if (size + ecc_notice_size + uboot_log_size + pmsg_log_size >= 0) {
		*buf = kmalloc(size + ecc_notice_size + uboot_log_size + pmsg_log_size + strlen(pmsg_log_mark)
				+ 5 + 1, GFP_KERNEL);
#else
...
#ifdef CONFIG_PSTORE_TVOS_PMSG
			if (prz_ok(prz_pmsg)) {
				snprintf(*buf + size + ecc_notice_size, strlen(pmsg_log_mark)+5, "\n\n%s\n\n",
						pmsg_log_mark);
				memcpy(*buf + size + ecc_notice_size + strlen(pmsg_log_mark)+5,
						persistent_ram_old(prz_pmsg), pmsg_log_size);
			}

			persistent_uboot_log(*buf + size + pmsg_log_size + strlen(pmsg_log_mark) + 5 + ecc_notice_size);
			*id = 0;
			*type = PSTORE_TYPE_CONSOLE;
#else
			persistent_uboot_log(*buf + size + ecc_notice_size);
#endif
		}
	}
	...
	if (is_console)
#ifdef CONFIG_PSTORE_TVOS_PMSG
		return size + ecc_notice_size + uboot_log_size + pmsg_log_size + strlen(pmsg_log_mark) + 5;
#else
		return size + ecc_notice_size + uboot_log_size;
#endif
	else
