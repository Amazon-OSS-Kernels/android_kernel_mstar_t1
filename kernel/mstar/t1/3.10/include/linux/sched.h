 ...
 * trace (or NULL if the entire call-chain of the task should be shown).
 *
 * This function will not show exception stack memory
 */
extern void show_call_trace(struct task_struct *task, unsigned long *sp);
...
struct sched_domain {
	...
	unsigned int mt_lbprof_nr_balance_failed;/* initialized to 0 */
#endif
...
#if defined(CONFIG_MP_HMP_GTS_SCHEDULER_CUSTOMIZED_RULES)
struct hmp_domain_cpus {
    int cpu_num;
    int is_big_core[NR_CPUS];
    int big_core_mask_bit;
    int little_core_mask_bit;
};
#endif
...
extern struct hmp_domain_cpus hmp_all_cpus;
extern struct proc_dir_entry *proc_scheduler_rule;
/* Decide which rule is enable. If  Rule 5 is enable, proc_rule_applied[5]=1. Otherwise, default is 0.*/
extern unsigned long proc_rule_applied[MAX_RULES+1];
extern volatile int ir_boost_flag;
extern volatile struct timer_list ir_boost_timer;
/*Set time interval for Rule 7 interval of process moved to big core */
#define RULE7_IR_INTERVAL_ON_BIG_CORE 1 /* sec*/


enum proc_rule_applied {
    RULE_RESERVED               = 0,
    RULE_FORK_MIGRATION         = 1,    /* enabled by default*/
    RULE_WAKEUP_MIGRATION       = 2,    /* enabled by default */
    RULE_FORCE_MIGRATION        = 3,    /* enabled by default*/
    RULE_IDLE_PULL_MIGRATION    = 4,    /* enabled by default*/
    RULE_OFFLOAD_MIGRATION      = 5,    /* enabled by default */
    RULE_APP_LAUNCH_MIGRATION   = 6,
    RULE_IR_MIGRATION           = 7,
    RULE_BENCHMARK_MIGRATION    = 8,
    RULE_LIGHT_TASK_MIGRATION   = 9,
    RULE_PAIR_MIGRATION         = 10,
    RULE_GROUP_MIGRATION        = 11,
    RULE_INTERRUPT_MIGRATION    = 12,
    RULE_BOOT_MIGRATION         = 13,
    RULE_BROWSER_MIGRATION      = 14,
};
...
#endif
#if defined(CONFIG_SCHED_HMP) && defined(CONFIG_MP_HMP_GTS_SCHEDULER_CUSTOMIZED_RULES)
    unsigned int opt_rule;  /* >0 apply rule */
#endif
};
