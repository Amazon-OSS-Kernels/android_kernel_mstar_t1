...
void mt_lbprof_update_state_has_lock(int cpu, int rq_cnt)
{
	...
	if (delta >= 1000000 && unbalance) {
		...
		if (0 == start_output) {
			...
			snprintf(strings, 128, "%llu.%06lu 0 %s", nsec_high(start), nsec_low(start), pre_state);
			trace_sched_lbprof_update(strings);
		}
		...
		snprintf(strings, 128, "%llu.%06lu %llu %s%lu %d %d ", nsec_high(now), nsec_low(now), delta,
...
void mt_lbprof_update_status(void)
{
	...
	for_each_present_cpu(cpu) {
		...
		snprintf(cpu_load_info_tmp, 8, "%3lu.%02lu ", (cpu_load)/100, (cpu_load)%100);
	...
	period_time_32 /= 10;
	{
		...
		snprintf(strings, 128,
			...
			(10000 - (unbalance_slice_ns) / period_time_32)/100, (10000 - (unbalance_slice_ns) / period_time_32)%100,
			cpu_load_info,
			(unbalance_slice_ns / period_time_32)/100, (unbalance_slice_ns / period_time_32)%100,
			(time_slice_ns[0][0] / period_time_32)/100, (time_slice_ns[0][0] / period_time_32)%100,
			(time_slice_ns[0][1] / period_time_32)/100, (time_slice_ns[0][1] / period_time_32)%100,
			(time_slice_ns[0][2] / period_time_32)/100, (time_slice_ns[0][2] / period_time_32)%100,
			(time_slice_ns[1][0] / period_time_32)/100, (time_slice_ns[1][0] / period_time_32)%100,
			(time_slice_ns[1][1] / period_time_32)/100, (time_slice_ns[1][1] / period_time_32)%100,
			(time_slice_ns[1][2] / period_time_32)/100, (time_slice_ns[1][2] / period_time_32)%100,
			period_time_32);
