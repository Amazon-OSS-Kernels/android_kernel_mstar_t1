...
static void __init mm_init(void)
{
	...
	mstar_pm_base = (ptrdiff_t)ioremap(IO_PHYS, 0xA00000);
 ...
 */
	...
	/* Filter out Serial number if present in boot_command_line from beging logged,
	 due to privacy reasons.
	   Note : the creation of __cmdline is in do_bootm_linux in uboot */
	/* Note: sizeof() returns length including the terminal null character . So subtracting 1 */
	if (strncmp(boot_command_line, "androidboot.serialno", sizeof("androidboot.serialno")-1) == 0) {
		#define SN_LOG_LENGTH (sizeof("androidboot.serialno")+17)
		/* SN_LOG_LENGTH = length of ("androidboot.serialno=") + 18(for SN) */
		pr_notice("Kernel command line: androidboot.serialno=GXXXXXXXXXXXXXXX %s\n",\
		(boot_command_line + SN_LOG_LENGTH));
	} else
	...
	early_irq_init();
	init_IRQ();					// we need to modify the NR_IRQS definition, this should be defined @ chip hal layer, see arch/arm64/include/asm/irq.h 
	tick_init();
	init_timers();
	hrtimers_init();
	softirq_init();
...
extern void print_cpu_workqueue_info(int only_show);
struct task_struct *show_work_info_tsk;
void show_work_info(void* arg);
unsigned int msleep_time = 5000;
unsigned int if_show_cpu_workqueue_info = 1;
void show_work_info(void* arg)
{
	while(1)
	{
		msleep(msleep_time);
		if(if_show_cpu_workqueue_info)
			print_cpu_workqueue_info(1);
		else
			pr_debug("\033[35mFunction = %s, Line = %d, stop show_work_info\033[m\n", __PRETTY_FUNCTION__, __LINE__);
	}
}
#endif
...
static int __ref kernel_init(void *unused)
{
...
#ifdef CONFIG_MP_DEBUG_TOOL_SHOW_CPU_WORKQUEUE_INFO
    show_work_info_tsk = kthread_create(show_work_info, NULL, "JOE_TEST");
    kthread_bind(show_work_info_tsk, 0);
    if (IS_ERR(show_work_info_tsk)) {
	pr_debug("create kthread for JOE_TEST fail\n");
            show_work_info_tsk = NULL;
			BUG_ON(1);
    }else
        wake_up_process(show_work_info_tsk);
#endif
	...
	pr_debug("\033[31m[Error]Function = %s, Line = %d, while using ARM chips and SPARSE_MEMORY, you need to enable CONFIG_HOLES_IN_ZONE\033[m\n", __PRETTY_FUNCTION__, __LINE__);
	pr_debug("\033[31m[Error]Function = %s, Line = %d, while using ARM chips and SPARSE_MEMORY, you need to enable CONFIG_HOLES_IN_ZONE\033[m\n", __PRETTY_FUNCTION__, __LINE__);
	pr_debug("\033[31m[Error]Function = %s, Line = %d, while using ARM chips and SPARSE_MEMORY, you need to enable CONFIG_HOLES_IN_ZONE\033[m\n", __PRETTY_FUNCTION__, __LINE__);
#endif
#endif
...
#error "Error, both CONFIG_MP_PLATFORM_ARM and CONFIG_MP_PLATFORM_MIPS are set, please select only one"
	...
	pr_debug("[AT][KR][start init][%u]\n", PiuTime);
	pr_debug("[AutoTest][Kernel][start Initprocess][%u]\n", PiuTime);
#endif
...
static noinline void __init kernel_init_freeable(void)
{
	...
	printk(KERN_DEBUG "%s",KERN_CL);
	printk(KERN_DEBUG "%s",KERM_CL);
#endif
