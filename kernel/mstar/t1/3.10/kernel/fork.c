...
/*
*  For MStar scheduler customized rules
*/
#if defined(CONFIG_SCHED_HMP) && defined(CONFIG_MP_HMP_GTS_SCHEDULER_CUSTOMIZED_RULES)
struct proc_dir_entry *proc_scheduler_rule;
#define MAX_PROC_SIZE 30
static char proc_data[MAX_PROC_SIZE];
unsigned long proc_rule_applied[MAX_RULES+1];
volatile int ir_boost_flag = 0;
volatile struct timer_list ir_boost_timer;
struct hmp_domain_cpus hmp_all_cpus;
#endif
...
/*
*  MStar scheduler customized rules module
*/
#if defined(CONFIG_SCHED_HMP) && defined(CONFIG_MP_HMP_GTS_SCHEDULER_CUSTOMIZED_RULES)

//Proc for scheduler customized rule
int read_scheduler_customized_rule_proc(char *buf, char **start, off_t offset, int count, int *eof, void *data){
    int loop;
    proc_rule_applied[8]=1;
    for (loop=0;loop<MAX_RULES;loop++){
		if(loop<5)
			proc_rule_applied[loop+1]=1;
		if(proc_rule_applied[loop+1]==1){
			pr_debug("Rule %d : enable\n", loop+1);
		}else{
			pr_debug("Rule %d : disable\n", loop+1);
		}
    }
    return 0;
}
int write_scheduler_customized_rule_proc(struct file *filp, const char *buf,int count, void *data){
	int rule_enable_num=0;
	char proc_data_clone[MAX_PROC_SIZE];
	unsigned long test_string2int;

	//Initialize scheduler customized rules applied
	memset(proc_data, 0, MAX_PROC_SIZE);
	memset(proc_rule_applied,0,MAX_RULES+1);
	if(copy_from_user(proc_data, buf, count))
		return -EFAULT;

	// Deal with input stream from char to int
	char* proc_data_pt;
	strcpy(proc_data_clone,proc_data);
	char* proc_data_pt_ori=proc_data_clone;
	while((proc_data_pt=strsep(&proc_data_pt_ori,",")) != NULL){
		if(proc_data_pt == "") continue;
		test_string2int=simple_strtoul(proc_data_pt,NULL,10);
		proc_rule_applied[test_string2int]=1;
		rule_enable_num++;
	}
//	printk("#rule_enable=%d\n",rule_enable_num);
	return count;
}
...
static const struct file_operations scheduler_customized_rule_fops = {
    .owner      = THIS_MODULE,
    .read = read_scheduler_customized_rule_proc,
    .write = write_scheduler_customized_rule_proc,
    .open = proc_open,
    .release = proc_close,
};
int init_scheduler_customized_rule(void)
{
	proc_scheduler_rule = proc_create("scheduler_customized_rule", 0666, NULL, &scheduler_customized_rule_fops);
	if(!proc_scheduler_rule){
        printk(KERN_INFO "Error creating proc entry: MStar scheduler_customized_rule");
        return -ENOMEM;
    }

	return 0;
}
module_init(init_scheduler_customized_rule)
#endif
