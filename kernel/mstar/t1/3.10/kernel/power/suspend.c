...
static int suspend_enter(suspend_state_t state, bool *wakeup)
{
	...
	BUG_ON(irqs_disabled());
#if defined(CONFIG_MP_MSTAR_STR_BASE)
	if ((str_cnt + 1) == mstr_cnt) {
		pr_info("PM: Resuming from STR.... Total Times [%d]\n", mstr_cnt);
		str_cnt++;
	}
#endif
 Enable_cpus:
...
static int enter_state(suspend_state_t state)
{
    ...
    if(!is_mstar_str()) {
        // Kernel Mode STR, Load PM51 to DRAM here instead of SN
		MDrv_PM_CopyBin2TempBuffer();
    }
#endif
    set_state_entering();
#if (MP_USB_STR_PATCH==1)
 ...
 */
extern u32 MDrvDvfsProc(u32 dwInputCpuClock, u8 dwCpu);
int pm_suspend(suspend_state_t state)
{
	...
	while (atomic_read(&dvfs_finish) != 1)
	mdelay(100);

	pr_info("\033[0;32;31m%s Disable DVFS in STR\033[m\n", __func__);
	pr_info("Change Cluster 0 to 1.1Gz\n");
