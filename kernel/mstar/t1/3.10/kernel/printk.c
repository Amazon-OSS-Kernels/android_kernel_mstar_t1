		      ...
		      const char *text, u16 text_len)
{
	...
	if (console_suspended == 0) {
		tlen = snprintf(tbuf, sizeof(tbuf), "%c[%d:%s]",
				state, current->pid, current->comm);
	} else {
		tlen = snprintf(tbuf, sizeof(tbuf), "%c)", state);
	}
	/*printk prefix }*/
...
#ifdef CONFIG_SMP
	if (!buf)
		return snprintf(NULL, 0, "[%5lu.000000@%1d] ", (unsigned long)ts, current_cpu);
	return sprintf(buf, "[%5lu.%06lu@%1d] ",
