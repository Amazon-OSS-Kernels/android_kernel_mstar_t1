...
static int __init hmp_cpu_mask_setup(void)
{
    ...
    hmp_all_cpus.big_core_mask_bit = 0;
    hmp_all_cpus.little_core_mask_bit = 0;
#endif
...
#if defined(CONFIG_MP_HMP_GTS_SCHEDULER_CUSTOMIZED_RULES)
	    hmp_all_cpus.cpu_num++;
	    hmp_all_cpus.is_big_core[cpu] = hmp_cpu_is_fastest(cpu);
	    if (hmp_cpu_is_fastest(cpu))
		hmp_all_cpus.big_core_mask_bit |= (1 << cpu);
	    else
		hmp_all_cpus.little_core_mask_bit |= (1 << cpu);
#endif
		}
		dc++;
	}
 ...
 */
unsigned int hmp_up_threshold = 300;
...
select_task_rq_fair(struct task_struct *p, int sd_flag, int wake_flags)
{
	...
	if (unlikely(sd_flag & SD_BALANCE_FORK) && hmp_task_should_forkboost(p)) {
		...
		if (new_cpu != NR_CPUS) {
			...
			trace_sched_hmp_migrate(p, new_cpu, HMP_MIGRATE_FORK);
			return new_cpu;
		}
...
#if defined(CONFIG_MP_HMP_GTS_SCHEDULER_CUSTOMIZED_RULES)
    /*
    *Rule 7 user experience (IR)
    */

    if (ir_boost_flag && strstr(p->group_leader->comm, "tvos")) {
	hmp_domain_min_load(hmp_cpu_is_fastest(cpu) ? hmp_cpu_domain(cpu) : hmp_faster_domain(cpu), &temp_target_cpu, tsk_cpus_allowed(p));
