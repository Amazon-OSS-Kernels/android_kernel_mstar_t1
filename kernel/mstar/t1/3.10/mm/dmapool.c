		     ...
		     unsigned long top)
{
	...
	printk(KERN_DEBUG "%s%s(0x%016lx to 0x%016lx)\n", lvl, str, bottom, top);
	...
	for (first = bottom & ~31; first < top; first += 32) {
		...
		for (p = first, i = 0; i < 8 && p < top; i++, p += 4) {
			if (p >= bottom && p < top) {
				sprintf(str + i * 9, " %08x", *(unsigned int *)p);
			}
		}
		printk(KERN_DEBUG "%s%04lx:%s\n", lvl, first & 0xffff, str);
	}
}

#include "../drivers/mstar2/hal/maserati/cpu/arm64/include/mstar/mstar_chip.h"
 ...
 ready:
	...
	*handle = offset + page->dma;
#ifdef	DMAPOOL_DEBUG
	{
		...
		if (unlikely(MIU_FAIL_ADDR == _BUS2PA(*handle))) {
			printk(KERN_ERR "page->offset: 0x%016lx, &page->offset: 0x%016lx\n", page->offset,  &(page->offset));
			printk(KERN_ERR "page->dma: 0x%016lx, &page->dma: 0x%016lx\n", page->dma, &(page->dma));

			dump_mem(KERN_DEBUG, "page->offset: ", &(page->offset)-64,  &(page->offset)+64);
			dump_mem(KERN_DEBUG, "page->dma: ", &(page->dma)-64,  &(page->offset)+64);
	}
