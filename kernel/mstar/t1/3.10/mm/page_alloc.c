...
extern void get_cma_status(char *info, size_t count);
...
void show_mm_time(unsigned long time)
{
	...
	get_cma_status(cma_info, sizeof(cma_info));
...
void ZONE_CMA_COHERENCE_CHK(int nolock)
{
	...
	for_each_populated_zone(zone) {
		...
		//printk("\033[35mFunction = %s, Line = %d, check zone %s\033[m\n", __PRETTY_FUNCTION__, __LINE__, zone->name);
...
static void __adjust_managed_cma_page_count(struct zone *zone, long count)
{
	...
	printk(CMA_DEBUG "minfree[5]=%d pages, final CMA_threshold_1 = %d KB, CMA_threshold_2 = %d KB\n",
	...
	*/
    ...
    if(((global_page_state(NR_FREE_PAGES)) << (PAGE_SHIFT - 10)) > CMA_threshold_2){
         zone->max_try_movable = pageblock_nr_pages;
         zone->max_try_cma = 0;
         //printk("all allocate from Normal");
    }
    else if(((global_page_state(NR_FREE_PAGES)) << (PAGE_SHIFT - 10)) < CMA_threshold_1){
        //printk("prefer to use CMA. If no CMA, use normal page, instead");
        if (cma > 0) {
            zone->max_try_cma = cma;
        ...
        //printk("allocate based on ratio");
        if (movable > cma) {
...
void warn_alloc_failed(gfp_t gfp_mask, int order, const char *fmt, ...)
{
	...
	if (!order) {
		pr_warn("dump addition information for order 0 allocation failure\n");
		dump_task_mm();
...
int write_CMA_threshold_proc(struct file *file,const char *buf,int count,void *data )
{
    ...
    //printk("cur = %s\n", cur);
    th2 = cur;
	pr_debug("th1 = %s, th2 = %s\n", th1, th2);
    if((th1 == NULL) || (th2 == NULL)){
	pr_debug("TH1 or TH2 == NULL\n");
        return count;
    }
    if((th1[0] == '\0') || (th2[0] == '\0')){
	pr_debug("TH1 or TH2 == empty value\n");
        return count;
    }
    ...
    if(val_th1 > val_th2){
	pr_debug("invalid config: TH1 > TH2\n");
        return count;
    }

    CMA_threshold_1 = val_th1;
    CMA_threshold_2 = val_th2;
	pr_debug("set CMA_thresholds\n");
	pr_debug("CMA_threshold_1 = %d kB\nCMA_threshold_2 = %d kB\n", CMA_threshold_1, CMA_threshold_2);
    return count;
}
...
pr_debug("CMA_threshold proc initialized");
...
int __meminit init_per_zone_wmark_min(void)
{
  ...
  create_CMA_threshold_proc_entry();
	...
	printk(CMA_DEBUG "\n Init CMA_threshold_1 = %d KB, CMA_threshold_2 = %d KB\n",
		CMA_threshold_1, CMA_threshold_2);
#endif
	return 0;
}
...
static void dump_page_flags(unsigned long flags)
{
	...
	for (i = 0; i < ARRAY_SIZE(pageflag_names) && flags; i++) {
		...
		flags &= ~mask;
		pr_alert("%s%s", delim, pageflag_names[i].name);
		delim = "|";
	}

	/* check for left over flags */
	if (flags)
		pr_alert("%s%#lx", delim, flags);
...
static short g_early_kill_oom_adj_min = 529;

void set_early_kill_oom_adj_threshold(short oom_adj)
{
	if (oom_adj < g_early_kill_oom_adj_min) {
		g_early_kill_oom_adj_min = oom_adj;
		pr_err("set_early_kill_oom_adj_threshold set to %d\n", oom_adj);
	}
