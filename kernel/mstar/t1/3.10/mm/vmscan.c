...
void show_freemem_info(void)
{
	...
	for_each_populated_zone(zone) {
		...
		int type;
		printk_ratelimited(CMA_DEBUG "ZONE %s: \n", zone->name);
		...
		for (order = 0; order < MAX_ORDER; order++) {
			...
			for (type = 0; type < MIGRATE_TYPES; type++) {
				if(nr[order][type])
				{
					printk_ratelimited(CMA_DEBUG "[%c]%lu*%lukB ", migrate_types[type],
			...
			if(show_cnt)
				printk_ratelimited(CMA_DEBUG "\n");
		}
		printk_ratelimited(CMA_DEBUG "\ntotal= %lukB free\n", K(total));
		printk_ratelimited(CMA_DEBUG "%ld total pagecache pages\n\n", global_page_state(NR_FILE_PAGES));
	}
}
#endif
					...
					struct shrink_control *shrink)
{
	...
	do {
		...
		if(sc->priority == 0)
		{
		pr_err_ratelimited("@@a do_try_to_free_pages free: %llu, cmafree: %llu, pressure 0, order: %d\n",
...
loop_again:
	...
	do {
		...
		for (i = 0; i <= end_zone; i++) {
			...
			if(sc.priority == 0)
			{
				pr_err_ratelimited("@@a balance_pgdat zone%d pressure 0 free: %llu, cma_free: %llu, low_water_mark: %lu, for order: %d\n", i,
